...
const errorTestDiagnostics = { errors: [] as string[], unhandledErrors: [] as string[] };
...
test.describe('RankPilot Error Handling & Edge Cases - Comprehensive Testing', () => {
    ...
    test.describe('Network Error Handling', () => {
        ...
        test('Slow Network Recovery', async ({ page }) => {
            ...
            try {
                ...
                await page.route('**/api/**', async route => {
                    await new Promise(resolve => setTimeout(resolve, TEST_DATA.edgeCases.networkFailures.slowConnection));
                ...
                const loadingIndicator = await page.isVisible('.loading, [data-testid="loading"], .spinner');
                console.log(`   Loading Indicator Shown: ${loadingIndicator}`);

                // Wait for content to load
                await page.waitForTimeout(TEST_DATA.edgeCases.networkFailures.slowConnection + 1000);

                const contentLoaded = await page.isVisible('.main-content, [data-testid="content"]');
                ...
                errorTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
        ...
        test('Network Timeout Handling', async ({ page }) => {
            ...
            try {
                ...
                await page.route('**/api/**', async route => {
                    await new Promise(resolve => setTimeout(resolve, TEST_DATA.edgeCases.networkFailures.timeout + 1000));
                    await route.continue();
                });
                ...
                if (await keywordInput.isVisible()) {
                    ...
                    if (await searchBtn.isVisible()) {
                        ...
                        // Wait for timeout
                        await page.waitForTimeout(TEST_DATA.edgeCases.networkFailures.timeout + 2000);

                        const timeoutError = await page.isVisible('text=/timeout|network error|connection/i');
        ...
        test('Connection Loss Recovery', async ({ page }) => {
            ...
            try {
                ...
                if (await refreshBtn.isVisible()) {
                    ...
                    const offlineMsg = await page.isVisible('text=/offline|connection lost|network error/i');
                    const reconnectBtn = await page.isVisible('button[data-testid="reconnect"], .reconnect-btn');
    ...
    test.describe('API Error Handling', () => {
        ...
        test('HTTP Error Status Codes', async ({ page }) => {
            ...
            try {
                for (const statusCode of TEST_DATA.errorScenarios.apiErrors) {
                    ...
                    const errorMsg = await page.isVisible(`text=/error|${statusCode}|failed/i`);
                    const userFriendlyMsg = await page.isVisible('.error-message, [data-testid="error"]');

                    console.log(`   HTTP ${statusCode} - Error Shown: ${errorMsg}`);
                    console.log(`   HTTP ${statusCode} - User Friendly: ${userFriendlyMsg}`);
        ...
        test('API Rate Limiting', async ({ page }) => {
            ...
            try {
                ...
                const rateLimitMsg = await page.isVisible('text=/rate limit|too many|429/i');
                const cooldownMsg = await page.isVisible('text=/cooldown|wait|try again/i');
        ...
        test('Empty & Whitespace Inputs', async ({ page }) => {
            ...
            try {
                ...
                for (const input of inputs.slice(0, 3)) { // Test first 3 inputs
                    const inputType = await input.getAttribute('type') || await input.getAttribute('name') || 'text';
                    ...
                    if (await submitBtn.isVisible()) {
                        ...
                        const emptyError = await page.isVisible('.error, .validation-error, [data-testid="error"]');
                    ...
                    if (await submitBtn.isVisible()) {
                        ...
                        const whitespaceError = await page.isVisible('.error, .validation-error, [data-testid="error"]');
                        console.log(`   Whitespace ${inputType} - Validation: ${whitespaceError}`);
                    }
                }
            ...
            try {
                ...
                if (textareas.length > 0) {
                    ...
                    if (await submitBtn.isVisible()) {
                        ...
                        const sizeError = await page.isVisible('text=/too long|limit|maximum/i');
                        const truncatedMsg = await page.isVisible('text=/truncated|shortened/i');
        ...
        test('Special Characters & Unicode', async ({ page }) => {
            ...
            try {
                ...
                if (await keywordInput.isVisible()) {
                    ...
                    await keywordInput.fill(TEST_DATA.edgeCases.invalidInputs.specialChars);
                    ...
                    if (await searchBtn.isVisible()) {
                        ...
                        const specialCharError = await page.isVisible('.error, [data-testid="error"]');
                        const results = await page.isVisible('[data-testid="results"], .results');
                    ...
                    if (await searchBtn.isVisible()) {
                        ...
                        const unicodeError = await page.isVisible('.error, [data-testid="error"]');
                        const unicodeResults = await page.isVisible('[data-testid="results"], .results');
                ...
                if (numberInputs.length > 0) {
                    ...
                    for (const value of boundaryValues) {
                        ...
                        if (await saveBtn.isVisible()) {
                            ...
                            const boundaryError = await page.isVisible('.error, .validation-error, [data-testid="error"]');
                            const successMsg = await page.isVisible('.success, [data-testid="success"]');

                            console.log(`   Boundary ${value} - Error: ${boundaryError}, Success: ${successMsg}`);
                        }
                    }
    ...
    test.describe('Browser & Device Edge Cases', () => {
        ...
        test('Cookies Disabled', async ({ page }) => {
            ...
            try {
                ...
                const authRequired = await page.isVisible('.login-required, [data-testid="login"]');
        ...
        test('Local Storage Unavailable', async ({ page }) => {
            ...
            try {
                ...
                const storageError = await page.isVisible('text=/storage|localStorage/i');
    ...
    test.describe('Performance Edge Cases', () => {
        ...
        test('Memory Leak Prevention', async ({ page }) => {
            ...
            try {
                ...
                const pages = ['/keyword-tool', '/seo-audit', '/settings/profile', '/dashboard'];
                ...
                for (const pagePath of pages) {
                    ...
                    const memoryError = await page.isVisible('text=/memory|out of memory|heap/i');
        ...
        test('Concurrent Request Handling', async ({ page }) => {
            ...
            try {
                ...
                const loadingStates = await page.locator('.loading, [data-testid*="loading"]').all();
                const errorStates = await page.locator('.error, [data-testid*="error"]').all();
        ...
        test('CSS Flexbox/Grid Fallbacks', async ({ page }) => {
            ...
            try {
                ...
                const layoutBroken = await page.evaluate(() => {
                    const elements = document.querySelectorAll('.flex, .grid, [style*="display: flex"], [style*="display: grid"]');
                    let brokenCount = 0;
        ...
        test('Progressive Enhancement', async ({ page }) => {
            ...
            try {
                ...
                const readableContent = await page.evaluate(() => {
                    ...
                    const hasReadableText = body.textContent && body.textContent.length > 100;
    ...
    test.afterAll(() => {
        ...
        if (errorTestDiagnostics.errors.length === 0 && errorTestDiagnostics.unhandledErrors.length === 0) {
