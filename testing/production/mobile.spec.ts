...
test.describe("Mobile Compatibility", () => {
  test("mobile responsiveness validation", async ({ page }) => {
    ...
    const mobileScore =
      (report.summary.mobilePassed / report.summary.mobileTotal) * 100;
  ...
  test("touch target accessibility", async ({ page }) => {
    ...
    const touchTargetScore =
      touchTargets.length > 0
        ? (validTouchTargets / touchTargets.length) * 100
        : 100;
  ...
  test("mobile navigation usability", async ({ page }) => {
    ...
    if (await mobileMenuButton.isVisible()) {
      ...
      const mobileMenu = page
        .locator(
          '[data-testid="mobile-menu-content"], .mobile-menu, [role="menu"]'
        )
        .first();
  ...
  test("mobile text readability", async ({ page }) => {
    ...
    const readabilityScore =
      (readableTextCount / Math.min(textElements.length, 20)) * 100;
