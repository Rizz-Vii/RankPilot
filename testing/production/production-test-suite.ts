    ...
    try {
      ...
      this.results.push({
        ...
        details: passed
          ...
          : "Missing or incomplete CSP headers",
      ...
      if (await searchInput.isVisible()) {
        ...
        const alertFired = await this.page.evaluate(() => {
          return (
            ...
            window.alert.toString().includes("native")
          );
        });
        ...
        this.results.push({
          ...
          passed: !alertFired,
          details: alertFired
            ? "XSS vulnerability detected in search"
  ...
  private async testSQLInjection(): Promise<void> {
    try {
      ...
      for (const pattern of injectionPatterns) {
        const response = await this.page.goto(
          `/api/search?q=${encodeURIComponent(pattern)}`
        );
    ...
    try {
      ...
      for (const route of protectedRoutes) {
        ...
        const protectedProperly =
          redirectedToAuth || status === 401 || status === 403;
  ...
  private async testSessionSecurity(): Promise<void> {
    try {
      ...
      const passed =
        ...
        secureSessionsFound === sessionCookies.length;
      this.results.push({
...
export class LoadTestSuite {
  ...
  private async simulateUserSession(): Promise<number> {
    ...
    try {
      ...
      const endpoints = ["/", "/dashboard", "/neuroseo", "/api/health"];
...
export class MobileTestSuite {
  ...
  private async testMobileDevice(
    deviceName: string
