...
test.describe('RankPilot Agents API - Comprehensive Testing', () => {
    ...
    test.describe('Agent Enable/Disable Operations', () => {
        ...
        test('Agent Enable - Valid Request', async ({ page }) => {
            ...
            try {
                const response = await page.request.post(`${BASE_URL}/api/agents/enable`, {
                ...
                agentsTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
    ...
    test.describe('Agent Management Operations', () => {
        ...
        test('Agent Details - Get Specific Agent', async ({ page }) => {
            ...
            try {
                const response = await page.request.get(`${BASE_URL}/api/agents/${TEST_DATA.validAgentId}`);
        ...
        test('Agent Details - Invalid Agent ID', async ({ page }) => {
            ...
            try {
                const response = await page.request.get(`${BASE_URL}/api/agents/${TEST_DATA.invalidAgentId}`);
    ...
    test.describe('Agent Execution & Capabilities', () => {
        ...
        test('Agent Execution - Business Operations', async ({ page }) => {
            ...
            try {
                const response = await page.request.post(`${BASE_URL}/api/agents/execute`, {
                    data: {
        ...
        test('Agent Capabilities - List Available Actions', async ({ page }) => {
            ...
            try {
                const response = await page.request.get(`${BASE_URL}/api/agents/${TEST_DATA.validAgentId}/capabilities`);
                ...
                if (response.status() === 200) {
                    const data = await response.json();
                    console.log(`   Agent Capabilities: ${JSON.stringify(data, null, 2)}`);
    ...
    test.describe('Agent Rate Limiting & Performance', () => {
        ...
        test('Agent Rate Limiting - Multiple Requests', async ({ page }) => {
            ...
            const errorCount = responses.filter(r => r.status() === 400 || r.status() === 404).length;
            ...
            if (totalValidResponses > 0) {
                ...
                console.log('   ⚠️ Agent rate limiting not triggered, but some requests processed');
            }
        });
