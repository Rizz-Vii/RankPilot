  ...
  test.describe("Authentication Flow", () => {
    ...
    test("Login - Invalid Credentials", async ({ page }) => {
      ...
      try {
        ...
        await page.waitForSelector('.error-message, [data-testid="error"]', {
          timeout: 5000,
        });
    ...
    test("Login - Rate Limiting", async ({ page }) => {
      ...
      try {
        ...
        const rateLimited = await page.isVisible(
          '.rate-limit, [data-testid="rate-limit"]'
        );
  ...
  test.describe("Authorization & Access Control", () => {
    test("Premium Feature Access - Agency Tier", async ({ page }) => {
      ...
      try {
        ...
        const canAccess = await page.isVisible(
          '[data-testid="neuroseo-analysis"]'
        );
        const upgradePrompt = await page.isVisible(
    ...
    test("Admin Panel Access - Admin Role Required", async ({ page }) => {
      ...
      try {
        ...
        if (response.status() === 200) {
          ...
          console.log(
            "   ⚠️ Admin panel not accessible (expected for non-admin users)"
          );
        }
    ...
    test("Team Member Access - Team Features", async ({ page }) => {
      ...
      try {
        ...
        if (response.status() === 200) {
          ...
          console.log(
            "   ⚠️ Team features not accessible (may require higher tier)"
          );
        }
  ...
  test.describe("Security Headers & Protections", () => {
    test("Security Headers - API Responses", async ({ page }) => {
      ...
      try {
        ...
        const hasSomeSecurityHeaders = Object.keys(headers).some(
          (h) =>
        ...
        console.log(
          `   Some Security Headers Present: ${hasSomeSecurityHeaders}`
        );
