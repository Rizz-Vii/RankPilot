  ...
  test.describe("Keyword Tool Form", () => {
    test("Keyword Tool Form - Basic Display", async ({ page }) => {
      ...
      try {
        ...
        const formVisible = await page.isVisible(
          'form, [data-testid="keyword-form"]'
        );
        const inputField = await page.isVisible(
        ...
        formTestDiagnostics.errors.push(
      ...
      try {
        ...
        if (await inputField.isVisible()) {
          ...
          if (await submitBtn.isVisible()) {
            ...
            const validationMsg = await page.isVisible(
        ...
        if (await inputField.isVisible()) {
          ...
          if (await submitBtn.isVisible()) {
            ...
            const resultsVisible = await page.isVisible(
              '[data-testid="results"], .results, .keyword-results'
            );
          ...
          console.log(
            "   ⚠️ Form elements not found, skipping submission test"
          );
        }
      } catch (error) {
        formTestDiagnostics.errors.push(
  ...
  test.describe("SEO Audit Form", () => {
    test("SEO Audit Form - URL Input", async ({ page }) => {
      ...
      try {
        ...
        const urlInput = await page.isVisible(
          'input[type="url"], input[placeholder*="url"]'
        );
        const auditButton = await page.isVisible(
          'button[data-testid*="audit"], .audit-btn'
        );
      ...
      try {
        ...
        if (await urlInput.isVisible()) {
          ...
          const auditBtn = await page
            .locator('button[data-testid*="audit"], .audit-btn')
            .first();
          ...
          if (await auditBtn.isVisible()) {
            ...
            const urlError = await page.isVisible(
              '.url-error, [data-testid="url-error"]'
            );
  ...
  test.describe("Contact Form", () => {
    test("Contact Form - Basic Display", async ({ page }) => {
      ...
      try {
        ...
        const messageTextarea = await page.isVisible(
          'textarea[name="message"], textarea[placeholder*="message"]'
        );
        const sendButton = await page.isVisible(
        ...
        expect(nameInput || emailInput || messageTextarea || sendButton).toBe(
          true
        );
    ...
    test("Contact Form - Field Validation", async ({ page }) => {
      ...
      try {
        ...
        const submitBtn = await page
          .locator('button[type="submit"], [data-testid="send"]')
          .first();
        ...
        if (await submitBtn.isVisible()) {
          ...
          const validationErrors = await page
            .locator('.error, .validation-message, [data-testid*="error"]')
            .all();
          ...
          console.log(
            "   ⚠️ Submit button not found, skipping validation test"
          );
        }
      } catch (error) {
        formTestDiagnostics.errors.push(
      ...
      try {
        ...
        if (await submitBtn.isVisible()) {
          ...
          const successMsg = await page.isVisible(
            '.success, [data-testid="success"], .thank-you'
          );
    ...
    test("Form Loading States", async ({ page }) => {
      ...
      try {
        ...
        if (await submitBtn.isVisible()) {
          ...
          const loadingState = await page.isVisible(
            'button:disabled, .loading, [data-testid="loading"]'
          );
    ...
    test("Form Labels & ARIA", async ({ page }) => {
      ...
      try {
        ...
        const labels = await page
          .locator("label, [aria-label], [aria-labelledby]")
          .all();
