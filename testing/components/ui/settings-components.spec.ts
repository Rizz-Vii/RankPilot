  ...
  test.describe("User Profile Settings", () => {
    test("User Profile Settings - Display", async ({ page }) => {
      ...
      try {
        ...
        const profileForm = await page.isVisible(
          'form[data-testid="profile-form"], .profile-settings'
        );
        const nameInput = await page.isVisible(
          'input[name="name"], input[placeholder*="name"]'
        );
        const emailInput = await page.isVisible(
          'input[name="email"], input[type="email"]'
        );
        const saveButton = await page.isVisible(
        ...
        settingsTestDiagnostics.errors.push(
      ...
      try {
        ...
        if (await saveBtn.isVisible()) {
          ...
          const successMsg = await page.isVisible(
            '.success, [data-testid="success"], .profile-updated'
          );
  ...
  test.describe("Notification Settings", () => {
    test("Notification Settings - Toggle Controls", async ({ page }) => {
      ...
      try {
        ...
        const emailToggle = await page.isVisible(
          'input[type="checkbox"][name*="email"], [data-testid="email-notifications"]'
        );
        const pushToggle = await page.isVisible(
          'input[type="checkbox"][name*="push"], [data-testid="push-notifications"]'
        );
        const smsToggle = await page.isVisible(
          'input[type="checkbox"][name*="sms"], [data-testid="sms-notifications"]'
        );
    ...
    test("Notification Settings - Toggle Interaction", async ({ page }) => {
      ...
      try {
        ...
        if (await emailToggle.isVisible()) {
          ...
          console.log(
            "   ⚠️ Email toggle not found, skipping interaction test"
          );
        }
      } catch (error) {
        settingsTestDiagnostics.errors.push(
  ...
  test.describe("API Settings", () => {
    test("API Settings - Key Management", async ({ page }) => {
      ...
      try {
        ...
        const apiKeyInput = await page.isVisible(
          'input[name="apiKey"], input[type="password"]'
        );
        const generateBtn = await page.isVisible(
          'button[data-testid="generate-key"], .generate-key'
        );
        const revokeBtn = await page.isVisible(
          'button[data-testid="revoke-key"], .revoke-key'
        );
    ...
    test("API Settings - Webhook Configuration", async ({ page }) => {
      ...
      try {
        ...
        const webhookUrlInput = await page.isVisible(
          'input[name="webhookUrl"], input[placeholder*="webhook"]'
        );
        const testWebhookBtn = await page.isVisible(
          'button[data-testid="test-webhook"], .test-webhook'
        );
  ...
  test.describe("Theme Settings", () => {
    test("Theme Settings - Theme Selection", async ({ page }) => {
      ...
      try {
        ...
        const lightTheme = await page.isVisible(
          'input[value="light"], [data-testid="light-theme"]'
        );
        const darkTheme = await page.isVisible(
          'input[value="dark"], [data-testid="dark-theme"]'
        );
        const autoTheme = await page.isVisible(
          'input[value="auto"], [data-testid="auto-theme"]'
        );
    ...
    test("Theme Settings - Theme Switching", async ({ page }) => {
      ...
      try {
        ...
        if (await darkThemeOption.isVisible()) {
          ...
          console.log(
            "   ⚠️ Dark theme option not found, skipping theme switching test"
          );
        }
      } catch (error) {
        settingsTestDiagnostics.errors.push(
  ...
  test.describe("Language Settings", () => {
    test("Language Settings - Language Selection", async ({ page }) => {
      ...
      try {
        ...
        const languageSelect = await page.isVisible(
          'select[name="language"], [data-testid="language-select"]'
        );
        const englishOption = await page.isVisible(
          'option[value="en"], [data-testid="english"]'
        );
        const spanishOption = await page.isVisible(
          'option[value="es"], [data-testid="spanish"]'
        );
    ...
    test("Settings Save Error Handling", async ({ page }) => {
      ...
      try {
        ...
        if (await emailInput.isVisible()) {
          ...
          console.log(
            "   ⚠️ Email input not found, skipping error handling test"
          );
        }
      } catch (error) {
        settingsTestDiagnostics.errors.push(
    ...
    test("Settings Network Error Handling", async ({ page }) => {
      ...
      try {
        ...
        if (await saveBtn.isVisible()) {
          ...
          const networkError = await page.isVisible(
    ...
    test("Settings Screen Reader Support", async ({ page }) => {
      ...
      try {
        ...
        const formSections = await page
          .locator('[role="group"], [role="region"]')
          .all();
