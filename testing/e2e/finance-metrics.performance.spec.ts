...
test.describe("Finance metrics API performance & diagnostics", () => {
  test("p95 latency under 500ms (local) & diagnostics header parsing", async ({
    request,
  }) => {
    ...
    for (let i = 0; i < iterations; i++) {
      ...
      const resp = await request.get(
        "/api/finance/metrics?months=3&testUser=abbas_ali_rizvi@hotmail.com"
      );
      ...
      const diag =
        resp.headers()["x-finance-diagnostics"] ||
        resp.headers()["X-Finance-Diagnostics"];
    ...
    if (p95 > 800) {
      console.warn("Finance metrics latency elevated", {
        p50: Math.round(p50),
        p95: Math.round(p95),
        max: Math.round(max),
      });
