...
const gatedPaths: Array<{
  path: string;
  requires: "agency" | "enterprise";
  description: string;
}> = [
  {
    path: "/settings/api",
    requires: "enterprise",
    description: "API access settings",
  },
  {
...
test.describe("Feature gate consistency", () => {
  for (const g of gatedPaths) {
    test(`starter project: gated marker present for ${g.description}`, async ({
      page,
    }) => {
      ...
      expect(
        (await upgradeBanner.count()) + (await deniedMarker.count())
      ).toBeGreaterThan(0);
    });
    test(`enterprise project: no gating markers for ${g.description} when requires <= enterprise`, async ({
      page,
    }) => {
    ...
    if (g.requires === "agency") {
      test(`agency project: no gating markers for ${g.description}`, async ({
        page,
      }) => {
