...
async function globalSetup(config: FullConfig) {
    ...
    try {
        ...
        await page.locator('main, [data-testid], #root').first().waitFor({ state: 'visible', timeout: 10000 }).catch(() => { });
...
async function setupTestUser(page: Page) {
    ...
    try {
        ...
        if (!process.env.TEST_USER_EMAIL) process.env.TEST_USER_EMAIL = 'abbas_ali_rizvi@hotmail.com';
        if (!process.env.TEST_USER_PASSWORD) process.env.TEST_USER_PASSWORD = '123456';

        // Light ping to ensure app is responsive and AuthContext can initialize in tests
        await page.goto(`${process.env.TEST_BASE_URL || 'http://localhost:3000'}/login`, { waitUntil: 'domcontentloaded' }).catch(() => { /* non-fatal */ });
        console.log('Test user env configured', { email: process.env.TEST_USER_EMAIL });
    } catch (error) {
...
async function ensureLoggedInAndPersistState(context: BrowserContext, page: Page, baseURL: string) {
    try {
        ...
        await page.goto(`${baseURL.replace(/\/$/, '')}/login`, { waitUntil: 'domcontentloaded' }).catch(() => { });
        const emailInput = page.locator('input[type="email"], input[name*="email"]').first();
        const pwdInput = page.locator('input[type="password"]').first();
        const submit = page.locator('button[type="submit"], [data-testid="login-button"]').first();

        if (await emailInput.isVisible().catch(() => false) && await pwdInput.isVisible().catch(() => false)) {
            ...
            while (Date.now() - start < 5000) {
                ...
                const main = await page.locator('main, [role="main"], [data-testid="dashboard"], [data-testid*="dashboard"]').first().isVisible().catch(() => false);
                if (main) break;
        ...
        console.warn('Failed to persist auth storageState during setup; tests will attempt inline login.', e instanceof Error ? e.message : e);
    }
}

async function persistRoleStates(browser: Browser, baseURL: string) {
    // Define role credentials via env or sensible dev defaults
    const roles: Array<{ role: 'starter' | 'agency' | 'enterprise'; email: string; password: string }> = [
        { role: 'starter', email: process.env.TEST_STARTER_EMAIL || 'starter@rankpilot.com', password: process.env.TEST_STARTER_PASSWORD || '123456' },
        { role: 'agency', email: process.env.TEST_AGENCY_EMAIL || 'agency@rankpilot.com', password: process.env.TEST_AGENCY_PASSWORD || '123456' },
        { role: 'enterprise', email: process.env.TEST_ENTERPRISE_EMAIL || 'enterprise@rankpilot.com', password: process.env.TEST_ENTERPRISE_PASSWORD || '123456' },
    ];
    const outDir = path.resolve(process.cwd(), 'test-results', '.auth');
    fs.mkdirSync(outDir, { recursive: true });
    ...
    for (const r of roles) {
        ...
        try {
            ...
            await pg.goto(`${baseURL.replace(/\/$/, '')}/login`, { waitUntil: 'domcontentloaded' }).catch(() => { });
            ...
            if (emailVisible && pwdVisible) {
                ...
                const submit = pg.locator('button[type="submit"], [data-testid="login-button"]').first();
                if (await submit.isVisible().catch(() => false)) { await submit.click().catch(() => { }); }
                const start = Date.now();
                while (Date.now() - start < 5000) {
                    if (!/\/login/.test(pg.url())) break;
                    const main = await pg.locator('main, [role="main"], [data-testid="dashboard"]').first().isVisible().catch(() => false);
                    if (main) break;
            ...
            console.warn(`Could not save storageState for role ${r.role}, continuing`, e instanceof Error ? e.message : e);
