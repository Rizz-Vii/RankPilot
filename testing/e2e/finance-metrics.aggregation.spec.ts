...
test.describe('Finance metrics aggregation consistency', () => {
    test('MRR trend monotonic period ordering & kpi structural integrity', async ({ request }) => {
        const resp = await request.get('/api/finance/metrics?months=6&testUser=abbas_ali_rizvi@hotmail.com');
        ...
        const kpis: Array<{ key: string; trend?: number[]; value?: number; delta?: number }> = json.kpis || [];
