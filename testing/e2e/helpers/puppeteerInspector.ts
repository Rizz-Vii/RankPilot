...
type HTTPRequestLike = { url(): string; failure(): { errorText?: string } | null };
...
export async function captureWithPuppeteer(url: string, name: string) {
    ...
    const browser: BrowserLike = await puppeteer.launch({ headless: true, args: ['--no-sandbox', '--disable-setuid-sandbox'] });
    const page: PageLike = await browser.newPage();

    const consoleLogs: string[] = [];
    const networkErrors: Array<{ url: string; errorText?: string; status?: number }> = [];

    page.on('console', (msg: ConsoleMessageLike) => {
        if (msg.type() === 'error') consoleLogs.push(`[console.${msg.type()}] ${msg.text()}`);
    });
    ...
    try {
        ...
        if (consoleLogs.length) fs.writeFileSync(`${base}.console.log`, consoleLogs.join('\n'));
        if (networkErrors.length) fs.writeFileSync(`${base}.network.json`, JSON.stringify(networkErrors, null, 2));
        ...
        const state = await page.evaluate(() => {
            try {
                ...
                return {
                    ...
                    errorText: Array.from(document.querySelectorAll('.error,[data-testid="error"]')).slice(0, 3).map(e => e.textContent?.trim()).filter(Boolean),
                };
            } catch {
                return {};
            }
        ...
        logE2EError('puppeteer-capture-failed', { url, error: e instanceof Error ? e.message : String(e) });
    } finally {
        await browser.close();
    }
}
