...
test.describe("AI Visibility accuracy", () => {
  test("single query analysis produces consistent core metrics", async ({
    request,
  }) => {
    const body = {
    ...
    if (Array.isArray(json.visibility) && json.visibility.length) {
      ...
      for (const f of fixtureShape.visibilityFields)
        expect(first).toHaveProperty(f);
      for (const c of fixtureShape.citationInner)
        expect(first.citation).toHaveProperty(c);
      for (const o of fixtureShape.optimizationInner)
        expect(first.optimization).toHaveProperty(o);
    }
  });
});
