...
test.describe("AI Visibility drift tolerance", () => {
  ...
  test("current response within drift thresholds", async ({ request }) => {
    ...
    if (
      Array.isArray(baseline.visibility) &&
      Array.isArray(current.visibility)
    ) {
      if (baseline.visibility.length > 0) {
        expect(current.visibility.length).toBeGreaterThanOrEqual(
          Math.floor(baseline.visibility.length * 0.5)
        );
      }
    }
  });
