...
test.describe('BI Snapshot API + UI contract', () => {
    test('unified.latency routes match top latency table & hints consistency', async ({ page, request }) => {
        ...
        const latency: Record<string, { count: number; totalMs: number; maxMs: number; p95?: number | null }> = json.unified?.latency || {};
        ...
        const visible = await routeCells.first().isVisible().catch(() => false);
