...
async function genericPageOk(page: Page): Promise<boolean> {
  const mainVisible = await page
    .locator('main, [role="main"], [data-testid], #__next')
    .first()
    .isVisible()
    .catch(() => false);
  const upgradeVisible = await page
    .locator(
      '[data-testid="upgrade-cta"], button:has-text("Upgrade"), button:has-text("Subscribe")'
    )
    .first()
    .isVisible()
    .catch(() => false);
...
test.describe("Expanded User Journeys", () => {
  test.skip(
    process.env.E2E_RUN_EXTENDED !== "true",
    "Extended journeys disabled by default; set E2E_RUN_EXTENDED=true to enable"
  );
  test.beforeEach(async ({ page }) => {
    ...
    page.on("response", async (res) => {
      const s = res.status();
      if (s >= 400) logE2EError("bad-response", { url: res.url(), status: s });
    });

    // Attempt to ensure authenticated context for protected routes
    try {
      await retryGoto(page, `${BASE_URL}/login`, {
  ...
  test("Pricing to Checkout (or Billing) journey @smoke", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/pricing`, {
        waitUntil: "domcontentloaded",
      });
      ...
      await dismissBlockingOverlays(page);
      const plansPresent = await page
        .locator('[data-testid="plan"], .plan, [data-testid*="pricing"]')
        .first()
        .isVisible()
        .catch(() => false);
      const cta = page
        .locator(
          '[data-testid="select-plan"], button:has-text("Get started"), button:has-text("Upgrade"), button:has-text("Subscribe")'
        )
        .first();
      ...
      const paymentForm = await page
        .locator(
          '[data-testid="payment"], .payment-form, [data-testid="plan"], [data-testid="upgrade-cta"]'
        )
        .first()
        .isVisible()
        .catch(() => false);
      const ok =
        ...
        paymentForm ||
        plansPresent ||
        (await isLoginVisible(page)) ||
        (await genericPageOk(page));
      expect(ok).toBeTruthy();
    } catch (e) {
      await onFailureCapture(page, "pricing-checkout");
      throw e;
    }
  });
  ...
  test("Profile/Settings update basics @extended", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/profile`, {
        waitUntil: "domcontentloaded",
      }).catch(() => {});
      ...
      await retryGoto(page, `${BASE_URL}/settings`, {
        waitUntil: "domcontentloaded",
      });
      // lightweight title assertion if available
      const settingsTitle = await page.title().catch(() => "");
      if (settingsTitle)
        expect(settingsTitle).toMatch(/rankpilot|settings|profile/i);
      const settingsVisible = await page
        .locator(
          '[data-testid="settings"], [data-testid*="settings"], main, [role="main"]'
        )
        .first()
        .isVisible()
        .catch(() => false);
      const gate = await page
        .locator('[data-testid="upgrade-cta"], button:has-text("Upgrade")')
        .first()
        .isVisible()
        .catch(() => false);
      const ok =
      ...
      await onFailureCapture(page, "profile-settings");
      throw e;
    }
  });
  ...
  test("Integrations Hub presence and actions @extended", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/integrations`, {
        waitUntil: "domcontentloaded",
      }).catch(() => {});
      ...
      if (!/\/integrations/.test(page.url())) {
        await retryGoto(page, `${BASE_URL}/integration-hub`, {
          waitUntil: "domcontentloaded",
        }).catch(() => {});
      }
      // network sanity: ensure last response status is not catastrophic (non-fatal)
      const resp = await page
        .waitForResponse(() => true, { timeout: 2000 })
        .catch(() => null);
      if (resp)
        logE2EError("telemetry", {
          route: "/integrations",
          status: resp.status(),
        });
      const hub = page
        .locator(
          '[data-testid="integration-hub"], [data-testid*="integration"], [data-testid^="kpi"], [data-testid="chart"], table, main'
        )
        .first();
      const hasHub = await hub.isVisible().catch(() => false);
      const connectBtn = page
        .locator('button:has-text("Connect"), [data-testid*="connect"]')
        .first();
      ...
      await onFailureCapture(page, "integration-hub");
      throw e;
    }
  });
  ...
  test("NeuroSEO suite navigation @extended", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/neuroseo`, {
        waitUntil: "domcontentloaded",
      });
      const links = [
      ...
      for (const path of links) {
        await retryGoto(page, `${BASE_URL}${path}`, {
          waitUntil: "domcontentloaded",
        });
        const content = await page
          .locator(
            '[data-testid="section"], [data-testid^="kpi"], [data-testid*="neuro"], main'
          )
          .first()
          .isVisible()
          .catch(() => false);
        const gate = await page
          .locator('button:has-text("Upgrade"), [data-testid="upgrade-cta"]')
          .first()
          .isVisible()
          .catch(() => false);
        sawContent = sawContent || content;
      ...
      await onFailureCapture(page, "neuroseo-suite");
      throw e;
    }
  });
  ...
  test("Finance revenue view (gate-aware) @extended", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/finance/revenue`, {
        waitUntil: "domcontentloaded",
      });
      const chart = await page
        .locator(
          '[data-testid="chart"], .chart, [data-testid*="revenue"], [data-testid*="finance"]'
        )
        .first()
        .isVisible()
        .catch(() => false);
      ...
      await onFailureCapture(page, "finance-revenue");
      throw e;
    }
  });
  ...
  test("Team workspace navigation (gate-aware) @extended", async ({ page }) => {
    try {
      await retryGoto(page, `${BASE_URL}/team`, {
        waitUntil: "domcontentloaded",
      });
      const teamMain = await page
        .locator('[data-testid="team"], [data-testid*="team"], main')
        .first()
        .isVisible()
        .catch(() => false);
      const gate = await page
      ...
      await onFailureCapture(page, "team-workspace");
      throw e;
    }
  });
});
