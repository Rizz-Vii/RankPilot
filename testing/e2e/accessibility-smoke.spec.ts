...
test.describe('a11y smoke', () => {
    test('login page has no serious/critical violations', async ({ page }) => {
        ...
        if (serious.length) {
            console.warn('a11y serious/critical', serious.map(v => ({ id: v.id, impact: v.impact, nodes: v.nodes.length })));
        }
    ...
    test('dashboard or post-login surface is reachable', async ({ page }) => {
        ...
        const mainVisible = await page.locator('main, [role="main"], [data-testid="dashboard"]').first().isVisible().catch(() => false);
