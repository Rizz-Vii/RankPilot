...
test.describe('Finance metrics deterministic aggregation contract', () => {
    ...
    test('exact MRR trend + delta after deterministic seeding', async ({ request }) => {
        const { periods } = await seedDeterministicFinance(request, email);
        let json: any = null;
        for (let i = 0; i < 5; i++) {
            const r = await request.get(`/api/finance/metrics?months=6&testUser=${encodeURIComponent(email)}`);
            if (r.ok()) json = await r.json();
            const series: Array<{ period: string; mrr: number }> = json?.mrrSeries || [];
            if (series.filter(s => periods.includes(s.period)).length === periods.length) break;
            await new Promise(res => setTimeout(res, 200));
        }
        expect(json).toBeTruthy();
        const series: Array<{ period: string; mrr: number }> = json.mrrSeries || [];
        const seeded = series.filter(s => periods.includes(s.period)).sort((a, b) => a.period.localeCompare(b.period));
        ...
        if (mrrKpi) {
            const paidSeries = series.filter(s => s.mrr > 0).sort((a, b) => a.period.localeCompare(b.period));
            if (mrrKpi.value === 0) {
