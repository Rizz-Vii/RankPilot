...
test.describe('settings + RBAC smoke', () => {
    test('starter RBAC: shows upgrade banner and hides enterprise-only controls', async ({ page }) => {
        ...
        expect(bannerVisible || deniedVisible || enterpriseCount === 0).toBeTruthy();
    });
    ...
    test('profile/settings sections are navigable', async ({ page }) => {
        ...
        for (const t of tabs) {
            if (await t.isVisible().catch(() => false)) {
                ...
                const panel = page.locator('[role="tabpanel"], section[aria-labelledby]');
