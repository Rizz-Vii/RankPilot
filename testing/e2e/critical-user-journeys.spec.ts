...
async function dismissBlockingOverlays(page: Page) {
  ...
  try {
    ...
    const overlay = page
      .locator('.fixed.inset-0, [role="dialog"], .modal, .sheet')
      .first();
...
test.describe("Critical Journeys", () => {
  test.beforeEach(async ({ page }) => {
    ...
    page.on("response", async (res) => {
      ...
      if (status >= 400)
        logE2EError("bad-response", { url: res.url(), status });
    });
  });

  test("Home page loads with correct title and key elements", async ({
    page,
  }) => {
    try {
      ...
      const hero = page
        .locator('main, [data-testid="hero"], [data-testid="home"]')
        .first();
  ...
  test("Login form works and navigates to dashboard (or shows auth error) @smoke", async ({
    page,
  }) => {
    try {
      await page.goto(`${BASE_URL}/login`, { waitUntil: "domcontentloaded" });
      // If storageState already authenticated, we might be redirected or see user UI; accept this
      const alreadyAuthed = /(dashboard|profile|settings|onboarding)/.test(
        page.url()
      );
      if (alreadyAuthed) {
      ...
      const redirected = /(dashboard|profile|onboarding|settings)/.test(
        page.url()
      );
      const authErrorVisible = await page
        .locator('[role="alert"], .text-destructive-foreground')
        .first()
        .isVisible()
        .catch(() => false);
  ...
  test("Dashboard navigation shows sections @smoke", async ({ page }) => {
    try {
      ...
      try {
        await retryGoto(page, `${BASE_URL}/dashboard`, {
        ...
        if (emailVisible2 && pwdVisible2) {
          ...
          if (await loginBtn2.isVisible().catch(() => false)) {
            try {
              ...
              await dismissBlockingOverlays(page);
              await loginBtn2.click({ force: true });
            }
          }
      ...
      const content = page
        .locator(
          '[data-testid="dashboard-content"], [data-testid^="kpi"], [data-testid="chart"], table, main'
        )
        .first();
      const upgradeCta = page
        .locator(
          'button:has-text("Upgrade"), button:has-text("Subscribe"), [data-testid="upgrade-cta"]'
        )
        .first();
      ...
      const sectionOrKpi = page
        .locator(
          '.section-content, [data-testid="section"], [data-testid^="kpi"]'
        )
        .first();
  ...
  test("Form submission (contact or outreach) works and returns 2xx @extended", async ({
    page,
  }) => {
    try {
      ...
      const targets = [`${BASE_URL}/sales/outreach`, `${BASE_URL}/contact`];
      let navigated = false;
      ...
      const textInputs = page.locator(
        'input[type="text"], input[type="email"], input[name*="email"], textarea'
      );
      ...
      const submit = page
        .locator(
          'button[type="submit"], [data-testid="submit"], button:has-text("Send"), button:has-text("Send Calls"), button:has-text("Subscribe")'
        )
        .first();
  ...
  test("Health API returns 200 and KPIs shape is valid when present @smoke", async ({
    request,
    page,
  }) => {
    try {
