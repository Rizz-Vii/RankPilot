...
test.describe('@rbac Upgrade CTA redirects to billing', () => {
    test('starter user hitting enterprise-gated page sees Upgrade and routes to billing', async ({ page }, testInfo) => {
        const projectName = testInfo.project.name || '';
        test.skip(!/starter/i.test(projectName), 'Requires chromium-starter project with starter storageState');
    ...
    test('agency user hitting enterprise-gated page sees Upgrade and routes to billing', async ({ page }, testInfo) => {
        const projectName = testInfo.project.name || '';
        test.skip(!/agency/i.test(projectName), 'Requires chromium-agency project with agency storageState');
        ...
        // Auth guard
        if (/\/login/.test(page.url())) {
            const email = process.env.TEST_AGENCY_EMAIL || process.env.TEST_USER_EMAIL || 'abbas_ali_rizvi@hotmail.com';
            const password = process.env.TEST_AGENCY_PASSWORD || process.env.TEST_USER_PASSWORD || '123456';
            await AuthHelper.login(page, email, password);
            await page.goto('/marketing', { waitUntil: 'domcontentloaded' });
            await PageHelper.waitForPageLoad(page);
        }
    ...
    test('enterprise user can access enterprise-gated page without upgrade redirect', async ({ page }, testInfo) => {
        const projectName = testInfo.project.name || '';
        test.skip(!/enterprise/i.test(projectName), 'Requires chromium-enterprise project with enterprise storageState');
        ...
        // Auth guard
        if (/\/login/.test(page.url())) {
            const email = process.env.TEST_ENTERPRISE_EMAIL || process.env.TEST_USER_EMAIL || 'abbas_ali_rizvi@hotmail.com';
            const password = process.env.TEST_ENTERPRISE_PASSWORD || process.env.TEST_USER_PASSWORD || '123456';
            await AuthHelper.login(page, email, password);
            await page.goto('/marketing', { waitUntil: 'domcontentloaded' });
            await PageHelper.waitForPageLoad(page);
        }
        ...
        const hasMain = await page.locator('main, [role="main"], [data-testid], h1, h2').first().isVisible().catch(() => false);
