...
export default async function globalSetupHooks(config: FullConfig) {
    const baseURL = (config.projects[0]?.use as any)?.baseURL || process.env.TEST_BASE_URL || 'http://localhost:3000';
    ...
    for (const engine of browsers) {
        ...
        page.on('requestfailed', (req) => {
            logE2EError('requestfailed', { url: req.url(), failure: req.failure(), method: req.method() });
        });
        ...
        try {
            ...
            logE2EError('globalSetupHooks:navigate-failed', { baseURL, error: e instanceof Error ? e.message : String(e) });
        } finally {
            await browser.close();
        }
    }
