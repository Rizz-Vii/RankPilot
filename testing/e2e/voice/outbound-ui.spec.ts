...
test.describe("voice agent outbound UI", () => {
  ...
  test("accepts a number, calls with a pitch, and saves appointment", async ({
    page,
    baseURL,
    request,
  }) => {
    ...
    const hasPhoneField = await phoneField
      ...
      .catch(() => false);
    if (!hasPhoneField)
      test.skip(true, "voice page gated or input not available in this env");

    await phoneField.fill("+1 555 0101");
    await page
      .getByLabel("Pitch")
      .fill("Hi! Quick intro call to share RankPilot.");
