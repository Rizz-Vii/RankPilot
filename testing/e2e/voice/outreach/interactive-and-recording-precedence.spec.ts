...
test.describe('Sales Outreach: interactive & recording precedence', () => {
    ...
    test.beforeEach(async ({ page }) => {
        ...
        await page.context().setExtraHTTPHeaders({
            'x-probe-token': process.env.CRAWL_PROBE_TOKEN || '8ab3b3a95a0d9cf1b5bb2b61be5e3981',
        });
    });

    test('interactive toggle triggers Gather intent', async ({ page, baseURL }) => {
        ...
        if (!(await recipients.count())) test.skip(true, 'Outreach form not visible (feature-gated)');
        ...
        await page.route('**/api/voice/outbound', async (route) => {
            ...
            await route.fulfill({
                ...
                body: JSON.stringify({ callSid: 'TEST_SID', from: '+15551230000', callStatus: 'queued' })
            });
        });
    ...
    test('recordingUrl takes precedence over script', async ({ page, baseURL }) => {
        if (!baseURL) test.skip(true, 'no baseURL');
        ...
        await page.getByTestId('outreach-script').fill('This should be ignored by recording');
        ...
        await page.route('**/api/voice/outbound', async (route) => {
            ...
            await route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ callSid: 'TEST_SID_2', from: '+15551230000', callStatus: 'queued' })
            });
        });
