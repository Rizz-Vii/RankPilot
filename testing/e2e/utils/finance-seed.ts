 ...
 */
export async function seedDeterministicFinance(request: APIRequestContext, email: string) {
    const now = new Date();
    ...
    for (let idx = 0; idx < periods.length; idx++) {
        ...
        await request.post(`/api/test/finance/seed-invoice?testUser=${encodeURIComponent(email)}&amount=${amount}&status=paid&period=${p}`).catch(() => null);
    }
    // Current period draft outstanding invoice
    const currentPeriod = now.toISOString().slice(0, 7);
    await request.post(`/api/test/finance/seed-invoice?testUser=${encodeURIComponent(email)}&amount=75&status=draft&period=${currentPeriod}`).catch(() => null);
...
export async function seedExtendedBiLatency(request: APIRequestContext) {
    const core = ['/api/bi/snapshot', '/api/bi/snapshot?route=/api/finance/metrics'];
    const extras = [
