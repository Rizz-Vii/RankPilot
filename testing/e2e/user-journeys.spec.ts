    ...
    test.describe('New User Registration & Onboarding', () => {
        ...
        test('Complete User Registration Flow', async ({ page }) => {
            ...
            try {
                ...
                const confirmPasswordInput = await page.locator('input[name="confirmPassword"], input[placeholder*="confirm"]').first();

                if (await nameInput.isVisible()) await nameInput.fill(TEST_DATA.user.name);
                if (await emailInput.isVisible()) await emailInput.fill(TEST_DATA.user.email);
                if (await passwordInput.isVisible()) await passwordInput.fill(TEST_DATA.user.password);
                if (await confirmPasswordInput.isVisible()) await confirmPasswordInput.fill(TEST_DATA.user.password);

                // Submit registration
                const registerBtn = await page.locator('button[type="submit"], [data-testid="register"]').first();
                ...
                if (await registerBtn.isVisible()) {
                    ...
                    const successMsg = await page.isVisible('.success, [data-testid="registration-success"]');

                    console.log(`   Registration Successful: ${onDashboard || successMsg}`);

                    expect(onDashboard || successMsg).toBe(true);
                    console.log('   ✅ User registration flow functional');
                }
            } catch (error) {
                e2eTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
        ...
        test('User Onboarding Process', async ({ page }) => {
            ...
            try {
                ...
                const steps = await page.locator('.onboarding-step, [data-testid*="step"]').all();
                console.log(`   Onboarding Steps Found: ${steps.length}`);

                for (let i = 0; i < Math.min(steps.length, 3); i++) {
                    const step = steps[i];
                    const nextBtn = await page.locator('button[data-testid*="next"], .next-step').first();
                ...
                const completed = await page.isVisible('.onboarding-complete, [data-testid="onboarding-complete"]');
    ...
    test.describe('Keyword Research Workflow', () => {
            ...
            try {
                ...
                if (await keywordInput.isVisible()) {
                    ...
                    if (await searchBtn.isVisible()) {
                        ...
                        const resultsVisible = await page.isVisible('[data-testid="results"], .keyword-results, .results-table');
                        console.log(`   Keyword Results Visible: ${resultsVisible}`);

                        if (resultsVisible) {
                            // Test saving keywords
                            const saveBtn = await page.locator('button[data-testid="save-keywords"], .save-btn').first();

                            if (await saveBtn.isVisible()) {
                                await saveBtn.click();

                                const savedMsg = await page.isVisible('.saved, [data-testid="saved"]');
        ...
        test('Keyword Analysis & Export', async ({ page }) => {
            ...
            try {
                ...
                const analyzeBtn = await page.locator('button[data-testid="analyze"], .analyze-btn').first();
                ...
                if (await analyzeBtn.isVisible()) {
                    ...
                    const analysisVisible = await page.isVisible('.analysis, [data-testid="analysis"]');
                    console.log(`   Analysis Results Visible: ${analysisVisible}`);

                    if (analysisVisible) {
                        // Test export functionality
                        const exportBtn = await page.locator('button[data-testid="export"], .export-btn').first();

                        if (await exportBtn.isVisible()) {
                            // Note: In real test, would need to handle download
                            await exportBtn.click();

                            const exportMsg = await page.isVisible('.exported, [data-testid="exported"]');
    ...
    test.describe('SEO Audit Workflow', () => {
            ...
            try {
                ...
                if (await urlInput.isVisible()) {
                    ...
                    if (await auditBtn.isVisible()) {
                        ...
                        const auditResults = await page.isVisible('.audit-results, [data-testid="audit-results"]');
                        const scoreVisible = await page.isVisible('.score, [data-testid="score"]');
        ...
        test('Audit Report Generation & Sharing', async ({ page }) => {
            ...
            try {
                ...
                const reportBtn = await page.locator('button[data-testid="generate-report"], .report-btn').first();
                ...
                if (await reportBtn.isVisible()) {
                    ...
                    const reportVisible = await page.isVisible('.report, [data-testid="report"]');
                    console.log(`   Report Generated: ${reportVisible}`);

                    if (reportVisible) {
                        // Test sharing functionality
                        const shareBtn = await page.locator('button[data-testid="share"], .share-btn').first();

                        if (await shareBtn.isVisible()) {
                            await shareBtn.click();

                            const shareOptions = await page.isVisible('.share-options, [data-testid="share-options"]');
    ...
    test.describe('Project Management Workflow', () => {
        ...
        test('Create & Configure Project', async ({ page }) => {
            ...
            try {
                ...
                const nameInput = await page.locator('input[name="name"], input[placeholder*="project"]').first();
                const urlInput = await page.locator('input[name="url"], input[type="url"]').first();

                if (await nameInput.isVisible()) await nameInput.fill(TEST_DATA.project.name);
                if (await urlInput.isVisible()) await urlInput.fill(TEST_DATA.project.url);

                // Submit project creation
                const createBtn = await page.locator('button[type="submit"], [data-testid="create-project"]').first();
                ...
                if (await createBtn.isVisible()) {
                    ...
                    const projectCreated = await page.isVisible('.project-created, [data-testid="project-created"]');
        ...
        test('Project Settings & Team Management', async ({ page }) => {
            ...
            try {
                ...
                if (await projectLink.isVisible()) {
                    ...
                    const settingsTab = await page.locator('button[data-testid="settings-tab"], .settings-tab').first();

                    if (await settingsTab.isVisible()) {
                        await settingsTab.click();

                        // Test team management
                        const addMemberBtn = await page.locator('button[data-testid="add-member"], .add-member').first();

                        if (await addMemberBtn.isVisible()) {
                            await addMemberBtn.click();

                            const memberForm = await page.isVisible('.member-form, [data-testid="member-form"]');
    ...
    test.describe('Dashboard & Analytics Workflow', () => {
        ...
        test('Dashboard Overview & Navigation', async ({ page }) => {
            ...
            try {
                ...
                const kpiCards = await page.locator('.kpi-card, [data-testid="kpi"]').all();
                const charts = await page.locator('.chart, [data-testid="chart"]').all();
                const navigation = await page.locator('nav, [data-testid="nav"]').first();
                ...
                if (navLinks.length > 0) {
                    ...
                    const sectionLoaded = await page.isVisible('.section-content, [data-testid="section"]');
        ...
        test('Analytics Data Interaction', async ({ page }) => {
            ...
            try {
                ...
                const chart = await page.locator('.chart, [data-testid="chart"]').first();

                if (await chart.isVisible()) {
                    // Try to interact with chart (hover, click)
                    await chart.hover();

                    const tooltip = await page.isVisible('.tooltip, [data-testid="tooltip"]');
                    console.log(`   Chart Tooltip: ${tooltip}`);

                    // Test date range selector
                    const datePicker = await page.locator('.date-picker, [data-testid="date-range"]').first();

                    if (await datePicker.isVisible()) {
                        await datePicker.click();

                        const dateOptions = await page.isVisible('.date-options, [data-testid="date-options"]');
                        console.log(`   Date Options Visible: ${dateOptions}`);
                    }

                    console.log('   ✅ Analytics data interaction functional');
                } else {
                    console.log('   ⚠️ No charts found, skipping analytics interaction test');
                }
            } catch (error) {
                e2eTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
    ...
    test.describe('Subscription & Billing Workflow', () => {
        ...
        test('Subscription Upgrade Process', async ({ page }) => {
            ...
            try {
                ...
                const upgradeBtn = await page.locator('button[data-testid="upgrade"], .upgrade-btn').first();
                ...
                if (await upgradeBtn.isVisible()) {
                    ...
                    const paymentForm = await page.isVisible('.payment-form, [data-testid="payment"]');
        ...
        test('Billing History & Invoice Management', async ({ page }) => {
            ...
            try {
                ...
                const invoices = await page.locator('.invoice, [data-testid="invoice"]').all();
                const downloadBtn = await page.locator('button[data-testid="download"], .download-btn').first();

                console.log(`   Invoices Found: ${invoices.length}`);
                console.log(`   Download Button Visible: ${await downloadBtn.isVisible()}`);

                if (invoices.length > 0) {
                    // Test invoice download
                    await downloadBtn.click();

                    const downloadMsg = await page.isVisible('.downloaded, [data-testid="downloaded"]');
    ...
    test.describe('Cross-Feature Integration', () => {
        ...
        test('Keyword Research to SEO Audit Integration', async ({ page }) => {
            ...
            try {
                ...
                const suggestedKeywords = await page.isVisible('.suggested-keywords, [data-testid="suggested"]');
        ...
        test('Dashboard to Detailed Analytics Flow', async ({ page }) => {
            ...
            try {
                ...
                // Click on a KPI card
                const kpiCard = await page.locator('.kpi-card, [data-testid="kpi"]').first();
                ...
                if (await kpiCard.isVisible()) {
                    ...
                    const detailedView = await page.isVisible('.detailed-view, [data-testid="detailed"]');
                    const onAnalyticsPage = page.url().includes('/analytics');

                    console.log(`   Detailed Analytics View: ${detailedView || onAnalyticsPage}`);
            ...
            try {
                ...
                if (await keywordInput.isVisible()) {
                    ...
                    if (await searchBtn.isVisible()) {
                        ...
                        const retryBtn = await page.isVisible('button[data-testid="retry"], .retry-btn');
