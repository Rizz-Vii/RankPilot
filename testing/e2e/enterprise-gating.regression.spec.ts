...
test.describe("Enterprise gating regression", () => {
  test("enterprise user sees white-label page without upgrade markers", async ({
    page,
  }) => {
    const isEnterpriseProject = /enterprise/i.test(
    ...
    await expect(
      page.getByRole("heading", { name: /white-label/i })
    ).toBeVisible();
  });
});
