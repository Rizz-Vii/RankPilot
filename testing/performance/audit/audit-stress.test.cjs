...
async function runOne(i){
  try {
    const res = await audit.__testRunSeoAudit({ url: `https://stress${i}.example.com`, depth:1, plan:'admin', teamId:'teamStress', debugTeamLimit:1000 }, { uid: `userStress_${i}` });
    return { ok: true, res };
  } catch (e) {
...
describe('audit stress 20 parallel', () => {
  it('completes with <5% error rate and captures p95 crawl time', async function(){
    ...
    const N=20; const results = await Promise.allSettled(Array.from({length:N}, (_,i)=>runOne(i)));
  const settledValues = results.filter(r=>r.status==='fulfilled').map(r=>r.value);
  const successes = settledValues.filter(v=>v.ok);
  const failures = settledValues.filter(v=>!v.ok).concat(results.filter(r=>r.status==='rejected'));
  ...
  const crawlTimes = successes.map(s=>s.res?.timings?.crawl_time_ms ?? 0).sort((a,b)=>a-b);
  const p95Index = Math.min(crawlTimes.length-1, Math.floor(crawlTimes.length*0.95));
  ...
  assert.ok(deltaMB < 40, `Memory RSS delta too high: ${deltaMB.toFixed(2)}MB (limit <40MB)`);
  // Optionally log for diagnostics
  console.log(JSON.stringify({ N, failureRate, p95, min: crawlTimes[0], max: crawlTimes[crawlTimes.length-1], rssDeltaMB: deltaMB.toFixed(2) }));
  });
});
