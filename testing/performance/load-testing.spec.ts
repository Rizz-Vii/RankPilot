...
const ENABLE_PERF = process.env.E2E_RUN_PERF === '1' || process.env.RUN_PERF === '1';
test.skip(!ENABLE_PERF, 'Performance & load tests are disabled by default. Set E2E_RUN_PERF=1 to enable.');
...
const TEST_DATA = {
    ...
    scenarios: {
        navigation: ['/dashboard', '/keyword-tool', '/seo-audit', '/settings/profile'],
        actions: ['search', 'analyze', 'save', 'export'],
        dataSizes: ['small', 'medium', 'large']
    }
};

const performanceTestDiagnostics = { errors: [] as string[], metrics: {} as Record<string, any> };
    ...
    test.describe('Core Web Vitals', () => {
        ...
        test('First Contentful Paint (FCP)', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   FCP Threshold: ${TEST_DATA.performance.thresholds.firstPaint}ms`);
                console.log(`   FCP Passed: ${fcp <= TEST_DATA.performance.thresholds.firstPaint}`);

                expect(fcp).toBeLessThanOrEqual(TEST_DATA.performance.thresholds.firstPaint);
                console.log('   ✅ First Contentful Paint within threshold');
            } catch (error) {
                performanceTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
        ...
        test('Largest Contentful Paint (LCP)', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   LCP Threshold: ${TEST_DATA.performance.thresholds.largestContentfulPaint}ms`);
                console.log(`   LCP Passed: ${lcp <= TEST_DATA.performance.thresholds.largestContentfulPaint}`);

                expect(lcp).toBeLessThanOrEqual(TEST_DATA.performance.thresholds.largestContentfulPaint);
        ...
        test('First Input Delay (FID)', async ({ page }) => {
            ...
            try {
                ...
                if (await button.isVisible()) {
                    ...
                    console.log(`   FID Threshold: ${TEST_DATA.performance.thresholds.firstInputDelay}ms`);
                    console.log(`   FID Passed: ${fid <= TEST_DATA.performance.thresholds.firstInputDelay}`);

                    expect(fid).toBeLessThanOrEqual(TEST_DATA.performance.thresholds.firstInputDelay);
        ...
        test('Cumulative Layout Shift (CLS)', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   CLS Threshold: ${TEST_DATA.performance.thresholds.cumulativeLayoutShift}`);
                console.log(`   CLS Passed: ${cls <= TEST_DATA.performance.thresholds.cumulativeLayoutShift}`);

                expect(cls).toBeLessThanOrEqual(TEST_DATA.performance.thresholds.cumulativeLayoutShift);
        ...
        test('Page Load Times', async ({ page }) => {
            ...
            try {
                for (const pagePath of TEST_DATA.scenarios.navigation) {
                    ...
                    console.log(`   ${pagePath} Passed: ${loadTime <= TEST_DATA.performance.thresholds.pageLoad}`);

                    expect(loadTime).toBeLessThanOrEqual(TEST_DATA.performance.thresholds.pageLoad);
                }
    ...
    test.describe('Load Testing', () => {
        ...
        test('Light Load Simulation', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   Light Load - Avg Response Time: ${results.avgResponseTime}ms`);
        ...
        test('Medium Load Simulation', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   Medium Load - Avg Response Time: ${results.avgResponseTime}ms`);
        ...
        test('Stress Test - Heavy Load', async ({ page }) => {
            ...
            try {
                ...
                console.log(`   Heavy Load - Avg Response Time: ${results.avgResponseTime}ms`);
        ...
        test('Memory Leak Detection', async ({ page }) => {
            ...
            try {
                ...
                for (let i = 0; i < 5; i++) {
                    ...
                    await page.goto(`${RANKPILOT_APP_URL}${TEST_DATA.scenarios.navigation[i % TEST_DATA.scenarios.navigation.length]}`);
        ...
        test('Resource Cleanup', async ({ page }) => {
            ...
            try {
                ...
                const resourceGrowth = {
                    ...
                    stylesheets: finalResources.stylesheets - initialResources.stylesheets,
        ...
        test('Data Size Performance', async ({ page }) => {
            ...
            try {
                for (const size of TEST_DATA.scenarios.dataSizes) {
                    ...
                    performanceTestDiagnostics.metrics[`${size}_data_time`] = processingTime;
...
async function simulateLoad(page: any, concurrentUsers: number, duration: number) {
    const results = {
        totalRequests: 0,
    ...
    while (Date.now() < endTime) {
        try {
            ...
            const actions = [
                ...
                () => page.locator('input').first().type('test').catch(() => { })
            ];
