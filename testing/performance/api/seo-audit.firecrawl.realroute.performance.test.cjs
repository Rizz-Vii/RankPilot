...
Module._load = function (request, parent, isMain) {
  ...
  if (request === "@/lib/middleware/provenance") {
...
describe("firecrawl real route performance (T13)", () => {
  ...
  it("20 parallel runs meet p95 latency & error-rate thresholds and no significant memory leak", async function () {
    ...
    await Promise.all(
      Array.from({ length: runs }, async (_, i) => {
        ...
        const req = new NextRequestMock(
          `http://localhost/api/seo-audit/firecrawl?url=${encodeURIComponent(url)}&depth=1&limit=1`
        );
    ...
    expect(p95Latency).to.be.below(
      150,
      `p95 latency too high: ${p95Latency}ms`
    );
    ...
    expect(memDiff).to.be.below(
      5 * 1024 * 1024,
      `heap grew by ${(memDiff / 1024 / 1024).toFixed(2)}MB`
    );
  });
});
