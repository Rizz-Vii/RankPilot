...
async function fetchIdToken(baseURL: string): Promise<string | null> {
  try {
    const customTokRes = await fetch(
      `${baseURL}/api/test/auth/custom-token`
    ).catch(() => undefined);
    if (!customTokRes || !customTokRes.ok) return null;
    const customTokJson = (await customTokRes.json().catch(() => ({}))) as {
      token?: string;
    };
    ...
    const idTokJson = (await idTokRes.json().catch(() => ({}))) as {
      idToken?: string;
    };
...
test.describe("Sales Sequences API", () => {
  ...
  const probeToken =
    ...
    process.env.RL_PROBE_TOKEN ||
    "8ab3b3a95a0d9cf1b5bb2b61be5e3981";
  ...
  test("GET /api/sales/sequences returns list or forbidden with auth", async ({
    baseURL,
  }) => {
  ...
  test("POST create + run sequence (happy path, best-effort with testMode)", async ({
    baseURL,
  }) => {
    const req = await pwRequest.newContext({ baseURL });
    const headers = {
    ...
    const createRes = await req.post("/api/sales/sequences", {
      ...
      data: {
        ...
        steps: [{ id: "step1", type: "call", delayMinutes: 0 }],
        targets: [{ id: "t1", name: "Test Target", phone: "+15555550123" }],
        status: "draft",
      },
    });
    ...
    if (createStatus !== 201) {
      ...
      test.info().annotations.push({
        type: "skip-followup",
        description: `Create not permitted (status ${createStatus}); skipping run.`,
      });
      return;
    }
  ...
  test("POST create enforces admin permission (permission-denied when enforced)", async ({
    baseURL,
  }) => {
    const req = await pwRequest.newContext({ baseURL });
    const headers = {
