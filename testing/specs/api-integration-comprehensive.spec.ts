...
test.describe("API Integration - Authentication Endpoints", () => {
  ...
  test("POST /api/auth/register - new user registration", async ({
    request,
  }) => {
  ...
  test("POST /api/auth/reset-password - password reset", async ({
    request,
  }) => {
...
test.describe("API Integration - SEO Analysis Endpoints", () => {
  ...
  test("GET /api/seo/competitors - competitor analysis", async ({
    request,
  }) => {
    const response = await request.get(
      "/api/seo/competitors?domain=example.com"
    );
    expect([200, 400, 401, 404]).toContain(response.status());
  });
...
test.describe("API Integration - User Management Endpoints", () => {
  ...
  test("GET /api/users/subscription - subscription status", async ({
    request,
  }) => {
  ...
  test("POST /api/users/upgrade - subscription upgrade", async ({
    request,
  }) => {
...
test.describe("API Integration - Project Management Endpoints", () => {
  ...
  test("GET /api/projects/[id]/analytics - project analytics", async ({
    request,
  }) => {
...
test.describe("API Integration - Reporting Endpoints", () => {
  ...
  test("POST /api/reports/generate - generate custom report", async ({
    request,
  }) => {
  ...
  test("GET /api/reports/export - export report data", async ({ request }) => {
    const response = await request.get(
      "/api/reports/export?format=csv&reportId=123"
    );
    expect([200, 400, 401, 404]).toContain(response.status());
  });
  ...
  test("POST /api/reports/schedule - create scheduled report", async ({
    request,
  }) => {
...
test.describe("API Integration - Webhook & Integration Endpoints", () => {
  ...
  test("POST /api/integrations/google-analytics - GA integration", async ({
    request,
  }) => {
  ...
  test("GET /api/integrations/search-console - Search Console data", async ({
    request,
  }) => {
  ...
  test("GET /api/integrations/health - integrations health check", async ({
    request,
  }) => {
...
test.describe("API Integration - Error Handling & Rate Limiting", () => {
  ...
  test("Rate limiting - multiple rapid requests", async ({ request }) => {
    const promises = Array(10)
      .fill(null)
      .map(() => request.get("/api/test-rate-limit"));
  ...
  test("CORS headers validation", async ({ request }) => {
    ...
    expect(
      headers["access-control-allow-origin"] || headers["content-type"]
    ).toBeTruthy();
  });
  ...
  test("Health endpoint exposes crawler metrics", async ({ request }) => {
    ...
    keys.forEach((k) => {
      expect(
        Object.prototype.hasOwnProperty.call(body.crawler, k)
      ).toBeTruthy();
    });
    // Team quota aggregate may be null early in day but field should exist
    expect(
      Object.prototype.hasOwnProperty.call(body, "teamCrawlerQuota")
    ).toBeTruthy();
  });
});
