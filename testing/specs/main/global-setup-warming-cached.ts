...
async function globalSetup(config: FullConfig) {
  console.log(
    "🌟 Starting Enhanced RankPilot Test Suite with State Caching..."
  );
  ...
  try {
    ...
    throw new Error(
      "Development server not available: " + extractErrorMessage(error)
    );
  }
  ...
  try {
    ...
    await context.storageState({
      path: path.join(cacheDir, "warmed-storage-state.json"),
    });
  } catch (error: unknown) {
    console.log(
      "⚠️ Authentication failed, continuing with warming...",
  ...
  for (const pageConfig of warmingSequence) {
    console.log(
      `🔄 Warming ${pageConfig.name} (${pageConfig.priority} priority)...`
    );
      ...
      if (pageConfig.priority === "heavy") {
        ...
        try {
          await page.waitForSelector(
            '[data-testid*="chart"], .recharts-wrapper, [class*="framer-motion"]',
            {
              timeout: 15000,
            }
          );
  ...
  console.log(
    `📊 Cache manifest saved with ${Object.keys(cacheManifest.pages).length} pages`
  );

  const successfulWarms = warmingResults.filter((r) => r.success);
  console.log(
    `✅ Successfully cached: ${successfulWarms.length}/${warmingResults.length} pages`
  );
