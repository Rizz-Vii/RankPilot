...
test.describe("Health API semantic map aggregate adoption alerts", () => {
  test("adoption alert transitions warn -> cleared", async () => {
    ...
    const parseSemantic = (data: unknown) => {
      ...
      const metrics =
        metricsRaw && typeof metricsRaw === "object"
          ...
          : undefined;
      const unifiedRaw =
        metrics && (metrics as Record<string, unknown>)["unified"];
      const unified =
      ...
      const semantic =
        semanticRaw && typeof semanticRaw === "object"
          ? (semanticRaw as Record<string, unknown>)
          : undefined;
      const alertsRaw = root["alerts"];
      const alerts = Array.isArray(alertsRaw)
        ? (alertsRaw as Array<{ type?: string; level?: string }>)
        : [];
      return {
    ...
    const warn = midParsed.alerts.find(
      (a) => a.type === "semanticMapAggregateAdoption"
    );
    ...
    const cleared = highParsed.alerts.filter(
      (a) => a.type === "semanticMapAggregateAdoption"
    );
