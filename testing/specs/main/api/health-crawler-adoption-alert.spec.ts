...
test.describe('Health API crawler aggregate adoption alerts', () => {
    test('adoption alert transitions warn -> cleared', async () => {
        ...
        const parseCrawler = (data: unknown) => {
            if (!data || typeof data !== 'object') return { hits: 0, fallbacks: 0, adoption: 0, alerts: [] as Array<{ type?: string; level?: string }> };
            const root = data as Record<string, unknown>;
            const kpis = (root.kpis && typeof root.kpis === 'object') ? root.kpis as Record<string, unknown> : undefined;
            const crawler = (kpis && kpis.crawler && typeof kpis.crawler === 'object') ? kpis.crawler as Record<string, unknown> : undefined;
            const alerts = Array.isArray(root.alerts) ? root.alerts as Array<{ type?: string; level?: string }> : [];
            return {
        ...
        const warn = midParsed.alerts.find(a => a.type === 'crawlerAggregateAdoption');
        ...
        const adoptionAlerts = highParsed.alerts.filter(a => a.type === 'crawlerAggregateAdoption');
