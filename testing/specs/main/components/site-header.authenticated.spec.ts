...
test('Site header hides CTA and shows profile when authenticated', async ({ page, baseURL }) => {
    ...
    const cta = page.locator('a[href*="/register"], a:has-text("Start 7‑Day Free Trial")');
    const avatar = page.locator('button[aria-label*="profile"], img[alt*="avatar"], [data-testid="user-avatar"]');
