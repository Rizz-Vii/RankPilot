...
test.describe('Billing UI (FIN-02) - Accessibility & Regression', () => {
    ...
    test('has main landmark, heading structure, usage + history sections', async ({ page }) => {
        await orchestrator.userManager.loginAs('agency');
        await page.goto('/billing');
        await expect(page.locator('main[aria-label="Billing portal main content"]')).toBeVisible();
    ...
    test('keyboard navigation reaches key interactive elements (first 5 tabbable)', async ({ page }) => {
        await orchestrator.userManager.loginAs('agency');
        await page.goto('/billing');
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const el = await page.evaluate(() => {
                ...
                if (!a) return null;
                return a.getAttribute('data-testid') || a.getAttribute('aria-label') || a.textContent?.trim()?.slice(0, 40) || a.tagName;
            });
    ...
    test('pagination load more appends invoices without duplicates (network intercept)', async ({ page }) => {
        await orchestrator.userManager.loginAs('agency');
        ...
        await page.route('/api/billing/invoices*', async (route, req) => {
            ...
            if (url.searchParams.get('cursor')) {
                ...
                return route.fulfill({
                    ...
                    body: JSON.stringify({
                        invoices: [
                            { id: 'inv-extra-1', period: '2025-05', amount: 12, description: 'Extra Invoice 1', date: '2025-05-05T00:00:00.000Z', status: 'paid' },
                            { id: 'inv-extra-2', period: '2025-04', amount: 18, description: 'Extra Invoice 2', date: '2025-04-04T00:00:00.000Z', status: 'paid' }
                        ],
                        hasMore: false
                    })
                });
        ...
        if (loadMoreTriggered) {
            await expect(page.getByTestId('invoice-row').filter({ hasText: 'Extra Invoice 1' })).toBeVisible();
        }
    });

    test('handles payment method API failure gracefully', async ({ page }) => {
        await orchestrator.userManager.loginAs('agency');
        await page.route('/api/billing/payment-method', route => route.fulfill({ status: 500, body: '{}' }));
        await page.goto('/billing');
