...
async function login(page: Page) {
  ...
  try {
    ...
    if (
      await page
        .getByRole("heading", { name: /Finance Dashboard/ })
        .first()
  ...
  await page
    .waitForURL(/dashboard|app|finance/, { timeout: 30000 })
    .catch(() => {});
}

async function pollInvoicesCount(
  page: Page,
  token: string | null,
  timeoutMs = 20000
) {
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    try {
      const url = token
        ? "/api/finance/metrics"
        : "/api/finance/metrics?testUser=admin@rankpilot.com";
...
test.describe("Finance Live Metrics (live KPI emergence)", () => {
  ...
  test("seeding invoice produces live KPIs and hides mock banner", async ({
    page,
  }) => {
    await login(page);
    ...
    try {
      ...
      if (simple.ok()) {
        const js = await simple.json();
        if (!("version" in js))
          console.warn("[finance-live-metrics] simple health missing version");
      }
    } catch {}
    // Ensure mocks allowed initially so banner may appear (consistent state), then seed invoice.
    await page.addInitScript(() => {
      localStorage.setItem("allowFinanceMocks", "true");
    });
    await page.reload();
    ...
    if (token) {
      await page.request.post(
        "/api/test/finance/seed-invoice?status=paid&amount=123",
        { headers: { Authorization: `Bearer ${token}` } }
      );
    } else {
      await page.request.post(
        "/api/test/finance/seed-invoice?status=paid&amount=123&testUser=admin@rankpilot.com"
      );
    }
    ...
    if (!gotInvoice)
      console.warn(
        "[finance-live-metrics] invoicesCount missing after poll window; proceeding to UI validation"
      );
    await expect(
