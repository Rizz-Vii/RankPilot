...
async function login(page: Page) {
    ...
    await page.waitForURL(/dashboard|app|finance/, { timeout: 20000 }).catch(() => { });
}

test.describe('Finance Mock Gating', () => {
    test('mock banner visible under mock fallback then hidden after override', async ({ page }) => {
        ...
        await Promise.race([
            ...
            page.waitForSelector('[aria-label="Loading metric"]', { state: 'detached', timeout: 7000 }).catch(() => { })
        ]).catch(() => { });
