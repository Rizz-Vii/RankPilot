...
test.describe('Chat happy path (stream + cancel)', () => {
    ...
    test('customer chat streams response and supports cancel', async ({ page }) => {
        ...
        const input = page.locator('input[placeholder="Ask about your SEO performance..."]');
        ...
        // Cancel streaming
        const cancel = page.locator('button[aria-label="Cancel streaming response"]');
        await cancel.click();

        // Verify response contains canceled marker
        await expect(page.locator('.prose')).toContainText('Canceled', { timeout: 10000 });
    });

    test('customer chat completes stream and shows suggestion chips', async ({ page }) => {
        await orchestrator.userManager.loginAs('starter');
        ...
        const suggestion = page.locator('button[aria-label^="Suggestion:"]').first();
