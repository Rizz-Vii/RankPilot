...
test.describe('Navigation visibility and gating', () => {
    test('Locked enterprise marketing items are visible but disabled for starter', async ({ page, baseURL }) => {
        await page.goto(baseURL!);

        // Expand "Marketing Automation" group (match aria-label from EnhancedAppNav)
        await page.getByRole('button', { name: /Marketing Automation navigation group/i }).click();
    ...
    test('Team Dashboard is visible; Team Projects remains disabled for starter', async ({ page, baseURL }) => {
        await page.goto(baseURL!);

        // Management is default-expanded; verify Team Dashboard visibility directly
        await expect(page.getByRole('link', { name: /Team Dashboard/i })).toBeVisible();

        // Open Team Collaboration group and verify Projects is disabled (enterprise)
        await page.getByRole('button', { name: /Team Collaboration navigation group/i }).click();
