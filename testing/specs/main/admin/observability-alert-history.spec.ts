...
test.describe("@observability Observability Alert History", () => {
  test("seeds alert snapshot & displays alert history row", async ({
    page,
  }) => {
    ...
    const seed = await page.request.get(
      "/api/test/observability/seed-alert?type=provenanceCoverage&level=warn&value=95&ma7=96"
    );
    if (!seed.ok())
      test.skip(true, "Seeding endpoint unavailable (production environment)");
    ...
    const provenanceCell = rows
      .locator("td")
      .filter({ hasText: "provenanceCoverage" })
      .first();
