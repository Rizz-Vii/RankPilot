...
const sparklineIds = [
  "sparkline-provenance",
  "sparkline-latencyP95",
  "sparkline-cost",
  "sparkline-crawlerAdoption",
  "sparkline-semanticAdoption",
  "sparkline-teamUtil",
];
...
test.describe("@observability Admin Observability MA7 Overlays", () => {
  ...
  test("renders sparkline MA7 overlays", async ({ page }) => {
    ...
    await page.waitForSelector('[data-testid="prov-delta-smoothed"]', {
      timeout: 10000,
    });
    await page.waitForSelector('[data-testid="sparkline-provenance"]', {
      timeout: 10000,
    });
  ...
  test("alert filter narrows table", async ({ page }) => {
    ...
    const firstOption = await alertFilter
