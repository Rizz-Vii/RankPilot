...
test.describe("Feature - load-balancing", () => {
  ...
  test("should load load-balancing interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="load-balancing-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle load-balancing actions", async ({ page }) => {
    await page.goto("/load-balancing");
    await expect(
      page.locator('[data-testid="load-balancing-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate load-balancing data", async ({ page }) => {
    await page.goto("/load-balancing");
    await expect(
      page.locator('[data-testid="load-balancing-data"]')
    ).toBeVisible();
  });

  test("should display load-balancing correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/load-balancing");
    await expect(
      page.locator('[data-testid="load-balancing-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle load-balancing errors gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="load-balancing-error-fallback"]')
    ).toBeVisible();
  });
});
