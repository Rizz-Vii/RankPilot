...
test.describe("finance dashboard", () => {
  test("preflight readiness → KPIs render → exports available + provenance + actions + months persistence", async ({
    page,
    request,
    baseURL,
  }) => {
    ...
    test.info().annotations.push({
      ...
      description: JSON.stringify(
        readinessJson || { status: readiness.status() }
      ),
    });
    ...
    const hasKpi = await kpiRegion
      .locator("text=/\bMRR\b|\bLTV\b|\bChurn\b|On[- ]?Time/i")
      .first()
      .isVisible()
      .catch(() => false);
    if (!hasKpi) {
      // If labels differ, assert at least that there are metric cards rendered (divs with progress bars or badges)
      const metricCandidates = await kpiRegion
        .locator("[role='progressbar'], .badge, .rounded-xl")
        .count();
    ...
    const mockBanner = page.getByRole("alert", {
      name: "Finance mock data banner",
    });
    ...
    // 4) Modules present
    await expect(
      page.getByRole("heading", { name: "MRR Trend" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Invoice Aging" })
    ).toBeVisible();
    await expect(
      page.getByRole("heading", { name: "Finance Workbench" })
    ).toBeVisible();

    // 5) Export actions produce downloads
    const jsonButton = page.getByRole("button", {
      name: "Export finance snapshot JSON",
    });
    ...
    const csvButton = page.getByRole("button", {
      name: "Export finance snapshot CSV",
    });
    ...
    if (
      readinessOk &&
      readinessJson &&
      typeof readinessJson.invoicesCount === "number"
    ) {
    ...
    const workbench = page.getByRole("region", {
      name: "Finance workbench actions",
    });
    await workbench.waitFor({ state: "visible" });

    // Revenue Snapshot (optimistic timestamp update when snapshot exists)
    const revenueCard = page
      .locator("div", { hasText: "Revenue Snapshot" })
      .first();
    const revenueRunBtn = revenueCard.getByRole("button", {
      name: /Run|Running/i,
    });
    await expect(revenueRunBtn).toBeVisible();
    const revenueTs = page
      .locator("[aria-label='Latest automation snapshots']")
      .locator("text=Last Revenue Snapshot")
      .locator("xpath=..")
      .locator("time");
    const hadRevenue = await revenueTs.count().then((c) => c > 0);
    let prevRevenueTime = hadRevenue
      ? await revenueTs.first().getAttribute("dateTime")
      : null;
    ...
    const storedBefore = await page.evaluate(() =>
      window.localStorage.getItem("financeMonths")
    );
