...
test.describe('Health API AI Usage Contract', () => {
    test('returns aiUsage24h & subtoolUsage24h with expected shape', async () => {
        ...
        ['tokensIn', 'tokensOut'].forEach(k => expect(typeof body.aiUsage24h[k]).toBe('number'));
        ...
        if (body.kpis) {
            ['aiDailyTokensIn', 'aiDailyTokensOut', 'aiDailyCostEstimate'].forEach(f => { if (f in body.kpis) expect(body.kpis[f]).toBeGreaterThanOrEqual(0); });
        }
    });
});
