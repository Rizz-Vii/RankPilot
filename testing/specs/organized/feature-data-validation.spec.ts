...
test.describe("Feature - data-validation", () => {
  ...
  test("should load data-validation interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="data-validation-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-validation actions", async ({ page }) => {
    await page.goto("/data-validation");
    await expect(
      page.locator('[data-testid="data-validation-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate data-validation data", async ({ page }) => {
    await page.goto("/data-validation");
    await expect(
      page.locator('[data-testid="data-validation-data"]')
    ).toBeVisible();
  });

  test("should display data-validation correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/data-validation");
    await expect(
      page.locator('[data-testid="data-validation-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-validation errors gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="data-validation-error-fallback"]')
    ).toBeVisible();
  });
});
