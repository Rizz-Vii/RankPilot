...
test.describe("Feature - metrics-collection", () => {
  ...
  test("should load metrics-collection interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="metrics-collection-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle metrics-collection actions", async ({ page }) => {
    await page.goto("/metrics-collection");
    await expect(
      page.locator('[data-testid="metrics-collection-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate metrics-collection data", async ({ page }) => {
    await page.goto("/metrics-collection");
    await expect(
      page.locator('[data-testid="metrics-collection-data"]')
    ).toBeVisible();
  });

  test("should display metrics-collection correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/metrics-collection");
    await expect(
      page.locator('[data-testid="metrics-collection-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle metrics-collection errors gracefully", async ({
    ...
    await expect(
      page.locator('[data-testid="metrics-collection-error-fallback"]')
    ).toBeVisible();
  });
});
