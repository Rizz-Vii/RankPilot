...
test.describe('Feature Gating · Nav Matrix', () => {
    ...
    test.beforeEach(async ({ page }) => { auth = new EnhancedAuth(page); });

    test('starter tier gating matrix', async ({ page }) => {
        await auth.loginAndGoToDashboard(UNIFIED_TEST_USERS.starter);
        const { salesDashboard, financeDashboard, marketingDashboard, linkView, contentBriefs, aiVisibility } = selectors;
        for (const sel of [salesDashboard, financeDashboard, marketingDashboard, linkView, contentBriefs, aiVisibility]) {
            await expect(page.locator(sel)).toBeVisible();
        }
        await expect(page.locator(salesDashboard)).not.toHaveAttribute('aria-disabled', 'true');
        await expect(page.locator(financeDashboard)).not.toHaveAttribute('aria-disabled', 'true');
        for (const sel of [marketingDashboard, linkView, contentBriefs, aiVisibility]) {
    ...
    test('agency tier gating matrix', async ({ page }) => {
        ...
        const { marketingDashboard, linkView, contentBriefs, aiVisibility } = selectors;
        for (const sel of [linkView, contentBriefs, aiVisibility]) {
            await expect(page.locator(sel)).not.toHaveAttribute('aria-disabled', 'true');
        }
        await expect(page.locator(marketingDashboard)).toHaveAttribute('aria-disabled', 'true');
    });

    test('enterprise tier gating matrix', async ({ page }) => {
        await auth.loginAndGoToDashboard(UNIFIED_TEST_USERS.enterprise);
        await expect(page.locator(selectors.marketingDashboard)).not.toHaveAttribute('aria-disabled', 'true');
    });
});
