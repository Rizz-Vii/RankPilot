...
test.describe("Feature - data-sovereignty", () => {
  ...
  test("should load data-sovereignty interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="data-sovereignty-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-sovereignty actions", async ({ page }) => {
    await page.goto("/data-sovereignty");
    await expect(
      page.locator('[data-testid="data-sovereignty-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate data-sovereignty data", async ({ page }) => {
    await page.goto("/data-sovereignty");
    await expect(
      page.locator('[data-testid="data-sovereignty-data"]')
    ).toBeVisible();
  });

  test("should display data-sovereignty correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/data-sovereignty");
    await expect(
      page.locator('[data-testid="data-sovereignty-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-sovereignty errors gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="data-sovereignty-error-fallback"]')
    ).toBeVisible();
  });
});
