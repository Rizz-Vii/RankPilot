...
test.describe('Demo banners and behavior', () => {
    ...
    test('Content Analyzer blocks mock report when demo disabled', async ({ page }) => {
        // Force off
        await page.addInitScript(() => {
            window.localStorage.setItem('demoContent', 'false');
        });
        ...
        await page.getByRole('button', { name: 'Start NeuroSEO™ Analysis' }).click();

        // Expect error notice due to demo disabled (from allowContentAnalyzerMocks gating)
        await expect(page.getByText('Live NeuroSEO analysis is not yet available')).toBeVisible();
        ...
        await expect(page.getByText('Key Insights', { exact: false })).toBeVisible({ timeout: 15000 });
    });
});
