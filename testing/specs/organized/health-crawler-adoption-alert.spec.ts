...
test.describe('Health API crawler aggregate adoption alerts', () => {
    test('adoption alert transitions warn -> cleared', async () => {
        ...
        const warnAlert = (body.alerts as any[]).find(a => a.type === 'crawlerAggregateAdoption');
        ...
        const adoptionAlerts = (body.alerts as any[]).filter(a => a.type === 'crawlerAggregateAdoption');
