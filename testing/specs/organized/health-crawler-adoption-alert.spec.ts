...
test.describe("Health API crawler aggregate adoption alerts", () => {
  test("adoption alert transitions warn -> cleared", async () => {
    ...
    const alerts = Array.isArray((body as { alerts?: unknown }).alerts)
      ? (body.alerts as unknown[])
      : [];
    const warnAlert = alerts.find(
      (a) =>
        typeof a === "object" &&
        a !== null &&
        (a as Partial<Alert>).type === "crawlerAggregateAdoption"
    ) as Alert | undefined;
    ...
    const adoptionAlerts = alerts.filter(
      (a) =>
        typeof a === "object" &&
        a !== null &&
        (a as Partial<Alert>).type === "crawlerAggregateAdoption"
