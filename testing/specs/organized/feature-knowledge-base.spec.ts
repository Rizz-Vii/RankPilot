...
test.describe("Feature - knowledge-base", () => {
  ...
  test("should load knowledge-base interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="knowledge-base-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle knowledge-base actions", async ({ page }) => {
    await page.goto("/knowledge-base");
    await expect(
      page.locator('[data-testid="knowledge-base-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate knowledge-base data", async ({ page }) => {
    await page.goto("/knowledge-base");
    await expect(
      page.locator('[data-testid="knowledge-base-data"]')
    ).toBeVisible();
  });

  test("should display knowledge-base correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/knowledge-base");
    await expect(
      page.locator('[data-testid="knowledge-base-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle knowledge-base errors gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="knowledge-base-error-fallback"]')
    ).toBeVisible();
  });
});
