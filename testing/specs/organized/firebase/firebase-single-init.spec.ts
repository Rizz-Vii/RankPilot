...
test.describe("Firebase single initialization", () => {
  test("only logs initialization once across multiple navigations", async ({
    browser,
  }) => {
    ...
    page1.on("console", (msg) => {
      if (msg.type() === "log" && msg.text().includes(INIT_LOG))
        seen.push(msg.text());
    });
    ...
    expect
      .soft(
        seen.filter((l) => l.includes(INIT_LOG)).length,
        "Initialization log count"
      )
      .toBe(1);
