...
test.describe('Team Token Bucket Rate Limiting', () => {
    ...
    test('within limit succeeds and headers decrement', async ({ request }) => {
        for (let i = 0; i < 5; i++) {
            const res = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamA' } })
    ...
    test('within limit succeeds and success headers present', async ({ request }) => {
        process.env.ENABLE_TEAM_BUCKET_LIMIT = '1'
    ...
    test('exceed returns 429 with headers & retry-after', async ({ request }) => {
        for (let i = 0; i < 5; i++) {
            await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamB' } })
        }
        const res = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamB' } })
    ...
    test('refill restores tokens after time advance', async ({ request }) => {
        // Consume
        for (let i = 0; i < 5; i++) {
            await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamC' } })
        }
        ...
        const res = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamC' } })
        expect([200, 429]).toContain(res.status())
    })

    test('independent buckets across teams', async ({ request }) => {
        for (let i = 0; i < 5; i++) {
            await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamD' } })
            await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamE' } })
        }
        const dRes = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamD' } })
        const eRes = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamE' } })
    ...
    test('flag off -> legacy behavior (no team headers)', async ({ request }) => {
        ...
        const res = await request.get('/api/seo-audit', { headers: { 'x-team-id': 'teamZ' } })
        expect(res.status()).toBe(200)
