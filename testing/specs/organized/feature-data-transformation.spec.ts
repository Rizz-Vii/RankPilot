...
test.describe("Feature - data-transformation", () => {
  ...
  test("should load data-transformation interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="data-transformation-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-transformation actions", async ({ page }) => {
    await page.goto("/data-transformation");
    await expect(
      page.locator('[data-testid="data-transformation-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate data-transformation data", async ({ page }) => {
    await page.goto("/data-transformation");
    await expect(
      page.locator('[data-testid="data-transformation-data"]')
    ).toBeVisible();
  });

  test("should display data-transformation correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/data-transformation");
    await expect(
      page.locator('[data-testid="data-transformation-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle data-transformation errors gracefully", async ({
    ...
    await expect(
      page.locator('[data-testid="data-transformation-error-fallback"]')
    ).toBeVisible();
  });
});
