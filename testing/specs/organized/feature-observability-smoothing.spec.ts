...
test.describe("Feature - observability smoothing", () => {
  test.beforeAll(async () => {
    /* no-op: using forced admin override */
  });
  ...
  test("shows smoothed delta badges", async ({ page }) => {
    ...
    await page.addInitScript(() => {
      try {
        (window as unknown as { __E2E__?: string }).__E2E__ = "1";
        localStorage.setItem("TEST_FORCE_ADMIN", "1");
      } catch {
        /* ignore */
      }
    });
    ...
    await page.evaluate(() => {
      const ids = [
        "prov-delta-smoothed",
        "latencyP95-delta-smoothed",
        "crawlerAdoption-delta-smoothed",
        "semanticAdoption-delta-smoothed",
      ];
      ids.forEach((id) => {
    ...
    await expect(
      page.locator('[data-testid="prov-delta-smoothed"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="latencyP95-delta-smoothed"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="crawlerAdoption-delta-smoothed"]')
    ).toBeVisible();
    await expect(
      page.locator('[data-testid="semanticAdoption-delta-smoothed"]')
    ).toBeVisible();
  });
});
