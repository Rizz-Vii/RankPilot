...
test.describe("Feature - network-security", () => {
  ...
  test("should load network-security interface", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="network-security-container"]')
    ).toBeVisible();
  });
  ...
  test("should handle network-security actions", async ({ page }) => {
    await page.goto("/network-security");
    await expect(
      page.locator('[data-testid="network-security-actions"]')
    ).toBeVisible();
  });
  ...
  test("should validate network-security data", async ({ page }) => {
    await page.goto("/network-security");
    await expect(
      page.locator('[data-testid="network-security-data"]')
    ).toBeVisible();
  });

  test("should display network-security correctly on mobile", async ({
    page,
  }) => {
    await page.setViewportSize({ width: 375, height: 667 });
    await page.goto("/network-security");
    await expect(
      page.locator('[data-testid="network-security-mobile"]')
    ).toBeVisible();
  });
  ...
  test("should handle network-security errors gracefully", async ({ page }) => {
    ...
    await expect(
      page.locator('[data-testid="network-security-error-fallback"]')
    ).toBeVisible();
  });
});
