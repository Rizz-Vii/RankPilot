...
const STATUS_PALETTE_REGEX =
  /(bg|text|border|ring)-(?:red|green|yellow|orange|blue|purple|amber)-(50|100|200|300|400|500|600|700|800|900)\b/;
...
test.describe("Design status color semantic compliance", () => {
  test("no raw status palette utilities remain in src/", () => {
    ...
    expect(
      offenders,
      "Replace raw palette utilities with semantic tokens (primary, accent, success, warning, destructive, etc.)"
    ).toEqual([]);
  });
});
