...
test.describe("RankPilot Security & Edge Cases - Comprehensive Validation", () => {
  ...
  test.describe("Input Validation & Sanitization", () => {
    test("XSS Prevention - Script Injection", async ({ page }) => {
      ...
      try {
        ...
        securityTestDiagnostics.errors.push(
      ...
      try {
        ...
        if (await keywordInput.isVisible()) {
          ...
          if (await searchBtn.isVisible()) {
            ...
            const sqlError = await page.isVisible(
              "text=/sql|database|syntax|injection/i"
            );
            const serverError = await page.isVisible(
              '.error-500, [data-testid="server-error"]'
            );
    ...
    test("Command Injection Prevention", async ({ page }) => {
      ...
      try {
        ...
        if (await urlInput.isVisible()) {
          await urlInput.fill(
            `https://example.com${TEST_DATA.maliciousInputs.commandInjection}`
          );

          const auditBtn = await page
            .locator('button[data-testid="start-audit"]')
            .first();
          ...
          if (await auditBtn.isVisible()) {
            ...
            const commandError = await page.isVisible(
              "text=/command|shell|exec|injection/i"
            );
            const systemError = await page.isVisible(
              '.system-error, [data-testid="system-error"]'
            );
    ...
    test("Path Traversal Prevention", async ({ page }) => {
      ...
      try {
        ...
        const fileAccess = await page.isVisible(
          "text=/etc/passwd|file not found|access denied/i"
        );
        const errorPage = await page.isVisible(
          '.error-403, .error-404, [data-testid="error"]'
        );
  ...
  test.describe("Authentication & Authorization", () => {
    test("Brute Force Protection", async ({ page }) => {
      ...
      try {
        ...
        for (let i = 0; i < 5; i++) {
          ...
          if (await emailInput.isVisible()) {
            await emailInput.fill(
              TEST_DATA.securityScenarios.bruteForce.username
            );
          }
          if (await passwordInput.isVisible()) {
            await passwordInput.fill(
              TEST_DATA.securityScenarios.bruteForce.passwords[i]
            );
          }

          const loginBtn = await page
            .locator('button[type="submit"], [data-testid="login"]')
            .first();
        ...
        const rateLimited = await page.isVisible(
          "text=/rate limit|too many|captcha|verification/i"
        );
        const captcha = await page.isVisible(
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential brute force vulnerability detected"
        );
    ...
    test("Session Management Security", async ({ page }) => {
      ...
      try {
        ...
        console.log(
          `   HttpOnly Cookies: ${httpOnlyCookies}/${cookies.length}`
        );

        // Check for session timeout
        await page.waitForTimeout(5000); // Wait a bit

        const sessionValid = await page.isVisible(
          '.user-menu, [data-testid="user"]'
        );
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential session management vulnerability detected"
        );
      ...
      try {
        ...
        for (const url of adminUrls) {
          try {
            ...
            const accessDenied = await page.isVisible(
              "text=/access denied|unauthorized|forbidden|403/i"
            );
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential authorization bypass vulnerability detected"
        );
        ...
        if (await messageTextarea.isVisible()) {
          ...
          if (await submitBtn.isVisible()) {
            ...
            const truncatedMsg = await page.isVisible(
              "text=/truncated|too long|limit/i"
            );
      ...
      try {
        ...
        if (await keywordInput.isVisible()) {
          await keywordInput.fill(
            TEST_DATA.maliciousInputs.unicode +
              TEST_DATA.maliciousInputs.specialChars
          );
          ...
          if (await searchBtn.isVisible()) {
            ...
            const encodingError = await page.isVisible(
              "text=/encoding|charset|unicode/i"
            );
            ...
            console.log(
              "   ✅ Unicode & special characters handling functional"
            );
          }
        }
      } catch (error) {
          ...
          for (const invalidInput of invalidInputs) {
            if (invalidInput) {
              ...
              if (await auditBtn.isVisible()) {
                ...
                const validationError = await page.isVisible(
                  '.error, .validation-error, [data-testid="error"]'
                );
                console.log(
                  `   Input "${invalidInput.substring(0, 20)}..." - Validation: ${validationError}`
                );
              }
            }
          }
  ...
  test.describe("Network & Protocol Security", () => {
    test("HTTPS Enforcement", async ({ page }) => {
      ...
      try {
        ...
        try {
          ...
          console.log(
            "   HTTP navigation blocked (expected for HTTPS-only sites)"
          );
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential HTTP vulnerability detected"
        );
    ...
    test("CORS Policy Validation", async ({ page }) => {
      ...
      try {
        ...
        expect(corsTest === "blocked" || typeof corsTest === "number").toBe(
          true
        );
    ...
    test("Content Security Policy", async ({ page }) => {
      ...
      try {
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential CSP vulnerability detected"
        );
    ...
    test("Error Message Sanitization", async ({ page }) => {
      ...
      try {
        ...
        const errorMsg = await page
          ...
          .first()
          .textContent();
        const stackTrace = await page.isVisible(
          "text=/stack|trace|at line|error in/i"
        );
        const sensitiveInfo = await page.isVisible(
          "text=/password|token|key|secret/i"
        );
        ...
        securityTestDiagnostics.vulnerabilities.push(
          "Potential information disclosure vulnerability detected"
        );
  ...
  test.afterAll(() => {
    ...
    if (
      securityTestDiagnostics.errors.length === 0 &&
      securityTestDiagnostics.vulnerabilities.length === 0
    ) {
