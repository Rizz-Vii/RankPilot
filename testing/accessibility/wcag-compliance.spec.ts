...
const accessibilityTestDiagnostics = {
  errors: [] as string[],
...
test.describe("RankPilot Accessibility - WCAG 2.1 AA Compliance", () => {
      ...
      try {
        ...
        if (skipLinks.length > 0) {
          ...
          if (href) {
            ...
            console.log(
              `   Focus Moved to Target: ${activeElement === href.substring(1)}`
            );
        ...
        accessibilityTestDiagnostics.errors.push(
    ...
    test("Landmarks - ARIA Landmarks", async ({ page }) => {
      ...
      try {
        ...
        const navLandmark = await page
          ...
          .first();
        const bannerLandmark = await page
    ...
    test("Tab Order - Logical Tab Sequence", async ({ page }) => {
      ...
      try {
        ...
        const focusableElements = await page
          .locator("input, button, select, textarea, a[href]")
          .all();
        ...
        if (focusableElements.length > 0) {
          ...
          expect(
            TEST_DATA.keyboardNavigation.tabOrder.includes(
        ...
        const hasFocusOutline = await page.evaluate(() => {
          ...
          return (
            ...
            computedStyle.border !==
              computedStyle.border.replace(/2px solid/, "1px solid")
          );
        });
  ...
  test.describe("Screen Reader Support", () => {
    ...
    test("ARIA Labels - Form Controls", async ({ page }) => {
      ...
      try {
        ...
        console.log(
          `   Labeled Form Controls: ${labeledControls}/${inputs.length}`
        );
      ...
      try {
        ...
        if (await submitBtn.isVisible()) {
          ...
          const errorMessages = await page
            .locator('.error, [role="alert"], [aria-live]')
            .all();
    ...
    test("Consistent Navigation - All Pages", async ({ page }) => {
      ...
      try {
        for (const pagePath of TEST_DATA.pages) {
          ...
          console.log(
            `   ${pagePath} - Nav: ${navExists}, Main: ${mainExists}`
          );
  ...
  test.afterAll(() => {
    ...
    if (
      accessibilityTestDiagnostics.errors.length === 0 &&
      accessibilityTestDiagnostics.violations.length === 0
    ) {
