...
describe('Firestore security rules - multi-tenant isolation', () => {
    ...
    before(async function () {
        ...
        if (!hostEnv) {
            console.warn('[rules-test] FIRESTORE_EMULATOR_HOST not set, skipping rules tests');
            this.skip();
            return;
        }
    ...
    it('allows user to read their own user doc, denies others', async () => {
        const admin = await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await adb.collection('users').doc('alice').set({ email: 'alice@example.com' });
            await adb.collection('users').doc('bob').set({ email: 'bob@example.com' });
        });
    ...
    it('subscriptions readable by owner and team member when teamId present', async () => {
        await testEnv.withSecurityRulesDisabled(async (ctx) => {
            ...
            await adb.collection('teams').doc('team1').set({ ownerId: 'alice', memberIds: ['alice', 'bob'] });
            await adb.collection('teams').doc('team1').collection('members').doc('bob').set({ role: 'member' });
            // Subscriptions doc keyed by userId per rules
            await adb.collection('subscriptions').doc('alice').set({ userId: 'alice', tier: 'starter', teamId: 'team1' });
        });

        const alice = testEnv.authenticatedContext('alice', {});
        const bob = testEnv.authenticatedContext('bob', {});
        const charlie = testEnv.authenticatedContext('charlie', {});
        await assertSucceeds(alice.firestore().collection('subscriptions').doc('alice').get());
        await assertSucceeds(bob.firestore().collection('subscriptions').doc('alice').get());
        await assertFails(charlie.firestore().collection('subscriptions').doc('alice').get());
    });

    it('invites_index is server-only (deny client writes)', async () => {
        const alice = testEnv.authenticatedContext('alice', {});
        await assertFails(alice.firestore().collection('invites_index').doc('x').set({ foo: 'bar' }));
    });
});
