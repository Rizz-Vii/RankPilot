...
interface SimpleRequestLike {
  headers: { get(k: string): string | undefined };
  json(): Promise<ExportChartBody>;
}
...
class MockRequest implements SimpleRequestLike {
  ...
  get headersObj() {
    return { get: (k: string) => this.headers.get(k.toLowerCase()) };
  }
  async json() {
    return this.body;
  }
...
function toNextRequest(body: ExportChartBody, token: string): PostArg {
  ...
  return {
    headers: r.headersObj as unknown as PostArg["headers"],
    json: () => r.json(),
  } as PostArg;
}
...
describe("Integration: POST export_chart", () => {
  ...
  beforeEach(() => {
    sinon.stub(adminAuth, "verifyIdToken").callsFake(
      async (_t: unknown) =>
        ({
          uid: "u1",
          aud: "test",
    ...
    const chartsColl = {
      doc: (_id: string) => ({ get: async () => chartDoc }),
    };
    sinon.stub(adminDb, "collection").callsFake(((name: string) => {
      ...
      if (name === "visualizations")
        return chartsColl as unknown as ReturnType<typeof adminDb.collection>;
      if (name === "analytics")
        return analyticsColl as unknown as ReturnType<
    ...
    (
      adminStorage as unknown as { bucket: () => { file: sinon.SinonStub } }
    ).bucket = sinon.stub().returns(bucketStub);
    const fakeFile = {
      save: sinon.stub().resolves(),
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/integration-chart.pdf"]),
    };
  ...
  it("returns signed URL for server-rendered PDF when artifact not supplied", async () => {
    ...
    expect(json.exportUrl).to.equal(
      "https://signed.example.com/integration-chart.pdf"
    );
  });
  ...
  it("returns signed URL for server-rendered SVG when artifact not supplied", async () => {
    ...
    const fakeFile = {
      ...
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/integration-chart.svg"]),
    };
    ...
    const req = toNextRequest(
      {
        action: "export_chart",
        data: { chartId: "c9", format: "svg", config: {} },
      },
      "tok"
    );
    const res = await POST(req);
    ...
    expect(json.success).to.equal(true);
    expect(json.exportUrl).to.equal(
      "https://signed.example.com/integration-chart.svg"
    );
  });
  ...
  it("returns signed URL when client artifact (SVG data URL) is provided", async () => {
    ...
    const fakeFile = {
      ...
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/integration-artifact.svg"]),
    };
    bucket.file.returns(fakeFile);

    const inlineSvg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="50" height="30"><rect width="50" height="30" fill="green"/></svg>';
    const artifact = `data:image/svg+xml;base64,${Buffer.from(inlineSvg, "utf-8").toString("base64")}`;

    const req = toNextRequest(
      {
        action: "export_chart",
        data: { chartId: "c9", format: "svg", config: {}, artifact },
      },
      "tok"
    );
    const res = await POST(req);
    ...
    expect(json.success).to.equal(true);
    expect(json.exportUrl).to.equal(
      "https://signed.example.com/integration-artifact.svg"
    );
