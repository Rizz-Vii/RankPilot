...
interface BucketStub {
  file: sinon.SinonStub<[string?], FakeFile>;
}
interface StorageStub {
  bucket: sinon.SinonStub<[], BucketStub>;
}
...
describe("Visualizations API exports", () => {
  ...
  beforeEach(() => {
    ...
    (adminStorage as unknown as { bucket: StorageStub["bucket"] }).bucket =
      storageStub.bucket;
  });
  ...
  it("persists client-provided artifact and returns signed URL (chart)", async () => {
    const fakeFile = {
      ...
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/chart.pdf"]),
    };
    bucketStub.file.returns(fakeFile);

    const artifactInput: Parameters<
      typeof vizApiTest.persistExportArtifact
    >[0] = {
      userId: "u1",
      kind: "chart",
      id: "c1",
      format: "pdf",
      artifact: "data:application/pdf;base64,UEsDBAoAAAAA",
  ...
  it("generates server-side export when no artifact is provided (chart PDF)", async () => {
    const fakeFile = {
      ...
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/chart-generated.pdf"]),
    };
    bucketStub.file.returns(fakeFile);
    ...
    const url = await vizApiTest.generateChartExport(chartDoc, "pdf", {
      title: "Test Chart",
    });
  ...
  it("generates server-side export for dashboard (JSON)", async () => {
    const fakeFile = {
      ...
      getSignedUrl: sinon
        .stub()
        .resolves(["https://signed.example.com/dashboard.json"]),
    };
    bucketStub.file.returns(fakeFile);

    const dashboardDoc: Parameters<
      typeof vizApiTest.generateDashboardExport
    >[0] = {
      id: "d1",
      userId: "u1",
      widgets: [
    ...
    const url = await vizApiTest.generateDashboardExport(
      dashboardDoc,
      "json",
      {}
    );
  ...
  it("generateServerArtifact returns PNG buffer from SVG when no PNG data URL provided", async () => {
    const svg =
      '<svg xmlns="http://www.w3.org/2000/svg" width="120" height="80"><circle cx="40" cy="40" r="35" fill="blue"/></svg>';
    const out = await generateServerArtifact(
      "png",
      { svg },
      { width: 120, height: 80 }
    );
  ...
  it("client export SVG includes inlined styles when includeStyles=true", async () => {
    ...
    const dataUrl = await d3VisualizationEngine.exportChart("chart-inline", {
      format: "svg",
      includeStyles: true,
    });
  ...
  it("client export SVG embeds @font-face when provided", async () => {
    ...
    const dataUrl = await d3VisualizationEngine.exportChart("chart-font", {
      ...
      embedFonts: [
        {
          family: "TestSans",
          src: "data:font/woff2;base64,d09GRgABAAAAA",
          format: "woff2",
          display: "swap",
        },
      ],
