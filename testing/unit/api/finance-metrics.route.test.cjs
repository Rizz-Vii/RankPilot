...
const { GET: financeGET } = require(
  path.resolve(__dirname, "../../../src/app/api/finance/metrics/route")
);

describe("API finance/metrics route (unit)", () => {
  it("returns 401 without auth", async () => {
    const req = {
      url: "http://localhost/api/finance/metrics?months=3",
      headers: { get: () => null },
    };
  ...
  it("returns 200 or tolerated status with Bearer test token (mock admin)", async () => {
    const headers = {
      get: (k) =>
        String(k).toLowerCase() === "authorization" ? "Bearer test" : null,
    };
    const req = {
      url: "http://localhost/api/finance/metrics?months=2",
      headers,
    };
    const res = await financeGET(req);
