...
const { GET: firecrawlGET } = require(
  path.resolve(__dirname, "../../../src/app/api/seo-audit/firecrawl/route.ts")
);
...
describe("API seo-audit/firecrawl route (unit)", () => {
  ...
  it("returns synthetic crawl when no api key", async () => {
    const req = {
      url: "http://localhost/api/seo-audit/firecrawl?url=https://example.com&limit=2",
    };
    const res = await firecrawlGET(req);
