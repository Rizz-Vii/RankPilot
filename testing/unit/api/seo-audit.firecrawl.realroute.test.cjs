...
try {
  const stubPath = path.join(
    process.cwd(),
    "testing/shims/next-server-stub.js"
  );
...
class HeadersMap {
  ...
  get(k) {
    return this._m.get(k.toLowerCase()) || null;
  }
  set(k, v) {
    this._m.set(k.toLowerCase(), String(v));
  }
}
class NextResponseMock {
  constructor(body, init) {
    ...
    this.headers = new HeadersMap(init?.headers);
  }
...
class NextRequestMock {
  constructor(url, headers) {
    this.url = url;
    this.headers = new HeadersMap(headers);
  }
}

Module._load = function (request, parent, isMain) {
  if (request === "next/server" || request === "virtual:next-server-stub") {
    ...
    const adminDb = {
      collection: (name) => ({
        doc: (id) => ({
          _key: `${name}/${id}`,
          get: async function () {
            const data = quotaStore.get(this._key);
      ...
      runTransaction: async (fn) =>
        fn({
          ...
          set: (ref, data) =>
            quotaStore.set(ref._key, {
              ...(quotaStore.get(ref._key) || {}),
              ...data,
            }),
        }),
    };
...
const routeMod = require(
  path.join(process.cwd(), "src/app/api/seo-audit/firecrawl/route.ts")
);
...
describe("REAL firecrawl route (Next.js) contract", () => {
  ...
  it("enforces rate limit (second call over limit=1)", async () => {
    ...
    const first = await realGET(
      new NextRequestMock(
        "http://localhost/api/seo-audit/firecrawl?url=https://ratelimit.com"
      )
    );
    expect(first.status).to.equal(200);
    const second = await realGET(
      new NextRequestMock(
        "http://localhost/api/seo-audit/firecrawl?url=https://ratelimit.com/2"
      )
    );
  ...
  it("robots.txt block returns 403 robots_blocked", async () => {
    ...
    const res = await realGET(
      new NextRequestMock(
        "http://localhost/api/seo-audit/firecrawl?url=https://robotsblock.com/path&depth=2"
      )
    );
    const body = await res.json();
