...
const { POST: webhookPOST } = require(path.resolve(__dirname, '../../../src/app/api/telephony/webhook/route.ts'));
...
describe('API telephony/webhook route (unit)', () => {
    ...
    it('returns TwiML for inbound calls', async () => {
        ...
        const req = {
            headers: new Map([['content-type', 'application/x-www-form-urlencoded']]),
            text: async () => 'Direction=inbound&From=%2B15555550123&To=%2B15551234567',
            json: async () => ({})
        };
        const res = await webhookPOST(req);
        expect(res.status).to.equal(200);
