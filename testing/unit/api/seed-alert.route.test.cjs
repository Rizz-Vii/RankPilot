...
Module._load = function (request, parent, isMain) {
    ...
    if (request === '@/lib/firebase-admin' || request === path.resolve(process.cwd(), 'src/lib/firebase-admin.ts')) {
...
const routePath = path.resolve(__dirname, '../../../src/app/api/test/observability/seed-alert/route.ts');
...
describe('API test/observability/seed-alert route (unit)', () => {
    it('GET returns 404 with provenance in production', async () => {
        const prevEnv = { NODE_ENV: process.env.NODE_ENV, CI_PRODUCTION: process.env.CI_PRODUCTION };
        process.env.NODE_ENV = 'production'; process.env.CI_PRODUCTION = '1';
        try {
            const res = await GET({ url: 'http://localhost/api/test/observability/seed-alert?type=provenanceCoverage' });
        ...
        } finally { process.env.NODE_ENV = prevEnv.NODE_ENV; process.env.CI_PRODUCTION = prevEnv.CI_PRODUCTION; }
    });

    it('GET seeds default alert in non-prod and returns ok true with provenance', async () => {
        const prevEnv = { NODE_ENV: process.env.NODE_ENV, CI_PRODUCTION: process.env.CI_PRODUCTION };
        process.env.NODE_ENV = 'test'; delete process.env.CI_PRODUCTION;
        try {
            const res = await GET({ url: 'http://localhost/api/test/observability/seed-alert?type=provenanceCoverage&level=warn' });
    ...
    it('POST seeds batch alerts and returns count with provenance', async () => {
        ...
        process.env.NODE_ENV = 'test'; delete process.env.CI_PRODUCTION;
        try {
            const mockReq = { json: async () => ({ alerts: [{ type: 'cacheHitRatio', level: 'warn', value: 90, ma7: 88 }] }) };
