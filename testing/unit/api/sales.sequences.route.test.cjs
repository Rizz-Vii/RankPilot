...
// Load handlers
const salesRoute = require(
  path.resolve(__dirname, "../../../src/app/api/sales/sequences/route.ts")
);
const runRoute = require(
  path.resolve(__dirname, "../../../src/app/api/sales/sequences/run/route.ts")
);
...
describe("API sales/sequences route (unit)", () => {
  ...
  function makeReq(method, body, headers) {
    return {
      ...
      json: async () => body || {},
      url:
        "http://localhost/api/sales/sequences" +
        (method === "DELETE" && body && body.query ? "?" + body.query : ""),
    };
  }
          ...
          where: () => ({
            limit: () => ({
              get: async () => ({
                empty: false,
                docs: [
                  { id: "t1", data: () => ({ planTier: tier, ownerId: "u1" }) },
                ],
              }),
            }),
          }),
              ...
              if (sub === "salesSequences") {
                return {
                  orderBy: () => ({
                    limit: () => ({
                      get: async () => ({
                        docs: store.map((x) => ({ id: x.id, data: () => x })),
                      }),
                    }),
                  }),
                  add: async (doc) => {
                    store.push({ id: "s1", ...doc });
                    return { id: "s1" };
                  },
                  doc: () => ({
                    set: async () => ({}),
  ...
  it("creates a sequence (admin required)", async () => {
    stubAuth();
    stubUserAccess();
    const res = await salesRoute.POST(
      makeReq("POST", {
        name: "Seq A",
        steps: [{ id: "st1", type: "call", delayMinutes: 0 }],
        targets: [{ id: "c1", phone: "+15555550123" }],
      })
    );
  ...
  it("runs a sequence (test mode)", async () => {
    ...
    const fetchStub = sinon
      .stub(global, "fetch")
      .resolves({ ok: true, json: async () => ({ callSid: "CA_TEST_1" }) });
            ...
            limit: () => ({
              get: async () => ({
                ...
                docs: [
                  {
                    id: "t1",
                    data: () => ({ planTier: "agency", ownerId: "u1" }),
                  },
                ],
              }),
            }),
          ...
          doc: () => ({
            ...
            collection: (sub) => {
              ...
              if (sub === "salesSequences") {
                return {
                  doc: () => ({
                    get: async () => ({ exists: true, data: () => seqDoc }),
                  }),
                };
              }
              if (sub === "salesExecutions") {
