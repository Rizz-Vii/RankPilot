...
function withEnv(key, value, fn) {
    ...
    return Promise.resolve(fn()).finally(() => { if (prev === undefined) delete process.env[key]; else process.env[key] = prev; });
}
...
const { POST: verifyCaptchaPOST } = require(path.resolve(__dirname, '../../../src/app/api/verify-captcha/route.ts'));
...
describe('API verify-captcha route (unit)', () => {
    ...
    it('returns 500 when server misconfigured (no secret)', async () => {
        ...
        const res = await withEnv('RECAPTCHA_SECRET_KEY', '', () => verifyCaptchaPOST(req));
    ...
    it('returns 400 when verification fails (success=false)', async () => {
        return withEnv('RECAPTCHA_SECRET_KEY', 'test-secret', async () => {
            ...
            const origFetch = global.fetch;
            global.fetch = async () => ({ ok: true, status: 200, json: async () => ({ success: false, 'error-codes': ['invalid-input-response'] }) });
            try {
                const res = await verifyCaptchaPOST(req);
                expect(res.status).to.equal(400);
                const body = await res.json();
