...
function makeFakeDb(
  ...
  const whereStub = sinon
    .stub()
    .returns({ orderBy: () => ({ limit: () => ({ get: async () => snap }) }) });
  // Sinon stub we expose as collection
  const colStub = sinon.stub().callsFake((name: string) => ({
    where: whereStub,
    add: async (obj: unknown) => {
      (added[name] ||= []).push(obj);
      return { id: `${name}-${(added[name] || []).length}` };
    },
  }));
  return {
...
describe("runDueAutomationTick", () => {
  it("processes due recipes and writes automationRuns", async () => {
    ...
    const db = makeFakeDb([
      {
        ...
        actionConfigs: { sendDigestEmail: { to: "x@example.com" } },
        nextRun: now,
      },
    ]);
    // Patch runTransaction for locking success
    // attach runTransaction dynamically (test helper only)
    (db.api as Record<string, unknown>).runTransaction = async (
      fn: (txn: unknown) => unknown
    ) =>
      fn({
        get: async () => ({ data: () => ({ running: false, lockedAt: null }) }),
        update: () => {
          /* no-op */
        },
      });
    ...
    const res = await (
      mod as unknown as {
        runDueAutomationTick: (
          api: unknown,
          date: Date
        ) => Promise<{ processed: number }>;
      }
    ).runDueAutomationTick(db.api, now);
