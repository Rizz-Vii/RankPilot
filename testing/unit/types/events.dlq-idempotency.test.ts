...
import { __resetDeadLettersTestOnly, getDeadLetters } from '../../../src/lib/events/dead-letter';
...
import { __resetUnifiedMetricsTestOnly, getUnifiedMetricsSnapshot } from '../../../src/lib/metrics/unified-metrics';
    ...
    it('drops duplicate events when idempotencyKey matches', async () => {
        ...
        const unsubscribe = registerWithRetry(ev, async () => { count += 1; }, {
            idempotencyKey: (e: unknown) => {
                const id =
                    e && typeof e === 'object' && 'payload' in e && e.payload && typeof e.payload === 'object'
