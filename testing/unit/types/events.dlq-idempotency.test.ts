...
import {
  ...
  getDeadLetters,
} from "../../../src/lib/events/dead-letter";
...
// Focused tests for retry -> DLQ path and idempotency within the ts-node types test suite.
...
describe("event retry: DLQ and idempotency", () => {
  ...
  it("sends to DLQ after maxAttempts exhausted and increments metrics", async () => {
    ...
    const unsubscribe = registerWithRetry(
      ev,
      async () => {
  ...
  it("drops duplicate events when idempotencyKey matches", async () => {
      ...
      },
      {
        idempotencyKey: (e: unknown) => {
          const id =
            ...
            "attrs" in (e.payload as Record<string, unknown>) &&
            (e.payload as { attrs?: { id?: unknown } }).attrs?.id;
    ...
    assert.equal(
      count,
      1,
      "handler should run only once for duplicate idempotency key"
    );
    const snap = getUnifiedMetricsSnapshot();
  ...
  it("applies jitter within expected bounds and counts retry success", async () => {
    ...
    const patchedTimeout = (
      ...
      return (originalSetTimeout as unknown as (...args: unknown[]) => unknown)(
        fn,
        0,
        ...rest
      );
    ...
    await new Promise<void>((res) => originalSetTimeout(res, 30));
    ...
    assert.ok(
      Math.abs(recordedDelays[0] - 12) <= 1,
      `delay1=${recordedDelays[0]} ~12`
    );
    assert.ok(
      Math.abs(recordedDelays[1] - 24) <= 1,
      `delay2=${recordedDelays[1]} ~24`
    );
