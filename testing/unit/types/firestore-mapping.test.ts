import {
  mapForecastSnapshot,
  mapMarketingCampaignDoc,
  mapSalesDeal,
  type ForecastSnapshotFirestore,
  type MarketingCampaignFirestore,
  type SalesDealFirestore,
...
describe("Firestore Mapping Helpers", () => {
  it("maps marketing campaign with defaults + date", () => {
    const now = new Date();
    const raw: MarketingCampaignFirestore = {
      period: "2025-08",
      ...
      spend: 10,
      revenue: 25,
      createdAt: new TsStub(now) as unknown as Date,
    };
  ...
  it("maps sales deal with stage fallback + numeric defaults", () => {
    const raw: SalesDealFirestore = {
      amount: undefined,
      stage: undefined,
      createdAt: new TsStub(new Date()) as unknown as Date,
    };
  ...
  it("maps forecast snapshot numeric defaults", () => {
    const raw: ForecastSnapshotFirestore = {
      period: "2025-W33",
      forecast: undefined,
      actual: undefined,
    };
