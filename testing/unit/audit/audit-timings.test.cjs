...
global.fetch = async () => ({
  ok: true,
  text: async () =>
    '<html><title>T</title><meta name="description" content="d"><h1>H1</h1></html>',
});
...
describe("audit callable timings", () => {
  it("includes timings with numeric fields and logical ordering", async () => {
    ...
    const res = await __testRunSeoAudit(
      { url: "https://timing.test", depth: 1 },
      { uid: "userX" }
    );
    expect(res).to.have.property("timings");
    expect(res.timings).to.include.keys([
      "crawl_time_ms",
      "analysis_time_ms",
      "total_time_ms",
    ]);
