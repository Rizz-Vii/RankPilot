  ...
  if (request === "firebase-admin/firestore") {
    ...
    const db = {
      ...
      runTransaction: async (fn) =>
        fn({ get: async () => fakeSnap(null), set: () => {} }),
    };
...
describe("audit callable concurrency & quota (T13)", () => {
  it("handles 10 parallel audit calls successfully", async () => {
    ...
    global.fetch = async (url) => ({
      ok: true,
      text: async () =>
        '<html><title>Test</title><meta name="description" content="d"><h1>H1</h1></html>',
    });
    ...
    const results = await Promise.all(
      Array.from({ length: N }, (_, i) =>
        __testRunSeoAudit(
          { url: `https://example.com/page${i}`, depth: 1 },
          { uid: "user1" }
        )
      )
    );
  ...
  it("enforces per-user quota (plan free) and returns error on exceed", async () => {
    ...
    for (let i = 0; i < limit + 1; i++) {
      try {
        await __testRunSeoAudit(
          { url: `https://quota.com/${i}`, depth: 1, plan: "free" },
          { uid: "userQuota" }
        );
      } catch (e) {
        threw++;
