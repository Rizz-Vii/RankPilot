...
function mockFetchEmpty(counter: { calls: number }) {
  ...
  if (url.includes('/stream') && method === 'POST') {
    ...
    controller.enqueue(encoder.encode('data: ' + JSON.stringify({ token: 'world! <rp_meta>{"actions":["Analyze CWV","Find keywords"]}</rp_meta>' }) + '\n\n'));
            controller.enqueue(encoder.encode('data: ' + JSON.stringify({ final: true, sessionId: 's1', tokensUsed: 2 }) + '\n\n'));
  ...
  const body = JSON.stringify({
    response: 'Hello from mock <rp_meta>{"actions":["Analyze CWV","Find keywords"]}</rp_meta>',
          sessionId: 's1',
      ...
      // GET history
      return new Response(JSON.stringify({ messages: [], sessionId: 's1', hasMore: false }), { status: 200, headers: { 'content-type': 'application/json' } });
    }
  ...
  try { localStorage.setItem('rp_chat_isOpen', '1'); localStorage.setItem('rp_chat_isMin', '0'); } catch {}
    ...
    root.render(
      <AuthContext.Provider value={authValue}>
        <CustomerChatBot initialSuggestions={["Analyze CWV","Find keywords"]} />
      </AuthContext.Provider>
    );

  // Suggestions should appear from initialSuggestions deterministically
  const suggestionBtn = await waitFor(() => container.querySelector('button[aria-label="Suggestion: Analyze CWV"]'), 1000) as HTMLButtonElement;
  suggestionBtn.click();
  await delay(0);
  const inputAfter = container.querySelector('input[placeholder="Ask about your SEO performance..."]') as HTMLInputElement;
