...
const {
  deriveSemanticMapAggregate,
  deriveNeuralCrawlerAggregate,
} = require("../../../scripts/scan-neuroseo-large-docs.ts");
...
describe("T14 aggregate derivation", () => {
  it("derives semantic map aggregate with counts/top slices", () => {
    const full = {
      ...
      overallScore: 88,
      topicClusters: Array.from({ length: 10 }).map((_, i) => ({
        topic: "t" + i,
        semanticScore: 70 + i,
        opportunity: "high",
      })),
      keywordAnalysis: Array.from({ length: 20 }).map((_, i) => ({
        keyword: "k" + i,
        semanticRelevance: 50 + i,
      })),
      contentAnalysis: {
        readabilityScore: 90,
        contentDepth: 80,
