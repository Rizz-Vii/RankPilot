...
function calcReduction(legacyDocs, aggDocs, matchFields) {
  const idx = new Map();
  aggDocs.forEach((d) => {
    const key = matchFields.map((f) => d[f] ?? "").join("|");
    idx.set(key, d);
  });
  let matched = 0,
    legacyBytes = 0,
  ...
  legacyDocs.forEach((ld) => {
    const key = matchFields.map((f) => ld[f] ?? "").join("|");
    const agg = idx.get(key);
    if (agg) {
      matched++;
      legacyBytes += approxSize(ld);
      aggBytes += approxSize(agg);
    }
  });
  if (!matched) return null;
  const avgLegacy = legacyBytes / matched;
  const avgAgg = aggBytes / matched;
