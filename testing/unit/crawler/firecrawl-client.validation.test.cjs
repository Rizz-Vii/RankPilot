...
describe('firecrawl-client validation', () => {
  beforeEach(() => {
    ...
    global.fetch = async () => ({ ok: true, json: async () => ({ pages: [{ url: 'https://example.com', status: 9999 }] }) });
  });
  ...
  it('passes canonicalUrl and metaDescription when present', async () => {
    ...
    global.fetch = async () => ({ ok: true, json: async () => ({ pages: [{ url: 'https://example.com', markdown: 'content', status: 200, title: 'Title', links: ['https://a'], canonicalUrl: 'https://example.com', metaDescription: 'Example description' }] }) });
    const { runFirecrawl } = require(clientPath);
    const res = await runFirecrawl('https://example.com');
