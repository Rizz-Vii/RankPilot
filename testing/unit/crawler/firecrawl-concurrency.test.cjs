...
Module._load = function(request, parent, isMain){
  ...
  if (request.endsWith('/metrics/unified-metrics')) { return { recordRouteLatency: () => {}, recordError: () => {}, recordFallback: () => {} }; }
...
function p95(values){ const s=[...values].sort((a,b)=>a-b); return s[Math.ceil(0.95*s.length)-1]; }
...
describe('firecrawl-client concurrency + perf (T13)', () => {
  it('20 parallel calls under p95 latency threshold, <5% errors, no significant mem leak', async () => {
    ...
    global.fetch = async (url) => {
      ...
      if (Math.random() < 0.03) return { ok: false, status: 500, json: async () => ({ error: 'synthetic' }) };
      return { ok: true, json: async () => ({ pages: [{ url, markdown: '# Title\nContent', status: 200, title: 'Title', links: [] }] }) };
    };
    ...
    await Promise.all(Array.from({ length: N }, async (_, i) => {
      ...
      try {
        const r = await runFirecrawl(`https://example.com/page${i+1}`, { limit: 1 });
    ...
    expect(errorRate).to.be.below(0.05, `error rate high: ${(errorRate*100).toFixed(1)}%`);
    expect(memDiff).to.be.below(5*1024*1024, `heap grew ${(memDiff/1024/1024).toFixed(2)}MB`);
  });
});
