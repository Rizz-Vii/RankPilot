...
const { enforceTeamRateLimit, TeamRateLimitError } = require(path.resolve(__dirname, '../../../src/lib/rate-limit/team-rate-limit.ts'));
...
function makeDbDouble() {
  ...
  return {
    ...
    runTransaction: async (fn) => fn({
      get: async () => ({ exists: true, data: () => ({ count: state.count, windowStart: { toMillis: () => state.windowStart } }) }),
      set: async (_ref, data) => { state.count = data.count; state.windowStart = data.windowStart?.toMillis ? data.windowStart.toMillis() : state.windowStart; },
    }),
    _state: state,
  };
}
