...
describe('ThemeSystem basic cycle', () => {
    it('cycles through themes updating classes & CSS vars', () => {
        ...
        order.forEach(mode => {
            ...
            expect(bodyClassName).to.match(/theme-(light|dark|high-contrast)/, 'theme class applied');
            if (mode === 'dark') {
                expect(themeSystem.isDark()).to.equal(true, 'dark mode should report isDark');
            }
            if (mode === 'high-contrast') {
                expect(themeSystem.isHighContrast()).to.equal(true, 'high-contrast flag set');
                expect(themeSystem.isDark()).to.equal(false, 'high-contrast should not report dark');
            }
        });
