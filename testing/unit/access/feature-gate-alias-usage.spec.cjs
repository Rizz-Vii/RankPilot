...
try {
  ...
  const match = content.match(
    /export const FEATURE_ALIASES:[\s\S]*?= {([\s\S]*?)};/
  );
  if (match) {
    const body = match[1];
  ...
  console.error(
    "[FEATURE_GATE_ALIAS_CHECK] Could not read access-control.ts",
    e
  );
  process.exit(1);
}
...
if (!aliasKeys.length) {
  console.warn(
    "[FEATURE_GATE_ALIAS_CHECK] No alias keys detected – skipping (treat as pass)"
  );
  process.exit(0);
}
...
for (const file of files) {
  ...
  while ((m = gateRegex.exec(content))) {
    const key = m[1];
    if (aliasSet.has(key))
      offenders.push({ file: file.replace(projectRoot + "/", ""), key });
  }
}
...
if (offenders.length) {
  ...
  console.log(
    "[FEATURE_GATE_ALIAS_CHECK] PASS - no FeatureGate uses alias keys",
    aliasKeys
  );
}
