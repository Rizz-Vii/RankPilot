...
describe('access-control entitlement warning de-noise', () => {
  ...
  before(() => {
    console.warn = (msg, ...rest) => { warned.push(String(msg)); }; // capture
  ...
  it('warns only once per entitlement key', () => {
    ...
    access.canAccessFeature(ua, 'priority_support');
    access.canAccessFeature(ua, 'priority_support');
    const matches = warned.filter(w => w.includes("Entitlement key 'priority_support'"));
    assert.strictEqual(matches.length, 1, 'should warn exactly once for priority_support');
  });
