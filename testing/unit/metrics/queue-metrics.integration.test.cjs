...
const { __resetQueueMetrics, recordQueueEnqueue, recordQueueStart, recordQueueDone } = require('../../../src/lib/metrics/queue-metrics.ts');
const { getUnifiedMetricsSnapshot } = require('../../../src/lib/metrics/unified-metrics.ts');
...
describe('queue metrics integration (DEV-QUEUE-01)', () => {
    ...
    it('handles no terminal tasks (successRatio null) then failure path', () => {
        ...
        if (!snapshot.queue) throw new Error('queue snapshot missing after failure');
