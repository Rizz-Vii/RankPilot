...
describe('aiClient', () => {
  ...
  it('uses latency budget to short-circuit long work', async () => {
    ...
    // Slow fetch mock
    global.fetch = () => new Promise(resolve => setTimeout(() => resolve({ ok: true, json: async () => ({ candidates: [{ content: { parts: [{ text: 'gem' }] } }] }) }), 100));
    const { chatComplete } = await freshImport();
    const out = await chatComplete({ messages: [{ role: 'user', content: 'x' }] });
    assert.equal(typeof out, 'string');
  });
