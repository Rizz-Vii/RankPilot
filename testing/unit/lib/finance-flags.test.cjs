...
describe('allowFinanceMocks()', () => {
  ...
  it('enables in dev by default when env unset', () => {
    ...
    if (prevFlag !== undefined) process.env.NEXT_PUBLIC_ALLOW_FINANCE_MOCKS = prevFlag; else delete process.env.NEXT_PUBLIC_ALLOW_FINANCE_MOCKS;
  });
  ...
  it('respects explicit env true/false', () => {
    ...
    if (prev !== undefined) process.env.NEXT_PUBLIC_ALLOW_FINANCE_MOCKS = prev; else delete process.env.NEXT_PUBLIC_ALLOW_FINANCE_MOCKS;
  });
});
