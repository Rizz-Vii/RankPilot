...
describe("processVoiceRecurringTick (unit)", () => {
  it("returns processed:0 when no due appointments", async () => {
    const {
      processVoiceRecurringTick,
    } = require("../../../functions/src/scheduled/voice-recurring");
    const fakeDb = makeFakeDb({ appts: [] });
    const res = await processVoiceRecurringTick(
      fakeDb,
  ...
  it("enqueues next occurrences and advances nextOccurrence for daily and weekly", async () => {
    ...
    // schedule is now + 60s
    assert.ok(
      Math.abs(new Date(q1.schedule).getTime() - (now.getTime() + 60000)) <= 5
    );
    ...
    assert.ok(
      new Date(u1.patch.nextOccurrence).getTime() -
        appts[0].nextOccurrence.toDate().getTime() ===
        86_400_000
    );
    assert.ok(
      new Date(u2.patch.nextOccurrence).getTime() -
        appts[1].nextOccurrence.toDate().getTime() ===
        7 * 86_400_000
    );
  });
});
