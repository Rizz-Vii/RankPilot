...
describe('Outreach payload builder', () => {
    it('builds ISO schedule and repeat when provided', () => {
        ...
        const buildOutboundPayload = mod.buildOutboundPayload || (mod.default && mod.default.buildOutboundPayload);
        ...
        assert.ok(p.schedule.endsWith(':00.000Z') || /:00Z$/.test(p.schedule), 'schedule should be ISO');
