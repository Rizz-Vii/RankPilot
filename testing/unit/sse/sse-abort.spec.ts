...
describe("SSE abort should not error (streaming route)", () => {
  it("aborts /api/streaming?action=sse gracefully", async function () {
    ...
    const body = (res as Response).body as
    ...
    const hasWebReader =
      typeof (body as ReadableStream<Uint8Array> | undefined)?.getReader ===
      "function";
    ...
    if (hasWebReader) {
      ...
      await new Promise<void>((resolve) => {
        const nodeBody = body as NodeJS.ReadableStream & {
          off?: (ev: string, fn: (...args: unknown[]) => void) => void;
          once?: (ev: string, fn: (...args: unknown[]) => void) => void;
        };
        const onData = () => {
        ...
        const cleanup = () => {
          try {
            nodeBody.off?.("data", onData);
            nodeBody.off?.("error", onError);
          } catch {}
        };
