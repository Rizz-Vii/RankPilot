...
test.describe('RankPilot Frontend-Backend Integration', () => {
    ...
    test.describe('Public Pages - Production Health', () => {
        ...
        test('Pricing Page - Subscription Tiers', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/pricing`, { waitUntil: 'domcontentloaded' });
            ...
            for (const tier of tierNames) {
                ...
                const tierElement = page.locator(`text="${tier}", button:has-text("${tier}"), a:has-text("${tier}"), [data-testid*="${tier.toLowerCase()}"]`).first();
                try {
                    await expect(tierElement).toBeVisible({ timeout: 5000 });
                    console.log(`   ✅ ${tier} tier visible`);
                } catch (error) {
                    console.log(`   ⚠️ ${tier} tier not found with standard selectors (may be styled differently)`);
                }
            }

            // Check for pricing information with more flexible selectors
            const priceElements = page.locator('[data-testid*="price"], .price, [class*="price"], [class*="pricing"], [class*="cost"], span:has-text("$"), div:has-text("$")');
        ...
        test('Features Page - NeuroSEO™ Suite Showcase', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/features`, { waitUntil: 'domcontentloaded' });
    ...
    test.describe('Authentication Flow Integration', () => {
        ...
        test('Login Page - Form Availability', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/login`, { waitUntil: 'domcontentloaded' });
            ...
            try {
                ...
                console.log(`   ⚠️ Submit button click blocked (likely by reCAPTCHA): ${error instanceof Error ? error.message : String(error)}`);
        ...
        test('Signup Page - Registration Flow', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/signup`, { waitUntil: 'domcontentloaded' });
            ...
            if (hasPasswordField) {
                ...
                console.log('   ⚠️ Password field not found (may be multi-step signup)');
            }
    ...
    test.describe('Protected Routes - Authentication Checks', () => {
        ...
        test('Dashboard Redirect - Unauthenticated Access', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/dashboard`, { waitUntil: 'domcontentloaded' });
            ...
            const hasAuthPrompt = await page.locator('text="sign in", text="login", text="authenticate", text="Sign In", text="Login", text="Authenticate"').count() > 0;
    ...
    test.describe('Feature Integration - NeuroSEO™ Suite Access', () => {
        ...
        test('NeuroSEO™ Dashboard Access - Tier Validation', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/neuroseo`, { waitUntil: 'domcontentloaded' });
        ...
        test('Keyword Tool - Public vs Premium Features', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/keyword-tool`, { waitUntil: 'domcontentloaded' });
            ...
            const upgradePrompts = page.locator('text="upgrade", text="premium", text="subscribe", text="pro", text="Upgrade", text="Premium", text="Subscribe", text="Pro"');
        ...
        test('Content Analyzer - Feature Availability', async ({ page }) => {
            ...
            await page.goto(`${RANKPILOT_APP_URL}/content-analyzer`, { waitUntil: 'domcontentloaded' });
        ...
        test('Mobile Navigation - Touch Targets', async ({ page }) => {
            ...
            if (hasMobileMenu) {
                ...
                try {
                    ...
                    console.log(`   ⚠️ Mobile menu click blocked (likely by reCAPTCHA): ${error instanceof Error ? error.message : String(error)}`);
                    // Don't fail the test if reCAPTCHA blocks the interaction
                }
            }
        });
