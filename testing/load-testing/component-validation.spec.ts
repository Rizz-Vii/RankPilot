...
test.describe('RankPilot Component Testing - UI Validation', () => {
        ...
        test('Feature Gate - Free Tier Access', async ({ page }) => {
            ...
            try {
                ...
                if (await featureGates.count() > 0) {
                    ...
                    const upgradePrompt = firstGate.locator(COMPONENT_SELECTORS.upgradePrompt);
                ...
                componentTestDiagnostics.errors.push(error instanceof Error ? error.message : String(error));
            ...
            try {
                ...
                for (const feature of premiumFeatures) {
                    ...
                    if (await element.count() > 0) {
                        ...
                        const parentGate = element.locator('..').locator(COMPONENT_SELECTORS.featureGate);
        ...
        test('Upgrade Prompts - Clear Call-to-Actions', async ({ page }) => {
            ...
            try {
                ...
                if (await upgradePrompts.count() > 0) {
                    ...
                    await expect(firstPrompt).toContainText(/upgrade|premium|pro|starter|agency|enterprise/i);
        ...
        test('Mobile Menu - Responsive Design', async ({ page }) => {
            ...
            try {
                ...
                if (await mobileMenu.count() > 0) {
                    ...
                    const menuToggle = page.locator('button[data-testid*="menu"], [aria-label*="menu"]').first();

                    if (await menuToggle.count() > 0) {
                        await menuToggle.click();

                        // Menu should expand or show navigation
                        const expandedMenu = page.locator('[data-testid*="menu"][aria-expanded="true"], .mobile-menu-open');
        ...
        test('Login Form - Authentication Interface', async ({ page }) => {
            ...
            try {
                ...
                if (await loginForm.count() > 0) {
                    ...
                    const passwordInput = loginForm.locator(COMPONENT_SELECTORS.passwordInput);
                    const submitButton = loginForm.locator(COMPONENT_SELECTORS.submitButton);
                    ...
                    const errorMessages = loginForm.locator('.error, [data-testid*="error"]');
        ...
        test('Signup Form - Registration Process', async ({ page }) => {
            ...
            try {
                ...
                if (await signupForm.count() > 0) {
                    ...
                    const passwordInput = signupForm.locator(COMPONENT_SELECTORS.passwordInput);
                    ...
                    const submitButton = signupForm.locator(COMPONENT_SELECTORS.submitButton);
                    await submitButton.click();

                    // Should show email validation error
                    const emailError = signupForm.locator('[data-testid*="email-error"], .email-error');
        ...
        test('Contact Form - Public Communication', async ({ page }) => {
            ...
            try {
                ...
                if (await contactForm.count() > 0) {
                    ...
                    // Check form fields
                    const emailInput = contactForm.locator('input[type="email"], [data-testid*="email"]');
                    const messageInput = contactForm.locator('textarea, [data-testid*="message"]');
                    ...
                    const submitButton = contactForm.locator('button[type="submit"], [data-testid*="submit"]');
                    await expect(submitButton).toBeVisible();
        ...
        test('Dashboard Layout - Main Interface', async ({ page }) => {
            ...
            try {
                ...
                if (await dashboard.count() > 0) {
                    ...
                    const metricsCards = dashboard.locator(COMPONENT_SELECTORS.metricsCard);
    ...
    test.describe('Error Handling Components - User Experience', () => {
        ...
        test('Loading States - User Feedback', async ({ page }) => {
            ...
            try {
                const loadingSpinners = page.locator(COMPONENT_SELECTORS.loadingSpinner);
    ...
    test.describe('Subscription Components - Billing Interface', () => {
        ...
        test('Pricing Table - Subscription Options', async ({ page }) => {
            ...
            try {
                ...
                if (await pricingTable.count() > 0) {
                    ...
                    const tiers = pricingTable.locator('[data-testid*="tier"], .pricing-tier');
        ...
        test('Subscription Status - Account Management', async ({ page }) => {
            ...
            try {
                const subscriptionStatus = page.locator(COMPONENT_SELECTORS.subscriptionStatus);
                ...
                if (await subscriptionStatus.count() > 0) {
                    ...
                    console.log(`   Subscription status: ${statusText?.substring(0, 50)}...`);
    ...
    test.describe('Performance Components - Loading Optimization', () => {
        ...
        test('Lazy Loading - Performance Optimization', async ({ page }) => {
            ...
            try {
                ...
                const lazyRequests = requests.filter(url =>
                    url.includes('lazy') || url.includes('chunk') || url.includes('dynamic')
                );
