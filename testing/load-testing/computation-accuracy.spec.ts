...
test.describe("RankPilot Computation & Data Accuracy Testing", () => {
    ...
    test("SEO Score Calculation - Valid Ranges", async ({ page }) => {
      ...
      try {
        ...
        for (const url of TEST_DATA.seoMetrics.sampleUrls) {
          ...
          const urlInput = page
            .locator('input[type="url"], [data-testid*="url"]')
            .first();
          const analyzeButton = page
            .locator('button:has-text("Analyze"), [data-testid*="analyze"]')
            .first();
          ...
          if (
            ...
            const scoreElements = page.locator(
              '[data-testid*="score"], .seo-score, .score-display'
            );
            ...
            for (const scoreText of scoreTexts) {
              ...
              if (scoreMatch) {
                ...
                const isValidRange =
                  score >= TEST_DATA.seoMetrics.expectedScores.minScore &&
                  score <= TEST_DATA.seoMetrics.expectedScores.maxScore;

                console.log(
                  `   Score ${score}: ${isValidRange ? "valid" : "invalid"}`
                );

                if (isValidRange) {
                  computationDiagnostics.accuracyChecks.push(
                    `Valid SEO score: ${score}`
                  );
                } else {
                  computationDiagnostics.accuracyChecks.push(
                    `Invalid SEO score: ${score}`
                  );
                }
              }
            }
      ...
      try {
        ...
        if ((await urlInput.count()) > 0 && (await analyzeButton.count()) > 0) {
          ...
          if (scores.length >= 2) {
            ...
            console.log(
              `   Score variance: ${variance} (${isConsistent ? "consistent" : "inconsistent"})`
            );

            computationDiagnostics.accuracyChecks.push(
              `Consistency check: variance=${variance}`
            );
          }
        }
        ...
        computationDiagnostics.accuracyChecks.push(
          `Consistency error: ${error instanceof Error ? error.message : String(error)}`
        );
      ...
      try {
        for (const keyword of TEST_DATA.keywordAnalysis.testKeywords) {
          ...
          const keywordInput = page
            .locator('input[type="text"], [data-testid*="keyword"]')
            .first();
          const searchButton = page
            .locator('button:has-text("Search"), [data-testid*="search"]')
            .first();

          if (
            (await keywordInput.count()) > 0 &&
            ...
            for (const metric of TEST_DATA.keywordAnalysis.expectedMetrics) {
              ...
              if (hasMetric) {
                ...
                computationDiagnostics.dataValidation.push(
                  `${keyword}-${metric}: ${metricValue}`
                );
              }
            }
          } else {
    ...
    test("Keyword Data Validation - Format Consistency", async ({ page }) => {
      ...
      try {
        ...
        if (
          ...
          const volumeElements = page.locator(
            '[data-testid*="volume"], .volume'
          );
          ...
          if ((await volumeElements.count()) > 0) {
            ...
            console.log(
              `   Volume format valid: ${isValidVolume} (${volumeText})`
            );
          }

          // Check difficulty format (should be 0-100)
          if ((await difficultyElements.count()) > 0) {
            const difficultyText = await difficultyElements
              .first()
              .textContent();
            ...
            if (difficultyMatch) {
              ...
              console.log(
                `   Difficulty format valid: ${isValidDifficulty} (${difficulty})`
              );
            }
          }
        ...
        computationDiagnostics.dataValidation.push(
          `Format validation error: ${error instanceof Error ? error.message : String(error)}`
        );
  ...
  test.describe("Performance Metrics Computation - Accuracy Verification", () => {
      ...
      try {
        ...
        const performanceData = await page.evaluate(() => {
          ...
          return {
            ...
            firstPaint:
              paint.find((entry) => entry.name === "first-paint")?.startTime ||
              0,
        ...
        const validations = {
          ttfb:
            performanceData.ttfb >=
              TEST_DATA.performanceData.validRanges.ttfb[0] &&
            performanceData.ttfb <=
              TEST_DATA.performanceData.validRanges.ttfb[1],
          domContentLoaded:
            performanceData.domContentLoaded >= 0 &&
            performanceData.domContentLoaded <= 10000,
          loadComplete:
            performanceData.loadComplete >= 0 &&
            performanceData.loadComplete <= 30000,
          firstPaint:
            performanceData.firstPaint >= 0 &&
            performanceData.firstPaint <= 10000,
          firstContentfulPaint:
            performanceData.firstContentfulPaint >= 0 &&
            performanceData.firstContentfulPaint <= 10000,
        };

        Object.entries(validations).forEach(([metric, isValid]) => {
          console.log(
            `   ${metric} valid: ${isValid} (${performanceData[metric as keyof typeof performanceData]}ms)`
          );
          computationDiagnostics.performanceMetrics.push(
    ...
    test("Performance Data Consistency - Measurement Accuracy", async ({
      page,
    }) => {
      ...
      try {
        ...
        const loadTimeVariance =
          Math.max(...loadTimeValues) - Math.min(...loadTimeValues);
        ...
        computationDiagnostics.performanceMetrics.push(
          `Consistency: TTFB=${ttfbConsistent}, Load=${loadTimeConsistent}`
        );
  ...
  test.describe("Data Accuracy Validation - Cross-Component Verification", () => {
    test("Dashboard Data Consistency - Component Synchronization", async ({
      page,
    }) => {
      ...
      try {
        ...
        computationDiagnostics.dataValidation.push(
          `Dashboard components: metrics=${hasMetrics}, charts=${hasCharts}, tables=${hasTables}`
        );
    ...
    test("Form Data Validation - Input Processing Accuracy", async ({
      page,
    }) => {
      ...
      try {
        ...
        if ((await contactForm.count()) > 0) {
          ...
          const nameInput = contactForm
            .locator('input[name="name"], [data-testid*="name"]')
            .first();
          const emailInput = contactForm
            .locator('input[name="email"], [data-testid*="email"]')
            .first();
          const messageInput = contactForm
            .locator('textarea[name="message"], [data-testid*="message"]')
            .first();
          ...
          if ((await messageInput.count()) > 0) {
            ...
            console.log(
              `   Message input filled: ${testData.message.length} characters`
            );
          }
          ...
          const validationErrors = contactForm.locator(
          ...
          if ((await submitButton.count()) > 0) {
            ...
            const successMessages = page.locator(
              "text=/success|submitted|thank you/i"
            );
            ...
            computationDiagnostics.dataValidation.push(
              `Form validation: success=${hasSuccess}, errors=${hasError}`
            );
          }
        } else {
    ...
    test("Search Results Accuracy - Query Processing", async ({ page }) => {
      ...
      try {
        const searchInput = page
          .locator('input[type="search"], [data-testid*="search"]')
          .first();
        const searchButton = page
          .locator('button:has-text("Search"), [data-testid*="search"]')
          .first();
        ...
        if (
          ...
          if (resultCount > 0) {
            ...
            const isRelevant =
              firstResult?.toLowerCase().includes(searchTerm.toLowerCase()) ||
              false;

            console.log(
              `   First result relevant: ${isRelevant} (${firstResult?.substring(0, 50)}...)`
            );

            computationDiagnostics.dataValidation.push(
              `Search accuracy: results=${resultCount}, relevant=${isRelevant}`
            );
          }
          ...
          const noResults = page.locator(
            "text=/no results|nothing found|0 results/i"
          );
          const hasNoResultsMessage = (await noResults.count()) > 0;

          console.log(
            `   No results message displayed: ${hasNoResultsMessage}`
          );

          computationDiagnostics.dataValidation.push(
            `No results handling: ${hasNoResultsMessage}`
          );
        } else {
