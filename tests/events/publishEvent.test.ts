...
import type {
  FirestoreLegacyCollectionRef,
  FirestoreLegacyDocRef,
} from "../../src/lib/events/publishEvent";
...
class InMemoryFirestore implements FirestoreLegacyCollectionRef {
  // also exposes dump() for test assertions
...
async function run() {
  ...
  if (
    writes.length !== 1 ||
    writes[0][0] !== `/orgs/org_abc/events/${res.eventId}`
  ) {
