...
(function run() {
    const off = on('bi.snapshot.requested', (evt) => {
        if (!evt || evt.type !== 'bi.snapshot.requested') throw new Error('unexpected type');
    });
    const delivered = emit('bi.snapshot.requested', { ts: Date.now(), source: 'test' });
    assert(delivered >= 1, 'should deliver to at least one subscriber');
    off();
    const delivered2 = emit('bi.snapshot.requested', { ts: Date.now(), source: 'test2' });
