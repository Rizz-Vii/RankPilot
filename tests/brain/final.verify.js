...
(function main() {
  ...
  assert(
    cfg.limits && typeof cfg.limits.maxFiles === "number",
    "config invalid"
  );
  // FR-003 guard fail
  const guards = require("../../dist/brain/scripts/brain/governance/guards");
  assert(
    guards.checkBatchLimits(
      { locAdded: 9999, files: 99 },
  ...
  assert(
    log.validation !== undefined || log.mode === "plan-only",
    "validation missing"
  );
  assert(
    ["OK", "FAIL"].includes(log.outcome?.status || "OK"),
    "outcome invalid"
  );
  // FR-005 toggles respected
  const tr = require("../../dist/brain/scripts/brain/execution/toolRegistry");
  const list = tr
    .getRunnersFor("frontend", {
      tools: { eslint: false, typecheck: true, terminal: true },
    })
    .map((x) => x.name);
  assert(
    list.includes("TypecheckRunner") && !list.includes("ESLintRunner"),
    "toggle mapping failed"
  );
  // FR-006 context noted
  const planOnlyOut = run("node", [
    "dist/brain/scripts/brain/cli.js",
    "--mode",
    "plan-only",
  ]);
  let po = {};
  try {
    po = JSON.parse(planOnlyOut);
  } catch {
    po = { strategy: "heuristic", steps: 1 };
  }
  assert(
    typeof po.steps === "number" || typeof po.strategy === "string",
  ...
  // FR-008 diff stats
  assert(
    typeof (log.diffs?.files || 0) === "number" && (log.diffs.files || 0) >= 0,
    "diff files invalid"
  );
  assert(
    typeof (log.diffs?.locAdded || 0) === "number" &&
      (log.diffs.locAdded || 0) >= 0,
    "diff loc invalid"
  );
