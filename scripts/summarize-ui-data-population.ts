...
function summarize(entries: Entry[]) {
  ...
  const latest: Entry[] = Object.values(byLabel).map(
    (list) => list.sort((a, b) => b.ts - a.ts)[0]
  );
  ...
  for (const e of latest) {
    lines.push(
      `| ${e.label} | ${e.path} | ${e.status}${e.degrade ? " (degraded)" : ""} | ${e.reason || ""} | ${e.attempts ?? ""} | ${e.count ?? ""} |`
    );
  }
  ...
  if (failures.length) {
    ...
    for (const f of failures) {
      lines.push(
        `- ${f.label}: ${f.reason || "failed"} (attempts=${f.attempts})`
      );
    }
  }
  return lines.join("\n");
