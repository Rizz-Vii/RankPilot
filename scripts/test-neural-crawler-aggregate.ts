...
async function simulateClientCrawl(url: string) {
  ...
  const historyRef = await db
    .collection("neuralCrawlerHistory")
    .add({ userId, url, status: "completed", createdAt: new Date() });
  const legacyDoc = {
    userId,
    ...
    links: [
      { href: "/a", text: "A", type: "internal" },
      { href: "https://ext", text: "E", type: "external" },
    ],
    technicalData: {
      loadTime: 1.1,
      pageSize: 12345,
...
async function main() {
  ...
  assert.strictEqual(
    data.readingTime,
    legacyDoc.readingTime,
    "readingTime mismatch"
  );
  assert.strictEqual(
    data.imagesCount,
    legacyDoc.images.length,
    "imagesCount mismatch"
  );
  assert.strictEqual(
    data.linksInternal + data.linksExternal,
    legacyDoc.links.length,
    "links total mismatch"
  );
  assert.strictEqual(
    data.issuesCount,
    legacyDoc.issues.length,
    "issuesCount mismatch"
  );
  assert.strictEqual(
    data.entitiesCount,
    legacyDoc.entities.length,
    "entitiesCount mismatch"
  );
...
main().catch((e) => {
  console.error("T14 neural crawler aggregate test FAIL", e);
  process.exit(1);
});
