...
let UNIFIED_TEST_USERS:
  | Record<string, { email: string; password: string }>
  | undefined;
try {
  UNIFIED_TEST_USERS =
    require("../testing/config/unified-test-users").UNIFIED_TEST_USERS;
} catch {
...
const ROUTES_FILE =
  ...
  path.resolve(process.cwd(), "artifacts", "next-routes.txt");
const OUT_PATH = path.resolve(
  process.cwd(),
  "artifacts",
  "route-crawl-report.json"
);
...
function shouldCapture(msg: ConsoleMessage): {
  ...
  const cspHit =
    /content security policy|refused to.*inline|unsafe-inline/.test(lower);
  const connClosed =
    lower.includes("connection closed") || lower.includes("net::err_http2");
  ...
  if (type === "error" || connClosed || cspHit || resourceFail || firebaseFail)
    return { type: "error", text };
  if (
    type === "warning" &&
    (cspHit || lower.includes("deprecated") || lower.includes("blocked"))
  )
...
async function visitPage(
  ...
  page.on("console", (msg: ConsoleMessage) => {
    ...
    if (cap.type === "warning" && cap.text)
      result.consoleWarnings!.push(cap.text);
  });
...
async function maybeLogin(
  ...
  try {
    const role = (process.env.CRAWL_LOGIN_ROLE || "enterprise") as
      | "enterprise"
      | "admin";
    const email =
      process.env.CRAWL_LOGIN_EMAIL || UNIFIED_TEST_USERS?.[role]?.email;
    const password =
      process.env.CRAWL_LOGIN_PASSWORD || UNIFIED_TEST_USERS?.[role]?.password;
    ...
    await page
      .waitForURL(/\/(dashboard|adminonly|app)(\/.*)?$/, { timeout: 30000 })
      .catch(() => {});
...
const GET_STREAM_PATHS = new Set<string>(["/api/insights/stream"]);
...
async function probeApi(
  ...
  if (
    (result.status === 405 || POST_ONLY.has(pathLabel)) &&
    !(result.status && result.status >= 200 && result.status < 300)
  ) {
    try {
      const res2 = await context.request.post(url, {
        timeout: 15000,
  ...
  const preflightMethod: "GET" | "POST" = GET_STREAM_PATHS.has(pathLabel)
    ? "GET"
    : isStream
      ? "POST"
      : "GET";
...
async function main() {
  ...
  const report: Report = {
    ...
    total: pages.length + apis.length,
    pages: [],
    apis: [],
  };
  ...
  const pageErrors = report.pages.reduce(
    (n, r) => n + (r.consoleErrors?.length || 0),
    0
  );
  const pageFails = report.pages.reduce(
    (n, r) => n + (r.requestFailures?.length || 0),
    0
  );
  const api401 = report.apis.filter((r) => r.status === 401).length;
  const api4xx5xx = report.apis.filter(
    (r) => (r.status ?? 0) >= 400 && r.status !== 401
  ).length;
  console.log(`\nReport written: ${OUT_PATH}`);
  console.log(
    `Summary: pages=${report.pages.length} (consoleErrors=${pageErrors}, failedRequests=${pageFails}), apis=${report.apis.length} (401(auth-required)=${api401}, other4xx/5xx=${api4xx5xx})`
  );
}
