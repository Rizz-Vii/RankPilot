...
async function main() {
  ...
  try {
    ...
    try {
      raw = run("npx eslint . -f json");
    } catch {
      try {
        raw = run("npx next lint --format json");
      } catch (e3) {
        raw = e3.stdout || "";
      }
    }
  ...
  if (parsed.length === 0) {
    try {
      parsed = await runMetrics();
    } catch {
      /* degraded */
    }
  }
  const { errorCount, warningCount, ruleTally } = aggregate(parsed);
  const topRules = Object.entries(ruleTally)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 15);
  const snapshot = {
  ...
  if (parsed.length === 0) {
    ...
    snapshot.note =
      "ESLint patch failure encountered; metrics collection degraded (no files).";
  }
