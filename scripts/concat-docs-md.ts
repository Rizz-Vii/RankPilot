...
interface FileEntry { rel: string; abs: string; date?: string; mtime: number; }
...
function main() {
    ...
    entries.sort((a, b) => {
        if (a.date && b.date) return a.date.localeCompare(b.date) || a.rel.localeCompare(b.rel);
    ...
    for (const e of entries) {
        ...
        try {
            ...
            if (stat.size > 200 * 1024) { // 200KB safety cap
    ...
    console.log(`Wrote ${entries.length} markdown files into ${path.relative(ROOT, OUTPUT)}`);
}
