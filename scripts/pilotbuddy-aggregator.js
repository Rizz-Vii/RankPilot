...
function getProjectMetrics() {
  ...
  try {
    ...
    const testFiles = listFilesRecursive("tests").filter((f) =>
      f.endsWith(".spec.ts")
    );
    metrics.testFiles = testFiles.length;

    // Count components (recursive)
    const componentFiles = listFilesRecursive("src/components").filter((f) =>
      f.endsWith(".tsx")
    );
    metrics.components = componentFiles.length;

    // Count documentation (recursive)
    const docFiles = listFilesRecursive("docs").filter((f) =>
      f.endsWith(".md")
    );
    ...
    const scriptFiles = fs.existsSync("scripts")
      ? fs
          .readdirSync("scripts")
          .filter(
            (f) => f.endsWith(".js") || f.endsWith(".ps1") || f.endsWith(".sh")
          )
      : [];
    ...
    try {
      const pilotScriptFiles = listFilesRecursive("pilotScripts").filter(
        (f) =>
          f.endsWith(".js") ||
          f.endsWith(".ps1") ||
          f.endsWith(".sh") ||
...
`;
  ...
  const pilotScripts = Object.entries(packageScripts)
    .filter(
      ([key, value]) => key.includes("pilot") || key.includes("pilotbuddy")
    )
    .sort();
...
try {
  ...
  console.log("\n📋 Project Summary:");
  console.log(
    `   Tests: ${metrics.testFiles} | Components: ${metrics.components} | Docs: ${metrics.documentationFiles}`
  );
