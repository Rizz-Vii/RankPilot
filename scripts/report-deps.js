...
async function run(){
  ...
  console.log('\nMissing (referenced but not installed):', Object.keys(result.missing||{}).join(', ')||'None');
  console.log('\nPeer Issues (not checked):', Object.keys(result.invalidDirs||{}).length);
  console.log('\nScripts summary (core set):', Object.keys(pkg.scripts).filter(s=>['dev','build','start','test','lint'].includes(s)).join(', '));
}
run();
