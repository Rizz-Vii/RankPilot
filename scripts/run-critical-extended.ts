...
if (!enabled) {
    console.log('[critical-extended] Skipped (set CRITICAL_EXTENDED=1 to enable)');
    process.exit(0);
}

const commands: Array<[string, string[]]> = [
    // New crawler adoption alert Playwright spec
    ['npx', ['playwright', 'test', 'testing/specs/main/api/health-crawler-adoption-alert.spec.ts', '--config=playwright.config.role-based.ts', '--project=api-testing']],
...
for (const [cmd, args] of commands) {
    ...
    if (res.status !== 0) {
        console.error(`[critical-extended] Failed command: ${cmd} ${args.join(' ')}`);
