...
export const ALLOWED_TRIAGE_ACTIONS = [
  "none",
  "start_delegation",
  "enqueue_ts_fixes",
  "start_and_enqueue",
...
export function evaluateAgentTriageOutput(
  raw: unknown,
  metrics?: { guardrailFailures: number }
): ParsedTriage | null {
  ...
  try {
    ...
    try {
      utilMod = require("../../../../scripts/brain/triage-util.js");
    } catch {
      /* ignore */
    }
  }
  if (
    utilMod &&
    typeof (utilMod as Record<string, unknown>).extractFirstJsonObject ===
      "function"
  ) {
    extractFirstJsonObject = (
      utilMod as { extractFirstJsonObject: (s: string) => unknown }
    ).extractFirstJsonObject;
  } else {
    extractFirstJsonObject = (s: string) => {
      try {
        return JSON.parse(s);
      } catch {
        return undefined;
...
export function buildTriageInstructions(diag: {
  tsErrors: number;
  lintErrors: number;
  lintWarnings: number;
}) {
