 ...
 */
import { existsSync, writeFileSync as fsWrite, mkdirSync, readFileSync, rmSync, writeFileSync } from 'fs';
...
interface LintCycleResult { planned: number; drift?: number; adaptiveMax?: number; considered: number; skippedDueToHash?: boolean }
 
const runTwoAgentLintCycle = twoAgentMod.runTwoAgentLintCycle as (opts: { maxTasks?: number }) => Promise<LintCycleResult>;
...
const INTERVAL_MS = Number(process.env.TWO_AGENT_PLAN_LOOP_INTERVAL_MS || 45000); // 45s default
const START = Date.now();
const TRIGGER_FILE = process.env.TWO_AGENT_DELEGATION_TRIGGER_FILE || '.codex/tmp/delegation-trigger.signal';
const TRIGGER_ENABLED = process.env.TWO_AGENT_DELEGATION_TRIGGER !== '0';

function mins() { return (Date.now() - START) / 60000; }
    ...
    try {
        ...
        if (existsSync(PID_FILE)) {
            ...
            if (!isNaN(priorPid) && priorPid !== process.pid) {
                ...
                if (procExists && !FORCE) {
                    console.log(`[two-agent:plan-loop] existing instance detected (pid=${priorPid}); exiting secondary invocation (set TWO_AGENT_PLAN_LOOP_FORCE=1 to override).`);
                    process.exit(0);
                } else if (procExists && FORCE) {
                    console.log(`[two-agent:plan-loop] FORCE override active; proceeding despite existing pid=${priorPid}.`);
                } else {
                    // Stale lock
        ...
        process.on('exit', () => { try { rmSync(PID_FILE); } catch { /* ignore */ } });
        process.on('SIGINT', () => { try { rmSync(PID_FILE); } catch { /* ignore */ } process.exit(0); });
        process.on('SIGTERM', () => { try { rmSync(PID_FILE); } catch { /* ignore */ } process.exit(0); });
    } catch { /* ignore lock errors */ }
}

async function main() {
    ensureSingleInstance();
    console.log(`[two-agent:plan-loop] start maxIters=${MAX_ITERS} maxMinutes=${MAX_MINUTES} intervalMs=${INTERVAL_MS}`);
    for (let iter = 1; iter <= MAX_ITERS; iter++) {
        ...
        try {
            const cycle = await runTwoAgentLintCycle({ maxTasks: Number(process.env.TWO_AGENT_MAX_TASKS || 5) });
            // Heartbeat (cheap overwrite)
            try {
                const hb = { ts: new Date().toISOString(), iter, planned: cycle.planned, considered: cycle.considered, drift: cycle.drift ?? null, adaptiveMax: cycle.adaptiveMax ?? null, skippedDueToHash: !!cycle.skippedDueToHash };
                mkdirSync(path.dirname(HEARTBEAT_FILE), { recursive: true });
                writeFileSync(HEARTBEAT_FILE, JSON.stringify(hb));
            } catch { /* ignore heartbeat errors */ }
            console.log(`[two-agent:plan-loop] iter=${iter} planned=${cycle.planned} considered=${cycle.considered} drift=${cycle.drift ?? 'n/a'} adaptiveMax=${cycle.adaptiveMax ?? 'n/a'}${cycle.skippedDueToHash ? ' (skippedDueToHash)' : ''}`);
            if (TRIGGER_ENABLED && cycle.planned > 0) {
                try {
                    const abs = path.resolve(TRIGGER_FILE);
                    mkdirSync(path.dirname(abs), { recursive: true });
                    writeFileSync(abs, String(Date.now()));
                    console.log(`[two-agent:plan-loop] delegation trigger emitted → ${abs}`);
                } catch (e) {
            ...
            if (cycle.planned === 0 && cycle.skippedDueToHash) {
                ...
                console.log(`[two-agent:plan-loop] unchanged hash; extended sleep ${backoff}ms`);
                await sleep(backoff);
            } else {
