...
function formatList<T>(items: T[], fmt: (v: T, i: number) => string): string {
  return items
    .map((v, i) => `  ${dim((i + 1).toString().padStart(2, "0"))} ${fmt(v, i)}`)
    .join("\n");
}
...
async function main() {
  ...
  if (!v.ok) throw new Error("config invalid");
  ...
  if (mode === "mission") {
    const previousMission = (() => {
      try {
        if (fs.existsSync("artifacts/brain/previousMission.json"))
          return JSON.parse(
            fs.readFileSync("artifacts/brain/previousMission.json", "utf8")
          );
      } catch {}
      return undefined;
    })();
    ...
    const delta = previousMission
      ? {
          tsErrors:
            mission.diagnostics.typecheck.errors -
            (previousMission.diagnostics?.typecheck?.errors || 0),
          lintErrors:
            mission.diagnostics.lint.errors -
            (previousMission.diagnostics?.lint?.errors || 0),
          lintWarnings:
            mission.diagnostics.lint.warnings -
    ...
    if (asJson) {
      ...
      console.log(
        banner("Mission Snapshot") +
          ...
          (delta
            ? `  ${dim("[Δ TS=" + (delta.tsErrors >= 0 ? "+" : "") + delta.tsErrors + " LE=" + (delta.lintErrors >= 0 ? "+" : "") + delta.lintErrors + " LW=" + (delta.lintWarnings >= 0 ? "+" : "") + delta.lintWarnings + "]")}`
            : "") +
          ...
          (mission.immediateSteps.length
            ? formatList(
                mission.immediateSteps,
                (s) => green(s.title) + dim(" — " + s.rationale)
              )
            : dim("  (none)")) +
          "\n"
      );
      ...
      .map((f) => {
        try {
          ...
          return {
            runId: obj.runId,
            mode: obj.mode,
            status: obj.outcome?.status,
            files: obj.diffs?.files,
            loc: obj.diffs?.locAdded,
            timeMs: obj.metrics?.elapsedMs,
          };
        } catch {
          return null;
        }
    ...
    try {
      mission = JSON.parse(
        fs.readFileSync("artifacts/brain/currentMission.json", "utf8")
      ) as Mission;
    } catch {}
    if (asJson) {
      console.log(
        JSON.stringify({
          ...
          mission: mission
            ? {
                tsErrors: mission.diagnostics.typecheck.errors,
                lintErrors: mission.diagnostics.lint.errors,
                lintWarnings: mission.diagnostics.lint.warnings,
              }
            : null,
        })
      );
      ...
      console.log(
        banner("Loop Status") +
          ...
          summaries
            .map(
              (r, i) =>
                `  ${dim(String(i + 1).padStart(2, "0"))} ${green(r!.runId)} ${dim(r!.mode)} ${r!.status === "OK" ? green("OK") : red("FAIL")} ${dim(`files=${r!.files || 0} loc=${r!.loc || 0} t=${r!.timeMs || 0}ms`)}`
            )
            .join("\n") +
          "\n" +
          (mission
            ? `${cyan("Current Mission")}: TS=${mission.diagnostics.typecheck.errors} LintE=${mission.diagnostics.lint.errors} LintW=${mission.diagnostics.lint.warnings}\n`
    ...
    let mission: Mission | undefined = undefined;
    try {
      if (fs.existsSync("artifacts/brain/currentMission.json"))
        mission = JSON.parse(
          fs.readFileSync("artifacts/brain/currentMission.json", "utf8")
        );
    } catch {}
    ...
    if (asJson) {
      ...
      console.log(
        banner("Enhancement Priorities") +
          ...
          formatList(
            analysis.ordering.slice(0, 8),
            (o) =>
              `${o.rank <= 3 ? yellow(o.rank.toString()) : dim(o.rank.toString())} ${green(o.title)} ${dim(o.reason)}`
          ) +
          "\n"
      );
    }
    ...
    const intervalMs =
      Number(
        arg("intervalMs", process.env.BRAIN_AUTOPULSE_INTERVAL_MS || "60000")
      ) || 60000;
    const results: Array<{
      cycle: number;
      tsErrors: number;
    ...
    for (let i = 0; i < cycles; i++) {
      ...
      const mission = runMissionCycle();
      const p: BrainPlan = plan(
        [
          {
            id: "autopulse-" + i,
            title: "autopulse context refresh",
            raw: "refresh",
            domain: "docs",
            status: "TODO",
          } as Task,
    ...
    if (asJson) {
      ...
      console.log(
        banner("Autopulse Summary") +
          `${cyan("Cycles")}: ${results.length} interval=${intervalMs}ms\n` +
          results
            .map(
              (r) =>
                `  ${dim(String(r.cycle).padStart(2, "0"))} TS=${r.tsErrors} LintE=${r.lintErrors} LintW=${r.lintWarnings} steps=${r.steps} ${dim(r.elapsedMs + "ms")}`
            )
            .join("\n") +
          "\n"
      );
  ...
  if (process.env.BRAIN_PLANNER_DEBUG === "1") {
    try {
      console.error(
        dim(
          `[planner:tasks] parsed=${tasks.length} sample="${tasks
            .slice(0, 5)
            .map((t) => t.title)
            .join(" | ")}"`
        )
  ...
  tasks = tasks.map((t) => ({
    ...t,
    domain: classify(t.title + " " + (t.raw || "")),
  }));
  const useOpenAI = process.env.BRAIN_USE_OPENAI === "1";
  const p: BrainPlan = useOpenAI
    ? await planWithOpenAI(tasks, cfg, 8)
    : plan(tasks, { contextKb: 8 });
  ...
  const completionRate = Number(
    process.env.BRAIN_COST_COMPLETION_RATE || "0.0000020"
  );
  ...
  const preflightEstimate = {
    files: Math.min(tasks.length, cfg.governance?.maxBatchTasks || 10),
    locAdded: estLoc,
  };
  const runId = "cli-" + Date.now();
  // Persist enriched plan JSON with step file hints (if present)
  try {
    const stepsArr: unknown[] = Array.isArray(p.steps)
      ? (p.steps as unknown[])
      : [];
    const enriched = {
      ...p,
      steps: stepsArr.map((s) => {
        ...
        const runners = Array.isArray(r.runners)
          ? r.runners.filter((x): x is string => typeof x === "string")
          : undefined;
  ...
  if (
    process.env.BRAIN_ENQUEUE_FROM_PLAN === "1" &&
    (p.steps?.length || 0) > 0
  ) {
    try {
      ...
      if (!fs.existsSync(queueFile)) {
        ...
        fs.writeFileSync(
          queueFile,
          JSON.stringify({
            meta: "delegation queue (JSON Lines). Each line: pending task.",
          }) + "\n"
        );
      }
      const existing = fs
      ...
      const existingIds = new Set(
        existing
          .map((t) =>
            t &&
            typeof t === "object" &&
            "taskId" in (t as Record<string, unknown>) &&
            typeof (t as { taskId?: unknown }).taskId === "string"
              ? (t as { taskId: string }).taskId
              : ""
          )
          .filter(Boolean)
      );
      ...
      for (const step of p.steps || []) {
        ...
        if (
          filesUnknown &&
          typeof filesUnknown === "object" &&
          "files" in (filesUnknown as Record<string, unknown>)
        ) {
        ...
        if (
          process.env.BRAIN_FILTER_NOFILE_STEPS === "1" &&
          (!files || !files.length)
        )
          continue;
        const task = {
          taskId,
          summary: `Brain planned step ${idx} (${step.kind || "do"})`,
          status: "pending",
          createdAt: nowIso,
          updatedAt: nowIso,
          files,
      ...
      if (newLines.length)
        fs.appendFileSync(queueFile, newLines.join("\n") + "\n");
    } catch {}
  }
  if (watch) {
    ...
    const trigger = () => {
      ...
      const run = () => {
        try {
          console.log(dim("[watch] change detected -> refreshing plan-only"));
        } catch {}
        try {
          require("./state/memory").recordMemory({
            ts: Date.now(),
            source: "brain",
            kind: "watch-trigger",
            status: "ok",
            meta: { reason: "fs-change" },
          });
        } catch {}
        ...
        const proc = spawn(
          "node",
          ["dist/brain/scripts/brain/cli.js", "--mode", "plan-only"],
          {
            stdio: "ignore",
            detached: true,
            env: { ...process.env, BRAIN_AUTOTRIGGERED: "1" },
          }
        );
        proc.unref();
      };
      if (nowTs - last < debounceMs) {
    ...
    watchDirs.forEach((d) => {
      if (fs.existsSync(d))
        fs.watch(d, { recursive: true }, (_evt, _file) => trigger());
    });
  }
  ...
  const toolsInvoked = Array.from(
    new Set(
      tasks.flatMap((t) =>
        getRunnersFor(t.domain || "docs", cfg).map((r) => r.name)
      )
    )
  );
  ...
  if (mode === "execute") {
    validation = await runValidators({
      ...
      forceFail:
        forceValidatorFail ||
        process.env.PB_BRAIN_FORCE_VALIDATION_FAIL === "1",
    });
    const exec = await runBatch(tasks, {
      mode,
      cfg,
      preflightEstimate: verifyGuardFail
        ? { files: 99, locAdded: 9999 }
        : preflightEstimate,
    });
    diffs = exec.diffs;
    ...
    for (const _ of groups) {
      ...
      if (Date.now() - tStart > cfg.budget.timeSeconds * 1000) {
        aborted = true;
        followUps.push({ type: "budget", note: "time exceeded" });
        break;
      }
      if (tokenUsed > cfg.budget.token) {
        aborted = true;
        followUps.push({ type: "budget", note: "token exceeded" });
        break;
      }
    }
  } else {
    ...
    if (asJson) {
      ...
      console.log(
        JSON.stringify({
          ...
          cost: {
            promptRate,
            completionRate,
            estCost: Number(estCost.toFixed(6)),
          },
        })
      );
    } else {
      ...
      const loopSuggestion = forceCodex
        ? "codex (forced)"
        : p.steps.some((s) => {
              const step = s as { contextNote?: unknown };
              const note =
                typeof step.contextNote === "string" ? step.contextNote : "";
      ...
      console.log(
        banner("Plan Summary") +
          ...
          (useOpenAI && tokenMeta.totalTokens
            ? `${cyan("Token Usage")}: prompt=${promptTokens} completion=${completionTokens} total=${tokenMeta.totalTokens} estCost=$${estCost.toFixed(4)}${tokenMeta.model ? " model=" + tokenMeta.model : ""}\n`
            : "") +
            ...
            .map((s, i: number) => {
              const r =
                s && typeof s === "object"
                  ...
                  : {};
              const note =
                typeof r.contextNote === "string" ? r.contextNote : "";
              const kind = typeof r.kind === "string" ? r.kind : "step";
              const runners = Array.isArray(r.runners)
                ? r.runners.filter((x): x is string => typeof x === "string")
                : [];
              let loop: "aider" | "codex";
              if (forceCodex) loop = "codex";
              else if (runners.includes("codex")) loop = "codex";
              else if (/lint|type/i.test(note)) loop = "aider";
              else loop = "codex";
              const tag =
                loop === "aider" ? yellow("[aider]") : cyan("[codex]");
  ...
  if (outcome.status === "FAIL") {
    try {
      ...
      fs.writeFileSync(
        ...
        JSON.stringify({ reason: "validation", tasks }, null, 2)
      );
    } catch {}
    if (aborted)
      followUps.push({
        type: "action",
        suggestion: "Reduce scope or increase budget",
      });
  }
  await writeRunLog({
    ts: Date.now(),
    ...
    metrics: {
      ...
      budget: {
        tokenUsed,
        tokenBudget: cfg.budget.token,
        timeUsedMs: Date.now() - tStart,
        timeBudgetMs: cfg.budget.timeSeconds * 1000,
      },
    },
  });
  if (
    !asJson &&
    (mode === "execute" || mode === "dry-run" || mode === "auto")
  ) {
    console.log(
      banner("Run Outcome") +
        `${cyan("Date")}: ${tsHuman}  ${cyan("Melb")}: ${melbTime} UTCΔ=${tzDiffStr}\n` +
        ...
        (followUps.length
          ? "\n" +
            ...
            formatList(
              followUps,
              (f) => yellow(f.type) + dim(" " + (f.note || f.suggestion || ""))
            )
          : "") +
        "\n"
    );
