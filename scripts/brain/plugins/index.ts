...
export interface BrainPluginModule {
  runners?: ToolRunner[];
  validators?: Array<
    (ctx: unknown) => Promise<{ name: string; status: string; note?: string }>
  >;
}
...
export function loadPlugins(): LoadedPlugins {
  ...
  const sourceDirCandidate = path.join(
    process.cwd(),
    "scripts",
    "brain",
    "plugins"
  ...
  const runningFromDist =
    /dist[\\/ ]brain[\\/ ]/.test(compiledDirCandidate) &&
    fs.existsSync(path.join(compiledDirCandidate, "index.js"));
  const dir =
    runningFromDist && fs.existsSync(compiledDirCandidate)
  ...
  try {
    ...
    const files = fs
      .readdirSync(dir)
      .filter(
        (f) => f.endsWith(".plugin.js") || (allowTs && f.endsWith(".plugin.ts"))
      );
    ...
    console.error(
      "[plugins] plugin directory read failed",
      (e as Error).message
    );
  }
