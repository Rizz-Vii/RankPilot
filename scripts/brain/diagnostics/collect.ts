...
function run(label: string, cmd: string, args: string[], maxOutput = 8000) {
    ...
    const truncated = out.length > maxOutput ? out.slice(0, maxOutput) + `\n...<truncated ${out.length - maxOutput} chars>` : out;
    return { label, code: res.status ?? 0, out: truncated, durationMs: Date.now() - started };
}

function extractTsErrorCount(raw: string): number {
    const m = raw.match(/Found (\d+) error/); if (m) return Number(m[1]);
...
export function collectDiagnostics(): DiagnosticsResult {
    ...
    const { errors: lintErrors, warnings: lintWarnings } = extractLintCounts(lint.out);
    ...
    try {
        ...
        fs.writeFileSync(path.join(dir, 'diagnostics-latest.json'), JSON.stringify(diag, null, 2));
    } catch { }
    return diag;
}
