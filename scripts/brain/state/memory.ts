...
export function recordMemory(ev: MemoryEvent) {
  try {
    ...
    try {
      ...
      const diff = melbOffset - serverOffset;
      ...
      ev.meta = {
        ...(ev.meta || {}),
        date: ev.meta?.date || dateStr,
        melbTime: ev.meta?.melbTime || melbTime,
        utcDelta: ev.meta?.utcDelta || delta,
      };
    } catch {}
    ...
    if (prior && now - prior.ts < 60_000) {
      ...
      try {
        const raw = fs.existsSync(MEM_FILE)
          ? fs.readFileSync(MEM_FILE, "utf8").trimEnd()
          : "";
        ...
        if (lines.length) {
          ...
          try {
            ...
            fs.appendFileSync(
              MEM_FILE,
              JSON.stringify({ ...ev, __aggKey: key, aggCount: prior.count }) +
                "\n"
            );
          }
        } else {
    ...
    try {
      ...
      /* pruning non-critical */
    }
  } catch (e) {
    if (process.env.DEBUG_MEMORY)
      console.error("[memory] write failed", (e as Error).message);
  }
}

function brainDistReady() {
  return fs.existsSync(
    path.join(process.cwd(), "dist", "brain", "scripts", "brain", "cli.js")
  );
}
...
function maybeTriggerBrain() {
  ...
  lastTrigger = now;
  const cmd = brainDistReady()
    ? "node dist/brain/scripts/brain/cli.js --mode plan-only"
