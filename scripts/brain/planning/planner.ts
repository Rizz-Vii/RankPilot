...
interface MissionLike {
  ...
  immediateSteps?: { title: string; rationale: string }[];
}
interface MemoryEventLike {
  source?: string;
  kind?: string;
  status?: string;
  id?: string;
...
export interface BrainPlan {
  steps:
    | NormalizedStep[]
    | { kind: string; taskId: string; domain: string; contextNote?: string }[];
...
function loadMission(): MissionLike | undefined {
  try {
    ...
    if (fs.existsSync(f))
      return JSON.parse(fs.readFileSync(f, "utf8")) as MissionLike;
  } catch {
    /* ignore errors reading mission */
  }
  return undefined;
}
...
function loadRecentMemory(limit = 40): MemoryEventLike[] {
  try {
    ...
    return lines
      .map((l) => {
        try {
          return JSON.parse(l) as MemoryEventLike;
        } catch {
          return null;
        }
      })
...
export function plan(batch: Task[], opts?: { contextKb?: number }): BrainPlan {
  ...
  const failedIds = new Set(
    mem
      .filter((m) => m.kind === "task-complete" && m.status === "fail")
      .map((m) => m.id)
      .filter((v): v is string => Boolean(v))
  );
  const prioritized = [
    ...batch.filter((b) => failedIds.has(b.id)),
    ...batch.filter((b) => !failedIds.has(b.id)),
  ];
  const diagNote =
    tsErrors !== undefined
      ? ` diag:ts=${tsErrors},lintE=${lintErrors},lintW=${lintWarnings}`
      : "";
  const forceCodex = process.env.BRAIN_FORCE_CODEX === "1";
  const steps = prioritized.map((t) => ({
    kind: "do-task",
    taskId: t.id,
    domain: t.domain || "docs",
    contextNote: `files:${ctx.files.length}${diagNote}`,
    runners: forceCodex ? ["codex"] : undefined,
  }));
...
interface PlannerCfg {
  ...
  tokens?: { plannerModel?: string; temperature?: number };
  [k: string]: unknown;
}
export async function planWithOpenAI(
  ...
  const use =
    Boolean(
      cfg?.tools && (cfg.tools as Record<string, unknown>).openaiPlanner
    ) &&
    process.env.OPENAI_API_KEY &&
    process.env.BRAIN_USE_OPENAI === "1";
  if (process.env.BRAIN_PLANNER_DEBUG === "1") {
    console.log(
      `[planner:openai] useFlag=${process.env.BRAIN_USE_OPENAI} keyPresent=${!!process.env.OPENAI_API_KEY} keyLen=${(process.env.OPENAI_API_KEY || "").length} gpt5Len=${(process.env.OPENAI_GPT5_KEY || "").length} orgLen=${(process.env.OPENAI_ORGANIZATION || "").length} model=${process.env.BRAIN_OPENAI_MODEL || cfg?.tokens?.plannerModel || "gpt-4o-mini"} temp=${cfg?.tokens?.temperature ?? 0.2}`
    );
  }
  if (!use && process.env.BRAIN_FORCE_OPENAI_STRATEGY !== "1")
  ...
  const missionLines = mission
    ? [
        "Mission summary: " +
          mission.summary +
          ` (tsErrors=${mission.diagnostics?.typecheck?.errors}, lintErrors=${mission.diagnostics?.lint?.errors}, lintWarnings=${mission.diagnostics?.lint?.warnings})`,
        "Immediate mission steps:",
        ...(mission.immediateSteps || [])
          .slice(0, 3)
          .map((s, i: number) => `  ${i + 1}. ${s.title} :: ${s.rationale}`),
      ]
  ...
  const prompt = [
    ...
    ...mem
      .slice(-10)
      .map((m) => ` - ${m.source}:${m.kind}:${m.status || ""}:${m.id || ""}`),
  ...
  try {
    ...
    const model =
      process.env.BRAIN_OPENAI_MODEL ||
      cfg.tokens?.plannerModel ||
      "gpt-4o-mini";
      ...
      messages: [
        {
          role: "system",
          content:
            "You are an expert software planner. Output ONLY a JSON array of planning steps. No commentary.",
        },
    ...
    if (!parsed.length)
      console.error("[planner:openai] parse-fail; rawLen=", raw.length);
    ...
    for (const p of parsedArr) {
      if (
        p &&
        typeof p === "object" &&
        "taskId" in (p as Record<string, unknown>) &&
        typeof (p as { taskId?: unknown }).taskId === "string" &&
        "files" in (p as Record<string, unknown>) &&
        Array.isArray((p as { files?: unknown[] }).files)
      ) {
        const taskId = (p as { taskId: string }).taskId;
        const files = (p as { files: unknown[] }).files.filter(
          (f): f is string => typeof f === "string" && f.length < 260
        );
    ...
    for (const s of steps as Array<NormalizedStep & { files?: string[] }>) {
      ...
      if (cand && cand.length) {
        s.files = cand
          .filter(
            (f) =>
              /^(src|scripts|docs|functions|package\.json|README\.md)/.test(
                f
              ) &&
              (exists(f) || /package\.json|README\.md$/.test(f))
          )
          .slice(0, 4);
      }
    }
    ...
    if (process.env.BRAIN_FORCE_OPENAI_STRATEGY === "1") {
      const fb = plan(batch, { contextKb: ctxKb });
      return {
        steps: fb.steps as NormalizedStep[],
        strategy: "openai",
        meta: {
          fallback: true,
          model:
            (process.env.BRAIN_OPENAI_MODEL as string) ||
            (cfg as PlannerCfg).tokens?.plannerModel ||
            "gpt-4o-mini",
        },
      };
    }
