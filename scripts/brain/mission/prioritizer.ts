...
export interface EnhancementPreference { rank: number; id: number; title: string; reason: string; }
export interface EnhancementAnalysis { ts: number; ordering: EnhancementPreference[]; context: { tsErrors: number; lintErrors: number; lintWarnings: number; immediateSteps: number }; rawMission?: unknown; }
...
export function prioritizeEnhancements(mission?: unknown): EnhancementAnalysis {
    const m = mission as { diagnostics?: { typecheck?: { errors?: number }; lint?: { errors?: number; warnings?: number } }; immediateSteps?: unknown[] } | undefined;
    ...
    const scores: Record<number, { score: number; title: string; reason: string }> = {
        1: { score: 10, title: 'Auto-generate tasks from immediateSteps', reason: 'Turns mission output into executable queue tasks.' },
        2: { score: 4, title: 'Add lineage IDs', reason: 'Improves traceability and refinement loops.' },
        3: { score: 2, title: 'Incremental brainThinking diff', reason: 'Reduces noise; quality-of-life improvement.' },
        4: { score: 8, title: 'Feed mission diagnostics into planner context', reason: 'Directly increases planning relevance.' },
        5: { score: 6, title: 'Adaptive retry tasks', reason: 'Automates recovery from persistent failures.' }
    };
    if (tsErrors > 0) { scores[4].score += 4; scores[5].score += 3; }
    if (lintErrors > 0) { scores[1].score += 3; scores[4].score += 2; }
    if (immediateSteps > 0) { scores[1].score += 2; scores[2].score += 2; }
    if (lintWarnings > 150) { scores[3].score += 1; }
    ...
    const ordering = Object.entries(scores)
        .map(([id, v]) => ({ id: Number(id), title: v.title, score: v.score, reason: v.reason }))
        .sort((a, b) => b.score - a.score)
        .map((o, i) => ({ rank: i + 1, id: o.id, title: o.title, reason: `${o.reason} (score=${o.score})` }));

    const analysis: EnhancementAnalysis = { ts: Date.now(), ordering, context: { tsErrors, lintErrors, lintWarnings, immediateSteps }, rawMission: mission };
    const dir = path.join(process.cwd(), 'artifacts', 'brain');
    fs.mkdirSync(dir, { recursive: true });
    fs.appendFileSync(path.join(dir, 'brainAnswers.jsonl'), JSON.stringify(analysis) + '\n');
    try { recordMemory && recordMemory({ ts: analysis.ts, source: 'brain', kind: 'answer', status: 'ok', meta: { top: ordering[0]?.id, ordering: ordering.slice(0, 3).map(o => o.id) } }); } catch { }
    return analysis;
}
