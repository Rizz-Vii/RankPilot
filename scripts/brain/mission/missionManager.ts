...
export function runMissionCycle(): Mission {
  ...
  if (tsErrors > 0)
    thoughts.push(
      "Observation: Type errors present; prioritizing type stability."
    );
  if (lintErrors > 0)
    thoughts.push(
      "Observation: Lint errors degrade code health; auto-fix recommended."
    );
  if (lintWarnings > 100)
    thoughts.push(
      "Observation: High warning volume; consider rule refinement or batch suppression strategy."
    );
  ...
  if (tsErrors > 0)
    immediateSteps.push({
      id: "fix-types",
      title: "Resolve top TypeScript errors",
      rationale: "Type stability prerequisite for further refactors.",
    });
  if (lintErrors > 0)
    immediateSteps.push({
      id: "lint-autofix",
      title: "Run ESLint autofix for safe rules",
      rationale: "Reduce toil and surface structural issues.",
    });
  if (lintWarnings > 0 && lintWarnings <= 200)
    immediateSteps.push({
      id: "lint-triage",
      title: "Triage remaining lint warnings",
      rationale: "Convert noisy rules or batch address readability issues.",
    });
  ...
  if (tsErrors > 0) {
    try {
      ...
      files.forEach((f, idx) =>
        immediateSteps.push({
          id: `fix-ts-${idx + 1}`,
          title: `Fix TS errors in ${f}`,
          rationale: "Reduce compiler error count for stability.",
        })
      );
    } catch {
      /* ignore */
  ...
  if (!immediateSteps.length)
    thoughts.push("System clean: no immediate remediation required.");
  ...
  const mission: Mission = {
    ...
    diagnostics: {
      ...
      lint: {
        ...
        rawExcerpt: diag.lint.rawExcerpt,
      },
    },
    immediateSteps,
  ...
  fs.appendFileSync(
    ...
    JSON.stringify({
      ts: mission.ts,
      thoughts,
      metrics: { tsErrors, lintErrors, lintWarnings },
    }) + "\n"
  );
  ...
  if (
    process.env.BRAIN_ENQUEUE_MISSION_STEPS === "1" &&
      ...
      try {
        recordMemory &&
          recordMemory({
            ts: Date.now(),
            source: "brain",
            kind: "mission-enqueue",
            status: "ok",
            meta: { lineage, enqueued: newLines.length },
          });
      } catch {}
    } catch {
      /* swallow */
