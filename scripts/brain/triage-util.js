...
function computeFallbackTriageAction(diagnostics) {
  ...
  if (tsErrors > 10 && lintErrors > 50) {
    return {
      action: "start_and_enqueue",
      rationale: `High combined debt ts=${tsErrors} lint=${lintErrors}.`,
    };
  }
  if (tsErrors > 0 && lintErrors > 0) {
    return {
      action: "start_delegation",
      rationale: `Both ts (${tsErrors}) & lint (${lintErrors}) errors present.`,
    };
  }
  if (tsErrors > 0) {
    return {
      action: "enqueue_ts_fixes",
      rationale: `Only ts errors (${tsErrors}).`,
    };
  }
  return {
    action: "start_delegation",
    rationale: `Only lint errors (${lintErrors}).`,
  };
}
