...
export function parseTasks(inputs: { checkList?: string; queues?: string; changeLog?: string; todos?: string }): Task[] {
  ...
  if (!out.length) {
    ...
    if (clRaw) {
      ...
      const bullets = clRaw.split(/\n/).filter(l => /^(\s*(\d+\.|-|\*)\s+)/.test(l) && !/\(DONE\)/i.test(l)).slice(0, 300);
      bullets.forEach((ln, i) => out.push({ id: `CLF-${i}`, title: ln.slice(0, 110), raw: ln, domain: 'unknown', status: 'TODO' }));
    }
    const ch = safeRead('docs/CHANGE_LOG.md'); if (ch) {
      const todoLines = ch.split('\n').filter(l => /TODO|BACKLOG|PENDING/i.test(l));
      todoLines.slice(0, 120).forEach((ln, i) => out.push({ id: `CHD-${i}`, title: ln.slice(0, 110), raw: ln, domain: 'unknown', status: 'TODO' }));
    }
    const inc = safeRead('docs/INCOMPLETE_CODE_AUDIT.md'); if (inc) {
      inc.split(/\n/).filter(l => l.trim().length > 6).slice(0, 120).forEach((ln, i) => out.push({ id: `INC-${i}`, title: ln.slice(0, 110), raw: ln, domain: 'unknown', status: 'TODO' }));
    }
  }
  // Lightweight domain heuristic
  for (const t of out) {
    const text = t.raw.toLowerCase();
    if (/firestore|api|backend|function|cloud|cron/.test(text)) t.domain = 'backend';
    else if (/ui|page|component|react|next\.js|route/.test(text)) t.domain = 'frontend';
