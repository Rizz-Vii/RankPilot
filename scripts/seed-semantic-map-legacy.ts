...
function makeLargeDoc(i: number, userId: string): LargeSemanticDoc {
    ...
    const nodes = Array.from({ length: 40 }).map((_, n) => ({ id: `n${i}_${n}`, label: `L${n}`, type: 'concept', score: +(rand(i * 300 + n)).toFixed(2) }));
    const edges = Array.from({ length: 60 }).map((_, e) => ({ source: `n${i}_${e % 40}`, target: `n${i}_${(e * 3) % 40}`, weight: +(Math.random() * 1.5).toFixed(2) }));
...
async function main() {
    ...
    console.log(`[seed-semantic-map-legacy] Seeding ${count} legacy semanticMapResults docs (userId=${userId})`);
    let written = 0;
    ...
    console.log('Next: run `npm run scan:neuroseo-large:ci` then backfill and size report:');
...
main().catch(e => { console.error('[seed-semantic-map-legacy] FAILED', e); process.exit(1); });
