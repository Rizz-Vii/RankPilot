...
class TestRunner {
    ...
    async runTestSuite(suiteName, testFiles, description) {
        ...
        for (const testFile of testFiles) {
            ...
            const result = await this.runCommand(command, `Test: ${path.basename(testFile)}`);
        ...
        this.log(`📊 Suite Results: ${suiteResults.passed}/${suiteResults.total} passed`, 'info');
        ...
        const testSuites = [
            {
                ...
                description: 'Testing core API endpoints for MCP, Agents, NeuroSEO, and Authentication',
                files: [
            ...
            },
            {
                ...
                description: 'Testing UI components including dashboard, forms, and settings',
                files: [
            ...
            },
            {
                ...
                files: [
                    'testing/performance/load-testing.spec.ts'
                ]
            }
        ];
    ...
    generateReport() {
        ...
        this.log(`   Total Duration: ${(this.testResults.duration / 1000).toFixed(2)}s`, 'info');
        ...
        this.testResults.suites.forEach(suite => {
            ...
            this.log(`     Failed: ${suite.failed}`, suite.failed > 0 ? 'error' : 'info');
            this.log(`     Success Rate: ${suiteSuccessRate}%`, suiteSuccessRate >= '90' ? 'success' : 'warning');
        ...
        if (failedTests.length > 0) {
            ...
            failedTests.forEach(test => {
                ...
                this.log(`     Duration: ${(test.duration / 1000).toFixed(2)}s`, 'info');
            });
        }
        ...
        if (this.testResults.failed > 0) {
            this.log(`   • Review and fix ${this.testResults.failed} failed tests`, 'warning');
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            await runner.runCommand('npm install -g @playwright/test', 'Install Playwright');
            await runner.runCommand('npx playwright install', 'Install Playwright browsers');
        }
