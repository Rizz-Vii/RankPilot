...
function parseEnvFile(pathFile) {
  ...
  lines.forEach((line, i) => {
    ...
    if (/^[A-Z0-9_]+$/.test(key))
      map.set(key, { value: line.slice(eq + 1), index: i });
  });
  return { lines, map };
}
...
function isPlaceholder(v) {
  ...
  return (
    ["your_value_here", "replace_me_dev_secret"].includes(low) ||
...
function hydrateFirebase() {
  ...
  if (sa.private_key)
    upsertKey("FIREBASE_PRIVATE_KEY", sa.private_key.replace(/\n/g, "\\n"));
...
if (
  ...
  (process.env.FIREBASE_PRIVATE_KEY && process.env.FIREBASE_PROJECT_ID)
) {
  hydrateFirebase();
}
  ...
  try {
    ...
    if (repoTarget) {
      const repoResp = await fetch(
        `https://api.github.com/repos/${repoTarget}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
            "User-Agent": "env-hydrator",
      ...
      if (repoResp.ok) {
        ...
        if (repo.default_branch)
          upsertKey("GITHUB_REPO_DEFAULT_BRANCH", repo.default_branch);
        if (repo.visibility)
          upsertKey("GITHUB_REPO_VISIBILITY", repo.visibility);
      }
    }
  } catch (e) {
...
(async () => {
  ...
  if (dryRun) {
    ...
    for (const c of changes) {
      console.log(
        `  ${c.action.toUpperCase()}: ${c.key}${c.old ? ` (was ${String(c.old).slice(0, 20)}...)` : ""}`
      );
    }
  ...
  if (changes.length) {
    envData.lines.push(
      "",
      `# Hydrated by hydrate-env.mjs ${new Date().toISOString()}`
    );
    fs.writeFileSync(envLocalPath, envData.lines.join("\n"));
  }
  console.log(
    `[hydrate-env] Completed. Changes: ${changes.length}. Overwrite=${overwrite}`
  );
})();
