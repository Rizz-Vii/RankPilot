...
function walk(dir: string) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    else if (TARGET_EXT.test(entry.name)) scanFile(full);
  }
}
...
if (findings.length) {
  console.error(
    "\x1b[33mTenant scope linter: potential unscoped queries found (report written).\x1b[0m"
  );
  findings
    .slice(0, 80)
    .forEach((f) =>
      console.error(
        ` - ${f.file}:${f.line} collection=${f.collection} :: ${f.code}`
      )
    );
  fs.writeFileSync(
    "tenant-scope-report.json",
