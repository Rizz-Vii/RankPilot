...
async function generateForUser(tier: string) {
  ...
  try {
    ...
    const loginBtn = page
      .locator(
        '[data-testid="login-button"], button:has-text("Log in"), button:has-text("Login"), button:has-text("Sign In")'
      )
      .first();
    ...
    try {
      await page.waitForSelector(
        '[data-testid="dashboard-content"], main, .main-content, [data-testid="user-menu"]',
        { timeout: 45000 }
      );
      console.log(
        "✅ Authentication appeared successful (dashboard indicator found)"
      );
    } catch {
      ...
      if (
        ...
        console.warn(
          "⚠️ Authentication may have failed or not fully propagated; storageState will still be saved for debugging"
        );
      }
    }
    ...
    console.error(
      `❌ Failed to generate storageState for ${user.email}:`,
...
async function main() {
  ...
  const tiers = args.length
    ? args
    : ["admin", "starter", "agency", "enterprise"];
  await ensureOutDir();
  for (const t of tiers) {
    ...
    if (!ok)
      console.warn(
        `Generation for ${key} failed — check the app auth state or credentials`
      );
  }
}
