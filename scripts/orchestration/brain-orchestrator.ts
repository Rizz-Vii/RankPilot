...
for (const d of [OUT_DIR_CODEX, OUT_DIR_AIDER, LOG_DIR]) {
...
const BACKEND_TASK_PREFIX = new Set([
  "T26",
  "T27",
  "T28",
  "T30",
...
const FRONTEND_TASK_PREFIX = new Set([
  "T18",
  "T19",
  "T20",
  "T21",
...
function classifyTasks(parsed: ParsedTask[]) {
  ...
  return {
    backend: backend.slice(0, MAX_TASKS),
    frontend: frontend.slice(0, MAX_TASKS),
  };
}
...
function sampleRepoContext(): string {
  ...
  for (const c of candidates) {
    if (fs.existsSync(path.resolve(ROOT, c))) {
      const txt = fs
        .readFileSync(path.resolve(ROOT, c), "utf8")
        .split(/\n/)
        .slice(0, 60)
        .join("\n");
      out += `\n===== ${c} (head 60) =====\n${txt}\n`;
...
function writePrompt(kind: "backend" | "frontend", content: string): string {
  const file = path.resolve(
    kind === "backend" ? OUT_DIR_CODEX : OUT_DIR_AIDER,
    `${TS}-${kind}.txt`
  );
...
function runTool(kind: "backend" | "frontend", promptFile: string) {
  ...
  if (kind === "backend") {
    const res = spawnSync(CODEx_BIN, ["exec", "-p", prompt], {
      stdio: "inherit",
    });
  ...
  const res = spawnSync(
    AIDER_BIN,
    ["--model", "gpt-5-mini", "--message", prompt, "--yes"],
    { stdio: "inherit" }
  );
  return { skipped: false, status: res.status };
}
...
async function main() {
  ...
  const summary = {
    backend: backend.map((t) => t.id),
    frontend: frontend.map((t) => t.id),
    executed: RUN && !DRY_SAVE_ONLY,
  };
  fs.appendFileSync(
    path.resolve(LOG_DIR, `${new Date().toISOString().slice(0, 10)}.log`),
    JSON.stringify({ summary, backendFile, frontendFile }) + "\n"
  );
  console.log(
    JSON.stringify(
      {
  ...
  global.fetch = function (url: string, opts: unknown) {
    return new Promise((resolve, reject) => {
      ...
      const bodyVal = ((): string | Buffer | undefined => {
        if (typeof (o as { body?: unknown }).body === "string")
          return (o as { body?: unknown }).body as string;
        const maybeBuf = (o as { body?: unknown }).body as unknown;
        return typeof Buffer !== "undefined" && Buffer.isBuffer(maybeBuf)
          ? (maybeBuf as Buffer)
          : undefined;
      })();
