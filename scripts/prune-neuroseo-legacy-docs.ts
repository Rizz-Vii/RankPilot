...
interface Summary {
  ...
  dryRun: boolean;
  confirm: boolean;
  targetCrawler?: number;
  targetSemantic?: number;
  adoption?: { crawler?: number | null; semantic?: number | null };
  counts: Record<
    string,
    { scanned: number; candidates: number; deleted: number }
  >;
}

function approxSize(d: unknown) {
  try {
...
async function buildAggregateKeyIndex(
  db: FirebaseFirestore.Firestore,
  collection: string
) {
  const snap = await db
    .collection(collection)
    .select("userId", "url", "historyId")
    .get();
  const set = new Set<string>();
  snap.forEach((d) => {
    const data = d.data() as unknown as Record<string, unknown>;
    const historyId =
      typeof data.historyId === "string" ? data.historyId : undefined;
...
async function main() {
  ...
  if (
    (targetCrawler &&
      adoption.crawler != null &&
      adoption.crawler < targetCrawler) ||
    (targetSemantic &&
      adoption.semantic != null &&
      adoption.semantic < targetSemantic)
  ) {
    console.error("[prune] ABORT thresholds not met", {
      adoption,
      targetCrawler,
      targetSemantic,
    });
  ...
  for (const pair of colPairs) {
    const { candidates, scanned } = await collectCandidates(
      db,
      pair.legacy,
      pair.agg,
      threshold,
  ...
  console.log(
    "[prune] candidates",
    allCandidates
      .map((c) => ({ collection: c.collection, id: c.id, size: c.size }))
      .slice(0, 20)
  );
  if (confirm && !dryRun) {
  ...
  if (out) {
    try {
      ...
      fs.writeFileSync(
        out,
        JSON.stringify({ summary, candidates: allCandidates }, null, 2)
      );
