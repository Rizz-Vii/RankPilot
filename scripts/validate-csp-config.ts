...
function extractCSPFromFirebase(): CSPConfig {
  ...
  const headersUnknown = (config?.hosting?.headers?.[0]?.headers ??
    []) as unknown;
  let cspHeader = "";
  if (Array.isArray(headersUnknown)) {
    const found = headersUnknown.find(
      (h: unknown): h is { key: unknown; value: unknown } =>
        typeof h === "object" && h !== null && "key" in h && "value" in h
    );
    if (
      found &&
      typeof found.key === "string" &&
      found.key === "Content-Security-Policy" &&
...
function validateConfig(config: CSPConfig): {
  passed: boolean;
  missing: string[];
} {
  ...
  REQUIRED_DOMAINS.scriptSrc.forEach((domain) => {
    if (
      !config.scriptSrc.some(
        (d) =>
          d.includes(domain.replace("*", "")) ||
...
function main() {
  ...
  configs.forEach((config) => {
    ...
    if (validation.passed) {
      console.log(
        chalk.green(`✅ ${config.file}: All required domains present`)
      );
    } else {
  ...
  if (allPassed) {
    ...
    console.log(
      chalk.yellow("💡 Fix the missing domains in the respective files")
    );
  }
