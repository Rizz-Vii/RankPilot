...
const qu =
  require("./queue-utils.ts") ||
  (globalThis as unknown as { __QUEUE_UTILS__?: unknown }).__QUEUE_UTILS__;
...
const predicate = (t: TaskLike) => {
  ...
  if (match && !(taskId.includes(match) || summary.includes(match)))
    return false;
  if (status && t.status !== status) return false;
  if (
    olderMinutes &&
    t.updatedAt &&
    now - new Date(t.updatedAt).getTime() < olderMinutes * 60_000
  )
    return false;
  return true;
};
...
console.log(
  `Matched ${matches.length} tasks (total ${queue.length}). Mode=${doHose ? "HOSE" : "PURGE"}${dry ? " DRY" : ""}`
);
