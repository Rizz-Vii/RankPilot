...
function processQueuePass(): number {
  const res = spawnSync("npm", ["run", "delegate:process"], {
    stdio: "inherit",
    env: { ...process.env, AIDER_AUTORUN: "1" },
  });
  return res.status ?? 0;
}
...
function handleTrigger(mtimeMs: number) {
  ...
  console.log(
    `[triggered-runner] trigger detected at ${nowISO()} mtime=${mtimeMs}`
  );
  for (let pass = 1; pass <= MAX_PASSES; pass++) {
    ...
    if (code !== 0) {
      console.warn(
        `[triggered-runner] delegate:process exit code=${code}; aborting trigger cycle`
      );
      break;
    }
...
function poll() {
  ...
  if (IDLE_EXIT_MIN > 0) {
    ...
    if (idleMin >= IDLE_EXIT_MIN) {
      console.log(
        `[triggered-runner] idle ${idleMin.toFixed(2)}m >= ${IDLE_EXIT_MIN}m; exiting.`
      );
      process.exit(0);
    }
  }
...
function initWatch() {
  ...
  if (fs.existsSync(TRIGGER_FILE)) {
    try {
      lastMtime = fs.statSync(TRIGGER_FILE).mtimeMs;
    } catch {
      /* ignore */
    }
  }
  try {
    fs.watch(path.dirname(TRIGGER_FILE), (evt, filename) => {
      if (!filename) return;
      if (
        path.resolve(path.dirname(TRIGGER_FILE), filename.toString()) ===
          TRIGGER_FILE &&
        fs.existsSync(TRIGGER_FILE)
      ) {
        try {
          const st = fs.statSync(TRIGGER_FILE);
          if (st.mtimeMs > lastMtime) {
