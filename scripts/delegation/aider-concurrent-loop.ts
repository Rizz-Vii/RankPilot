...
let queueMetrics:
  | undefined
  | {
      recordQueueStart: (c?: number) => void;
      recordQueueDone: (success: boolean, c?: number) => void;
    } = (() => {
  try {
...
const PARALLEL = Math.min(
  4,
  Math.max(1, parseInt(process.env.AIDER_MAX_PARALLEL || "2", 10))
);
const INTERVAL_MS = parseInt(process.env.AIDER_LOOP_INTERVAL || "12000", 10);
const MODEL_CANDIDATES = (
  process.env.AIDER_MODEL_CANDIDATES || "gpt-5-mini,gpt-4.1-mini,gpt-4o-mini"
)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
const active = new Map<
  string,
  { proc: unknown; files: string[]; started: number }
>();
...
function hashTasks(tasks: QueueTask[]) {
...
function writeQueue(tasks: QueueTask[]) {
  ...
  for (const line of raw.slice(1)) {
    ...
    try {
      ...
      if (byId.has(obj.taskId)) out.push(JSON.stringify(byId.get(obj.taskId)));
      else out.push(line);
    } catch {
      out.push(line);
    }
...
function acquireFiles(files: string[]): boolean {
  for (const f of files) {
    if (locks.get(f)) return false;
  }
  files.forEach((f) => locks.set(f, 1));
  return true;
}
function releaseFiles(files: string[]) {
  files.forEach((f) => locks.delete(f));
}
function markRunning(task: QueueTask) {
  const tasks = readQueue();
  const now = new Date().toISOString();
  for (const t of tasks) {
    if (t.taskId === task.taskId && t.status === "pending") {
      t.status = "running";
      t.updatedAt = now;
    }
  }
...
function markDone(taskId: string, success: boolean) {
  ...
  const now = new Date().toISOString();
  for (const t of tasks) {
    if (t.taskId === taskId && t.status === "running") {
  ...
  writeQueue(tasks);
  try {
    queueMetrics?.recordQueueDone(success, 1);
  } catch {}
}
function ledger(taskId: string, durationMs: number, success: boolean) {
  const line = {
    ts: Date.now(),
    taskId,
    durationMs,
    success,
    model: MODEL,
    parallelRemaining: active.size,
  };
  fs.appendFileSync(LEDGER, JSON.stringify(line) + "\n");
}
function buildMessage(t: QueueTask): string {
  const base = t.summary || "";
  const lintRuleMatch = base.match(/Resolve (.*?) /);
  const rule = lintRuleMatch ? lintRuleMatch[1] : "";
  return `Resolve ONLY the targeted issue described. Rule: ${rule}. Keep changes minimal & safe. Summary: ${base}`;
}
function launch(task: QueueTask) {
  ...
  markRunning(task);
  try {
    queueMetrics?.recordQueueStart(1);
  } catch {}
  const started = Date.now();
  const aiderArgs = [
    "--model",
    MODEL,
    ...(process.env.AIDER_ARGS || "").split(/\s+/).filter(Boolean),
    ...files,
    "--yes",
    "--message",
    buildMessage(task),
  ...
  if (!process.env.OPENAI_API_KEY && !process.env.OPENAI_GPT5_KEY) {
    console.log(`[aider:loop] skip ${task.taskId} missing OPENAI key`);
    markDone(task.taskId, false);
    if (files.length) releaseFiles(files);
    return false;
  }
  const proc = spawn("aider", aiderArgs, {
    stdio: "inherit",
    env: { ...process.env, AIDER_AUTORUN: "1", AIDER_PARALLEL: "1" },
  });
  active.set(task.taskId, { proc, files, started });
  (proc as { on: (evt: string, cb: (code: number | null) => void) => void }).on(
    "exit",
    (code) => {
...
function cycle() {
  ...
  if (hash !== lastSnapshotHash) {
    ...
    const inst = process.env.AIDER_INSTANCE
      ? `#${process.env.AIDER_INSTANCE}`
      : "";
    console.log(
      `[aider:loop${inst}] snapshot`,
  ...
  for (const t of tasks) {
    ...
    if (/^\[CODEX]/.test(t.summary || "")) continue;
    const files = t.files || [];
