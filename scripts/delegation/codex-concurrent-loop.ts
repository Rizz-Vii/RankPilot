...
try {
  recordMemory = require("../brain/state/memory").recordMemory;
} catch {
...
const SELECTIVE =
  process.env.CODEX_SELECTIVE === "1" || process.env.CODEX_SELECTIVE === "true";
...
const RECYCLE_FAILED_MS = parseInt(
  process.env.CODEX_RECYCLE_FAILED_MS || "0",
  10
); // disabled unless set
const MAX_FAILED_ATTEMPTS = parseInt(
  process.env.CODEX_MAX_FAILED_ATTEMPTS || "2",
  10
);
...
function launch(task: QueueTask) {
  ...
  const proc = spawn(parts.shift()!, parts, {
    stdio: "inherit",
    env: { ...process.env, CODEX_MODE: "1", TARGET_TASK: task.taskId },
  });
...
function cycle() {
  ...
  if (hash !== lastSnapshotHash) {
    ...
    const inst = process.env.CODEX_INSTANCE
      ? `#${process.env.CODEX_INSTANCE}`
      : "";
    console.log(
      `[codex:loop${inst}] snapshot`,
