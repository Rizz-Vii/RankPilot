...
interface CandidateSpec { taskId: string; summary: string; files: string[]; message?: string; }

const ROOT = process.cwd();
const CHECKLIST = path.resolve(ROOT, 'checkList.txt');

function fileExists(rel: string) { return fs.existsSync(path.resolve(ROOT, rel)); }

const TASK_MAPPINGS: Record<string, () => CandidateSpec | null> = {
    T29: () => ({
        taskId: 'DEL-T29-EVENT-EXPLORER-FILTERS',
        summary: 'Implement Event Explorer filters (type/source/date) + CSV export scaffolding',
        files: [
            'src/app/(app)/admin/events/page.tsx',
            'src/lib/events',
        ],
        message: 'Add client-side filters + CSV button to Event Explorer page. Keep diff minimal; no backend changes.'
    }),
    T53: () => ({
        ...
        message: 'Insert short Architecture Delta section placeholder if not present; <40 LOC.'
    }),
    T54: () => ({
        taskId: 'DEL-T54-EVENT-SCHEMA-REF',
        summary: 'Add Event Schema reference stub',
        files: ['docs/FIRESTORE_SCHEMAS.md'],
        message: 'Add brief /orgs/{orgId}/events schema block (fields list) if absent; no large rewrite.'
    }),
    T55: () => ({
        taskId: 'DEL-T55-PR-TEMPLATE',
        summary: 'Insert risk checklist into PR template',
        files: ['.github/pull_request_template.md'],
        message: 'Append or create risk checklist section; avoid changing existing text heavily.'
    }),
    T56: () => ({
        ...
        message: 'If script missing TODO markers, add TODO comment about future indexing; no functional changes.'
    }),
    T57: () => ({
        ...
        message: 'Add/ensure stub that exits non-zero if docs/CHANGE_LOG.md missing required section header when behavior files changed.'
    }),
    T58: () => ({
        taskId: 'DEL-T58-ONBOARDING-VALIDATION',
        summary: 'Onboarding validation script stub',
        files: ['scripts/verify-env.ts'],
        message: 'Ensure verify-env.ts includes placeholder for onboarding aggregate check; add TODO comment.'
    })
};
...
function scanChecklistFor(ids: string[]): Set<string> {
    ...
    ids.forEach(id => { if (new RegExp(`^${id}\\b`, 'm').test(text)) found.add(id); });
    return found;
}
...
function main() {
    ...
    console.log(JSON.stringify({ autoEnqueue: { considered: targetIds.length, present: Array.from(present), enqueued: added } }));
}
