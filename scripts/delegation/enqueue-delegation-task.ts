...
if (!taskId || !filesArg || !summary) {
    console.error('Usage: npm run delegate:enqueue -- --taskId=DEL-FOO --files=path1,path2 --summary="Short description"');
    process.exit(1);
}

if (!/^DEL-[A-Z0-9\-]+$/.test(taskId)) {
    console.error('taskId must start with DEL- and be uppercase alphanumerics/dashes');
    process.exit(1);
}
