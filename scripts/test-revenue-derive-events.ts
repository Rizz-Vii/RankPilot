...
if (u2.status !== "canceled")
  throw new Error("u2 should be canceled (no latest period invoice)");
...
if (metrics.churnRatePct <= 0)
  throw new Error("Churn should be positive due to u2 cancellation");

console.log("PASS test-revenue-derive-events", {
  mrr: metrics.mrr,
  arr: metrics.arr,
  churn: metrics.churnRatePct,
  active: u1.status,
  canceled: u2.status,
});
