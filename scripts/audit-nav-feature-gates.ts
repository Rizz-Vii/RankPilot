...
const featureKeyRegex =
  /\b([a-zA-Z0-9_]+): \{[^}]*?(requiredTier|requiresAdmin)/g;
...
for (const block of navSource.matchAll(navItemRegex)) {
  const obj = block[0];
  const href = block[1];
  const requiredTier = /requiredTier:\s*\"(starter|agency|enterprise)\"/.exec(
    obj
  )?.[1];
  const feature = /feature:\s*\"([a-zA-Z0-9_]+)\"/.exec(obj)?.[1];
  const adminOnly = /adminOnly:\s*true/.test(obj);
  navItems.push({
    title: /title:\s*\"([^\"]+)\"/.exec(obj)?.[1] || href,
    href,
    requiredTier,
    feature,
    adminOnly,
...
for (const item of navItems) {
  if (
    item.requiredTier &&
    !item.feature &&
    !allowNoFeature.has(item.href) &&
    !item.adminOnly
  ) {
    findings.push({
      severity: "WARN",
      code: "NAV_NO_FEATURE",
      message: `Nav item ${item.href} has tier ${item.requiredTier} but no feature key`,
      context: item,
    });
  }
  if (item.feature && !featureKeys.has(item.feature)) {
    findings.push({
      ...
      message: `Nav item ${item.href} references unknown feature ${item.feature}`,
      context: item,
    });
  }
}
...
for (const item of navItems.filter((i) => i.feature)) {
  ...
  for (const candidate of candidates) {
    if (fs.existsSync(candidate)) {
      ...
      if (
        new RegExp(`<FeatureGate[^>]*feature=\\"${item.feature}\\"`).test(src)
      ) {
        matched = true;
        break;
      }
  ...
  if (!matched) {
    findings.push({
      severity: "WARN",
      code: "MISSING_FEATURE_GATE",
      message: `Route ${item.href} with feature ${item.feature} missing FeatureGate wrapper`,
      context: item,
    });
  }
}
...
for (const [feature, files] of featureGateUsages.entries()) {
  if (!featureKeys.has(feature)) {
    findings.push({
      severity: "ERROR",
      code: "GATE_UNKNOWN_FEATURE",
      message: `FeatureGate uses unknown feature ${feature}`,
      context: { files },
    });
  }
}
...
const usedFeatures = new Set<string>([
  ...navItems.filter((i) => i.feature).map((i) => i.feature!),
  ...featureGateUsages.keys(),
]);
// Precompute ignore lines
const ignoreOrphanLines = new Map<
  string,
...
for (let i = 0; i < accessLines.length; i++) {
  ...
  if (line.startsWith("// audit:ignore-orphan")) {
    ...
    for (let j = i + 1; j < Math.min(i + 4, accessLines.length); j++) {
      const featureLineMatch = /^(\s*)([a-zA-Z0-9_]+):\s*\{/.exec(
        accessLines[j]
      );
      if (featureLineMatch) {
        const fk = featureLineMatch[2];
        ignoreOrphanLines.set(fk, {
          category: categoryMatch?.[1] || "unspecified",
...
for (const fk of featureKeys) {
  if (!usedFeatures.has(fk)) {
    ...
    findings.push({
      severity: "INFO",
      code: "ORPHAN_FEATURE",
      message: `Feature ${fk} not referenced in nav or page gates`,
      context: fk,
    });
  }
}
...
const report = {
  stats: { errors, warns, infos, total: findings.length },
  findings,
};
