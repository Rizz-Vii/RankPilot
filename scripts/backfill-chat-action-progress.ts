...
async function run() {
  ...
  for (const userDoc of usersCol) {
    ...
    for (const sRef of sessionDocs) {
      ...
      const pending: string[] = Array.isArray(pendingRaw)
        ? pendingRaw.map(String)
        : [];
      const progress =
        data?.actionProgress && typeof data.actionProgress === "object"
          ? (data.actionProgress as Record<string, unknown>)
          : {};
      if (!pending.length || !progress || typeof progress !== "object")
        continue;
      ...
      const filtered = pending.filter(
        (p) => !completedSet.has(String(p).toLowerCase().trim())
      );
      ...
      if (doFull && msgCount >= 6) {
        // respect summarizer interval (MESSAGE_INTERVAL)
        try {
  ...
  console.log(
    JSON.stringify(
      { examined, modified, summarized, fullMode: doFull },
      null,
      2
    )
  );
