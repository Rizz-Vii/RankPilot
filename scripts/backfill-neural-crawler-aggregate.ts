...
function deriveAggregate(d: LegacyDoc) {
  const internalLinks =
    d.links?.filter((l) => l.type === "internal").length || 0;
  const externalLinks =
    d.links?.filter((l) => l.type === "external").length || 0;
  const headingsCounts = d.headings
    ? Object.fromEntries(
        Object.entries(d.headings).map(([k, v]) => [
          k,
...
async function main() {
  ...
  console.log(
    `[backfill-neural-crawler-aggregate] start batchSize=${batchSize} dryRun=${dryRun}`
  );
  ...
  while (true) {
    ...
    console.log(
      `[backfill] scanned=${totalScanned} written=${totalWritten} skipped=${totalSkipped}`
    );
    if (docs.length < batchSize) break;
  }
  console.log(
    `[backfill-neural-crawler-aggregate] complete scanned=${totalScanned} written=${totalWritten} skipped=${totalSkipped}`
  );
}
