...
async function visit(browserContext: BrowserLikeContext, url: string) {
  ...
  const context = browserContext.newContext
    ? await browserContext.newContext()
    : browserContext;
  ...
  try {
    ...
    console.log(
      `   • Done: ${url} – ${statusTxt}; CSP: ${result.csp ? "present" : "absent"}; errors=${errCount}, warnings=${warnCount}, failedRequests=${failCount}`
    );
...
🔐 Starting auth session for: ${user}`);
    ...
    try {
      ...
      const navResponse = await page.goto(session.loginUrl, {
        waitUntil: "domcontentloaded",
        timeout: 30000,
      });
      ...
      if (/\/(dashboard|adminonly|app)(\/.*)?$/.test(currentPath)) {
        ...
        console.log(
          `   • Already authenticated; redirected to ${session.finalUrl}`
        );
      }
      ...
      if (!session.success && emailSel && passSel) {
        ...
        await page
          .waitForURL(/\/(dashboard|adminonly|app)(\/.*)?$/, { timeout: 45000 })
          .catch(() => {});
        session.finalUrl = page.url();
        session.success = /\/(dashboard|adminonly|app)/.test(
          new URL(session.finalUrl).pathname
        );
        console.log(
          `   • Post-login URL: ${session.finalUrl} | success=${session.success}`
        );
      }
      ...
      const totalErrors = routeResults.reduce(
        (n, r) => n + r.consoleErrors.length,
        0
      );
      const totalWarns = routeResults.reduce(
        (n, r) => n + r.consoleWarnings.length,
        0
      );
      console.log(
        `✅ Finished ${user} session – success=${session.success}; pages=${routeResults.length}; errors=${totalErrors}; warnings=${totalWarns}`
      );
    }
  }
  return results;
...
async function main() {
  ...
  try {
    ...
    for (const p of PUBLIC_PATHS) {
      ...
      const context = await browser.newContext({
        ...(probeHdr
          ? { extraHTTPHeaders: { "x-probe-token": probeHdr } }
          : {}),
      });
  ...
  const totalPublicErrors = report.publicRoutes.reduce(
    (n, r) => n + r.consoleErrors.length,
    0
  );
  const totalPublicWarns = report.publicRoutes.reduce(
    (n, r) => n + r.consoleWarnings.length,
    0
  );
  console.log(`\n📄 Wrote crawl/auth report to ${OUT_PATH}`);
  console.log(
    `   • Public routes: ${report.publicRoutes.length} | errors=${totalPublicErrors} | warnings=${totalPublicWarns}`
  );
  const sessSummary = report.authSessions
    .map((s) => `${s.user}: ${s.success ? "ok" : "fail"}`)
    .join(", ");
