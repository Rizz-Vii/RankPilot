...
type NeuroMetrics = {
  neuro?: { analysisRuns?: number; analysisCacheHits?: number };
};
...
async function main() {
  const before = await fetchMetrics().catch(() => ({
    neuro: { analysisRuns: 0, analysisCacheHits: 0 },
  }));
  const baseRuns = before.neuro?.analysisRuns || 0;
  const baseHits = before.neuro?.analysisCacheHits || 0;
  ...
  if (!sawCachedEvent && hitsDelta < 1) {
    console.warn(
      "METRICS REGISTRY TEST WARN: no cache hit observed (possible very fast recompute)."
    );
  }
  assert(runsDelta >= 1, "expected at least one analysis run");
  console.log("METRICS REGISTRY TEST PASS", {
    runsDelta,
    hitsDelta,
    sawCachedEvent,
  });
...
main().catch((e) => {
  console.error("METRICS REGISTRY TEST FAIL", e);
  process.exit(1);
});
