...
function parseArgs() {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--uid") out.uid = args[++i];
      ...
      out.include = (out.include as RegExp[] | undefined) || [];
      (out.include as RegExp[]).push(new RegExp(args[++i]));
    } else if (a === "--exclude") {
      out.exclude = (out.exclude as RegExp[] | undefined) || [];
      (out.exclude as RegExp[]).push(new RegExp(args[++i]));
...
async function run() {
  const { uid, url, max, include, exclude, chunk, overlap } = parseArgs() as {
    uid?: string;
    url?: string;
    max?: number;
    include?: RegExp[];
