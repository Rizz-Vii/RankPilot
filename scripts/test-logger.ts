...
(async () => {
  ...
  // capture
  console.info = (l: string, ..._args: unknown[]) => {
    lines.push(String(l));
  };
  console.warn = (l: string, ..._args: unknown[]) => {
    lines.push(String(l));
  };
  console.error = (l: string, ..._args: unknown[]) => {
    lines.push(String(l));
  };
  ...
  parsed.forEach((p, idx) => {
    if (!p) failures.push(`Line ${idx} not JSON`);
      ...
      if (typeof ts !== "string" || !isIso(ts))
        failures.push(`Line ${idx} timestamp not ISO`);
      const level = (p as Record<string, unknown>).level;
      if (typeof level !== "string" || !level)
        failures.push(`Line ${idx} missing level`);
      ...
      if (msg === "audit.event" && audit !== true)
        failures.push("Audit flag missing");
      if (msg === "fallback.applied" && degraded !== true)
        failures.push("Degraded flag missing");
    }
  });
  if (failures.length) {
