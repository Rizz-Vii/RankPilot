...
const project =
  projectArgIdx !== -1
    ? process.argv[projectArgIdx + 1] || projectDefault
    : projectDefault;
...
const desired = {
  ...
  NEXT_PUBLIC_FIREBASE_API_KEY: get(
  ...
  NEXT_PUBLIC_FIREBASE_PROJECT_ID: get(
  ...
  NEXT_PUBLIC_FIREBASE_APP_ID: get(
    "NEXT_PUBLIC_FIREBASE_APP_ID",
    "FIREBASE_APP_ID"
  ),

  // Firebase Admin
  FIREBASE_ADMIN_PROJECT_ID: get(
    "FIREBASE_ADMIN_PROJECT_ID",
    "FIREBASE_PROJECT_ID"
  ),
  FIREBASE_ADMIN_PRIVATE_KEY: get(
    "FIREBASE_ADMIN_PRIVATE_KEY",
    "FIREBASE_PRIVATE_KEY"
  ),
  FIREBASE_ADMIN_CLIENT_EMAIL: get(
...
if (!desired.TEST_ADMIN_EMAIL)
  desired.TEST_ADMIN_EMAIL = "test.admin@example.com";
if (!desired.TEST_ADMIN_PASSWORD)
  desired.TEST_ADMIN_PASSWORD = "AdminPassw0rd!";

// Generate a probe token if missing to avoid deploy failures; 32 hex chars
if (!desired.CRAWL_PROBE_TOKEN)
  desired.CRAWL_PROBE_TOKEN = crypto.randomBytes(16).toString("hex");

// Verify firebase binary
const firebaseBinCandidate = path.join(
  root,
  "node_modules",
  ".bin",
  process.platform === "win32" ? "firebase.cmd" : "firebase"
);
const firebaseBin = fs.existsSync(firebaseBinCandidate)
  ? firebaseBinCandidate
  : "firebase";
...
for (const name of keys) {
  ...
  const res = spawnSync(firebaseBin, args, {
    input: val + "\n",
    encoding: "utf8",
  });
  ...
  const nonFatalWarning =
    /not managed by Cloud Functions for Firebase/i.test(msgText) ||
    /already exists/i.test(msgText);
...
console.log(
  ...
  JSON.stringify(
    {
      ...summary,
      errors: summary.errors.map((e) => ({ name: e.name, code: e.code })),
    },
    null,
    2
  )
...
if (summary.failed > 0) {
  console.error(
    '\nNote: If failures mention auth, run "npx firebase login" or set FIREBASE_TOKEN, then re-run this script with --project',
    project
  );
  process.exit(1);
}
