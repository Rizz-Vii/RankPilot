...
const asObj = (u: unknown): Record<string, unknown> => (u && typeof u === 'object') ? u as Record<string, unknown> : {};
        ...
        const req = http.request({ hostname: 'localhost', port: process.env.PORT || 3000, path: '/api/neuroseo/stream', method: 'POST', headers: { 'Content-Type': 'application/json', 'Content-Length': Buffer.byteLength(data) } }, res => {
            ...
            res.on('end', () => {
                ...
                for (const block of buf.split('\n\n')) {
                    ...
                    try { events.push({ type: ev, data: JSON.parse(dataLine) as unknown }); } catch { /* ignore */ }
                }
                resolve({ events });
            });
        });
...
async function main() {
    ...
    if (fb) { assert.equal(String(asObj(fb!.data).provenance || ''), 'synthetic'); }
    ...
    if (endLive && endCache && completeEv) {
        ...
        assert(dCache < dLive, `cache duration not faster: live=${dLive} cache=${dCache}`);
    }
