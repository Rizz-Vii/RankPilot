...
async function warmConversational() {
  await post("/api/ai/conversational-seo", {
    action: "start",
    userId: "kpi-user",
    userTier: "starter",
  });
...
async function run() {
  ...
  const k =
    h.kpis && typeof h.kpis === "object"
      ? (h.kpis as Record<string, unknown>)
      : {};
  // Presence assertions
  [
  ...
  assert(
    typeof k.provenanceCoveragePct === "number",
    "provenanceCoveragePct not numeric"
  );
  // routesP95 should have entries for at least one route
  const routeKeys = Object.keys(
    k.routesP95 && typeof k.routesP95 === "object"
      ? (k.routesP95 as Record<string, unknown>)
      : {}
  );
  assert(routeKeys.length >= 1, "expected at least one route p95 entry");
  console.log("KPI TEST PASS", {
    routeCount: routeKeys.length,
    coverage: k.provenanceCoveragePct,
    p95Overall: k.p95LatencyOverall,
  });
}
run().catch((e) => {
  console.error("KPI TEST FAIL", e);
  process.exit(1);
});
