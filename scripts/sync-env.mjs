...
for (const f of codeFiles) {
  ...
  let m;
  while ((m = codeVarPattern1.exec(txt))) codeKeys.add(m[1]);
...
function parseEnvFile(p) {
  ...
  lines.forEach((line, i) => {
    ...
    if (/^[A-Z0-9_]+$/.test(key))
      map.set(key, { value: line.slice(eq + 1), line, index: i });
  });
  return { lines, map };
}
...
function placeholderFor(key) {
  ...
  if (
    /TOKEN|KEY|SECRET|PASSWORD|PRIVATE|DSN|API|CLIENT_ID|CLIENT_SECRET/.test(
      key
    )
  )
...
function shouldOverwrite(value) {
  ...
  return (
    [
      ...
      "YOUR_VALUE_HERE",
      "your_value_here",
      "replace_me_dev_secret",
    ].includes(v) || /placeholder/i.test(v)
  );
}
...
if (!dryRun) {
  if (!onlyLocal) {
    if (exampleAdded.length)
      exampleData.lines.push(
        "",
        "# Added by sync-env.mjs: " + new Date().toISOString()
      );
    fs.writeFileSync(examplePath, exampleData.lines.join("\n"));
  }
  if (!onlyExample) {
    if (localAdded.length || localUpdated.length)
      localData.lines.push(
        "",
        "# Synced by sync-env.mjs: " + new Date().toISOString()
      );
...
console.log(
  "[env:sync] Mode:",
  onlyExample ? "example-only" : onlyLocal ? "local-only" : "both"
);
if (dryRun) console.log("[env:sync] DRY RUN - no files written");
if (!onlyLocal)
  console.log(`[env:sync] .env.example additions: ${exampleAdded.length}`);
if (!onlyExample)
  console.log(
    `[env:sync] .env.local additions: ${localAdded.length} updates: ${localUpdated.length}`
  );
if (exampleAdded.length)
  console.log("  Added to example:", exampleAdded.join(", "));
if (localAdded.length) console.log("  Added to local:", localAdded.join(", "));
if (localUpdated.length)
  console.log("  Updated in local:", localUpdated.join(", "));
