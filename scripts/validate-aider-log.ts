...
function run() {
  ...
  if (kb > 200)
    console.warn(
      `[warn] Size ${kb.toFixed(1)}KB exceeds 200KB rotation threshold.`
    );
  const lines = fs
  ...
  lines.forEach((line, i) => {
    ...
    try {
      ...
      for (const f of REQUIRED)
        if (!(f in obj)) {
          console.error(`[error] Line ${i + 1} missing field ${f}`);
          errors++;
        }
      const locAdded = obj["locAdded"];
      const locRemoved = obj["locRemoved"];
      if (typeof locAdded !== "number" || typeof locRemoved !== "number") {
        console.error(
          `[error] Line ${i + 1} locAdded/locRemoved must be numbers`
        );
        errors++;
      }
    } catch (e: unknown) {
  ...
  console.log(
    `Validation OK: ${lines.length - 1} record(s), size ${kb.toFixed(1)}KB.`
  );
}
