...
const onlyNamespaces = (takeEqFlag("--only-namespaces") || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
function writeEnv(map, originalText) {
  ...
  for (const k of Object.keys(map)) {
    if (!seen.has(k)) out.push(k + "=" + map[k].raw);
  }
  return (
    out.join("\n") +
...
if (!project) {
  console.error(
    "[hydrate-firebase-config] No project specified (use --project or set FIREBASE_PROJECT_ID)."
  );
  process.exit(1);
}
...
let jsonRaw;
try {
  jsonRaw = execSync(`firebase functions:config:get --project ${project}`, {
    stdio: ["ignore", "pipe", "pipe"],
  }).toString();
} catch (err) {
  console.error(
    "[hydrate-firebase-config] Failed to fetch config:",
    err.status,
    err.message
  );
  process.exit(1);
...
if (onlyNamespaces.length) {
  configObj = Object.fromEntries(
    Object.entries(configObj).filter(([k]) => onlyNamespaces.includes(k))
  );
}
...
const summary = {
  project,
  totalRemote: flat.length,
  applied: changes.length,
  actions: changes.reduce((a, c) => {
    a[c.action] = (a[c.action] || 0) + 1;
    return a;
  }, {}),
};
console.log(
  "[hydrate-firebase-config] Summary:",
...
if (changes.length) {
  ...
  for (const c of changes) {
    console.log(
      `  ${c.key} -> ${c.action} (${c.from ? mask(c.from) : "∅"} => ${mask(c.to)})`
    );
  }
}
if (DRY_RUN) {
...
if (changes.length) {
  ...
  console.log(
    "[hydrate-firebase-config] Wrote .env.local (backup at",
    backup + ")"
  );
} else {
