...
function main() {
  ...
  const requiresLog = files.some(f => /^(src\/|functions\/|firestore\.rules|next\.config|package\.json)/.test(f));
  if (!requiresLog) return;
  const hasChangeLog = files.some(f => f === 'docs/CHANGE_LOG.md' || f === 'CHANGE_LOG.md');
  if (!hasChangeLog) {
    console.error('\n❌ CHANGE_LOG ENFORCEMENT: Detected runtime-impacting changes without CHANGE_LOG.md update.');
    console.error('Files:');
    files.filter(f => /^(src\/|functions\/|firestore\.rules|next\.config|package\.json)/.test(f)).forEach(f => console.error(' -', f));
...
try { main(); } catch (e) { console.error('validate-changelog failed:', e?.message); process.exit(1); }
