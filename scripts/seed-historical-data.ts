...
const TEAM_CHANNELS = [
  ...
  },
  {
    id: "sales",
    name: "Sales",
    description: "Sales pipeline updates",
    type: "sales",
  },
  {
    id: "marketing",
  ...
  },
  {
    id: "finance",
    name: "Finance",
    description: "Finance & billing status",
    type: "finance",
  },
  {
    id: "announcements",
...
const FLAG_ADD_MISSING_CHAT_PARTICIPANTS = argv.includes(
  "--addMissingChatParticipants"
);
...
async function getSeedUsers(
  ...
  for (const doc of snap.docs) {
    ...
    const role =
      typeof raw.role === "string" && roleSet.has(raw.role)
        ? (raw.role as string)
        : "free";
  ...
  if (skipped)
    console.log(`  ↺ Skipped ${skipped} user doc(s) missing uid/email`);
  return users;
}
...
async function ensureSharedTeam(
  ...
  const memberIds = Array.from(
    new Set(users.map((u) => u.uid).filter(Boolean))
  );
  const members = users
  ...
  if (!memberIds.length) {
    throw new Error(
      "No valid users to attach to shared team (all missing uid/email)"
    );
  }
  if (snap.exists) {
    ...
    const needsMemberIds =
      ...
      memberIds.some((id) => !data.memberIds.includes(id));
    if (needsMemberIds) {
      await ref.set(
        { memberIds, members, updatedAt: FieldValue.serverTimestamp() },
        { merge: true }
      );
    ...
    await ref.set({
      ...
      description:
        "Cross-tier shared test team for chat & collaboration features",
...
async function seedChatHistory(
  ...
  if (!existing.empty && !FLAG_FORCE_CHAT_RESEED) {
    ...
    if (FLAG_ADD_MISSING_CHAT_PARTICIPANTS) {
      ...
      (await messagesRef.limit(500).get()).docs.forEach((d) =>
        authorIdsInMessages.add(d.data().authorId)
      );
      ...
      if (newAuthors.length) {
          ...
          for (const ch of TEAM_CHANNELS) {
            ...
            batch.set(msgRef, {
              ...
              timestamp: Timestamp.fromDate(
                new Date(
                  now.getTime() -
                    seededNumber(author.uid + ch.id, 1, 6) * 60 * 60 * 1000
                )
              ),
              type: "text",
              reactions: {},
        ...
        console.log(
          `  ✓ Injected ${newAuthors.length} new participant(s) into existing chat history`
        );
      } else {
      ...
      console.log(
        "  (chat) Messages exist – skipping (use --forceChatReseed to override or --addMissingChatParticipants)"
      );
    }
    return;
  }
  ...
  const contentSamples = {
    general: [
      "Daily standup sync",
      "Deploy succeeded",
      "Reviewing metrics spike",
      "Reminder: security rotation",
    ],
    sales: [
      "New lead added to pipeline",
      "Deal moved to negotiation",
      "Monthly quota progress 72%",
      "Lead score anomaly review",
    ],
    marketing: [
      "Email campaign CTR 18%",
      "Content calendar updated",
      "Social presence growth +12%",
      "A/B test variant B winning",
    ],
    finance: [
      "Invoice RP-1004 paid",
      "MRR updated",
      "Refund processed",
      "Expense report validated",
    ],
    announcements: [
      "Platform maintenance scheduled",
      "New feature flag enabled",
      "Policy update published",
      "Quarterly roadmap posted",
    ],
  ...
  for (let day = CHAT_DAYS; day >= 1; day--) {
    ...
    const msgsToday = seededNumber(
      "chat_" + dayDate.toISOString(),
      Math.max(2, CHAT_MESSAGES_PER_DAY - 2),
    ...
    for (let i = 0; i < msgsToday; i++) {
      ...
      const author =
        users[
          seededNumber(
            "user_" + channelId + dayDate.toISOString() + i,
            0,
            users.length - 1
          )
        ];
      ...
      const content =
        samples[
          seededNumber(
            "ct_" + channelId + dayDate.toISOString() + i,
            0,
            samples.length - 1
          )
        ];
      const ts = new Date(
        dayDate.getTime() +
          seededNumber("ofs_" + i + channelId, 0, 60 * 60 * 1000)
      );
  ...
  console.log(
    `  ✓ Seeded ${ops} chat messages across ${TEAM_CHANNELS.length} channels`
  );
}
...
async function seedMonthlyUsage(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  for (const p of periods) {
    ...
    const usage = {
      ...
      competitorReports: Math.round(
        5 *
          factor *
          (user.role === "enterprise" ? 8 : user.role === "agency" ? 4 : 1)
      ),
      apiCalls: Math.round(
        200 *
          factor *
...
async function seedAnalyticalDocs(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  ...
  for (const p of periods) {
    for (let i = 0; i < ANALYSES_PER_MONTH; i++) {
      const created = new Date(
        p.start.getTime() +
          i * 2 * 24 * 60 * 60 * 1000 +
          seededNumber(user.uid + "an" + i + p.period, 0, 6) * 60 * 60 * 1000
      );
      ...
      if (!(await ref.get()).exists) {
        await ref.set({
          ...
          engines: {},
          summary: {
            overallScore: seededNumber(id, 60, 90),
            keyFindings: ["Historical backfill"],
            quickWins: ["Review historical patterns"],
          },
          status: "completed",
      ...
      if (!(await ref.get()).exists) {
        await ref.set({
          ...
          createdAt: Timestamp.fromDate(
            new Date(p.start.getTime() + i * 24 * 60 * 60 * 1000)
          ),
          status: "completed",
          testData: true,
        });
    ...
    for (let i = 0; i < LINK_ANALYSES_PER_MONTH; i++) {
      ...
      if (!(await ref.get()).exists) {
        await ref.set({
          ...
          createdAt: Timestamp.fromDate(
            new Date(p.start.getTime() + 12 * 60 * 60 * 1000)
          ),
          status: "completed",
          testData: true,
        });
  ...
  console.log(
    `  ✓ ${user.email}: analyses ${analysisCounter}, keywordResearch ${krCounter}, linkAnalyses ${linkCounter}`
  );
}
    ...
    for (let i = 0; i < ACTIVITIES_PER_MONTH; i++) {
      ...
      const date = new Date(
        p.start.getTime() +
          ...
          seededNumber("hr" + i + user.uid, 0, 20) * 60 * 60 * 1000
      );
      const type =
        DOMAIN_ACTIVITY_TYPES[
          seededNumber(
            user.uid + "t" + p.period + i,
            0,
            DOMAIN_ACTIVITY_TYPES.length - 1
          )
        ];
...
function tierScale(role: string) {
  switch (role || "free") {
    ...
    case "starter":
      return 2;
    default:
      return 1; // free/admin (admin treated as observer)
  }
}
  ...
  for (const p of periods) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const createdAt = new Date(
        p.start.getTime() +
          seededNumber(id + "day", 0, 25) * 24 * 60 * 60 * 1000
      );
      const closedAt = isClosed
        ? new Date(
            createdAt.getTime() +
              seededNumber(id + "cls", 1, 15) * 24 * 60 * 60 * 1000
          )
        : null;
      await ref.set({
        id,
...
async function seedMarketingCampaigns(
  ...
  for (const p of periods) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if ((await ref.get()).exists) continue;
      const channel =
        channels[seededNumber(id + "chn", 0, channels.length - 1)];
      const impressions = seededNumber(id + "imp", 1000 * scale, 25000 * scale);
      const clicks = Math.max(
        1,
        Math.round(impressions * (0.02 + seededNumber(id + "ctr", 1, 5) / 100))
      );
      const leads = Math.round(
        clicks * (0.05 + seededNumber(id + "conv", 1, 10) / 100)
      );
      const spend =
        channel === "paid"
          ? seededNumber(id + "sp", 100 * scale, 5000 * scale)
          : seededNumber(id + "sp", 10 * scale, 500 * scale);
...
async function seedFinanceInvoices(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  ...
  for (const p of periods) {
    for (let i = 0; i < INVOICES_PER_MONTH; i++) {
      ...
      if ((await ref.get()).exists) continue;
      const amountBase =
        user.role === "starter" ? 99 : user.role === "agency" ? 499 : 1999; // example plan pricing
      const issuedAt = new Date(
        p.start.getTime() + seededNumber(id + "day", 0, 3) * 24 * 60 * 60 * 1000
      );
      const paid = seededNumber(id + "paid", 0, 100) > 5; // 95% paid
      const paidAt = paid
        ? new Date(
            issuedAt.getTime() +
              seededNumber(id + "lag", 1, 10) * 24 * 60 * 60 * 1000
          )
        : null;
      await ref.set({
        id,
...
async function seedContentBriefs(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  ...
  for (const p of periods) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const createdAt = new Date(
        p.start.getTime() + seededNumber(id + "dt", 0, 20) * 24 * 60 * 60 * 1000
      );
      await ref.set({
        id,
        userId: user.uid,
          ...
          outline: [
            {
              ...
              subheadings: ["Context", "Goal"],
              keyPoints: ["Historical seeding"],
              wordCount: 250,
            },
            {
              heading: "Main Strategy",
              subheadings: ["Tactics"],
              keyPoints: ["Optimize content"],
              wordCount: 600,
            },
          ],
          targetAudience: {
            demographics: "Test audience",
            painPoints: ["Need data"],
            goals: ["Improve SEO"],
            expertiseLevel: "beginner",
          },
          seoGuidelines: {
            primaryKeyword: targetKeyword,
            secondaryKeywords: [targetKeyword + " strategy"],
            semanticKeywords: [targetKeyword + " guide"],
            keywordDensity: 1.2,
...
async function seedCompetitorAnalyses(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  // Competitor analyses for starter+ only
  if (["free", "admin"].includes(user.role)) {
    console.log(
      `  ↺ ${user.email}: competitorAnalyses skipped (role=${user.role})`
    );
    return;
  }
  let created = 0;
  ...
  for (const p of periods) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if ((await ref.get()).exists) continue;
      const createdAt = new Date(
        p.start.getTime() + seededNumber(id + "ca", 0, 25) * 24 * 60 * 60 * 1000
      );
      const competitors = [
        "example.com",
        "competitor-a.com",
        "competitor-b.com",
      ].slice(0, 2 + seededNumber(id + "ct", 0, 1));
      await ref.set({
        id,
        userId: user.uid,
        competitors,
        analysis: {
          overview: competitors.map((domain, idx) => ({
            domain,
            domainAuthority: 50 + idx * 5,
            organicTraffic: 10000 + idx * 5000,
            organicKeywords: 1500 + idx * 500,
            backlinks: 300 + idx * 120,
            estimatedValue: 20000 + idx * 8000,
          })),
          keywordGaps: [],
          contentGaps: [],
          backLinkGaps: [],
...
async function seedSerpDataForUser(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc
) {
  ...
  for (const kw of keywords) {
    ...
    const results = Array.from({ length: 10 }, (_, i) => ({
      position: i + 1,
      title: `Result ${i + 1} for ${kw}`,
      url: `https://result-${i + 1}.${user.role}.demo.com/${kw.replace(/\s+/g, "-")}`,
      snippet: `Snippet ${i + 1} about ${kw}`,
    }));
    await ref.set({
      id,
      userId: user.uid,
      ...
      results,
      createdAt: Timestamp.fromDate(
        new Date(Date.now() - seededNumber(id, 1, 20) * 24 * 60 * 60 * 1000)
      ),
      testData: true,
    });
    created++;
...
async function augmentUserProfileAndSettings(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc
) {
...
async function seedContentAnalysesLite(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  ...
  for (const p of periods) {
    ...
    if ((await ref.get()).exists) continue;
    const createdAt = new Date(
      p.start.getTime() + seededNumber(id + "c", 0, 25) * 24 * 60 * 60 * 1000
    );
    await ref.set({
      id,
      userId: user.uid,
      ...
      analysis: {
        ...
        technicalSeo: {
          titleTag: { present: true, length: 55, optimized: true },
          metaDescription: { present: true, length: 150, optimized: true },
          altTags: { total: 10, missing: 1, optimized: 7 },
          internalLinks: 12,
          externalLinks: 4,
        },
        contentQuality: {
...
async function seedSeoAuditsLite(
  db: FirebaseFirestore.Firestore,
  user: TestUserDoc,
  periods: { period: string; start: Date; end: Date }[]
) {
  ...
  if (["free", "admin"].includes(user.role)) {
    console.log(`  ↺ ${user.email}: seoAudits skipped (role=${user.role})`);
    return;
  }
  let created = 0;
  for (const p of periods.filter((_, idx) => idx % 3 === 0)) {
    ...
    await ref.set({
      ...
      createdAt: Timestamp.fromDate(
        new Date(p.start.getTime() + 5 * 24 * 60 * 60 * 1000)
      ),
      period: p.period,
      testData: true,
    });
...
async function run() {
  ...
  if (!users.length) {
    console.error(
      "No test users (testAccount=true) found. Run enhanced test user seeder first."
    );
    process.exit(1);
  }
  console.log(
    `Found ${users.length} seedable users${FLAG_ALL_USERS ? " (allUsers mode)" : ""}`
  );
