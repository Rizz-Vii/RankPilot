...
const ignore = new Set(['node_modules','.next','dist','out','functions/lib']);
...
const assignRequireRe = /^(const|let|var)\s+([A-Za-z0-9_$]+)\s*=\s*require\(['"]([^'"()]+)['"]\);?$/;
const destructureRequireRe = /^(const|let|var)\s+\{([^}]+)\}\s*=\s*require\(['"]([^'"()]+)['"]\);?$/;

function transform(content){
  const lines = content.split(/\n/);
  const imports=[]; const kept=[]; let changed=false;
...
console.log(`[codemod] require-to-import complete. Files changed: ${converted}`);
