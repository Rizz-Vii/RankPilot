...
function listFiles(dir){
  ...
  for(const entry of fs.readdirSync(dir)){
    ...
    if(isDir(full)) out.push(...listFiles(full)); else out.push(full);
  }
  return out;
}

const files = listFiles(path.join(root, 'src'));
const stubPattern = /^export\s+\{\s*default\s*\}\s+from\s+'\.\/(.+?)\.tsx';?(?:\s*export\s+\*\s+from\s+'\.\/.+?';?)?\s*$/s;
const removed = [];
...
for(const f of files){
  ...
  text = text.replace(importRegex, (m, pre, spec, post) => {
    ...
    if(rewriteMap.has(last)){
      ...
      const newSpec = spec.replace(/([^/]+)$/,(seg)=> seg.replace(/\.js$/,'') + '.tsx');
      changed = true;
