...
const EXEMPTIONS = new Set<string>([
  /* intentionally empty; deprecated automation run-due endpoint removed */
]);
  ...
  try {
    ...
    const msg =
      ...
      typeof (e as { message?: unknown }).message === "string"
        ...
        : String(e);
    console.warn(
      "[provenance-audit] Failed to load .provenance-audit.json:",
      msg
    );
  }
  return {};
...
files.forEach((f) => {
  ...
  const extraHints = (CONFIG.extraHints || [])
    .map((s) => {
      try {
  ...
  if (!hasMarker) {
    violations.push({
      ...
      reason: "No provenance marker heuristically detected",
    });
    return;
  }
  if (STRICT) {
    ...
    if (!isStreaming && !/withProvenance/.test(txt)) {
      violations.push({
        file: rel,
        reason: "Strict mode: missing withProvenance wrapper",
      });
    }
    ...
    if (/\/stream\//.test(f) && !/enforceProvenanceOnChunk/.test(txt)) {
      violations.push({
        file: rel,
        reason:
          "Strict mode: streaming endpoint missing enforceProvenanceOnChunk",
      });
    }
  }
});
      ...
      JSON.stringify(
        {
          ...
          violations,
          allowedReasonCodes: PROVENANCE_REASON_CODES,
        },
        null,
        2
      )
  ...
  if (violations.length) {
    ...
    for (const v of violations) {
      ...
      if (process.env.CI) {
        ...
        console.error(
          `::warning file=${v.file}::Provenance violation: ${v.reason}`
        );
      }
    }
    console.error(
      JSON.stringify({ count: violations.length, violations }, null, 2)
    );
    process.exit(1);
  } else {
...
async function runTableDataProvenanceChecks(
  origin: string,
  timeoutMs: number
): Promise<Finding[]> {
  const out: Finding[] = [];
  ...
  try {
    ...
    if (jsonRes.ok) {
      ...
      if (!("__provenance" in body)) {
        out.push({
          file: "runtime:/api/table-data?format=json",
          reason: "Missing __provenance on JSON payload",
        });
      }
    } else {
      out.push({
        file: "runtime:/api/table-data?format=json",
        reason: `HTTP ${jsonRes.status} calling table-data`,
      });
    }
    ...
    if (csvRes.ok) {
      ...
      if (!prov) {
        out.push({
          file: "runtime:/api/table-data?format=csv",
          reason: "Missing x-provenance header on CSV response",
        });
      }
    } else {
      out.push({
        file: "runtime:/api/table-data?format=csv",
        reason: `HTTP ${csvRes.status} calling table-data`,
      });
    }
  } catch (e: unknown) {
    ...
    if (requireServer) {
      const msg =
        ...
        typeof (e as { message?: unknown }).message === "string"
          ...
          : String(e);
      out.push({
        file: "runtime:/api/table-data",
        reason: `Server unavailable for runtime provenance audit: ${msg}`,
      });
    } else {
      console.warn(
        "[provenance-audit] Skipping runtime /api/table-data checks (server not available)."
      );
    }
  } finally {
    clearTimeout(t);
...
async function runExtraRuntimeChecks(
  origin: string,
  timeoutMs: number
): Promise<Finding[]> {
  const out: Finding[] = [];
  ...
  for (const check of extra) {
    const url = check.path.startsWith("http")
      ? check.path
      : `${origin}${check.path}`;
    const controller = new AbortController();
    const t = setTimeout(() => controller.abort(), timeoutMs);
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401 || res.status === 403) {
          console.warn(
            `[provenance-audit] Skipping ${check.path} (${check.type}) due to auth (${res.status}). Falling back to static scan.`
          );
          continue;
        }
        out.push({
          file: `runtime:${check.path}`,
          reason: `HTTP ${res.status} calling ${check.type}`,
        });
        continue;
      }
      if (check.type === "json") {
        ...
        if (!ct.includes("text/event-stream"))
          out.push({
            file: `runtime:${check.path}`,
            reason: "SSE endpoint missing text/event-stream content-type",
          });
        ...
        try {
          ...
          if (reader) {
            ...
            if (!r || (r as { done?: boolean }).done) {
              out.push({
                file: `runtime:${check.path}`,
                reason: "SSE stream did not yield initial chunk",
              });
            }
          }
        } catch {}
      ...
      if (requireServer)
        out.push({
          file: `runtime:${check.path}`,
          reason: `Server unavailable: ${msg}`,
        });
      else
        console.warn(
          `[provenance-audit] Skipping runtime ${check.path} (${check.type}) check (server not available).`
        );
    } finally {
      clearTimeout(t);
    }
...
(async () => {
  ...
  const origin =
    args.origin || process.env.PROV_ORIGIN || "http://localhost:3000";
  const timeoutMs = Number(
    args.timeoutMs || process.env.PROV_TIMEOUT_MS || 5000
  );
