...
async function run() {
    ...
    const accept = await request('PUT', '/api/team/invite', { inviteId, token }, newUserToken);
    if (accept.status !== 200) { console.error('FAIL accept invite', accept.status, accept.json); process.exit(1); }
    console.log('TEAM-01 invite flow tests passed');
}

run().catch(e => { console.error('TEAM-01 invite test error', e); process.exit(1); });
