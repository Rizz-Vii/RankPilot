  ...
  agency: {
    ...
    testData: {
      ...
      teamMembers: [
        ...
        {
          email: "content-specialist@seo-masters-agency.demo.com",
          role: "editor",
        },
  ...
  enterprise: {
    ...
    testData: {
      ...
      teamMembers: [
        ...
        {
          email: "regional-manager-apac@global-tech-corp.demo.com",
          role: "editor",
        },
        {
          email: "analyst-team-lead@global-tech-corp.demo.com",
          role: "editor",
        },
    ...
    try {
      ...
      try {
        ...
        firebaseUser = await auth.createUser({
          ...
          password:
            EXISTING_TEST_USERS[tier as keyof typeof EXISTING_TEST_USERS]
              .password,
      ...
      for (const [index, project] of userData.testData.projects.entries()) {
        ...
        await db
          ...
          .set({
            ...
            lastAnalyzedAt: Timestamp.fromDate(
      ...
      for (const [
        index,
        analysis,
      ] of userData.testData.sampleAnalyses.entries()) {
              ...
              semanticMap: {
                ...
                results: {
                  ...
                  keywordDensity: Object.fromEntries(
                    analysis.keywords.map((k) => [k, Math.random() * 3])
                  ),
                  readabilityScore: 72,
                  contentDepth: 8,
                },
              ...
              rewriteGen: {
                ...
                results: {
                  ...
                  metaDescriptionSuggestions: [
                    `Learn about ${analysis.keywords[0]} with our comprehensive guide`,
                  ],
      ...
      if (tier === "agency" || tier === "enterprise") {
        ...
        await db
          ...
          .set({
            ...
            plan: tier,
            memberIds: (userData.testData.teamMembers || []).map(
              (member) => `${userData.uid}_member_${member.email.split("@")[0]}`
            ),
            members:
              userData.testData.teamMembers?.map((member) => ({
            ...
            projects: userData.testData.projects.map(
              (_, index) => `${userData.uid}_project_${index + 1}`
            ),
            settings: {
              allowInvites: true,
            ...
            usage: {
              ...
              totalReports: Math.floor(
                userData.testData.sampleAnalyses.length / 2
              ),
      ...
      await db
        .collection("usage")
        .doc(`${userData.uid}_${currentMonth}`)
        .set({
          ...
          limits: userData.limits,
          overage: {
            neuroSeoAnalyses: Math.max(
              0,
              userData.usage.neuroSeoAnalyses -
                (userData.limits.neuroSeoAnalyses === -1
                  ? 999999
                  : userData.limits.neuroSeoAnalyses)
            ),
            keywordSearches: Math.max(
              0,
              userData.usage.keywordSearches -
                (userData.limits.keywordSearches === -1
                  ? 999999
                  : userData.limits.keywordSearches)
            ),
            contentAnalyses: Math.max(
              0,
              userData.usage.contentAnalyses -
                (userData.limits.contentAnalyses === -1
                  ? 999999
                  : userData.limits.contentAnalyses)
            ),
            competitorReports: Math.max(
              0,
              userData.usage.competitorReports -
                (userData.limits.competitorReports === -1
                  ? 999999
                  : userData.limits.competitorReports)
            ),
            apiCalls: Math.max(
              0,
              userData.usage.apiCalls -
                (userData.limits.apiCalls === -1
                  ? 999999
                  : userData.limits.apiCalls)
            ),
          },
  ...
  async cleanTestData() {
    ...
    for (const userData of Object.values(ENHANCED_TEST_PROFILES)) {
      try {
        ...
        console.log(
          `    ⚠️  Warning: Could not clean all data for ${userData.email}`
        );
      }
    }
