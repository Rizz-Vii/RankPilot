...
async function run() {
  ...
  try {
    await page.evaluate(async () => {
      try {
        ...
        const s1 = new EventSource(
          "/api/streaming/real-time?action=sse&clientId=smoke"
        );
        setTimeout(() => s1.close(), 2000);
        const s2 = fetch("/api/streaming?action=metrics")
          .then(() => {})
          .catch(() => {});
        void s2;
      } catch {}
  ...
  const connClosed = consoleMsgs.filter((t) =>
    /Connection closed\.|The operation was aborted|NetworkError/i.test(t)
  );
