...
async function run() {
  ...
  if (!owner || !newUser || !member || !altUser) {
    console.log("SKIP: missing one of required tokens");
    return;
  }

  // Non-admin invite attempt
  const attemptEmail = `neg_member_${Date.now()}@example.com`;
  const nonAdminInvite = await req(
    "POST",
    "/api/team/invite",
    { email: attemptEmail, role: "member" },
    member
  );
  assert(
    nonAdminInvite.status === 403,
    `Expected 403 for non-admin invite, got ${nonAdminInvite.status}`
  );
  ...
  const create = await req(
    "POST",
    "/api/team/invite",
    { email: targetEmail, role: "member" },
    owner
  );
  assert(
    create.status === 200 && create.json.inviteId && create.json.token,
    "Owner create invite failed"
  );
  const inviteId = create.json.inviteId;
  ...
  assert(
    reuse.status === 400,
    `Expected 400 on token reuse, got ${reuse.status}`
  );

  // Expired invite scenario: create second invite then expire
  const expEmail = `expired_${Date.now()}@example.com`;
  const createExp = await req(
    "POST",
    "/api/team/invite",
    { email: expEmail, role: "member" },
    owner
  );
  assert(createExp.status === 200, "Failed to create invite for expiry");
  const expId = createExp.json.inviteId;
  const expToken = createExp.json.token;
  // Expire (PATCH helper) - need teamId; require TEAM_ID env (owner's team)
  const teamId = process.env.TEST_OWNER_TEAM_ID;
  if (teamId) {
    const expire = await req(
      "PATCH",
      "/api/team/invite",
      { inviteId: expId, teamId, minutesAgo: 120 },
      owner
    );
    assert(expire.status === 200, "Expire helper failed");
    const acceptExpired = await req(
      "PUT",
      "/api/team/invite",
      { inviteId: expId, token: expToken },
      newUser
    );
    assert(
      acceptExpired.status === 410,
      `Expected 410 for expired invite got ${acceptExpired.status}`
    );
  } else {
  ...
  const createMismatch = await req(
    "POST",
    "/api/team/invite",
    { email: mismatchEmail, role: "member" },
    owner
  );
  assert(createMismatch.status === 200, "Create mismatch invite failed");
  const mmId = createMismatch.json.inviteId;
  const mmToken = createMismatch.json.token;
  const mismatchAccept = await req(
    "PUT",
    "/api/team/invite",
    { inviteId: mmId, token: mmToken },
    altUser
  );
  assert(
    mismatchAccept.status === 403,
    `Expected 403 mismatch got ${mismatchAccept.status}`
  );
  ...
  const createBackfill = await req(
    "POST",
    "/api/team/invite",
    { email: backfillEmail, role: "member" },
    owner
  );
  assert(createBackfill.status === 200, "Create backfill invite failed");
  const bfId = createBackfill.json.inviteId;
  const bfToken = createBackfill.json.token;
  // Delete index doc via admin endpoint
  const delIdx = await req(
    "DELETE",
    `/api/admin/invites/index?inviteId=${bfId}`,
    undefined,
    owner
  );
  assert(delIdx.status === 200, `Index delete failed ${delIdx.status}`);
  // Accept with new user to force scan path
  const bfAccept = await req(
    "PUT",
    "/api/team/invite",
    { inviteId: bfId, token: bfToken },
    newUser
  );
  assert(bfAccept.status === 200, `Backfill accept failed ${bfAccept.status}`);

  if (process.exitCode) {
    console.error("TEAM-01 negative invite tests FAILED");
    process.exit(1);
  } else {
...
run().catch((e) => {
  console.error("TEAM-01 negative invite test error", e);
  process.exit(1);
});
