...
if (!creds || !creds.project_id || !creds.client_email || !creds.private_key) {
  console.error(
    "[ERROR] Missing Firebase Admin credentials. Provide serviceAccount.json (not committed) or set FIREBASE_ADMIN_PROJECT_ID, FIREBASE_ADMIN_CLIENT_EMAIL, FIREBASE_ADMIN_PRIVATE_KEY."
  );
  process.exit(2);
}
initializeApp({
...
(async () => {
  try {
    ...
    if (!teamSnap.exists) {
      ...
      const projects = Array.isArray(teamData.projects)
        ? teamData.projects
        : [];
      const integrations = Array.isArray(teamData.integrations)
        ? teamData.integrations
        : [];
      ...
      console.log(
        "- integrations:",
        integrations.map((i) => `${i.id}:${i.status}`).join(", ")
      );

      if (!hasUidInMemberIds && !hasUidInMembers) {
        console.warn(
          "[MISMATCH] UID not found in team membership. Team lookup via array-contains may fail."
        );
      }
    }
    ...
    const qSnap = await db
      .collection("teams")
      .where("memberIds", "array-contains", uidArg)
      .get();
    console.log(
      "\nQuery teams where memberIds contains UID => matched:",
      qSnap.size
    );
