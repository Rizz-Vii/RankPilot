...
async function main(): Promise<void> {
    ...
    const headers: Record<string, string> = { 'Content-Type': 'application/json', 'x-test-team-limit': '1' };
    ...
    if (second.status !== 429) {
        console.error('Expected second request to be rate limited. Status:', second.status);
        process.exit(1);
    }
    ...
    console.log('[team-rate-limit] PASS', { retryAfter });
}
