...
function readRegistry() {
  ...
  const arr = (m[0].match(/['"]([^'"]+)['"]/g) || []).map((q) => q.replace(/^['"]|['"]$/g, ''));
  const types = new Set(arr);
  const prefixes = new Set(arr.map((t) => (t.includes('.') ? t.split('.')[0] : null)).filter(Boolean));
  return { types, prefixes };
}

function* walk(dir) {
  const ignore = new Set(['node_modules', '.next', 'out', 'dist', '.git', 'functions/lib']);
