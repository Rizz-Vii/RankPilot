...
import {
  ...
  createTriageAgent,
  runAgentOnce,
} from "../../src/lib/ai/agents/agentAdapter";
...
async function main() {
  ...
  const weatherAgent = await createSimpleAgent({
    name: "WeatherAgent",
    instructions:
      "Provide concise weather summaries using the provided tool results only.",
    tools: [
      {
        name: "get_weather",
  ...
  const triage = await createTriageAgent(
    "Triage",
    [
      "Route user queries:",
  ...
  const run1 = await runAgentOnce(triage, {
    input: "What is the weather in Paris today?",
  });
  const run2 = await runAgentOnce(triage, {
    input: "Give 2 core SEO ranking factors.",
  });
