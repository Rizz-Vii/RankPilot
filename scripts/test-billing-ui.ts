...
if (!process.env.FIRESTORE_EMULATOR_HOST) {
  console.warn(
    "FIN-02 billing test skipped (FIRESTORE_EMULATOR_HOST not set)."
  );
  process.exit(0);
}
...
(async () => {
  ...
  try {
    ...
    for (const inv of invoices) {
      await setDoc(doc(db, "financeInvoices", inv.id), {
        ...inv,
    ...
    const dbIsFirestore =
      ...
      typeof (dbMaybe as { collection?: unknown }).collection !== "undefined";
    if (!dbIsFirestore) {
      console.error("Emulator Firestore not available");
      process.exit(1);
    }
    const result = await fetchBillingData(db as unknown as Firestore, userId, {
      invoiceLimit: 10,
    });

    const assertions: string[] = [];
    if (!result.subscription || result.subscription.tier !== "agency")
      assertions.push("subscription tier mismatch");
    if (result.invoices.length !== invoices.length)
      assertions.push("invoice count mismatch");
    if (result.effectiveMonthly !== 49 + 10)
      assertions.push(
        `effectiveMonthly expected 59 got ${result.effectiveMonthly}`
      );
    if (!result.nextInvoice || result.nextInvoice.id !== "inv-sep")
      assertions.push("nextInvoice should be upcoming open invoice");
