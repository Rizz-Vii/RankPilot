...
(async () => {
    ...
    try {
        ...
        await admin.collection('users').doc('user1').set({ email: 'u1@example.com', role: 'user' });
        await admin.collection('users').doc('user2').set({ email: 'u2@example.com', role: 'user' });
        await admin.collection('users').doc('adminUser').set({ email: 'admin@example.com', role: 'admin' });
        ...
        try { await assertFails(user1.collection('users').doc('user2').set({ email: 'hack@example.com', role: 'user' })); } catch { failures.push('user1 should be denied creating user2 doc'); }

        // 3. NeuroSEO analysis create with mismatched userId blocked
        try { await assertFails(user1.collection('neuroSeoAnalyses').doc('a1').set({ userId: 'user2', overallScore: 50, createdAt: new Date(), urls: [], hashKey: 'hk', __provenance: 'test' })); } catch { failures.push('mismatched neuroSeoAnalyses create should fail'); }

        // 4. Marketing campaign create with mismatched userId blocked
        try { await assertFails(user1.collection('marketingCampaigns').doc('c1').set({ userId: 'user2', name: 'x', createdAt: new Date() })); } catch { failures.push('mismatched marketingCampaigns create should fail'); }
        ...
        await admin.collection('teams').doc('teamA').set({ ownerId: 'user1', memberIds: ['user1'], name: 'Alpha' });
        ...
        await assertSucceeds(user1.collection('teams').doc('teamA').collection('members').doc('user2').set({ userId: 'user2', role: 'member', status: 'active', joinedAt: Timestamp.now() }));
        // user2 (now member) cannot add another member
        try { await assertFails(user2.collection('teams').doc('teamA').collection('members').doc('userX').set({ userId: 'userX', role: 'member', status: 'active', joinedAt: Timestamp.now() })); } catch { failures.push('member should not add other members'); }

        // 6. Invite creation restricted to owner/admin; member attempt denied
        try { await assertFails(user2.collection('teams').doc('teamA').collection('invites').doc('invite1').set({ email: 'new@example.com', createdAt: Timestamp.now() })); } catch { failures.push('member should not create invites'); }
        await assertSucceeds(user1.collection('teams').doc('teamA').collection('invites').doc('invite2').set({ email: 'new@example.com', createdAt: Timestamp.now() }));
        ...
        await assertSucceeds(user1.collection('financeInvoices').doc('inv1').set({ userId: 'user1', teamId: 'teamA', amount: 100, createdAt: Timestamp.now() }));
        ...
        try { await assertFails(user1.collection('presence').doc('user2').set({ state: 'online', ts: Timestamp.now() })); } catch { failures.push('user1 should not write presence for user2'); }

        // 9. User1 cannot create usage doc under users/user2/usage
        try { await assertFails(user1.collection('users').doc('user2').collection('usage').doc('u1').set({ m: 1 })); } catch { failures.push('user1 should not create usage for user2'); }

        // 10. Cross-user audits access denied
        await assertSucceeds(user2.collection('audits').doc('a2').set({ userId: 'user2', createdAt: Timestamp.now() }));
        ...
        await assertSucceeds(user1.collection('projects').doc('p1').set({ userId: 'user1', teamId: 'teamA', createdAt: Timestamp.now() }));
        ...
        try { await assertFails(user1.collection('supportMessages').doc('m1').set({ any: 'x' })); } catch { failures.push('user1 should not create supportMessages'); }
        await assertSucceeds(admin.collection('supportMessages').doc('m1').set({ createdAt: Timestamp.now(), subject: 's' }));
        await assertSucceeds(admin.collection('supportMessages').doc('m1').collection('replies').doc('r1').set({ body: 'reply', createdAt: Timestamp.now() }));
        ...
        try { await assertFails(user2.collection('teams').doc('teamA').collection('reports').doc('r1').set({ createdAt: Timestamp.now() })); } catch { failures.push('non-member should not create team report'); }
        await assertSucceeds(user1.collection('teams').doc('teamA').collection('reports').doc('r1').set({ createdAt: Timestamp.now(), title: 'T' }));
        ...
        try { await assertFails(user2.collection('teams').doc('teamB').set({ ownerId: 'user1', memberIds: ['user1'], name: 'Beta' })); } catch { failures.push('user2 should not create team with different ownerId'); }
        ...
        await assertSucceeds(user1.collection('subscriptions').doc('user1').set({ status: 'active', tier: 'starter' }));
