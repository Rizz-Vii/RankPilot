...
function log(msg: string) { console.log(`[brain:prune] ${msg}`); }

function prunePlanFiles() {
    if (!fs.existsSync(ART_DIR)) return;
    const planFiles = fs.readdirSync(ART_DIR)
    ...
    if (toDelete.length) {
        ...
        if (!DRY) toDelete.forEach(f => { try { fs.unlinkSync(path.join(ART_DIR, f)); } catch { } });
    }
}
