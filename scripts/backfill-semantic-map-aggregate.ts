...
async function processBatch(
  ...
  for (const doc of snaps) {
    ...
    const agg = deriveSemanticMapAggregate(
      data as unknown as SemanticMapFullDoc
    );
    if (!dryRun) {
      await db.collection("semanticMapResultsAgg").add({
        ...agg,
        createdAt: agg.createdAt || FieldValue.serverTimestamp(),
      });
    }
    written++;
  }
...
async function main() {
  ...
  console.log(
    `[backfill-semantic-map-aggregate] start batchSize=${batchSize} dryRun=${dryRun}`
  );
  ...
  while (true) {
    ...
    const { written, skipped, oversizedConsidered } = await processBatch(
      db,
      docs,
      dryRun
    );
    ...
    totalOversized += oversizedConsidered;
    console.log(
      `[backfill] scanned=${totalScanned} oversized=${totalOversized} written=${totalWritten} skipped=${totalSkipped}`
    );
    if (docs.length < batchSize) break;
  }
  console.log(
    `[backfill-semantic-map-aggregate] complete scanned=${totalScanned} oversized=${totalOversized} written=${totalWritten} skipped=${totalSkipped}`
  );
