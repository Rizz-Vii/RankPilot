...
function collectFromFiles(files) {
  ...
  for (const f of files) {
    ...
    let m; while ((m = codeVarPattern1.exec(txt))) set.add(m[1]);
...
// Categorize
const missingInExample = [...codeVars].filter(k=>!exampleVars.has(k)).sort();
const unusedInCode = [...exampleVars.keys()].filter(k=>!codeVars.has(k)).sort();
...
console.log(`Unused in code (present only in example): ${result.counts.unusedInCode}`);
...
console.log('  node scripts/list-env-keys.mjs --export > export-env.sh && source export-env.sh');
