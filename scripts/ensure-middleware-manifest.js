...
function parseMatchersFromSource() {
    ...
    for (const file of candidates) {
        ...
        const m = src.match(/export\s+const\s+config\s*=\s*\{[\s\S]*?matcher\s*:\s*([^\n;]+)[\s\S]*?\}/);
        if (m) {
            try {
                const val = eval(`(${m[1]})`);
...
function main() {
    try {
        ...
        if (fs.existsSync(manifestPath)) {
            try {
                ...
                if (existing && typeof existing === 'object') {
                    console.log('[ensure-middleware-manifest] Existing middleware-manifest.json detected. Skipping.');
                    return;
                }
            } catch {
        ...
        console.log('[ensure-middleware-manifest] Created minimal middleware-manifest.json');
    } catch (err) {
        console.warn('[ensure-middleware-manifest] Non-fatal error:', err && err.message ? err.message : err);
    }
}
