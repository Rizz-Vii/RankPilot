...
const AI_PATH_HINT =
  /(ai\/.+|neuroseo\/.+|automation\/.+|seo-audit\/run|mcp\/neuroseo\/enhanced)/i;
...
function check(file: string) {
  ...
  const isStreaming =
    /enforceProvenanceOnChunk/.test(src) || /text\/event-stream/.test(src);
  if (isStreaming) return; // skip streaming
  const exportsWith =
    /export\s+const\s+(GET|POST|PUT|PATCH|DELETE)\s*=\s*withProvenance/.test(
      src
    );
  if (!exportsWith)
    findings.push({
      file: rel,
      reason: "missing withProvenance wrapper export",
    });
}
