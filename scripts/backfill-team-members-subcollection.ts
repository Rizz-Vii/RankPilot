...
function init() {
  ...
  try {
    ...
    console.error(
      "Failed to initialize Firebase Admin (missing serviceAccount.json)",
      e
    );
    process.exit(1);
  }
...
async function run() {
  ...
  for (const doc of teamsSnap.docs) {
    ...
    const members: Array<Record<string, unknown>> = Array.isArray(rawMembers)
      ? (rawMembers.filter((m) => m && typeof m === "object") as Array<
