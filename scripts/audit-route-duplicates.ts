...
interface RouteFile { route: string; source: string; file: string; }
...
function collectPagesApi(root: string): RouteFile[] {
    ...
    const walk = (dir: string) => {
        for (const entry of fs.readdirSync(dir)) {
            ...
            if (stat.isDirectory()) walk(full); else if (/\.(t|j)sx?$/.test(entry)) {
...
function collectAppApi(root: string): RouteFile[] {
    ...
    const walk = (dir: string) => {
        for (const entry of fs.readdirSync(dir)) {
            const full = path.join(dir, entry);
            const stat = fs.statSync(full);
            if (stat.isDirectory()) walk(full); else if (/^route\.(t|j)sx?$/.test(entry)) {
...
function main() {
    ...
    for (const [route, files] of dupes) {
        console.log(`  ${route}`);
        files.forEach(f => console.log(`    - (${f.source}) ${path.relative(root, f.file)}`));
    }
