...
function collectMessageStats(parsed) {
  ...
  for (const f of parsed) {
    ...
    for (const m of f.messages) {
      ...
      if (m.ruleId) {
        withRule++;
        sampleRules.add(m.ruleId);
        if (sampleRules.size > 5) break;
      }
    }
  }
  ...
  if (!forceCli) {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed) && parsed.length) {
        ...
        if (debug)
          console.log(
            "[metrics:update] Reused cached metrics file %s totalMessages=%d withRuleId=%d",
            candidate,
            stats.total,
            stats.withRule
          );
        ...
        if (debug)
          console.log(
            "[metrics:update] Cached metrics degraded (no ruleIds) -> forcing fresh run"
          );
      }
    }
    ...
    let prog = [];
    try {
      prog = await runMetrics();
    } catch (e) {
      if (debug) console.log("[metrics:update] runMetrics failed", e.message);
    }
    if (Array.isArray(prog) && prog.length) {
      const stats = collectMessageStats(prog);
      if (debug)
        console.log(
          "[metrics:update] Programmatic metrics messages=%d ruleIds=%d",
          stats.total,
          stats.withRule
        );
      ...
      if (debug)
        console.log(
          "[metrics:update] Programmatic metrics degraded; falling back to CLI"
        );
    }
  } else if (debug) {
    console.log(
      "[metrics:update] METRICS_FORCE_CLI set: skipping cache + programmatic path"
    );
  }
  // CLI path minimal flat config
  const cliRaw = run(
    "npx eslint -c scripts/eslint-metrics.flat.mjs 'src/**/*.{ts,tsx,js,jsx}' -f json --no-error-on-unmatched-pattern"
  );
  const parsedCli = safeJSON(cliRaw) || [];
  const statsCli = collectMessageStats(parsedCli);
  if (debug)
    console.log(
      "[metrics:update] CLI metrics messages=%d ruleIds=%d sample=%o",
      statsCli.total,
      statsCli.withRule,
      statsCli.sampleRules
    );
  try {
    fs.writeFileSync(
      "/tmp/metrics-fresh.json",
      JSON.stringify(parsedCli, null, 2)
    );
  } catch {}
  try {
    const { errorCount, warningCount, ruleTally } = aggregate(parsedCli);
    fs.writeFileSync(
      "/tmp/metrics-summary.json",
      JSON.stringify(
        {
          errors: errorCount,
          warnings: warningCount,
          ruleCounts: ruleTally,
          files: parsedCli.length,
...
function buildSnapshot(parsed) {
  ...
  const ordered = Object.keys(ruleTally)
    .sort()
    .map((k) => `${k}:${ruleTally[k]}`)
    .join("|");
  const checksum = createHash("sha256")
    ...
    .digest("hex")
    .slice(0, 16);
  const degradedRuleIds =
    Object.keys(ruleTally).length === 1 &&
    (Object.keys(ruleTally)[0] === "(no-rule)" ||
      Object.keys(ruleTally)[0] === "internal");
  return {
    ts: new Date().toISOString(),
    files: parsed.length,
    errors: errorCount,
    warnings: warningCount,
    topRules: Object.entries(ruleTally)
      .sort((a, b) => b[1] - a[1])
      .slice(0, 15),
    ruleCounts: ruleTally,
...
function regenerateHeatmap(parsed) {
  ...
  for (const f of parsed) {
    const counts = {};
    for (const m of f.messages || []) {
      if (m.ruleId && FOCUS.has(m.ruleId))
  ...
  fs.writeFileSync(
    "artifacts/lint-heatmap.json",
    JSON.stringify(
      { generated: new Date().toISOString(), focus: Array.from(FOCUS), top },
      null,
      2
    )
  );
      ...
      JSON.stringify(
        {
          ...
          files: parsed.length,
          generated: new Date().toISOString(),
          checksum: snap.ruleCountsChecksum,
        },
        null,
        2
      )
