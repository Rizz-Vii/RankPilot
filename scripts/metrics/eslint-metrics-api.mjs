...
export function aggregate(results) {
  ...
  if (totalMessages > 0 && Object.keys(ruleTally).length === 0) {
    if (debug)
      console.log(
        "[metrics:aggregate] Empty tally after first pass; applying fallback inclusion strategy"
      );
    errorCount = 0;
    warningCount = 0; // reset
    for (const file of results) {
      for (const msg of file.messages) {
  ...
  if (debug)
    console.log(
      "[metrics:aggregate] files=%d messages=%d errors=%d warnings=%d distinctRules=%d",
      results.length,
      totalMessages,
      errorCount,
      warningCount,
      Object.keys(ruleTally).length
    );
