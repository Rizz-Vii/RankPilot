...
function main() {
  ...
  const runtimeTouched = staged.some(
    (f) => f.startsWith("src/") || f.startsWith("functions/")
  );
  if (!runtimeTouched) return; // no runtime changes
  const changelogTouched = staged.some(
  ...
  if (!changelogTouched) {
    ...
    console.error(
      "Add an entry summarizing the behavior change (or rationale if purely internal refactor) then stage it."
    );
    process.exit(2);
  }
  // Optional lightweight diff size guidance
  const diffStat = execSync("git diff --cached --shortstat", {
    encoding: "utf8",
  }).trim();
  console.log(
    `[CHANGE_LOG ENFORCEMENT] OK (runtime + CHANGE_LOG present). Diff: ${diffStat}`
  );
}
