...
function init() {
  if (!getApps().length) {
    ...
    const fromFile = (() => {
      ...
      try {
        const raw = JSON.parse(fs.readFileSync(keyPath, "utf8")) as Record<
          string,
          unknown
        >;
        const projectId =
    ...
    if (!sa) {
      throw new Error(
        "Missing Firebase Admin credentials. Set GOOGLE_APPLICATION_CREDENTIALS or FIREBASE_ADMIN_* env vars, or provide serviceAccount.json locally."
      );
    }
