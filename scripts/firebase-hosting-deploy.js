  ...
  return new Promise((resolve, reject) => {
      ...
      if (code === 0) {
        try {
          ...
          const jsonLine = lines.find(
            (l) => l.startsWith("{") && l.includes('"status"')
          );
    ...
    child.on("error", (error) => {
      originalConsole.error(
        `[${ts()}] 🚫 Failed to start Firebase CLI: ${error.message}`
      );
      reject(error);
    });
  });
