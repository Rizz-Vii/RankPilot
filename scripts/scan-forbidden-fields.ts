...
async function scanCodebase(root: string): Promise<string[]> {
  ...
  function walk(p: string) {
    ...
    const isUi =
      /\/app\//.test(p) || /\/hooks\//.test(p) || /\/components\//.test(p);
    const writeLikely =
      WRITE_KEYWORDS.some((k) => text.includes(k)) &&
      PERSIST_HINTS.some((h) => text.includes(h));
    ...
    FORBIDDEN_FIELDS.forEach((f) => {
      ...
      if (re.test(text)) {
        ...
        violations.push(
          `Potential persistence of forbidden field '${f}' in ${p}`
        );
      }
    });
  }
...
async function scanFirestore(): Promise<string[]> {
  ...
  if (!Array.isArray(a.apps) || a.apps.length === 0) {
    try {
      a.initializeApp && a.initializeApp({ projectId: project });
    } catch {
      /* ignore */
    }
  }
    ...
    s.forEach((doc: unknown) => {
      ...
      FORBIDDEN_FIELDS.forEach((f) => {
        if (data && typeof data === "object" && f in data)
          violations.push(
            `Firestore doc marketingCampaigns/${d.id ?? "unknown"} contains forbidden field '${f}'`
          );
      });
    });
  } catch {
    /* ignore firestore errors in local non-emulator context */
  }
  return violations;
}
...
(async () => {
  ...
  if (all.length) {
    ...
    console.log(
      "Forbidden fields scan passed (no forbidden references or persisted fields found)."
    );
  }
})();
