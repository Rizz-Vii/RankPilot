  ...
  function walk(dir: string, rel: string) {
    ...
    for (const e of entries) {
      if (e.isDirectory()) {
        ...
        const routeFile =
          fs.existsSync(path.join(next, "route.ts")) ||
          fs.existsSync(path.join(next, "route.js"));
        const indicator = aiIndicators.some((k) => relNext.includes(k));
        if (routeFile && indicator)
          found.push("/api/" + relNext.replace(/\\/g, "/"));
        walk(next, relNext);
      }
    }
  }
...
async function main() {
  ...
  if (missing.length) {
    ...
    missing.forEach((m) =>
      console.error(
        ` - ${m.path} status=${m.status} error=${m.error || "no __provenance"}`
      )
    );
    fs.writeFileSync(
      "provenance-enumeration.json",
      JSON.stringify({ results }, null, 2)
    );
    process.exit(1);
  } else {
    console.log(
      "Provenance enumeration passed for endpoints:",
...
main().catch((e) => {
  console.error("Enumerator error", e);
  process.exit(1);
});
