...
async function adoption(db: FirebaseFirestore.Firestore) {
  ...
  return {
    crawlerAggregateAdoptionApprox:
      crawlerDenom > 0 ? +((crawlerAgg / crawlerDenom) * 100).toFixed(2) : null,
    semanticMapAggregateAdoptionApprox:
      semanticDenom > 0
...
async function runSizeReport(): Promise<{
  semantic: PairStats;
  crawler: PairStats;
}> {
  ...
  const db = gf2();
  async function collectPairSizes(
    legacyCol: string,
    aggCol: string,
    matchFields: string[]
  ) {
    ...
    const legacySnap = await db.collection(legacyCol).limit(5000).get();
    ...
    if (!legacySnap.empty) {
      ...
      for (const doc of legacySnap.docs) {
        ...
        if (aggDoc) {
          ...
          stats.aggBytes += Buffer.byteLength(
            JSON.stringify(aggDoc.data() as unknown)
          );
        }
      }
      if (stats.matched > 0) {
  ...
  const semantic = await collectPairSizes(
    "semanticMapResults",
    "semanticMapResultsAgg",
    ["userId", "url"]
  );
  const crawler = await collectPairSizes(
...
async function main() {
  ...
  const [sizeReport, adopt] = await Promise.all([
    runSizeReport(),
    adoption(db),
  ]);
  const snapshot = {
...
main().catch((e) => {
  console.error("[snapshot-size] FAILED", e);
  process.exit(1);
});
