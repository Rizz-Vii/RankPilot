...
const ONLY_LISTED = (getFlagValue("only-listed") || "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
...
if (!fs.existsSync(BACKUP_FILE)) {
  console.error(
    "[merge-env-backup] No .env.local.backup file found. Aborting."
  );
  process.exit(1);
}
...
if (changes.length) {
  const backupOut = path.join(
    ROOT,
    `.env.local.merge.${new Date().toISOString().replace(/[:]/g, "-")}.bak`
  );
  ...
  fs.writeFileSync(
    ENV_FILE,
    newContent +
      "\n# Merged by merge-env-backup.mjs " +
      new Date().toISOString() +
      "\n",
    "utf8"
  );
  console.log(
    "[merge-env-backup] Wrote updated .env.local (previous version stored at",
    backupOut + ")"
  );
} else {
