...
async function createUnifiedTestUsers() {
  ...
  const allUsers: UnifiedTestUser[] = [
    ...(Object.values(UNIFIED_TEST_USERS) as UnifiedTestUser[]),
    DEV_USER,
  ];
  ...
  for (const user of allUsers) {
    try {
      ...
      try {
        ...
        console.log(
          `  ✅ Auth user already exists: ${user.email} (UID: ${authUser.uid})`
        );
      } catch (error: unknown) {
        ...
        if (err.code === "auth/user-not-found") {
          ...
          try {
            ...
            console.log(
              `  ✅ Created Auth user: ${user.email} (UID: ${authUser.uid})`
            );
      ...
      try {
        ...
        console.log(
          `  ✅ Created/updated Firestore document for: ${user.email} (UID: ${actualUid})`
        );
      } catch (firestoreError) {
        console.error(
          `  ❌ Failed to create Firestore document for ${user.email}:`,
          firestoreError
        );
      }
      ...
      try {
        ...
        console.log(
          `  ✅ Set custom claims for: ${user.email} (UID: ${actualUid})`
        );
      } catch (claimsError) {
        console.error(
          `  ⚠️ Failed to set custom claims for ${user.email}:`,
          claimsError
        );
      }
      ...
      if (user.tier !== "free") {
        ...
        try {
          ...
          console.log(
            `  ✅ Created subscription document for: ${user.email} (UID: ${actualUid})`
          );
        } catch (subError) {
          console.error(
            `  ⚠️ Failed to create subscription document for ${user.email}:`,
            subError
          );
        }
      }
      ...
      try {
        await firestore
          .collection("usage")
          .doc(actualUid)
          .set(usageDoc, { merge: true });
        console.log(
          `  ✅ Created usage tracking document for: ${user.email} (UID: ${actualUid})`
        );
      } catch (usageError) {
        console.error(
          `  ⚠️ Failed to create usage document for ${user.email}:`,
          usageError
        );
      }
    } catch (error: unknown) {
  ...
  for (const user of allUsers) {
    try {
      ...
      try {
        ...
        if (err.code !== "auth/user-not-found") {
          console.error(
            `  ⚠️ Failed to delete Auth user ${user.email}:`,
            err.message ?? extractErrorMessage(error)
          );
        }
      }
  ...
  for (const user of allUsers) {
    try {
      ...
      if (authUser && userDoc.exists) {
        ...
        console.log(
          `  ✅ ${user.displayName}: Auth ✓, Firestore ✓, Tier: ${userData?.subscriptionTier}, UID: ${actualUid}`
        );
        successCount++;
      } else {
        console.log(
          `  ❌ ${user.displayName}: Missing data (UID: ${actualUid})`
        );
      }
    } catch (error: unknown) {
      const errMsg = extractErrorMessage(error);
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      default:
        console.log(
          "Usage: npm run create-test-users [cleanup|create|verify|reset]"
        );
        break;
    }
  } catch (error: unknown) {
