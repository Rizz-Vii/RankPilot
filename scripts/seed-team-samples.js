...
async function seedChat() {
  ...
  console.log(
    `Seeded ${total} chat messages across ${Object.keys(messagesByChannel).length} channels`
  );
}
...
async function enrichProjects() {
  ...
  for (const docSnap of snap.docs) {
    ...
    updates.push(
      db.collection("projects").doc(docSnap.id).set(enriched, { merge: true })
    );
  }
