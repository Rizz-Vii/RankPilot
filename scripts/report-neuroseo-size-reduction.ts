  ...
  if (!legacySnap.empty) {
    ...
    for (const doc of legacySnap.docs) {
      ...
      if (aggDoc) {
        ...
        const aggRaw: unknown = (
          aggDoc as unknown as { data: () => unknown }
        ).data();
...
async function main() {
  ...
  if (out) {
    try {
      ...
      console.log(`[size-reduction] wrote ${out}`);
    } catch (e: unknown) {
      const msg =
        e && typeof e === "object" && "message" in e
          ? String((e as { message?: unknown }).message)
          : String(e);
      console.error("[size-reduction] failed to write OUTPUT_FILE", msg);
    }
  }
}

main().catch((e) => {
  console.error("[size-reduction] FAILED", e);
  process.exit(1);
});
