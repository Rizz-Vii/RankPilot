...
async function main(): Promise<void> {
  ...
  if (!doc.exists) {
    console.warn(
      "Persistence doc not found (may be using mock admin). Skipping assert."
    );
    return;
  }
  const data = doc.data()!;
  ...
  console.log("NEU-02 persistence OK", {
    hashKey: data.hashKey,
    provenance: data.__provenance,
  });
}
main().catch((e) => {
  console.error("NEU-02 persistence FAIL", e);
  process.exit(1);
});
