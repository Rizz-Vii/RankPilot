...
const REPORT_PATH =
  ...
  path.resolve(process.cwd(), "artifacts", "route-crawl-report.json");
const OUT_MD =
  process.env.OUT_MD ||
  path.resolve(process.cwd(), "artifacts", "route-crawl-summary.md");
...
function run() {
  ...
  lines.push(
    `APIs: ${apis.length} total → OK: ${twoxx}, 4xx: ${fourxx}, 5xx: ${fivexx}`
  );

  // Actionable: list 4xx/5xx that are not likely expected
  const actionable = apis.filter(
    (a) => (a.status ?? 0) >= 400 && !isIgnored(a) && !likelyExpected(a)
  );
