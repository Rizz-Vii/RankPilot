...
const SCOPED_EMPTY_WIDGET = process.env.SCOPED_EMPTY_WIDGET_ID || 'edge-scoped-empty';
...
async function csv(params: Record<string, string>) {
    const qs = new URLSearchParams({ widgetId, format: 'csv', ...params }).toString();
    const res = await http('GET', `/api/table-data?${qs}`);
...
async function run() {
    ...
    const emptyScoped = await json({ widgetId: SCOPED_EMPTY_WIDGET, sort: 'metric.asc', page: '1', pageSize: '10', teamId: SCOPED_TEAM, userId: SCOPED_USER });
    assert(Array.isArray(emptyScoped.rows), 'Scoped rows not array');
    assert(emptyScoped.rows.length === 0, 'Scoped empty dataset should return 0 rows');
    ...
    for (let i = 1; i < stringAsc.rows.length; i++) {
        ...
        assert(!Number.isNaN(prev) && !Number.isNaN(curr), 'Non-numeric string encountered in value');
        assert(prev <= curr, `value.asc order violated at index ${i}: ${prev} > ${curr}`);
    }

    // 3) Large CSV all-cap via fallback: request an oversized total and ensure cap applied
    const text = await csv({ widgetId: FALLBACK_WIDGET, sort: 'metric.asc', all: 'true', total: '100000' });
    ...
    assert(dataLines === 2000, `CSV cap mismatch: expected 2000, got ${dataLines}`);
