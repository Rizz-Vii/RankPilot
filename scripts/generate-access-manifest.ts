...
function extractFeatures(src: string): Record<string, FeatureDef> {
  ...
  while ((m = objRegex.exec(src))) {
    ...
    if (!key) continue;
    const tierMatch = /requiredTier:\s*"(free|starter|agency|enterprise)"/.exec(
      body
    );
...
function extractNavRoutes(src: string): RouteDef[] {
  ...
  while ((b = blockRegex.exec(src))) {
    ...
    const tier = /requiredTier:\s*\"(free|starter|agency|enterprise)\"/.exec(
      block
    )?.[1] as Tier | undefined;
...
function extractPageGates(files: string[]): PageGate[] {
  ...
  for (const f of files) {
    ...
    while ((m = gateRegex.exec(content))) {
      ...
      const tier = /requiredTier=\"(free|starter|agency|enterprise)\"/.exec(
        attrs
      )?.[1] as Tier | undefined;
      const adminOnly = /adminOnly(=\{?true\}?|\s)/.test(attrs) || undefined;
      gates.push({
        file: f.replace(projectRoot + "/", ""),
        route: pathToRoute(f),
        feature,
        requiredTier: tier,
        adminOnly,
...
function main() {
  ...
  if (!accessSrc || !navSrc) {
    console.error(
      "Required files missing: access-control.ts or enhanced-nav.ts"
    );
    process.exit(1);
  }
  ...
  for (const r of navRoutes) {
    if (r.feature && !featureSet.has(r.feature)) {
      findings.push({
        severity: "ERROR",
        code: "NAV_UNKNOWN_FEATURE",
        message: `Nav href ${r.href} references unknown feature ${r.feature}`,
        context: r,
      });
    }
  }
  for (const p of pageGates) {
    if (p.feature && !featureSet.has(p.feature)) {
      findings.push({
        severity: "ERROR",
        code: "PAGE_UNKNOWN_FEATURE",
        message: `Page ${p.file} references unknown feature ${p.feature}`,
        context: p,
      });
    }
  }
  ...
  for (const r of navRoutes.filter((x) => x.feature)) {
    ...
    if (!match) {
      findings.push({
        severity: "WARN",
        code: "ROUTE_MISSING_GATE",
        message: `Route ${r.href} has feature ${r.feature} but no matching FeatureGate in page`,
        context: { route: r.href, feature: r.feature },
      });
    }
  }
  ...
  for (const fk of featureSet) {
    ...
    findings.push({
      severity: "INFO",
      code: "ORPHAN_FEATURE",
      message: `Feature ${fk} not referenced in nav or pages`,
    });
  }
  ...
  console.log(
    `[ACCESS_MANIFEST] written -> ${path.relative(projectRoot, outFile)} (features:${Object.keys(features).length} routes:${navRoutes.length} pages:${pageGates.length})`
  );
  if (errors) {
