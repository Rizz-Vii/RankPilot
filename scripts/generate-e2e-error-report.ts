...
function isNoise(ev: ErrorEvent): boolean {
  ...
  if (
    /Failed to fetch RSC payload .* Falling back to browser navigation/i.test(
      text
    )
  )
    return true;
  ...
  if (/Refused to apply a stylesheet.*style-src directive.*CSP/i.test(text))
    return true;
  ...
  if (
    /firestore\.googleapis\.com\/google\.firestore/i.test(url) &&
    /ABORTED|cancelled/i.test(failureText)
  )
    return true;

  // Google Identity toolkit ping noise
  if (
    /www\.googleapis\.com\/identitytoolkit\//i.test(url) &&
    /ABORTED|cancelled/i.test(failureText)
  )
    return true;

  // Stripe iframes/static noise
  if (/js\.stripe\.com\//i.test(url) && /ABORTED|cancelled/i.test(failureText))
    return true;
  ...
  if (msg === "text" && /^--- .* ---$/.test(String(ev.meta?.text || "")))
    return true;
...
function suggestFixesFromSignals(signals: ErrorEvent[]): string[] {
  const hints: string[] = [];
  const hasNetwork = signals.some((e) =>
    /requestfailed|bad-response/.test(e.message)
  );
  const hasClient = signals.some((e) =>
    /pageerror|console\.error/.test(e.message)
  );
  const hasServerDown = signals.some((e) =>
    /ECONNREFUSED|ENOTFOUND|timeout/i.test(e._raw || "")
  );
  const hasCORS = signals.some((e) =>
    /access control check|CORS/i.test(
      (e.meta?.error || e.meta?.text || e._raw || "") as string
    )
  );

  if (hasNetwork)
    hints.push(
      "- Network failures detected: check API routes, auth state, or rate limits."
    );
  if (hasCORS)
    hints.push(
      "- CORS error: ensure server allows origin and Authorization header for this route."
    );
  if (hasClient)
    hints.push(
      "- Client exceptions: open linked page and inspect stack traces; verify feature flags and env vars."
    );
  if (hasServerDown)
    hints.push(
      "- Server not reachable: ensure dev server is running or set TEST_BASE_URL to a deployed URL."
    );
  if (!hints.length)
    hints.push(
      "- Review screenshots, HTML snapshots, and traces for UI state at failure."
    );
  return hints;
}
...
function main() {
  ...
  for (const ev of events) {
    if (isNoise(ev)) noise.push(ev);
    else signals.push(ev);
  }
  ...
  try {
    if (report) {
      ...
      if (total > 0)
        summaryLine = `Summary: ${passed} passed, ${failed} failed, ${skipped} skipped, ${flaky} flaky (total ${total})`;
    }
  } catch {
    // ignore
  }
  ...
  const traces = artifacts.filter(
    (f) => f.endsWith(".zip") && /trace/.test(path.basename(f))
  );
  ...
  md.push(
    `Signal events: ${signals.length} | Suppressed noise: ${noise.length}`
  );
  md.push("");
  if (signals.length) {
    ...
    md.push("```");
    const lastSignals = signals
      .slice(-100)
      .map((e) => e._raw || JSON.stringify(e));
    md.push(lastSignals.join("\n"));
    md.push("```");
  } else if (lines.length) {
    md.push(
      "_No actionable errors after noise filtering. Showing last 20 noise lines for context._"
    );
    md.push("");
    md.push("```");
