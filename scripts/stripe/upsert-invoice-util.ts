...
export async function upsertFinanceInvoice(invoice: MinimalStripeInvoiceLike) {
    ...
    try {
        ...
        const firstLine = (firstLineUnknown && typeof firstLineUnknown === 'object') ? firstLineUnknown as Record<string, unknown> : undefined;
        const price = firstLine && typeof firstLine.price === 'object' && firstLine.price !== null ? firstLine.price as Record<string, unknown> : undefined;
        const priceMeta = price && typeof price.metadata === 'object' && price.metadata !== null ? price.metadata as Record<string, unknown> : undefined;
        const planTierFromLine = priceMeta && typeof priceMeta.planTier === 'string' ? priceMeta.planTier : undefined;
        const planTierFromMeta = invoice.metadata && typeof invoice.metadata.planTier === 'string' ? invoice.metadata.planTier : undefined;
        ...
        logger.degraded('invoice.upsert.failed', { invoiceId: invoice.id, error: message });
    }
}
