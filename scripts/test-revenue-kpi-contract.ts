...
function computeHookStyleKpis(invoices: InvoiceDoc[]) {
  ...
  for (const inv of invoices) {
    (byPeriod[inv.period] ||= []).push(inv);
  }
  const lastPaid = byPeriod[last].filter((i) => i.status === "paid");
  const sum = (arr: InvoiceDoc[], f: (x: InvoiceDoc) => number) =>
  ...
  const prevMrr = prev
    ...
    : mrr;
  const onTimePaid = lastPaid.filter(
    (i) => i.paidAt && i.dueAt && i.paidAt.getTime() <= i.dueAt.getTime()
  );
...
(function run() {
  ...
  const prevDate = new Date(
    Date.UTC(basePeriod.getUTCFullYear(), basePeriod.getUTCMonth() - 1, 5)
  );
  const curPeriod = ym(basePeriod);
  const prevPeriod = ym(prevDate);
  ...
  const invoices: InvoiceDoc[] = [
    // Previous period paid invoices
    {
      ...
      status: "paid",
      issuedAt: prevDate,
      paidAt: new Date(prevDate.getTime() + 2 * 86400000),
      dueAt: new Date(prevDate.getTime() + 5 * 86400000),
    },
    {
      userId: "u2",
      period: prevPeriod,
      ...
      status: "paid",
      issuedAt: prevDate,
      paidAt: new Date(prevDate.getTime() + 1 * 86400000),
      dueAt: new Date(prevDate.getTime() + 7 * 86400000),
    },
  ...
  const { mrr, prevMrr, onTimePct, outstanding } =
    computeHookStyleKpis(invoices);
  ...
  console.log("[revenue-kpi-contract:test] PASS", {
    mrr,
    prevMrr,
