...
async function currentAdoption(): Promise<Adoption> {
  ...
  return {
    ...
    semantic: semanticDen
      ? +((semanticAgg / semanticDen) * 100).toFixed(2)
      : null,
  };
}
...
function updateSustainFile(ok: boolean, sustainDays: number): boolean {
  ...
  if (fs.existsSync(file)) {
    ...
    if (last?.startsWith(new Date().toISOString().slice(0, 10)))
      return entries.length >= sustainDays; // already recorded today
  }
  if (ok) {
    entries.push(`${new Date().toISOString().slice(0, 10)} OK`);
  } else {
    entries = [];
  }
...
async function main() {
  ...
  const ok =
    (adoption.crawler ?? 0) >= target && (adoption.semantic ?? 0) >= target;
  const sustained = updateSustainFile(ok, sustainDays);
  console.log(
    JSON.stringify({ adoption, target, ok, sustained, sustainDays }, null, 2)
  );
  if (
    sustained &&
    (process.env.CONFIRM === "1" || process.env.DRY_RUN === "0")
  ) {
    console.log(
      "[prune-if-ready] Sustained adoption achieved – invoking prune script (dryRun unless CONFIRM=1)"
    );
    const args = ["scripts/prune-neuroseo-legacy-docs.ts"];
    const env = {
      ...process.env,
      DRY_RUN: process.env.CONFIRM === "1" ? "0" : "1",
      CONFIRM_PRUNE: process.env.CONFIRM === "1" ? "1" : "0",
...
main().catch((e) => {
  console.error("[prune-if-ready] failed", e);
  process.exit(1);
});
