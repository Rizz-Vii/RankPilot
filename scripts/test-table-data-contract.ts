...
async function run() {
    ...
    const pageDesc = await getJSON({ sort: 'valueNum.desc', page: '1', pageSize: '10' });
    const pageAsc = await getJSON({ sort: 'valueNum.asc', page: '1', pageSize: '10' });
    assert(pageDesc.rows.length > 0 && pageAsc.rows.length > 0, 'No rows for sort contract');
    ...
    if (Number.isFinite(firstDesc) && Number.isFinite(firstAsc)) {
        assert(firstDesc >= firstAsc, 'Sort contract failed: desc first < asc first');
    }

    // 2) Pagination: page 2 should not equal page 1 when dataset > pageSize
    const page2 = await getJSON({ sort: 'valueNum.desc', page: '2', pageSize: '10' });
