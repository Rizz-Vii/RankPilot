...
async function main() {
  ...
  const legacySnap = await db
    ...
    .get();
  if (legacySnap.empty) {
    console.log("No legacy docs to verify.");
    return;
  }
  ...
  for (const d of picks) {
    ...
    if (aggSnap.empty) {
      console.warn("Missing aggregate for", d.id);
      mismatches++;
      continue;
    }
    const aggUnknown = aggSnap.docs[0].data() as unknown;
    const agg =
      aggUnknown && typeof aggUnknown === "object"
        ? (aggUnknown as Record<string, unknown>)
        : {};
    const images = Array.isArray((data as Record<string, unknown>).images)
      ? ((data as Record<string, unknown>).images as unknown[])
      : [];
    const links = Array.isArray((data as Record<string, unknown>).links)
      ? ((data as Record<string, unknown>).links as unknown[])
      : [];
    const issues = Array.isArray((data as Record<string, unknown>).issues)
      ? ((data as Record<string, unknown>).issues as unknown[])
      : [];
    const entities = Array.isArray((data as Record<string, unknown>).entities)
    ...
    const expected = {
      ...
      linksInternal: links.filter(
        (l: unknown) =>
          l &&
          typeof l === "object" &&
          (l as Record<string, unknown>).type === "internal"
      ).length,
      linksExternal: links.filter(
        (l: unknown) =>
          l &&
          typeof l === "object" &&
          (l as Record<string, unknown>).type === "external"
      ).length,
    ...
    for (const k of Object.keys(expected) as (keyof typeof expected)[]) {
      ...
      if (aggVal !== expected[k]) {
        console.error("Mismatch", k, "legacy=", expected[k], "agg=", aggVal);
        mismatches++;
        break;
      }
    }
  ...
  if (mismatches) {
    console.error(
      `Verification FAILED mismatches=${mismatches}/${picks.length}`
    );
    process.exit(1);
  }
