...
function load(): Entry[] {
    ...
    return fs.readFileSync(LEDGER, 'utf8').trim().split(/\n/).filter(Boolean).map(l => { try { return JSON.parse(l) as Entry } catch { return null as any } }).filter(Boolean)
}
...
function aggregate(entries: Entry[]) {
    ...
    for (const e of recent) {
        ...
        b.count++; b.inTokens += e.input_tokens; b.outTokens += e.output_tokens; b.toolCalls += e.tool_calls; if (e.success) b.success++
    }
