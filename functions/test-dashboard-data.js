  ...
  try {
    ...
    if (analysesSnapshot.size > 0) {
      ...
      analysesSnapshot.docs.forEach((doc, index) => {
        ...
        const date =
          data.completedAt?.toDate?.()?.toISOString()?.split("T")[0] || "N/A";
    ...
    if (keywordSnapshot.size > 0) {
      console.log(
        `   ✅ Found ${keywordSnapshot.size} keyword research records`
      );
      ...
      keywordSnapshot.docs.forEach((doc, index) => {
        ...
        const date =
          data.createdAt?.toDate?.()?.toISOString()?.split("T")[0] || "N/A";
        console.log(
          `   🔑 Research ${index + 1}: "${query}" (${keywords} keywords, ${date})`
        );
      });
    } else {
    ...
    if (projectsSnapshot.size > 0) {
      projectsSnapshot.docs.forEach((doc, index) => {
        const data = doc.data();
        console.log(
          `   📋 Project ${index + 1}: "${data.name}" (${data.domain || "No domain"})`
        );
      });
    } else {
    ...
    const currentScore =
      analysesSnapshot.size > 0
        ? analysesSnapshot.docs[0].data().summary?.overallScore || 0
        : 0;
    const previousScore =
      analysesSnapshot.size > 1
        ? analysesSnapshot.docs[1].data().summary?.overallScore || currentScore
        : currentScore;
    const change = currentScore - previousScore;

    console.log(
      `   📊 SEO Score: ${currentScore} (${change >= 0 ? "+" : ""}${change} change)`
    );
    ...
    if (currentScore > 0) {
      ...
      console.log(
        '   ⚠️  SEO Score Chart: Would show "No analysis data yet" message'
      );
    }
