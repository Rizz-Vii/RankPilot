...
const cache = new Map<
  string,
  { data: KeywordSuggestionsResponse; expiry: number }
>();
...
function getRealAI(): GenAI | null {
  try {
    ...
    const candidate: unknown =
      mod && typeof mod === "object" && "getAI" in mod
  ...
  async (request) => {
    ...
    try {
      ...
      try {
        ...
        logger.warn("Could not fetch user plan, defaulting to free", {
          userId,
        });
      }
      ...
      if (quotaLimit !== -1 && used >= quotaLimit) {
        throw new HttpsError(
          "resource-exhausted",
          "Daily keyword research limit reached"
        );
      }
      ...
      if (cached && Date.now() < cached.expiry) {
        ...
        return {
          ...
          quota: {
            limit: quotaLimit,
            used,
            remaining: quotaLimit === -1 ? -1 : quotaLimit - used,
          },
        };
      }
      ...
      try {
        ...
        globalSnap.forEach((d) => {
          ...
          const sugg = Array.isArray(s["suggestions"])
            ? (s["suggestions"] as unknown[])
            : [];
          sugg.slice(0, 15).forEach((item) => {
            if (item && typeof item === "object" && "semanticCluster" in item) {
              const c = String(
                (item as Record<string, unknown>).semanticCluster || "general"
              );
        ...
        if (!recentSnap.empty) {
          ...
          recentSnap.forEach((d) => {
            const s = d.data() as Record<string, unknown>;
            const suggArr = Array.isArray(s["suggestions"])
              ? (s["suggestions"] as unknown[])
              : [];
        ...
        logger.warn("corpus_build_failed", {
      ...
      if (
        ...
        (data.crawlUrls?.length || query.split(" ").length <= 4)
      ) {
        try {
          const targetList =
            data.crawlUrls && data.crawlUrls.length > 0
              ? data.crawlUrls.slice(0, 3)
              : [
          ...
          for (const u of targetList) {
            ...
            if (resp.ok) {
              ...
              const md =
                j && typeof j === "object" && "markdown" in j
                  ? String((j as Record<string, unknown>).markdown)
                  : "";
              if (md) crawlContext += `\nURL:${u}\n${md.slice(0, 800)}`;
            }
          ...
          logger.warn("firecrawl_keyword_context_failed", {
            error: e instanceof Error ? e.message : String(e),
          });
        }
      }
      ...
      const { suggestions, source: genSource } = await generateKeywords(
        ...
        includeMetrics,
        {
          forceReal: plan === "admin" || data.forceReal,
          corpusSummary,
          crawlContext,
        }
      );
...
5. STRICT JSON ONLY: { "keywords": [ ... ] }`;
    ...
    if (useReal) {
      try {
        ...
        logger.warn("Real AI generation failed, falling back to mock", {
          error: realErr instanceof Error ? realErr.message : String(realErr),
        });
      }
    }
    if (!rawOutput) {
      rawOutput = await getMockAI(prompt, undefined, {
        temperature: 0.7,
    ...
    const raw: KeywordSuggestion[] =
      ...
      Array.isArray((parsedResult as Record<string, unknown>).keywords)
        ? ((parsedResult as Record<string, unknown>)
            .keywords as unknown as KeywordSuggestion[])
        : [];
    if (!raw.length) {
      logger.warn(
    ...
    const normalized = raw.map((k) => ({
      ...
      opportunities: Array.isArray(k.opportunities)
...
async function generateRelatedQueries(
  ...
  try {
    ...
    try {
      ...
      logger.warn("Real AI related queries failed, falling back to mock", {
        error: e instanceof Error ? e.message : String(e),
      });
    }
    if (!output) {
      output = await getMockAI(prompt, undefined, {
        temperature: 0.8,
        maxOutputTokens: 200,
      });
    }
...
function getFallbackKeywords(
  ...
  return variations.slice(0, count).map((keyword, index) => ({
    ...
    intent: ["informational", "commercial", "transactional", "navigational"][
      index % 4
    ] as "informational" | "commercial" | "transactional" | "navigational",
    semanticCluster: deriveCluster(keyword),
    topicalRelevance: Math.floor(Math.random() * 100),
    opportunities: ["Improve CTR", "Capture long-tail"].slice(
      0,
