...
function computeNextFromCron(now: Date, expr: string): Date | null {
  ...
  const minutes: number[] =
    minRaw === "*" ? Array.from({ length: 60 }, (_, i) => i) : [Number(minRaw)];
  const hours: number[] =
    hourRaw === "*"
...
export async function runDueAutomationTick(
  ...
  for (const docSnap of snap.docs) {
    ...
    const recipe: AutomationRecipe = {
      ...
      lastRun:
        (data.lastRun as { toDate?: () => Date } | undefined)?.toDate?.() ||
        null,
      nextRun:
        (data.nextRun as { toDate?: () => Date } | undefined)?.toDate?.() ||
        null,
      lockedAt:
        (data.lockedAt as { toDate?: () => Date } | undefined)?.toDate?.() ||
        null,
    ...
    const lockOk = await db.runTransaction(async (tx) => {
      ...
      const lockedAt = (
        d?.lockedAt as { toDate?: () => Date } | undefined
      )?.toDate?.();
      const lockExpired =
        !lockedAt || now.getTime() - lockedAt.getTime() > 10 * 60_000; // 10 minutes
    ...
    try {
      ...
      for (const action of recipe.actions) {
        ...
        try {
          ...
          if (action === "sendDigestEmail") {
            ...
            const range: "30d" | "90d" | "ytd" = str(cfg?.["range"], "30d") as
              | "30d"
              | "90d"
              | "ytd";
            ...
            if (!deals.length)
              ...
              const pipeline = deals
                .filter(
                  (d) => (d as Record<string, unknown>).status !== "ClosedLost"
                )
                .reduce(
              ...
              const closedWon = deals.filter(
                (d) => (d as Record<string, unknown>).status === "ClosedWon"
              ).length;
              await db.collection("salesMetricsSnapshots").add({
                userId: recipe.userId,
                teamId: recipe.teamId || null,
                range,
                pipeline,
            ...
            await db.collection("salesForecastSnapshots").add({
              userId: recipe.userId,
              teamId: recipe.teamId || null,
              period,
            ...
            const invSnap = await db
              ...
              .get();
            const invoices = invSnap.docs.map(
              (d) => d.data() as Record<string, unknown>
            );
            if (!invoices.length)
              actionsResults.push({
              ...
              const periods = Array.from(
                new Set(
                  invoices.map((i) => (i as Record<string, unknown>).period)
                )
              ).sort();
              const last = periods.at(-1)!;
              const current = invoices.filter(
                (i) => (i as Record<string, unknown>).period === last
              );
              const paid = current.filter(
                (i) => (i as Record<string, unknown>).status === "paid"
              );
              ...
              await db.collection("financeRevenueSnapshots").add({
                userId: recipe.userId,
                teamId: recipe.teamId || null,
                period: last,
                mrr,
            ...
            invoices
              ...
              .forEach((i) => {
                const due = (i as { dueAt?: { toDate?: () => Date } }).dueAt
                  ?.toDate?.()
                  ?.getTime?.();
                if (!due) return;
            ...
            await db.collection("financeInvoiceAgingSummaries").add({
              userId: recipe.userId,
              teamId: recipe.teamId || null,
              buckets,
              createdAt: now,
            ...
            actionsResults.push({
              type: action,
              status: "skipped",
              message: "Unsupported in scheduler",
            });
          }
        } catch (e) {
          actionsResults.push({
            type: action,
            status: "error",
            message: (e as { message?: string })?.message || "Action failed",
          });
        }
      }
      ...
      logger.error("runDueAutomationScheduler recipe error", {
        id: recipe.id,
