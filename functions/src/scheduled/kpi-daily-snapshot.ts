...
export async function runKpiDailySnapshot(now: Date = new Date()) {
  ...
  try {
    ...
    logger.warn(
      "kpiDailySnapshot.aiUsageQueryFailed",
  ...
  let revenueMrr = 0,
    ...
    revenueOnTimePct: number | undefined;
  try {
    ...
    if (!revSnap.empty) {
      ...
      revSnap.docs.forEach((d) => {
        const data = d.data() as Record<string, unknown>;
        all.push(data);
        if (data.status === "paid") paid.push(data);
      });
      revenueMrr = paid.reduce(
      ...
      if (paid.length) {
        const onTime = paid.filter((i) => {
          const paidAt = (
            i as { paidAt?: { toDate?: () => Date } }
          ).paidAt?.toDate?.();
          const due = (
            i as { dueAt?: { toDate?: () => Date } }
          ).dueAt?.toDate?.();
    ...
    logger.warn(
      "kpiDailySnapshot.revenueAggregationFailed",
      e instanceof Error ? e.message : String(e)
    );
  }
  ...
  try {
    ...
    if (uDoc.exists) {
      ...
      semanticMapAggregateAdoptionPct = readNum(
        u,
      ...
      if (fallbackRatePct === null)
        fallbackRatePct = readNum(u, "fallbackRatePct");
      cacheHitRatio = readNum(u, "cacheHitRatio");
      rateLimitRejectionRate = readNum(u, "rateLimitRejectionRate");
    }
  } catch (e) {
    logger.warn(
      "kpiDailySnapshot.unifiedMetricsLoadFailed",
      e instanceof Error ? e.message : String(e)
    );
  }
  ...
  try {
    ...
    const push = (
      ...
      if (cond) alerts.push({ type, level, message, value, threshold });
    };
    // Use values loaded above (provenanceCoveragePct etc.)
    push(
      (provenanceCoveragePct ?? 0) < 100,
      "critical",
      "provenanceCoverage",
      provenanceCoveragePct ?? null,
      100,
      "Provenance coverage below 100%"
    );
    if (fallbackRatePct != null)
      push(
        fallbackRatePct > 18,
        ...
        fallbackRatePct,
        18,
        "Fallback rate above target"
      );
    // Adoption thresholds (critical <50, warn 50-80)
    if (crawlerAggregateAdoptionPct != null) {
      push(
        crawlerAggregateAdoptionPct < 50,
        "critical",
        "crawlerAggregateAdoption",
        crawlerAggregateAdoptionPct,
        50,
        "Crawler aggregate adoption below 50%"
      );
      if (crawlerAggregateAdoptionPct >= 50)
        push(
          crawlerAggregateAdoptionPct < 80,
          "warn",
          "crawlerAggregateAdoption",
          crawlerAggregateAdoptionPct,
          80,
          "Crawler aggregate adoption below 80%"
        );
    }
    if (semanticMapAggregateAdoptionPct != null) {
      push(
        semanticMapAggregateAdoptionPct < 50,
        "critical",
        "semanticMapAggregateAdoption",
        semanticMapAggregateAdoptionPct,
        50,
        "Semantic map aggregate adoption below 50%"
      );
      if (semanticMapAggregateAdoptionPct >= 50)
        push(
          semanticMapAggregateAdoptionPct < 80,
          "warn",
          "semanticMapAggregateAdoption",
          semanticMapAggregateAdoptionPct,
          80,
          "Semantic map aggregate adoption below 80%"
        );
    }
    // Latency (overall p95 derived) thresholds: warn >600, critical >1200
    if (p95LatencyOverall != null) {
      push(
        p95LatencyOverall > 1200,
        "critical",
        "latencyOverallP95",
        p95LatencyOverall,
        1200,
        "Overall p95 latency >1200ms"
      );
      if (p95LatencyOverall > 600 && p95LatencyOverall <= 1200)
        push(
          true,
          "warn",
          "latencyOverallP95",
    ...
    if (rateLimitRejectionRate != null)
      push(
        rateLimitRejectionRate > 3,
        "warn",
        "rateLimitRejectionRate",
        rateLimitRejectionRate,
        3,
        "Rate limit rejection rate above target"
      );
    ...
    try {
      const recentSnap = await db
        ...
        .limit(7)
        .get();
      const rows = recentSnap.docs.map(
      ...
      ma7Provenance = avg(
        rows
          .map(
            (r) =>
              (r as { provenanceCoveragePct?: number }).provenanceCoveragePct
          )
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      ma7CrawlerAdoption = avg(
        rows
          .map(
            (r) =>
              (r as { crawlerAggregateAdoptionPct?: number })
                .crawlerAggregateAdoptionPct
          )
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      ma7SemanticAdoption = avg(
        rows
          .map(
            (r) =>
              (r as { semanticMapAggregateAdoptionPct?: number })
                .semanticMapAggregateAdoptionPct
          )
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      ma7FallbackRate = avg(
        rows
          .map(
            (r) =>
              (r as { fallbackRatePct?: number; fallbackRate?: number })
                .fallbackRatePct ??
              (r as { fallbackRatePct?: number; fallbackRate?: number })
                .fallbackRate
          )
      ...
      ma7LatencyP95 = avg(
        rows
          .map((r) => (r as { p95LatencyOverall?: number }).p95LatencyOverall)
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      ma7CacheHitRatio = avg(
        rows
          .map((r) => (r as { cacheHitRatio?: number }).cacheHitRatio)
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      ma7RateLimitRejectionRate = avg(
        rows
          .map(
            (r) =>
              (r as { rateLimitRejectionRate?: number }).rateLimitRejectionRate
          )
          .filter((v: number | undefined): v is number => typeof v === "number")
      );
      // Exponential smoothing (alpha=0.3) using existing kpiDaily order (rows newest->oldest)
      const alpha = 0.3;
      const provenanceSeries = rows
        .map(
          (r) => (r as { provenanceCoveragePct?: number }).provenanceCoveragePct
        )
        .filter((v: number | undefined): v is number => typeof v === "number")
        .slice()
      ...
      try {
        await db.collection("kpiDaily").doc(dateKey).update({
          smoothedProvenance,
          smoothedLatencyP95,
          smoothedCrawlerAdoption,
          smoothedSemanticAdoption,
          updatedAt: FieldValue.serverTimestamp(),
        });
      } catch (e) {
        logger.warn(
          "kpiDailySnapshot.smoothingPersistFailed",
          e instanceof Error ? e.message : String(e)
        );
      }
      // Include smoothing fields when writing alerts snapshot below
      (globalThis as unknown as Record<string, unknown>).__SMOOTHING = {
        smoothedProvenance,
        smoothedLatencyP95,
        smoothedCrawlerAdoption,
        smoothedSemanticAdoption,
      ...
      logger.warn(
        "kpiAlertsDaily.movingAverageFailed",
        e instanceof Error ? e.message : String(e)
      );
    }
    ...
    try {
      ...
      logger.warn(
        "kpiDailySnapshot.ma7PersistFailed",
        e instanceof Error ? e.message : String(e)
      );
    }
    await db.runTransaction(async (tx) => {
      ...
      if (!aSnap.exists)
        tx.set(alertsRef, { ...base, createdAt: FieldValue.serverTimestamp() });
      else tx.update(alertsRef, base);
    });
    ...
    try {
      ...
      logger.warn(
        "kpiAlertsDaily.retentionFailed",
        e instanceof Error ? e.message : String(e)
      );
    }
  } catch (e) {
    logger.warn(
      "kpiAlertsDaily.persistFailed",
      e instanceof Error ? e.message : String(e)
    );
  }
  ...
  try {
    ...
    logger.warn(
      "kpiDailySnapshot.retentionFailed",
      e instanceof Error ? e.message : String(e)
    );
  }

  logger.info("kpiDailySnapshot.complete", {
    date: dateKey,
    aiTokensIn,
    aiTokensOut,
    aiCostEstimate: aiCost,
