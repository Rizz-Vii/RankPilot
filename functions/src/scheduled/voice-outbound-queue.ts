...
type TwilioCallsApi = { create(args: TwilioCallCreateArgs): Promise<TwilioCallCreateResult> };
type TwilioClient = { calls: TwilioCallsApi };
type TwilioCtor = new (sid: string, token: string) => TwilioClient;

function buildTwiml(opts: { sayText: string; voice: string; language: string; rate: number; recordingUrl?: string | null; interactive?: boolean }) {
    const { sayText, voice, language, rate, recordingUrl, interactive } = opts;
    const prosodyOpen = rate && rate !== 1 ? `<prosody rate="${Math.round(rate * 100)}%">` : "";
...
export async function processVoiceOutboundQueueTick(injectedDb?: ReturnType<typeof getFirestore>, injectedNow?: Date) {
    ...
    try {
        ...
        logger.error('voiceOutboundQueue.twilio_import_error', { error: e instanceof Error ? e.message : String(e) });
    }
    ...
    for (const doc of snap.docs) {
        ...
        const to = data.to;
        if (!to) {
            await doc.ref.update({ status: 'skipped', reason: 'missing_to', updatedAt: FieldValue.serverTimestamp() });
            continue;
        }
        ...
        try {
            ...
            const voice = data.config?.voice || process.env.TWILIO_SAY_VOICE || 'alice';
            const language = data.config?.language || process.env.TWILIO_SAY_LANGUAGE || 'en-US';
            const rate = Number(data.config?.rate || process.env.TWILIO_SAY_RATE || 1);
            ...
            const sayText = script || 'Hello. This is RankPilot. We are confirming your appointment. Goodbye.';
            const twiml = buildTwiml({ sayText, voice, language, rate, recordingUrl, interactive });
            ...
            if (twilioClient && from) {
                try {
                    ...
                    logger.error('voiceOutboundQueue.call_failed', { id: doc.id, error: String(e) });
                }
            } else {
                logger.error('voiceOutboundQueue.twilio_not_configured', { hasClient: !!twilioClient, hasFrom: !!from });
            }
            ...
            try {
                ...
                const callDocData = {
                    ...
                    config: { voice, language, rate, recordingUrl, interactive, script: script || null },
                ...
                logger.warn?.('voiceOutboundQueue.voice_calls_write_failed', { id: doc.id, error: String(e) });
            }

            await doc.ref.update({ status: 'processed', callSid: callSid || null, processedAt: FieldValue.serverTimestamp() });
            processed++;
        } catch (e) {
            logger.error('voiceOutboundQueue.item_error', { id: doc.id, error: String(e) });
            try { await doc.ref.update({ status: 'error', error: String(e), updatedAt: FieldValue.serverTimestamp() }); } catch { }
        }
    }
