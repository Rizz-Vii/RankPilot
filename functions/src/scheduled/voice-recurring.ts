...
export async function processVoiceRecurringTick(
  ...
  const snap = await db
    ...
    .get()
    .catch((e: unknown) => {
      logger.error("voiceRecurring.query_error", {
    ...
    try {
      ...
      await db.collection("voice_outbound_queue").add({
        ...
        config: {
          ...
          interactive:
            typeof cfg.interactive === "boolean"
              ? cfg.interactive
      ...
      const next = new Date(
        base.getTime() + (repeat === "daily" ? 86_400_000 : 7 * 86_400_000)
      );
      ...
      logger.error("voiceRecurring.enqueue_error", {
        id: docSnap.id,
        error: String(e),
      });
    }
...
export const processVoiceRecurring = scheduler.onSchedule(
  ...
  async () => {
    await processVoiceRecurringTick();
  }
);
