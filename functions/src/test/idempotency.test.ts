...
import { hasProcessedEvent, recordProcessedEvent, writeEvidence } from '../lib/billing/idempotency.js';
...
describe('stripe idempotency helpers', () => {
    ...
    it('writes evidence documents without PII', async () => {
        const db = getFirestore();
        const id = 'evt_test_2';
        await writeEvidence(db, { eventId: id, type: 'invoice.payment_succeeded', invoiceId: 'in_123', customerId: 'cus_123', subscriptionId: 'sub_123', status: 'paid', requestId: 'req_123' });
        const snap = await db.collection('billing_evidence').where('eventId', '==', id).limit(1).get();
