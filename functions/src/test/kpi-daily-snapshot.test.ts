...
describe("kpiDailySnapshot contract", () => {
  ...
  it("purges docs older than retention window (kpiDaily & kpiAlertsDaily)", async () => {
    ...
    await db.collection("kpiDaily").doc(pastKey).set({
      date: pastKey,
      aiTokensIn: 0,
      aiTokensOut: 0,
      aiCostEstimate: 0,
    ...
    await db.collection("kpiAlertsDaily").doc(pastKey).set({
      date: pastKey,
      alerts: [],
      createdAt: new Date(),
      updatedAt: new Date(),
    ...
    expect(oldAlerts.exists, "old kpiAlertsDaily doc should be purged").to.be
      .false;
  });
});
