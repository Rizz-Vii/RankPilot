...
describe("audit team quota", function () {
  ...
  const baseReq = {
    ...
    checkMobile: true,
    plan: "starter",
    teamId: "team_test_quota",
    debugTeamLimit: 2,
  };
  it("enforces team quota after limit reached", async () => {
    const auth = { uid: "user_quota_1" } as { uid: string };
    try {
      const first = (await __testRunSeoAudit(baseReq, auth)) as unknown as {
        quota?: { team?: { used?: number } };
      };
      expect(first.quota?.team?.used).to.equal(1);
      const second = (await __testRunSeoAudit(baseReq, auth)) as unknown as {
        quota?: { team?: { used?: number } };
      };
      expect(second.quota?.team?.used).to.equal(2);
      let threw = false;
      try {
        await __testRunSeoAudit(baseReq, auth);
      } catch (e: unknown) {
        threw = true;
        const code = (e as { code?: string }).code;
        const msg = String((e as { message?: string }).message || "");
        expect(code === "resource-exhausted" || msg.includes("quota")).to.be
          .true;
      }
      ...
      else (this as unknown as { test?: { skip?: () => void } }).test?.skip?.();
    }
  });
});
