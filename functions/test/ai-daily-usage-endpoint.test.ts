...
describe("Daily AI usage endpoint (range)", () => {
  ...
  before(async () => {
    await db
      ...
      .set({
        ...
        tokensOut: 20,
        costEstimate: 0.01,
        models: { "gpt-4o": { tokensIn: 10, tokensOut: 20 } },
        seeded: true,
    ...
    await db
      ...
      .set({
        ...
        tokensOut: 40,
        costEstimate: 0.05,
        models: { "gpt-4o": { tokensIn: 30, tokensOut: 40 } },
        seeded: true,
        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
      });
  });
  it("returns both days in range", async () => {
    const res = await fetch(
      `http://localhost:3000/api/admin/ai-usage/daily?start=${day1}&end=${day2}`
    );
    ...
    const dates = rows
      .map((r: unknown) =>
        r && typeof r === "object" && "date" in (r as Record<string, unknown>)
          ? String((r as Record<string, unknown>).date)
          : ""
      )
      .filter(Boolean)
      .sort();
