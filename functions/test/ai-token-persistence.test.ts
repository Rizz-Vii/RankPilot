...
describe("AI Memory Manager - token persistence integration", () => {
  it("fires persistDailyUsage with usage from provider", async () => {
    ...
    (
      aiMemoryManager as unknown as {
        __overridePersistDailyUsageForTest: (
          fn: (p: string, i: number, o: number, m: string) => Promise<void>
        ) => void;
      }
    ).__overridePersistDailyUsageForTest(async (provider, inT, outT, model) => {
      captured = { provider, inT, outT, model };
    });

    (
      aiMemoryManager as unknown as {
        __overrideOpenAIInvokerForTest: (
          fn: (
            s: { model: string; apiKey: string },
            req: { prompt: string; model: string },
            t: number
          ) => Promise<{
            ok: boolean;
            content: string;
    ...
    ).__overrideOpenAIInvokerForTest(async () => ({
      ok: true,
      content: "TOKENIZED",
      usage: { in: 33, out: 21 },
    }));
    ...
    const res = await aiMemoryManager.processRequest({
      prompt: "Count these tokens please",
      model: "gpt-4o",
    });
