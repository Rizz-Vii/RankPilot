...
describe('kpiDailySnapshot', () => {
    ...
    it('aggregates today ai usage, revenue metrics, and enforces retention', async () => {
        ...
        const existing = await db.collection('aiUsageDaily').where('date', '==', dateKey).get();
        for (const d of existing.docs) await d.ref.delete();
        // Seed aiUsageDaily for two providers
        await db.collection('aiUsageDaily').doc(`${dateKey}_openai`).set({ date: dateKey, provider: 'openai', tokensIn: 100, tokensOut: 150, costEstimate: 0.12 });
        await db.collection('aiUsageDaily').doc(`${dateKey}_gemini`).set({ date: dateKey, provider: 'gemini', tokensIn: 50, tokensOut: 70, costEstimate: 0.045 });
        ...
        const old = new Date(Date.now() - 95 * 86400_000).toISOString().slice(0, 10);
        await db.collection('kpiDaily').doc(old).set({ date: old, aiTokensIn: 1, aiTokensOut: 1, aiCostEstimate: 0.0001, createdAt: today, updatedAt: today, _schema: 1 });
    ...
    it('enriches provenance & latency percentiles when unifiedMetricsDaily export exists', async () => {
        ...
        await db.collection('aiUsageDaily').doc(`${dateKey}_openai`).set({ date: dateKey, provider: 'openai', tokensIn: 10, tokensOut: 20, costEstimate: 0.01 });
