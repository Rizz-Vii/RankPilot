...
describe("audit crawler modernization (T10)", () => {
  ...
  it("falls back deterministically when fetch fails and Firecrawl disabled", async () => {
    ...
    try {
      const res = (await __testPerformWebCrawl(
        "https://example.com",
        1,
        true
      )) as unknown as {
        raw: { synthetic?: boolean };
        loadTime: number;
        headings: { h1: string[] };
      };
  ...
  it("respects robots disallow (blocked link not fetched) in BFS fallback", async () => {
    const calls: string[] = [];
    const htmlRoot =
      '<html><head><title>Root</title><meta name="description" content="desc"/></head><body><a href="/blocked/page">b</a><a href="/allowed/page">a</a><h1>Main</h1></body></html>';
    const htmlAllowed =
      "<html><head><title>Allowed</title></head><body><h1>Allowed</h1></body></html>";
    ...
    (
      ...
      if (url.endsWith("/allowed/page"))
        return new Response(htmlAllowed, { status: 200 });
      if (url === "https://example.org/")
        return new Response(htmlRoot, { status: 200 });
    ...
    try {
      const res = (await __testPerformWebCrawl(
        ...
        2,
        false
      )) as unknown as { raw: { directFetch?: boolean } };
