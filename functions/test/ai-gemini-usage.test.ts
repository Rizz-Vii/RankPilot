...
describe("AI Memory Manager - Gemini usage metadata extraction", () => {
  it("extracts usage.in/out from promptTokens + completionTokens", async () => {
    ...
    (
      mm.__setGenkitFactoryForTest as unknown as (
        f: () => {
          generate: (p: string) => Promise<{
            text: () => string;
    ...
    const res = await (
      mm.processRequest as (x: {
        prompt: string;
        model: string;
      }) => Promise<{ content: string }>
    )({ prompt: "Hello Gemini usage accounting", model: "gemini-pro" });
  ...
  it("falls back to estimator when usage fields missing", async () => {
    const mm = aiMemoryManager as unknown as Record<string, unknown>;
    mm.services = new Map([
      ["gemini", { model: "gemini-pro", apiKey: "test", timeout: 2000 }],
    ]);
    (
      mm.__setGenkitFactoryForTest as unknown as (
        f: () => { generate: (p: string) => Promise<{ text: () => string }> }
      ) => void
    )(() => ({
      generate: (prompt: string) =>
        Promise.resolve({
    ...
    (mm.persistDailyUsage as unknown as (
      p: string,
      inT: number,
      outT: number
    ) => Promise<void>) = async (_p: string, inT: number, outT: number) => {
      captured = { inT, outT };
    };
    await (
      mm.processRequest as (x: {
