...
describe("invoice webhook ordering", () => {
  before(async () => {
    await db.collection("users").doc("user_order").set({
  ...
  it("applies status transitions draft -> open -> paid", async () => {
    await upsertFinanceInvoice(
      mk({ status: "draft" }) as unknown as Stripe.Invoice,
      { allowUnpaid: true }
    );
    let doc = await db.collection("financeInvoices").doc("in_order_1").get();
    assert.equal(doc.data()!.status, "draft");
    await upsertFinanceInvoice(
      mk({ status: "open" }) as unknown as Stripe.Invoice,
      { allowUnpaid: true }
    );
