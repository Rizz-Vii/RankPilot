...
describe('T28 event mirroring scaffold', () => {
  ...
  beforeEach(() => {
    calls = [];
    (process.env as unknown as Record<string, string>).EVENT_MIRROR_ENABLED = '0';
  ...
  afterEach(() => {
    ...
    if (originalEnv === undefined) delete (process.env as unknown as Record<string, string>).EVENT_MIRROR_ENABLED;
    else (process.env as unknown as Record<string, string>).EVENT_MIRROR_ENABLED = originalEnv as string;
  });

  it('does nothing when flag is disabled', async () => {
    const ts = new Date('2025-01-01T00:00:00.000Z');
    const snap = makeSnap('1735689600000-deadbeef', { orgId: 'org_x', type: 'automation.run.started', ts });
  ...
  it('publishes minimal payload when enabled', async () => {
    (process.env as unknown as Record<string, string>).EVENT_MIRROR_ENABLED = '1';
    const ts = new Date('2025-01-01T00:00:00.000Z');
    const snap = makeSnap('1735689600000-deadbeef', { orgId: 'org_x', type: 'automation.run.started', ts });
    await mirrorEvent({ snapshot: snap, context: { params: { orgId: 'org_x', eventId: snap.id } } });
