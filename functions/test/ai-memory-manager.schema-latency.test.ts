...
describe("AI Memory Manager - schema validation and latency budget", () => {
  ...
  it("throws when strictSchema=true and content does not satisfy schema (JSON parse fails)", async () => {
    ...
    (
      mm as unknown as {
        makeAIRequest: (...args: unknown[]) => Promise<string> | string;
      }
    ).makeAIRequest = async () => "not json";
    let threw = false;
    try {
      ...
      (
        mm as unknown as {
          makeAIRequest: AIMemoryManagerPrivate["makeAIRequest"];
        }
      ).makeAIRequest = originalImpl;
    }
  ...
  it("returns response when strictSchema=false even if schema parsing fails", async () => {
    ...
    ).makeAIRequest = async () => "still not json";
    ...
    try {
      ...
      const structured =
        res && typeof res === "object" && "structured" in res
      ...
      // restore services
      (
        mm as unknown as { services: AIMemoryManagerPrivate["services"] }
      ).services = snapshot as AIMemoryManagerPrivate["services"];
    }
  });
