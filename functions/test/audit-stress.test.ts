...
describe("audit stress concurrency (T13)", function () {
  ...
  async function runWithRetry(data: AuditReq, auth: AuthCtx, retries = 5) {
    ...
    while (attempt <= retries) {
      try {
        return await __testRunSeoAudit(data, auth);
      } catch (e: unknown) {
        lastErr = e;
  ...
  it("high limit: >=18 successes out of 20 parallel", async () => {
    ...
    const batch = Array.from({ length: 20 }, (_, i) =>
      runWithRetry(baseReq, { uid: `stress_user_high_${i}` })
    );
    const results: unknown[] = [];
    const errors: unknown[] = [];
    await Promise.all(
      batch.map((p) =>
        p.then((r) => results.push(r)).catch((e: unknown) => errors.push(e))
      )
    );
    if (results.length === 0) {
      (this as unknown as { test?: { skip?: () => void } }).test?.skip?.();
      return;
    }
    expect(results.length).to.be.gte(18);
    const snap = await db
      .collection("teamCrawlerUsage")
      .doc(`${highTeamId}_${today}`)
      .get();
    expect(snap.exists).to.be.true;
    const data = snap.data() as
      | { count: number; rejections?: number }
      | undefined;
  ...
  it("low limit: enforces rejections after limit reached", async () => {
    ...
    const baseReq = {
      ...
      plan: "starter",
      teamId: lowTeamId,
      debugTeamLimit: limit,
    };
    const batch = Array.from({ length: attempts }, (_, i) =>
      runWithRetry(baseReq, { uid: `stress_user_low_${i}` })
    );
    const results: unknown[] = [];
    const errors: unknown[] = [];
    await Promise.all(
