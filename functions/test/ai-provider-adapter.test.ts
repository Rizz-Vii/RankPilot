...
type AIMgrPriv = {
  ...
  invokeAnthropic: () => Promise<{
    ok: boolean;
    content: string;
    usage?: { in: number; out: number };
  }>;
  invokeOpenAI: () => Promise<{
    ok: boolean;
    content: string;
    usage?: { in: number; out: number };
  }>;
  invokeGemini: () => Promise<{
    ok: boolean;
    content: string;
    usage?: { in: number; out: number };
  }>;
...
describe("AI Provider Adapter", () => {
  ...
  it("reorders providers putting preferred first (AI_PROVIDER)", async () => {
    ...
    const services: Services = (aiMemoryManager as unknown as AIMgrPriv)
      .services;
    const first = Array.from(services.keys())[0];
    expect(first).to.equal("anthropic");
    // Patch invokeAnthropic to avoid real network
    (aiMemoryManager as unknown as AIMgrPriv).invokeAnthropic = async () => ({
      ok: true,
      content: "anthropic-ok",
      usage: { in: 1, out: 2 },
    });
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "Hello",
  ...
  it("routes by capability when requested capability unsupported on first provider (prefers provider supporting capability)", async () => {
    ...
    (aiMemoryManager as unknown as AIMgrPriv).invokeGemini = async () => ({
      ok: true,
      content: "TOKENIZED",
      usage: { in: 5, out: 7 },
    });
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "Return JSON please",
      model: "gpt-4o",
  ...
  it("routes to provider with function_calling capability when primary lacks it", async () => {
    ...
    (aiMemoryManager as unknown as AIMgrPriv).invokeOpenAI = async () => {
      throw new Error("forced-openai-skip");
    };
    (aiMemoryManager as unknown as AIMgrPriv).invokeAnthropic = async () => ({
      ok: true,
      content: "ANTHROPIC-FC",
      usage: { in: 3, out: 4 },
    });
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "Call a function",
      model: "gpt-4o",
      options: { capability: "function_calling", latencyBudgetMs: 500 },
    });
  ...
  it("routes to provider with vision capability when requested and available", async () => {
    (aiMemoryManager as unknown as AIMgrPriv).services = new Map([
      [
        "anthropic",
        { model: "claude-3-5-sonnet-20241022", apiKey: "kA", timeout: 200 },
      ],
    ...
    (aiMemoryManager as unknown as AIMgrPriv).invokeAnthropic = async () => {
      throw new Error("anthropic-cannot-handle-vision");
    };
    (aiMemoryManager as unknown as AIMgrPriv).invokeOpenAI = async () => ({
      ok: true,
      content: "OPENAI-VISION",
      usage: { in: 2, out: 5 },
    });
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "Describe image",
      model: "claude-3-5-sonnet-20241022",
      options: { capability: "vision", latencyBudgetMs: 500 },
    });
  ...
  it("opens circuit after threshold consecutive failures and surfaces circuit_open error", async () => {
    ...
    for (let i = 0; i < 3; i++) {
      try {
        await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
          prompt: "Fail " + i,
          model: "gpt-4o",
        });
      } catch {
    ...
    const failures = (aiMemoryManager as unknown as AIMgrPriv).cbFailures.get(
      "openai"
    ) as { count: number; openedUntil: number };
  ...
  it("routes function_calling capability to provider that supports it (openai) when others do not", async () => {
    ...
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "call a function",
      model: "gpt-4o",
      options: { capability: "function_calling" },
    });
  ...
  it("retains preferred provider when capability (vision) supported by multiple providers", async () => {
    ...
    (aiMemoryManager as unknown as AIMgrPriv).invokeGemini = async () => ({
      ok: true,
      content: "VISION-GEMINI",
      usage: { in: 4, out: 6 },
    });
    const res = await (aiMemoryManager as unknown as AIMgrPriv).processRequest({
      prompt: "Describe this image",
      model: "gemini-pro",
      options: { capability: "vision" },
    });
