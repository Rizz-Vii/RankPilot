...
async function upstashIncr(
  ...
  try {
    ...
    if (count === 1 && expireSeconds > 0) {
      ...
      fetch(
        `${upstashUrl}/expire/${encodeURIComponent(key)}/${expireSeconds}`,
        {
          method: "GET",
          headers: { Authorization: `Bearer ${upstashToken}` },
          cache: "no-store",
...
const DEV_RELAX =
  process.env.RATE_LIMIT_DEV_RELAX === "1" &&
...
const DEFAULT_WEB_CAP = DEV_RELAX
  ? 1000
  : !DURABLE_ENABLED && process.env.NODE_ENV === "production"
    ? 30
    : 100;
const DEFAULT_API_CAP = DEV_RELAX
  ? 600
  : !DURABLE_ENABLED && process.env.NODE_ENV === "production"
    ? 15
    : 50;
const RL_MAX_REQUESTS = Number(process.env.RATE_LIMIT_MAX || DEFAULT_WEB_CAP);
const RL_API_MAX_REQUESTS = Number(
  process.env.API_RATE_LIMIT_MAX || DEFAULT_API_CAP
);
...
try {
  ...
  if (process.env.NODE_ENV === "production" && !g.__RP_RL_HEALTH_LOGGED__) {
    ...
    if (!DURABLE_ENABLED) {
      logger.warn("rate.limit.durable_store_missing", {
        note: "Using in-memory per-instance limiter",
        emergencyCaps: { web: RL_MAX_REQUESTS, api: RL_API_MAX_REQUESTS },
      });
    }
...
export async function rateLimit(req: NextRequest) {
  try {
    ...
    if (
      ...
      probeHeader === process.env.CRAWL_PROBE_TOKEN
    ) {
    ...
    if (teamResult && !teamResult.allowed) {
      ...
      return new NextResponse(
        ...
        {
          status: 429,
          headers: { ...teamResult.headers, "Cache-Control": "no-store" },
        }
      );
    }
    ...
    if (teamResult && teamResult.allowed) {
      Object.entries(teamResult.headers).forEach(([k, v]) =>
