...
import type { BatchExportConfig } from "@/lib/visualizations/chart-export-manager";
...
import {
  mergeServerWidget,
  serverToClientWidget,
  toServerCreate,
  toServerMutation,
} from "@/lib/dashboard/widget-mapping";
...
export const VisualizationDashboardBuilder: React.FC<
  VisualizationDashboardBuilderProps
> = ({ initialLayout, onSave, onExport, readOnly = false }) => {
  const [layout, setLayout] = useState<DashboardLayout>(
    initialLayout || createDefaultLayout()
  );
  // remote dashboard id (set after first successful server create)
  const [serverDashboardId, setServerDashboardId] = useState<string | null>(
    null
  );
    ...
    try {
      const cached =
        typeof window !== "undefined"
          ? localStorage.getItem("serverDashboardId")
          : null;
      ...
      try {
        const token =
          typeof window !== "undefined"
            ...
            : null;
        if (!token) return;
        void fetch("/api/dashboard/custom?action=dashboards", {
          ...
          .then((json) => {
            ...
            }> = Array.isArray(dashboardsUnknown)
              ? (dashboardsUnknown.filter(
                  (d) => d && typeof d === "object" && "id" in d
                ) as Array<{ id: string; widgets?: ServerDashboardWidget[] }>)
              : [];
            const serverDash = dashboards.find(
              (d) => d.id === serverDashboardId
            );
            if (!serverDash || cancelled) return;
            const serverWidgets: ServerDashboardWidget[] =
              serverDash.widgets || [];
            setLayout((prev) => {
              const existing = new Set(
                prev.widgets.map((w) => w.serverId || w.id)
              );
              const additions = serverWidgets
                .filter((sw) => !existing.has(sw.id))
                .map((sw) => serverToClientWidget(sw));
              if (additions.length === 0) return prev;
              return {
                ...prev,
                widgets: [...prev.widgets, ...additions],
                updated: new Date(),
              };
            });
          })
  ...
  const [sampleData, setSampleData] = useState<Map<string, ChartDataPoint[]>>(
    new Map()
  );
  ...
  const [tableData, setTableData] = useState<
    Map<
      string,
      { rows: TableRowType[]; total: number; loading: boolean; error?: string }
    >
  >(new Map());
  ...
  const renderChart = useCallback(
    async (widget: DashboardWidget) => {
      ...
      try {
        const { d3VisualizationEngine } = await import(
          "@/lib/visualizations/d3-visualization-engine"
        );
        ...
        switch (config.type) {
          ...
          case "pie":
            ...
            break;
          case "scatter":
            d3VisualizationEngine.createScatterPlot(
              `chart-${widget.id}`,
              config
            );
            break;
        ...
        try {
          sonnerToast.error("Render failed", { description: errMsg(error) });
        } catch {}
            ...
            if (json?.widget?.id) {
              ...
              setLayout((prev) => ({
                ...
                widgets: prev.widgets.map((w) =>
                  w.id === newWidget.id ? mergeServerWidget(w, json.widget) : w
                ),
              }));
            }
          })
  ...
  // Table helpers
  const getDefaultTableState = useCallback(
    (): {
      sortBy: TableSortBy;
      direction: TableDirection;
      page: number;
      pageSize: number;
    } => ({ sortBy: "metric", direction: "asc", page: 0, pageSize: 5 }),
    []
  );
  ...
  const handleSort = (
    widgetId: string,
    column: "metric" | "value" | "change"
  ) => {
    setTableState((prev) => {
      const next = new Map(prev);
      const current = next.get(widgetId) || getDefaultTableState();
      const direction: "asc" | "desc" =
        current.sortBy === column && current.direction === "asc"
          ? "desc"
          : "asc";
  ...
  const handlePageChange = (
    ...
    setTableState((prev) => {
      ...
      const newPage = Math.max(
        0,
        Math.min(current.page + delta, Math.max(0, totalPages - 1))
      );
  ...
  const buildTableQuery = useCallback(
    (
      ...
      opts?: { all?: boolean; format?: "json" | "csv" }
    ) => {
  ...
  const widgetsKey = React.useMemo(
    () => layout.widgets.map((w) => `${w.id}:${w.type}`).join("|"),
    [layout.widgets]
  );
  useEffect(() => {
    ...
    widgets.forEach((w) => {
      ...
      void fetch(`/api/table-data?${buildTableQuery(w.id, state)}`, {
        ...
        .then(async (res) => {
          ...
          setTableData((prev) => {
            const next = new Map(prev);
            next.set(w.id, {
              rows: json.rows as TableRowType[],
        ...
        .catch((err) => {
          ...
          try {
            sonnerToast.error("Failed to load table data", {
    ...
    void (async () => {
      try {
        ...
        await fetch("/api/dashboard/custom", {
          ...
          body: JSON.stringify({
            action: "update",
            dashboardId: serverDashboardId,
            updates: { widgetId, ...serverMutation },
          }),
        });
      } catch {
        /* ignore */
  ...
  const exportDashboard = async (format: "pdf" | "excel" | "json") => {
    ...
    try {
      ...
      try {
        sonnerToast.error("Dashboard export failed", {
                ...
                <>
                  ...
                  <div>
                    ...
                    <WidgetPropertiesPanel
                      widget={
                        layout.widgets.find((w) => w.id === selectedWidget)!
                      }
                      onUpdate={(updates) =>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {layout.widgets.map((widget) => (
              <Card
                key={widget.id}
                className={`${
                  selectedWidget === widget.id && !isPreviewMode
                    ? "ring-2 ring-primary"
                    : ""
                }`}
              ...
              >
                    ...
                    {widget.type === "chart" && (
                      <div className="flex items-center gap-2">
                        <ExportButton
                          ...
                          onDone={(url) => {
                            try {
                              sonnerToast.success("Chart export ready", {
                                description: "PNG generated",
                              });
                            } catch {}
                ...
                <CardContent>
                  {widget.type === "chart" ? (
                    ...
                    <div className="text-sm text-muted-foreground">
                      This is a sample text widget. You can add any content
                      here.
                    </div>
                  ) : (
                    <div className="text-sm">
                      {(() => {
                        const state =
                          tableState.get(widget.id) || getDefaultTableState();
                        const data = tableData.get(widget.id) || {
                          rows: [],
                          total: 0,
                          loading: false,
                        };
                        const totalPages = Math.max(
                          1,
                          Math.ceil((data.total || 0) / state.pageSize)
                        );
                        return (
                            ...
                            <div className="flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                <div className="text-xs text-muted-foreground">
                                  {data.loading
                                    ? "Loading…"
                                    : `${data.total} items • Page ${state.page + 1} of ${totalPages}`}
                                </div>
                                <Button
                                  variant="outline"
                                  size="sm"
                              ...
                              <div className="flex items-center gap-2">
                                <Select
                                  ...
                                  onValueChange={(v) =>
                                    handlePageSizeChange(widget.id, parseInt(v))
                                  }
                                >
                                ...
                                <div className="flex items-center">
                                  <Button
                                    ...
                                    disabled={state.page === 0 || data.loading}
                                  >
                                  ...
                                  <Button
                                    ...
                                    className="h-8 w-8"
                                    onClick={() =>
                                      handlePageChange(widget.id, 1, totalPages)
                                    }
                                    disabled={
  ...
  return (
    <div className="mt-2 space-y-3">
      ...
      <div>
        ...
        <Select
          value={widget.type}
          onValueChange={(val: DashboardWidget["type"]) =>
            onUpdate({ type: val })
          }
        >
      ...
      {widget.type === "chart" && (
        <div>
          ...
          <Select
            ...
            onValueChange={(val: ChartConfig["type"]) =>
              onUpdate({
                chartConfig: {
                  id: widget.chartConfig?.id || `chart_${widget.id}`,
                  width: widget.chartConfig?.width || 400,
                  height: widget.chartConfig?.height || 300,
                  margin: widget.chartConfig?.margin || {
                    top: 10,
                    right: 10,
                    bottom: 30,
                    left: 40,
                  ...
                  styling: widget.chartConfig?.styling || {},
                  type: val,
                },
              })
            }
...
async function ensureServerDashboard(token: string): Promise<string | null> {
  try {
    ...
    if (id && typeof window !== "undefined")
      localStorage.setItem("serverDashboardId", id);
    return id || null;
  } catch {
    return null;
  }
