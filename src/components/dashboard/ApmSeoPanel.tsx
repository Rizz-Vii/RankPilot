...
export default function ApmSeoPanel() {
  const [snap, setSnap] = useState<SeoSnapshot>({
    metaPresent: null,
    h1Count: null,
    missingAlt: null,
  });
  ...
  useEffect(() => {
    ...
    const metaMeta = meta?.metadata as
      | {
          length?: number;
          snippet?: string;
          titleLength?: number;
    ...
    setSnap({
      metaPresent: meta
        ? { value: meta.value, ts: meta.timestamp, meta: metaMeta }
        : null,
      h1Count: h1 ? { value: h1.value, ts: h1.timestamp, meta: h1Meta } : null,
      missingAlt: img
        ? { value: img.value, ts: img.timestamp, meta: imgMeta }
        : null,
    });
  }, []);
  ...
  const asMetaMeta = (
    ...
    return {
      ...
      titleLength:
        typeof x.titleLength === "number" ? x.titleLength : undefined,
      ...
      if (metric.name === "seo.meta_description.present") {
        setSnap((s) => ({
          ...
          metaPresent: {
            value: metric.value,
            ts: metric.timestamp,
            meta: asMetaMeta(metric.metadata),
          },
        }));
        ...
        setSnap((s) => ({
          ...
          missingAlt: {
            value: metric.value,
            ts: metric.timestamp,
            meta: asImgMeta(metric.metadata),
          },
        }));
      }
    };
    (
      enterpriseAPM as unknown as {
        on: (ev: string, cb: (m: APMMetric) => void) => void;
      }
    ).on("metric-recorded", handler);
    return () => {
      // Remove listener if available
      const anyApm = enterpriseAPM as unknown as {
        off?: (ev: string, cb: (m: APMMetric) => void) => void;
      };
      if (typeof anyApm.off === "function")
        anyApm.off("metric-recorded", handler);
    };
  }, []);

  const lastUpdated = useMemo(() => {
    const ts = Math.max(
      snap.metaPresent?.ts ?? 0,
      snap.h1Count?.ts ?? 0,
      snap.missingAlt?.ts ?? 0
    );
    if (!ts) return null;
        ...
        <div className="p-3 border rounded-md">
          ...
          <div className="text-xs text-muted-foreground">
            {snap.metaPresent?.value ? (
              <>
                Present
                {typeof snap.metaPresent.meta?.length === "number" &&
                  ` • ${snap.metaPresent.meta.length} chars`}
                {snap.metaPresent.meta?.snippet && (
                  <div
                    className="truncate mt-0.5"
                    title={snap.metaPresent.meta.snippet}
                  >
                    “{snap.metaPresent.meta.snippet}”
                  </div>
                )}
            ...
            {typeof snap.h1Count?.value === "number" ? (
              <>
                ...
                {snap.h1Count.meta?.firstH1 && (
                  <div
                    className="truncate mt-0.5"
                    title={snap.h1Count.meta.firstH1}
                  >
          ...
          <div className="text-xs text-muted-foreground">
            {typeof snap.missingAlt?.value === "number" ? (
              <>
                ...
                {snap.missingAlt.value > 0 &&
                  snap.missingAlt.meta?.samples &&
                  snap.missingAlt.meta.samples.length > 0 && (
                    <div className="mt-0.5">
                      <Badge
                        variant="outline"
                        className="text-[10px]"
                        title={snap.missingAlt.meta.samples.join(", ")}
                      >
                        {(() => {
                          try {
                            return `sample: ${new URL(snap.missingAlt!.meta!.samples![0]!, window.location.href).pathname}`;
                          } catch {
