...
import { AlertTriangle, BarChart3, Bot, Loader2, Maximize2, Minimize2, Send, Settings, Shield, TrendingUp, User, Users, X } from 'lucide-react';
...
What would you like to analyze today?`,
    ...
    const handleImageSelect = async (_e: React.ChangeEvent<HTMLInputElement>): Promise<void> => {
        try {
            ...
            if (file.size > 5 * 1024 * 1024) { setError('Image exceeds 5MB'); return; }
            setUploading(true);
        ...
        try {
            ...
            rec.onstop = async () => {
                try {
                    ...
                    if (blob.size > 10 * 1024 * 1024) { setError('Audio exceeds 10MB'); return; }
                    if (!user) { setError('Authentication required'); return; }
                    setUploading(true);
        ...
        try {
            ...
            try {
                ...
                if (streamRes.ok && streamRes.headers.get('content-type')?.includes('text/event-stream')) {
                    ...
                    // Final sanitize
                    try {
                        const html = DOMPurify.sanitize(accumulated.replace(/\n/g, '<br/>'));
                        setMessages(prev => prev.map(m => m.id === aiId ? { ...m, response: html } : m));
                    } catch { /* ignore */ }
                }
            } catch { /* fall back to non-stream */ } finally {
                setStreaming(false);
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    errMsg = (errorData as Record<string, unknown>)?.['error'] as string || errMsg;
                } catch {}
            ...
            const aiMessage: AdminChatMessage = {
                ...
                metadata: {
                    systemMetrics: Array.isArray(data.context?.dataUsed) && data.context.dataUsed.includes('system_metrics'),
                    performanceData: Array.isArray(data.context?.dataUsed) && data.context.dataUsed.includes('performance_data'),
                },
            };
                                            ...
                                            <div className="flex gap-3 justify-start">
                                                ...
                                                <div className="bg-muted rounded-lg p-3 text-sm">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        <span>{streaming ? 'Streaming response…' : 'Analyzing system data...'}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        )}
                                ...
                                <div className="p-4 border-t border-border">
                                    <div className="flex gap-2 items-center">
                                        ...
                                        {audioRecorderRef.current ? (
                                            <Button type="button" variant="ghost" size="icon" className="h-9 w-9 text-destructive" onClick={stopRecording} aria-label="Stop recording" title="Stop recording">
                                            ...
                                            <Button type="button" variant="ghost" size="icon" className="h-9 w-9" onClick={() => { void startRecording(); }} aria-label="Record voice note" title="Record voice note" disabled={isLoading || uploading || !isAdmin}>
                                        ...
                                        <Button
                                            ...
                                            disabled={!inputValue.trim() || isLoading || streaming || !isAdmin}
                                            size="sm"
                                    ...
                                    {(uploading || micBlocked) && (
                                        <div className="mt-2 text-xs text-muted-foreground">
                                            ...
                                            {micBlocked && !uploading && <span>Microphone blocked by permissions</span>}
                                        </div>
                                    )}
                                        ...
                                        {quickCommands.map((cmd, index) => (
                                            <Button
                                                ...
                                                onClick={() => {
                                                    ...
                                                    if (route && isAdmin && canNavigateTo('starter', route, { isAdmin: true })) {
