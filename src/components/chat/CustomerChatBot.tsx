...
const CHAT_MESSAGE_TYPES: readonly ChatMessageType[] = [
  "text",
  "image",
  "audio",
  "system",
...
const MessageRow = memo(function MessageRow({
  ...
  return (
    <div
      ...
      aria-label={
        msg.isUser ? `User message ${msg.message?.slice(0, 40)}` : "AI message"
      }
    ...
    >
      ...
      >
        ...
        {!msg.isUser && isCurrentStreaming && (
          <div
            className="flex items-center gap-2 mt-2 text-[11px] text-primary select-none"
            role="status"
            aria-live="polite"
          >
...
const coerceMsgType = (val: unknown): ChatMessage["type"] => {
  return typeof val === "string" &&
    (CHAT_MESSAGE_TYPES as readonly string[]).includes(val)
...
const normalizeMessages = (items: unknown[]): ChatMessage[] => {
  ...
  for (const m of items) {
    ...
    const fallbackTs =
      typeof mm.timestamp === "string" && mm.timestamp
        ? mm.timestamp
...
export default function CustomerChatBot({
  ...
  const [_previousActions, setPreviousActions] = useState<Set<string>>(
    new Set()
  );
  const [newActionHighlights, setNewActionHighlights] = useState<Set<string>>(
    new Set()
  );
  ...
  const userTier =
    (profile?.subscriptionTier as SubscriptionTier | undefined) || "free";
  ...
  useEffect(() => {
    try {
      localStorage.setItem("rp_chat_isOpen", isOpen ? "1" : "0");
    } catch {}
  }, [isOpen]);
  useEffect(() => {
    try {
      localStorage.setItem("rp_chat_isMin", isMinimized ? "1" : "0");
    } catch {}
  }, [isMinimized]);
  ...
  const handleManualScroll: React.UIEventHandler<HTMLDivElement> = useCallback(
    (e) => {
  ...
  const initSugKey = Array.isArray(initialSuggestions)
...
• Suggest schema markup for a product page`;
  ...
  useEffect(() => {
    const restore = async () => {
      ...
      try {
        const token = await (
          auth.currentUser || (user as AuthUserLike)
        )?.getIdToken?.();
        if (!token) throw new Error("No auth token");
        const res = await fetch("/api/chat/customer?limit=30", {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data.messages) && data.messages.length) {
            ...
            const mapped: ChatMessage[] = normalizeMessages(
            ...
            if (data.actionProgress && typeof data.actionProgress === "object")
              setCompletedActions(data.actionProgress);
  ...
  useEffect(() => {
    const count = messages.reduce(
      (acc, m) => acc + (m.type === "image" || m.type === "audio" ? 1 : 0),
      0
    );
  ...
  useEffect(() => {
    ...
    setPreviousActions((prev) => {
      ...
      if (newlyAdded.length) {
        ...
        if (newActionTimeoutRef.current)
          clearTimeout(newActionTimeoutRef.current);
  ...
  useEffect(() => {
    ...
    interface AugmentedPre extends HTMLPreElement {
      _rpCopyBtn?: boolean;
    }
    pres.forEach((node) => {
      ...
      btn.className =
        "absolute top-1 right-1 rounded-md bg-black/50 text-white text-[10px] px-2 py-1 hover:bg-black/70 transition focus:outline-none focus:ring-2 focus:ring-primary";
  ...
  const sendMessage = async () => {
    ...
    const classifyIntent = (text: string): string => {
      ...
      if (/core web vitals|lcp|cls|fid|tti|performance/.test(t))
        return "performance";
      if (/keyword|cluster|search volume|intent/.test(t))
        return "keyword_strategy";
      if (/schema|structured data|json-ld|rich snippet/.test(t))
      ...
      if (/rewrite|optimi(s|z)e|improve content|density/.test(t))
    ...
    try {
      ...
      const curr = (auth.currentUser || (user as AuthUserLike)) as
        | AuthUserLike
        | undefined;
      ...
      try {
        ...
        if (
          ...
          try {
            ...
            setMessages((prev) =>
              prev.map((m) =>
                m.id === aiId
                  ? { ...m, response: html, intent: meta?.intent || intent }
                  : m
              )
            );
          } catch {
        ...
        /* streaming failed, fallback to legacy */
      } finally {
        setStreaming(false);
      ...
      const doRequest = async (forced = false) =>
  ...
  const clearHistory = () => {
    setMessages([]);
    try {
      localStorage.removeItem("rp_chat_recent");
    } catch {}
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > maxMB * 1024 * 1024) {
      setError(`Image exceeds ${maxMB}MB`);
      return;
    }
    if (attachmentsUsed >= getAttachmentLimit(profile?.subscriptionTier)) {
      setError("Attachment quota reached");
      return;
    }
    ...
    try {
      ...
      try {
        await fetch("/api/chat/customer", {
          ...
          body: JSON.stringify({
            attachments: [
              { type: "image", mediaUrl: url, name: pendingImage.name },
            ],
          }),
        });
      } catch {
      ...
      try {
        ...
        if (token) {
          await fetch("/api/chat/customer", {
            ...
            body: JSON.stringify({
              attachments: [
                { type: "audio", mediaUrl: url, name: "voice-note.webm" },
              ],
            }),
          });
        }
  ...
  const onScrollAreaScroll: React.UIEventHandler<HTMLDivElement> = useCallback(
    (e) => {
      ...
      if (target.scrollTop < 40 && !loadingMore && hasMore && user) {
        ...
        void (async () => {
          try {
            ...
            const url = before
              ? `/api/chat/customer?limit=25&before=${encodeURIComponent(before)}`
              : `/api/chat/customer?limit=25`;
            ...
            if (res.ok) {
              ...
              if (Array.isArray(data.messages) && data.messages.length) {
                ...
                oldestMessageTsRef.current =
                  mapped[0]?.timestamp || oldestMessageTsRef.current;
                setHasMore(data.hasMore ?? false);
      ...
      setMessages((prev) =>
        prev.map((m) =>
          m.id === msgId
            ? {
                ...m,
                response: (m.response || "") + "<br/><em>[Canceled]</em>",
                type: "system",
              }
            : m
        )
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    const composing =
      (typeof nativeEvt === "object" &&
        nativeEvt !== null &&
        "isComposing" in nativeEvt &&
        (nativeEvt as { isComposing?: unknown }).isComposing === true) ||
      isComposing;
    if (composing) return;
  ...
  const ChatWindow = () => (
    <motion.div
      ref={chatWindowRef}
      initial={
        reducedMotion ? { opacity: 1 } : { opacity: 0, scale: 0.95, y: 20 }
      }
      animate={
        reducedMotion
          ? {
              opacity: 1,
              height: isMinimized ? "auto" : isMobile ? "70vh" : "600px",
            }
          : {
              opacity: 1,
              scale: 1,
      ...
      className={cn(
        ...
        isMobile
          ? "bottom-0 right-0 left-0 mx-auto w-full max-w-[520px] mb-0"
    ...
    >
      ...
      >
        ...
        {!isMinimized && (
          <CardContent
            className={cn(
              "relative flex flex-col min-h-0 p-0",
              isMobile
                ? "h-[calc(min(75vh,85dvh)-80px)]"
                : "h-[calc(600px-80px)]"
            )}
            style={
              isMobile
          ...
          >
              ...
              <div className="mb-2 rounded-xl border border-primary/20 bg-primary/10 p-3 text-[11px] leading-relaxed text-primary shadow-sm">
                <div className="font-semibold text-xs mb-1 flex items-center gap-2">
                  ...
                  {focusKeywords.length > 0 && (
                    <span className="font-normal text-[10px] text-primary truncate max-w-[160px]">
                      KW: {focusKeywords.slice(0, 5).join(", ")}
                    </span>
                  )}
                </div>
                <div
                ...
                {pendingActions.length > 0 && (
                  <div className="mt-2 space-y-1">
                    {pendingActions.slice(0, 6).map((a) => (
                      <label
                        key={a}
                        className="flex items-start gap-2 text-[10px] cursor-pointer select-none"
                      >
                        <input
                          type="checkbox"
                          ...
                          onChange={() => {
                            ...
                            if (actionUpdateDebounceRef.current)
                              clearTimeout(actionUpdateDebounceRef.current);
                              ...
                              void (async () => {
                                try {
                                  ...
                                  const res = await fetch(
                                    `/api/chat/customer/actions?sessionId=${sessionId}`,
                                    {
                                      method: "POST",
                                      headers: {
                                        "Content-Type": "application/json",
                                  ...
                                  if (res.ok) {
                                    ...
                                    if (json?.actionStats) {
                                      setActionStats({
                                        totalCompleted:
                                          json.actionStats.totalCompleted || 0,
                                        totalPending:
                                          json.actionStats.totalPending || 0,
                                        completionRate:
                                          json.actionStats.completionRate || 0,
                                      });
                                    }
                                  }
                                } catch {
                        ...
                        <span
                          className={cn(
                            ...
                            newActionHighlights.has(a) &&
                              !completedActions[a] &&
                              "bg-success/15 text-success px-1 rounded"
                          )}
                        >
                          {a}
                        </span>
                    ...
                    {actionStats && (
                      <div className="pt-1 text-[10px] text-primary flex items-center gap-2">
                        <span>
                          Actions: {actionStats.totalCompleted}/
                          {actionStats.totalCompleted +
                            actionStats.totalPending}
                        </span>
                        <span className="inline-block h-1.5 w-16 rounded bg-primary/15 overflow-hidden">
                          <span
                            className="block h-full bg-primary transition-all"
                            style={{
                              width: `${Math.round(actionStats.completionRate * 100)}%`,
                            }}
                          />
                        </span>
                        <span>
                          {Math.round(actionStats.completionRate * 100)}%
                        </span>
                      </div>
                    )}
                  </div>
            ...
            <ScrollArea
              className={cn("flex-1 h-full p-4", isMobile && "pt-3 pb-3")}
              onScrollCapture={(e) => {
                onScrollAreaScroll(e);
            ...
            >
              <div
                ...
                role="log"
                aria-live={streaming ? "polite" : "polite"}
              ...
              >
                ...
                {(failedImage || failedAudio) && (
                  <div className="bg-warning/15 border border-warning/30 rounded p-2 text-xs flex flex-col gap-1">
                    {failedImage && (
                      <div>
                        Image failed: {failedImage.name}{" "}
                        <button
                          className="underline"
                          onClick={() => {
                            setPendingImage(failedImage);
                            setFailedImage(null);
                          }}
                        >
                          retry
            ...
            {suggestions.length > 0 && (
              <div
                className="px-4 pb-2 pt-1 border-t border-border bg-gradient-to-r from-muted/50 via-background to-muted/50 overflow-x-auto scrollbar-none"
                  ...
                  {suggestions.map((s) => (
                    <button
                      ...
                      onClick={() => {
                        ...
                        if (
                          route &&
                          canNavigateTo(userTier as SubscriptionTier, route)
                        ) {
                          try {
                            router.push(route);
                          } catch {}
            ...
            <div
              className={cn(
                "border-t border-border bg-card/80 backdrop-blur supports-[backdrop-filter]:bg-card/60",
                isMobile ? "p-3" : "p-4"
              )}
              style={
                isMobile
            ...
            >
              <div
                className={cn("flex gap-2 items-center", isMobile && "gap-1")}
              >
                {/* Attachment buttons */}
                <input
                  ref={fileInputRef}
                ...
                <Button
                  ...
                  size="sm"
                  className={cn(
                    "relative bg-gradient-to-r from-primary to-accent hover:from-primary/90 hover:to-accent/90 disabled:opacity-60 disabled:cursor-not-allowed",
                    isMobile && "px-3"
                  )}
  ...
  return (
    <>
      <AnimatePresence>{!isOpen && chatToggleButton}</AnimatePresence>
