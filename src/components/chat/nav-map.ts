...
export function resolveSuggestionRoute(s: string): string | null {
    ...
    if (text.includes('core web vitals') || text.includes('lcp') || text.includes('cls') || text.includes('tti') || text.includes('performance')) return '/performance';
    if (text.includes('keyword') || text.includes('cluster') || text.includes('long-tail')) return '/keyword-tool';
    if (text.includes('index') || text.includes('crawl') || text.includes('coverage') || text.includes('sitemap') || text.includes('robots')) return '/seo-audit';
    if (text.includes('json-ld') || text.includes('schema') || text.includes('structured data')) return '/content-analyzer';
    if (text.includes('rewrite') || text.includes('snippet')) return '/neuroseo/rewrite-gen';
    if (text.includes('internal link')) return '/link-view';
    if (text.includes('competitor') || text.includes('serp') || text.includes('gap')) return '/competitors';
