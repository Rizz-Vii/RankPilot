...
const AdaptiveProgress: FC<{
  value: number;
  thresholds?: { good: number; warn: number };
  label: string;
  invert?: boolean;
}> = ({ value, thresholds = { good: 90, warn: 70 }, label, invert }) => {
  ...
  return (
    <div className="mt-2" role="group" aria-label={label}>
      ...
      <div className="-mt-2 h-2 w-full pointer-events-none relative">
        <div
          className={`absolute inset-0 ${color} rounded-full transition-all mix-blend-multiply opacity-60`}
...
export function PerformanceDashboard() {
  ...
  useEffect(() => {
    ...
    if (stored === "5m" || stored === "1h" || stored === "24h")
      setSelectedTimeRange(stored);
  ...
  const refreshStats = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      const totalCacheOps = Object.values(
        cacheStats as Record<string, CacheStat>
      ).reduce((sum: number, stat: CacheStat) => sum + (stat?.entries || 0), 0);
      const totalCacheHits = Object.values(
        cacheStats as Record<string, CacheStat>
      ).reduce(
        (sum: number, stat: CacheStat) =>
          sum + ((stat?.hitRate || 0) * (stat?.entries || 0)) / 100,
        0
      );
      ...
      setLatencySamples(
        performanceMonitor
          .getMetrics(undefined, timeRangeMs)
          .map((m) => m.duration || 0)
      );
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        ...
        aria-label="Performance dashboard header"
      >
        <div>
        ...
        <div className="flex items-center gap-2">
          <Tabs
            ...
            onValueChange={(value) =>
              setSelectedTimeRange(value as "5m" | "1h" | "24h")
            }
          >
            <TabsList aria-label="Select time range" role="tablist">
              <TabsTrigger value="5m" aria-label="5 minutes range">
                5m
              </TabsTrigger>
              <TabsTrigger value="1h" aria-label="1 hour range">
                1h
              </TabsTrigger>
              <TabsTrigger value="24h" aria-label="24 hours range">
                24h
              </TabsTrigger>
            </TabsList>
          </Tabs>
          ...
          <CardContent>
            ...
            <div className="mt-2">
              <LatencySparkline
                ...
                describedBy="latency-percentiles-desc"
              />
            </div>
          </CardContent>
        </Card>
      ...
      {latencySamples.length > 4 && (
        <div
          ...
          aria-describedby="latency-percentiles-desc"
        >
          <p id="latency-percentiles-desc" className="sr-only">
            Latency percentiles summarizing distribution for the selected time
            range.
          </p>
          <div className="flex flex-wrap gap-2 text-[11px]">
            {(() => {
              ...
              return [
                ...
                <span
                  key={x.label}
                  className="inline-flex items-center gap-1 rounded bg-muted/60 px-2 py-0.5 font-medium border border-border/50"
                >
          ...
          <div>
            <Button
              ...
              onClick={() => setShowPercentileTable((v) => !v)}
              aria-expanded={showPercentileTable}
              aria-controls="percentile-table"
            >
              {showPercentileTable
                ? "Hide Distribution Table"
              ...
              <table className="min-w-full text-[11px]">
                ...
                <tbody>
                  {["0.50", "0.75", "0.90", "0.95", "0.99"].map((p) => {
                    ...
                    const idx = Math.min(
                      ordered.length - 1,
                      Math.floor(parseFloat(p) * (ordered.length - 1))
                    );
      ...
      <Tabs defaultValue="operations" className="w-full">
        <TabsList aria-label="Detailed metrics sections">
          <TabsTrigger value="operations" aria-label="Operations metrics">
            Operations
          </TabsTrigger>
          <TabsTrigger value="cache" aria-label="Cache performance metrics">
            Cache Performance
          </TabsTrigger>
          <TabsTrigger value="errors" aria-label="Error analysis metrics">
            Error Analysis
          </TabsTrigger>
        </TabsList>
