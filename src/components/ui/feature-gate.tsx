...
export function FeatureGate({
  ...
  const hasAccess = userAccess
    ? canAccessCapability(userAccess, feature)
    : false;
  ...
  if (fallback) {
    return (
      <div className={className} data-testid="feature-gate-fallback">
        {fallback}
      </div>
    );
  }
...
function UpgradePrompt({
  ...
  const normalizedCurrent: SubscriptionTier = (
    TIER_HIERARCHY.includes(currentTier as SubscriptionTier)
      ? currentTier
      : "free"
  ) as SubscriptionTier;
  const requiredTier: SubscriptionTier =
    featureConfig.requiredTier || "starter";
  ...
  const tierColors = {
    free:
      ...
      " " +
      colors.projectStatus.completed.text,
    starter:
      colors.projectStatus.planning.bg +
      " " +
      colors.projectStatus.planning.text,
    ...
    <Card className="border-destructive/25 bg-destructive/5">
      <CardHeader>
        ...
        <CardDescription className="text-xs">
          Unknown feature: "{feature}". Please add it to FEATURE_ACCESS in
          access-control.ts
        </CardDescription>
      </CardHeader>
    </Card>
