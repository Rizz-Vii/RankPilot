    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h3 className={cn(typography.card.title, "truncate")}>{title}</h3>
          {subtitle && (
            <p className={cn(typography.card.subtitle, "mt-1")}>{subtitle}</p>
          )}
        </div>
...
export const EnhancedStatusCard: React.FC<StatusCardProps> = ({
  ...
  return (
    ...
    >
      ...
      {description && (
        <p className={cn(typography.card.subtitle, "mb-4")}>{description}</p>
      )}

      {lastUpdated && (
        <p className={cn(typography.ui.caption, "border-t border-border pt-3")}>
...
export const EnhancedProjectCard: React.FC<ProjectCardProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex-1 min-w-0">
          <h3 className={cn(typography.card.title, "mb-2")}>{title}</h3>
          <p className={cn(typography.card.subtitle, "line-clamp-2")}>
            {description}
          </p>
        </div>
        ...
        <div className="flex gap-2 ml-4">
          <span
            className={
              colors.projectStatus[status].bg +
              " " +
              colors.projectStatus[status].text +
          ...
          <span
            className={
              (priority === "low"
                ? colors.projectStatus.completed.bg +
                  " " +
                  colors.projectStatus.completed.text // Use completed as default for 'low'
                : colors.projectStatus[priority].bg +
                  " " +
                  colors.projectStatus[priority].text) +
              " px-2 py-1 rounded-md text-xs font-medium"
            }
          >
      ...
      <div className="flex items-center justify-between pt-4 border-t border-border">
        <div className="flex -space-x-2">
          {team.slice(0, 3).map((member, index) => (
            <div
              key={index}
              className={
                "w-8 h-8 rounded-full bg-muted border-2 border-background flex items-center justify-center"
              }
            >
        ...
        <span className={typography.ui.caption}>Due {dueDate}</span>
      </div>
    </div>
  );
};
