...
export function ThemeConfiguration({ className }: ThemeConfigurationProps) {
    ...
    const [customColors, setCustomColors] = useState(() => {
        ...
        if (typeof window !== 'undefined') {
            try {
                ...
                if (saved) {
                    const parsed = JSON.parse(saved) as { primary?: string; secondary?: string; accent?: string };
                    return {
                        primary: parsed.primary || preferences.customColors?.primary || 'var(--color-primary-500)',
                        secondary: parsed.secondary || preferences.customColors?.secondary || 'var(--color-gray-500)',
                        accent: parsed.accent || preferences.customColors?.accent || 'var(--color-success-500)'
                    };
                }
            } catch { }
        }
    ...
    const isHexColor = (v: string | undefined) => !!v && /^#([0-9a-fA-F]{6})$/.test(v);
    ...
    const primaryResolved = customColors.primary.startsWith('var(')
        ? (resolveCssVar('--color-primary') || resolveCssVar('--primary') || resolveCssVar('--color-primary-500'))
        : customColors.primary;
    const primaryColorForPicker = isHexColor(primaryResolved)
        ? primaryResolved
        : (typeof window !== 'undefined'
            ? (() => {
                const v = (resolveCssVar('--color-primary') || resolveCssVar('--primary') || resolveCssVar('--color-primary-500'));
    ...
    const secondaryResolved = customColors.secondary.startsWith('var(')
        ? (resolveCssVar('--color-secondary') || resolveCssVar('--secondary') || resolveCssVar('--color-gray-500'))
        : customColors.secondary;
    const secondaryColorForPicker = isHexColor(secondaryResolved)
        ? secondaryResolved
        : (() => { const hex = cssColorToHex(secondaryResolved); return hex || (isHexColor(primaryColorForPicker) ? primaryColorForPicker : defaultHex()); })();

    const accentResolved = customColors.accent.startsWith('var(')
        ? (resolveCssVar('--color-accent') || resolveCssVar('--accent') || resolveCssVar('--color-success-500'))
        : customColors.accent;
    const accentColorForPicker = isHexColor(accentResolved)
        ? accentResolved
        : (() => { const hex = cssColorToHex(accentResolved); return hex || (isHexColor(primaryColorForPicker) ? primaryColorForPicker : defaultHex()); })();
    ...
    const handleCustomColorChange = (colorType: 'primary' | 'secondary' | 'accent', color: string) => {
        ...
        if (typeof next === 'string') {
            if (next.startsWith('#')) {
                ...
                if (!isHexColor(next)) {
                    ...
                    const prior = (colorType === 'primary' ? primaryColorForPicker : colorType === 'secondary' ? secondaryColorForPicker : accentColorForPicker);
        ...
        const persistedForPreferences = {
            primary: colorType === 'primary' ? (valueForPreferences || preferences.customColors?.primary || primaryColorForPicker) : (preferences.customColors?.primary || primaryColorForPicker),
            secondary: colorType === 'secondary' ? (valueForPreferences || preferences.customColors?.secondary || secondaryColorForPicker) : (preferences.customColors?.secondary || secondaryColorForPicker),
            accent: colorType === 'accent' ? (valueForPreferences || preferences.customColors?.accent || accentColorForPicker) : (preferences.customColors?.accent || accentColorForPicker)
        };
    ...
    const resetToDefaults = () => {
        ...
        setPreferences({ customColors: { primary: undefined, secondary: undefined, accent: undefined } });
    };
    ...
    const exportTheme = async () => {
        ...
        try {
            ...
            if (w && w.document) {
                w.document.write('<pre style="white-space:pre-wrap;word-break:break-word;margin:0;padding:12px;font-family:monospace;"></pre>');
                w.document.title = 'Theme Export';
                const pre = w.document.querySelector('pre');
                if (pre) pre.textContent = config;
            } else {
                toast({ title: 'Clipboard unavailable', description: 'Opened export in a new tab.', variant: 'destructive' });
            }
        }
    };
        ...
        try {
            ...
            if (pasted) {
                try {
                    ...
                    setPreferences(parsed);
                    toast({ title: 'Theme imported', description: 'Applied pasted JSON.' });
                } catch {
                    toast({ title: 'Invalid theme JSON', description: 'Please check the pasted content.', variant: 'destructive' });
                }
            }
        }
    };

    const handleExportClick = () => { void exportTheme(); };
    const handleImportClick = () => { void importTheme(); };
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <p className="text-sm text-muted-foreground">
                                Accessibility controls live under the Accessibility tab in Settings. Use that tab to manage High Contrast, Reduced Motion, and related options.
                            </p>
                        </CardContent>
                    </Card>
                </TabsContent>
                    ...
                    <Card>
                        ...
                        <CardContent className="space-y-6">
                            ...
                            <div className="space-y-4">
                                ...
                                <p className="text-xs text-muted-foreground mt-2">If a custom color token cannot be resolved, the previous valid color will be used for preview.</p>
                            </div>
                        </CardContent>
                    </Card>
