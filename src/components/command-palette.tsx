      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <EnhancedButton
            variant="ghost"
            size="icon"
            aria-label={tr("commandPalette.open", "Command palette")}
        ...
        <TooltipContent>
          {tr("commandPalette.shortcut", "Ctrl/Cmd + K")}
        </TooltipContent>
      </Tooltip>
      ...
      <DialogContent className="max-w-2xl p-0 overflow-hidden">
        <div className="border-b px-4 py-3 flex items-center justify-between bg-muted/40">
          <h3 className="text-sm font-medium flex items-center gap-2">
            ...
            {tr("commandPalette.title", "Command Center")}
          </h3>
          <span className="text-xs text-muted-foreground">
            {tr("commandPalette.escHint", "Esc to close")}
          </span>
        </div>
        <div className="p-4 space-y-6">
          <section className="space-y-2">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {tr("commandPalette.section.search", "Search")}
            </h4>
            <GlobalSearch />
          </section>
          <section className="space-y-2">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {tr("commandPalette.section.interface", "Interface")}
            </h4>
            <div className="flex items-center gap-2 flex-wrap">
              ...
              >
                ...
                <span className="ml-2 text-xs hidden sm:inline">
                  {tr("commandPalette.theme.label", "Theme")}: {theme}
                </span>
              </EnhancedButton>
          ...
          <section className="space-y-2">
            <h4 className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
              {tr("commandPalette.section.account", "Account")}
            </h4>
            <div className="flex items-center gap-2 flex-wrap">
              {user ? (
                <EnhancedButton
                ...
                >
                  ...
                  {tr("commandPalette.signOut", "Sign Out")}
                </EnhancedButton>
              ) : (
                <EnhancedButton size="sm" variant="secondary" asChild>
                  <a href="/login">
                    <User className="h-4 w-4 mr-1" />{" "}
                    {tr("commandPalette.login", "Login")}
                  </a>
                </EnhancedButton>
              )}
            </div>
