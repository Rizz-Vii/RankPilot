...
interface Summary { totalUnique: number; keys: Record<string, number>; }
      ...
      {visible && (
        <div className="mt-1 max-h-48 overflow-auto bg-black/80 p-2 rounded w-[260px] space-y-1">
          {Object.entries(summary.keys).map(([k,c]) => (
            <div key={k} className="truncate"><span className="text-emerald-300">{c}×</span> <span className="opacity-80">{k}</span></div>
          ))}
        </div>
      )}
    </div>
