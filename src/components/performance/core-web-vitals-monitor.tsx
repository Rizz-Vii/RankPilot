...
export function CoreWebVitalsMonitor() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      onCLS((metric: BasicMetric) => {
        update({ cls: metric.value });
      });
      onINP((metric: BasicMetric) => {
        update({ fid: metric.value });
      }); // INP used as FID proxy
      onFCP((metric: BasicMetric) => {
        update({ fcp: metric.value });
      });
      onLCP((metric: BasicMetric) => {
        update({ lcp: metric.value });
      });
      onTTFB((metric: BasicMetric) => {
        update({ ttfb: metric.value });
      });
    } catch (error) {
    ...
    <Card className={spacing.component.base}>
      ...
      <CardContent className={spacing.stack.base}>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <Badge
                    className={cn(typography.ui.caption, ratingColors.badge)}
                  >
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg">
          <p className={typography.ui.helper}>
            <strong>Performance Guidelines:</strong> LCP &lt; 2.5s, FID &lt;
            100ms, CLS &lt; 0.1, TTFB &lt; 800ms
          </p>
        </div>
      </CardContent>
    </Card>
...
export function CoreWebVitalsWidget() {
  ...
  const [rating, setRating] = useState<"good" | "needs-improvement" | "poor">(
    "good"
  );
  ...
  useEffect(() => {
    try {
      type BasicMetric = { value: number };
      let metrics: { cls: number; fid: number; lcp: number } = {
        cls: 0,
        fid: 0,
        lcp: 0,
      };
  ...
  return (
    <div
      className={cn("flex items-center gap-2 p-2 rounded-lg", ratingColors.bg)}
    >
