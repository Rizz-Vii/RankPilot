...
export const LatencySparkline: React.FC<LatencySparklineProps> = ({
  ...
  const polyPoints = pointList
    .map((p) => `${p.x.toFixed(2)},${p.y.toFixed(2)}`)
    .join(" ");

  const handleMoveImmediate = useCallback(
    (clientX: number) => {
    ...
    (clientX: number) => {
      ...
      debounceRef.current = window.setTimeout(() => {
        if (lastClientX.current !== null)
          handleMoveImmediate(lastClientX.current);
      }, 32); // ~30fps debounce
    },
    [handleMoveImmediate]
  );
  ...
  if (!pointList.length)
    return (
      <div className="text-[11px] text-muted-foreground">No latency data</div>
    );
  ...
  return (
    <div
      ...
      aria-label="Latency distribution sparkline"
      ...
      onKeyDown={(e) => {
        ...
        if (e.key === "ArrowRight") {
          setHoverIdx((i) => Math.min(pointList.length - 1, (i ?? 0) + 1));
        }
        if (e.key === "ArrowLeft") {
    ...
    >
      ...
      >
        <defs>
          ...
          >
            <stop
              offset="0%"
              stopColor="hsl(var(--emerald-500,140,70%,40%))"
              stopOpacity="0.15"
            />
            <stop
              offset="60%"
              stopColor="hsl(var(--amber-500,40,90%,45%))"
              stopOpacity="0.12"
            />
            <stop
              offset="100%"
              stopColor="hsl(var(--rose-500,350,90%,55%))"
              stopOpacity="0.18"
            />
          </linearGradient>
        </defs>
      ...
      {active && (
        <div
          className="absolute -top-1 left-0 translate-x-1/2 pointer-events-none"
          style={{ left: `${active.x}%` }}
        >
