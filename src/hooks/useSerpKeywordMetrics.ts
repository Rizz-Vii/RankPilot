...
interface SerpResultItem { position?: number; title?: string; snippet?: string; }
interface SerpDoc { id: string; results?: SerpResultItem[]; createdAt?: { toDate?: () => Date } | Date; }
interface Result { loading: boolean; error?: string; kpis: Metric[]; rows: SerpDoc[]; }
    ...
    useEffect(() => {
        if (!user) return; let cancelled = false; (async () => {
            try {
                ...
                const featuredCount = docs.filter(d => (d.results || []).some(r => r.position === 1 && /result 1/i.test(r.title || ''))).length;
                const paaCount = docs.filter(d => (d.results || []).slice(0, 5).some(r => /snippet/i.test(r.snippet || ''))).length;
                const top3Presence = docs.filter(d => (d.results || []).some(r => (r.position ?? 99) <= 3)).length / docs.length * 100;
                const kpis: Metric[] = [
