...
interface Result { loading: boolean; error?: string; kpis: Metric[]; rows: unknown[]; }
    ...
    useEffect(() => {
        if (!user) return; let cancelled = false; (async () => {
            try {
                ...
                const byPeriod: Record<string, unknown[]> = {}; for (const d of filtered) { (byPeriod[d.period] ||= []).push(d); } const ordered = Object.keys(byPeriod).sort();
            ...
            } catch (e: unknown) { if (cancelled) return; const msg = e instanceof Error ? e.message : 'Unknown error'; setState({ loading: false, kpis: [], rows: [], error: msg }); }
