...
interface InvoiceDoc {
  ...
  status?: string;
  amount?: number;
  paidAt?: DateLike | Date | null;
  dueAt?: DateLike | Date | null;
}
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
          ...
          return {
            ...
            amount:
              typeof raw.amount === "number"
                ...
                : Number(raw.amount ?? 0),
            paidAt:
              (paidAtRaw && typeof paidAtRaw === "object") ||
              paidAtRaw instanceof Date
                ? (paidAtRaw as DateLike | Date)
                : null,
            dueAt:
              (dueAtRaw && typeof dueAtRaw === "object") ||
              dueAtRaw instanceof Date
                ? (dueAtRaw as DateLike | Date)
                : null,
          };
        });
        if (!docs.length) {
        ...
        const sum = (arr: InvoiceDoc[], f: (x: InvoiceDoc) => number): number =>
        ...
        const lastPaid: InvoiceDoc[] = byPeriod[last].filter(
          (i) => i.status === "paid"
        );
        ...
        const onTimePaid = lastPaid.filter((i) => {
          const paidAt = toDate(i.paidAt);
          const due = toDate(i.dueAt);
          return paidAt && due && paidAt.getTime() <= due.getTime();
        });
        const onTimePct = lastPaid.length
          ? (onTimePaid.length / lastPaid.length) * 100
          : 0;
        const outstanding = byPeriod[last].filter(
          (i) => i.status !== "paid"
        ).length;
        const kpis: Metric[] = [
          {
          ...
          },
          {
            key: "on_time",
            label: "On-Time %",
            value: Number(onTimePct.toFixed(1)),
            delta: 0,
            trend: ordered.map((p) => {
              const arr = (byPeriod[p] || []).filter(
                (i) => i.status === "paid"
              );
            ...
            intent: onTimePct > 90 ? "success" : "warning",
          },
          {
            key: "outstanding",
            label: "Outstanding",
            ...
            trend: ordered.map(
              (p) =>
                (byPeriod[p] || []).filter((i) => i.status !== "paid").length
            ),
            intent: outstanding === 0 ? "success" : "warning",
          },
        ];
        const rows: Array<Record<string, unknown>> = filtered
          .slice(0, 200)
          .map((f) => ({ ...f }));
        setState({ loading: false, kpis, rows });
      } catch (e: unknown) {
