...
export default function FeaturesPageClient() {
    ...
    const baseDelay = prefersReducedMotion || isMobile ? 0 : 0.1;
        ...
        <div className="min-h-[100dvh] sm:min-h-screen bg-background">
            ...
            <section className="py-24 px-4">
                <div className="max-w-4xl mx-auto text-center">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={0}>
                        <Badge className="mb-4" variant="secondary">
                            NeuroSEO™ Suite
                        </Badge>
                        <h1 className="text-5xl font-bold mb-6 bg-gradient-to-r from-primary to-primary/70 bg-clip-text text-transparent">
                            All‑in‑One NeuroSEO™ Platform
                        </h1>
            ...
            <section className="py-16 px-4">
                <div className="max-w-7xl mx-auto">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={1} className="text-center mb-12">
                        <h2 className="text-3xl font-bold mb-4 flex items-center justify-center gap-2">
                            <Brain className="h-8 w-8 text-primary" />
                            NeuroSEO™ Suite
                        </h2>
                        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
                            Six specialized, interoperable engines orchestrated for precision prioritization and measurable impact.
                        </p>
                    </motion.div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {neuroSEOFeatures.map((feature, index) => (
                            <motion.div key={feature.title} initial="hidden" animate="visible" variants={_fadeIn} custom={index + 2}>
                                <Card className="h-full hover:shadow-lg transition-all duration-300 group">
                                    <CardHeader>
                                        <div className="flex items-start justify-between">
                                            <feature.icon className="h-8 w-8 text-primary group-hover:scale-110 transition-transform" />
                                            {feature.badge && <Badge variant="secondary" className="text-xs">{feature.badge}</Badge>}
                                        </div>
            ...
            <section className="py-16 px-4 bg-muted/30">
                <div className="max-w-7xl mx-auto">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={8} className="text-center mb-12">
                        <h2 className="text-3xl font-bold mb-4">Core SEO Tools</h2>
                        <p className="text-lg text-muted-foreground">Essential SEO optimization and analysis tools</p>
                    </motion.div>

                    <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {seoTools.map((tool, index) => (
                            <motion.div key={tool.title} initial="hidden" animate="visible" variants={_fadeIn} custom={index + 9}>
            ...
            <section className="py-16 px-4">
                <div className="max-w-7xl mx-auto">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={13} className="text-center mb-12">
                        <h2 className="text-3xl font-bold mb-4">Competitive Intelligence</h2>
                        <p className="text-lg text-muted-foreground">Stay ahead with comprehensive competitor analysis</p>
                    </motion.div>

                    <div className="grid md:grid-cols-3 gap-6">
                        {competitiveIntelligence.map((feature, index) => (
                            <motion.div key={feature.title} initial="hidden" animate="visible" variants={_fadeIn} custom={index + 14}>
                                <Card className="h-full">
                                    <CardHeader>
                                        <div className="flex items-start justify-between">
                                            <feature.icon className="h-6 w-6 text-primary" />
                                            {feature.badge && <Badge variant="outline" className="text-xs">{feature.badge}</Badge>}
                                        </div>
            ...
            <section className="py-16 px-4 bg-muted/30">
                <div className="max-w-7xl mx-auto">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={17} className="text-center mb-12">
                        <h2 className="text-3xl font-bold mb-4">Management & Analytics</h2>
                        <p className="text-lg text-muted-foreground">Advanced reporting and team collaboration features</p>
                    </motion.div>

                    <div className="grid md:grid-cols-2 gap-8">
                        {managementFeatures.map((feature, index) => (
                            <motion.div key={feature.title} initial="hidden" animate="visible" variants={_fadeIn} custom={index + 18}>
                                <Card className="h-full">
                                    <CardHeader>
                                        <div className="flex items-start justify-between">
            ...
            <section className="py-24 px-4">
                <div className="max-w-4xl mx-auto text-center">
                    <motion.div initial="hidden" animate="visible" variants={_fadeIn} custom={20}>
                        <h2 className="text-4xl font-bold mb-6">Ready to Transform Your SEO Strategy?</h2>
