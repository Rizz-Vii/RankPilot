...
import { Check, CreditCard, Crown, Rocket, TrendingUp } from "lucide-react";
                    ...
                    <CardContent className="space-y-6">
                      ...
                      >
                        {subscription?.tier === plan.id ? (
                          ...
                          <Link
                            href={`/checkout?plan=${plan.id}&billingInterval=${isYearly ? "yearly" : "monthly"}`}
                          >
                            {"Upgrade to " + plan.name}
                          </Link>
                        ) : (
                          <Link
                            href={`/register?plan=${plan.id}&billingInterval=${isYearly ? "yearly" : "monthly"}`}
                          >
                      ...
                      {subscription?.tier === plan.id && !isYearly && (
                        <div className="mt-3 rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-sm text-warning-foreground">
                          Save ${plan.price * 12 - plan.yearlyPrice} per year by
                          switching to annual billing.
                          <div className="mt-2">
                            <Button asChild size="sm" variant="outline">
                              {user ? (
                                <Link
                                  href={`/checkout?plan=${plan.id}&billingInterval=yearly`}
                                >
                                  Switch to Annual
                                </Link>
                              ) : (
                                <Link
                                  href={`/register?plan=${plan.id}&billingInterval=yearly`}
                                >
                                  Switch to Annual
                                </Link>
                              )}
