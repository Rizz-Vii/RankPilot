...
export default async function RootLayout({
  ...
  if (bodyExtra.includes("colorblind-support"))
    flags.push("colorblind-support");
  const addDarkClass =
    (ssrThemeMode === "dark" && !ssrHighContrast) ||
  ...
  if (process.env.NODE_ENV === "development") {
    try {
      ...
      const AgentsToggleComp = (mod && mod.AgentsToggle) as
        | React.ComponentType
        | undefined;
      if (AgentsToggleComp) {
        agentsToggleElement = (
  ...
  const headerNonce =
    hdrs.get("x-nextjs-csp-nonce") ??
    hdrs.get("x-nonce") ??
    hdrs.get("x-rp-csp-nonce") ??
    undefined;
  const cspNonce =
    process.env.NODE_ENV === "production" ? headerNonce : undefined;
  return (
    <html
      lang={htmlLang}
      dir={htmlDir}
      suppressHydrationWarning
      className={`h-full ${ptSans.variable} ${spaceGrotesk.variable}`}
    >
      <head>{/* Head content only; font files handled by next/font */}</head>
