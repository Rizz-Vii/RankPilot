...
export async function GET(_: Request, ctx: RouteContext): Promise<Response> {
  ...
  const id =
    (Array.isArray(rawId) ? rawId[0] : rawId)?.replace(/\.jsonld$/i, "") || "";
        ...
        additionalProperty: [
          {
            "@type": "PropertyValue",
            name: "overallScore",
            value: (typeof data?.overallScore === "number"
              ? data?.overallScore
              : 0) as number,
          },
          {
            "@type": "PropertyValue",
            name: "provenance",
            value: (typeof data?.__provenance === "string"
              ? data?.__provenance
              : "live") as string,
          },
          {
            ...
            name: "topKeywords",
            value: Array.isArray(
              (data as { topKeywords?: unknown })?.topKeywords
            )
              ? (
                  (data as { topKeywords?: Array<{ keyword?: unknown }> })
                    .topKeywords || []
                )
                  .map((k) =>
                    k &&
