  ...
  async function GET(req: NextRequest) {
    ...
    try {
      ...
      const limit = Math.max(
        1,
        Math.min(20, Number.isFinite(limitRaw) ? limitRaw : 5)
      );
      // Optional hard cap on response size (number of pages returned)
      const respMaxRaw = Number(
        urlObj.searchParams.get("max") ||
          process.env.FIRECRAWL_RESPONSE_MAX ||
          "50"
      );
      const respMax = Math.max(
        1,
        Math.min(200, Number.isFinite(respMaxRaw) ? respMaxRaw : 50)
      );
      if (!target) {
        return NextResponse.json(
          enforceProvenance(
            { success: false, error: "url_required", provenance: "synthetic" },
            { path: "seo-audit/firecrawl", note: "validation" }
          ),
          { status: 400 }
        );
      }
      ...
      try {
        ...
        return NextResponse.json(
          enforceProvenance(
            { success: false, error: "malformed_url", provenance: "synthetic" },
            { path: "seo-audit/firecrawl", note: "validation" }
          ),
          { status: 400 }
        );
      ...
      if (!quota.allowed) {
        ...
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "seo-audit/firecrawl", note: "rate_limit" }
          ),
          {
            status: 429,
            headers: { "Retry-After": String(quota.retryAfterSeconds || 60) },
          }
        );
      }
      ...
      if (depth > 1) {
        try {
            ...
            if (
              ...
              return NextResponse.json(
                enforceProvenance(
                  ...
                  { path: "seo-audit/firecrawl", note: "robots" }
                ),
                { status: 403 }
              );
            }
          ...
          /* ignore robots failures per degradation policy */
        }
      }
      ...
      recordRouteLatency("seo-audit/firecrawl", elapsedMs);
      if (crawl.fallback) {
        recordCrawlerError(crawlTimeMs);
      } else {
        recordCrawlerSuccess(crawlTimeMs, 0);
      }
      ...
      const pages =
        totalPages > respMax ? crawl.pages.slice(0, respMax) : crawl.pages;
      const truncated = totalPages > respMax;
      const res = NextResponse.json(
        enforceProvenance(
          {
            success: true,
            provenance: crawl.fallback ? "synthetic" : "live",
            data: {
              pages,
              total: totalPages,
              returned: pages.length,
              truncated,
              fallback: !!crawl.fallback,
              degradedReason: crawl.degradedReason,
              timings: {
                quota_time_ms: quotaTimeMs,
      ...
      return NextResponse.json(
        enforceProvenance(
          ...
          { path: "seo-audit/firecrawl", note: "exception" }
        ),
        { status: 500 }
      );
    }
