...
const REGION = process.env.FIREBASE_REGION || process.env.NEXT_PUBLIC_FUNCTIONS_REGION || 'us-central1';
...
export const POST = withProvenance(async function (req: Request) {
    try {
        ...
        if (body && typeof body === 'object' && 'teamId' in body && typeof (body as Record<string, unknown>).teamId === 'string') {
            const teamId = (body as Record<string, unknown>).teamId as string;
            try {
                const res = await applyTeamRateLimit(teamId);
                if (res && res.allowed) {
                    (req as unknown as { _teamRateHeaders?: Record<string, string> })._teamRateHeaders = { ...res.headers, 'X-RateLimit-Policy': 'bucket' };
        ...
        const rateHeaders = (req as unknown as { _teamRateHeaders?: Record<string, string> })._teamRateHeaders;
        if (rateHeaders) {
        ...
        return NextResponse.json(enforceProvenance({ success: false, error: msg, provenance: 'synthetic' }, { path: 'seo-audit/run', note: 'exception' }), { status: 500 });
    }
}, { path: 'seo-audit/run' });
