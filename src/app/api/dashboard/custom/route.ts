...
import {
  sanitizeWidgetCreate,
  sanitizeWidgetMutation,
  type WidgetCreateInput,
  type WidgetMutation,
} from "@/types/dashboard-widget-mutation";

type WidgetPosition = { x: number; y: number; width: number; height: number };
interface WidgetUpdateInput extends WidgetMutation {
  widgetId: string;
}
    ...
    try {
      ...
      if (!authHeader?.startsWith("Bearer ")) {
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "dashboard/custom", note: "auth" }
          ),
          { status: 401 }
        );
      }
      ...
      if (!["agency", "enterprise", "admin"].includes(userTier)) {
        return NextResponse.json(
          enforceProvenance(
            {
              error:
                "Upgrade required - Custom dashboards available for Agency tier and above",
            },
            { path: "dashboard/custom", note: "tier" }
          ),
          { status: 403 }
        );
      }
        ...
        case "create":
          if (!body.name) {
            return NextResponse.json(
              enforceProvenance(
                ...
                { path: "dashboard/custom", note: "validation" }
              ),
              { status: 400 }
            );
          }
          ...
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "create" }
            )
          );
        case "update":
          if (!body.dashboardId) {
          ...
          if (body.widgetConfig && body.position) {
            ...
            const sanitized = sanitizeWidgetCreate(
              body.widgetConfig
            ) as WidgetCreateInput;
            ...
            return NextResponse.json(
              enforceProvenance(
                { success: true, widget, message: "Widget added successfully" },
                { path: "dashboard/custom", note: "add_widget" }
              )
            );
            ...
            return NextResponse.json(
              enforceProvenance(
                ...
                { path: "dashboard/custom", note: "update_widget" }
              )
            );
          }
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "invalid_update" }
            ),
            { status: 400 }
          );
        case "delete":
          ...
          if (body.updates?.widgetId) {
            ...
            return NextResponse.json(
              enforceProvenance(
                ...
                { path: "dashboard/custom", note: "remove_widget" }
              )
            );
          }
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "widget_id_required" }
            ),
            { status: 400 }
          );
        case "export":
          ...
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "export" }
            )
          );
        case "share":
          ...
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "share" }
            )
          );
        case "duplicate":
          ...
          const duplicatedDashboard =
            await customDashboardBuilder.duplicateDashboard(
              body.dashboardId,
              body.name,
              user.uid
            );
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "duplicate" }
            )
          );
        default:
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "invalid_action" }
            ),
            { status: 400 }
          );
      }
      ...
      return NextResponse.json(
        enforceProvenance(
          ...
          { path: "dashboard/custom", note: "exception" }
        ),
        { status: 500 }
      );
    }
    ...
    try {
      ...
      switch (action) {
        case "templates":
          ...
          return NextResponse.json(
            enforceProvenance(
              { success: true, templates, userTier },
              { path: "dashboard/custom", note: "templates" }
            )
          );
        case "dashboards":
          ...
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "dashboards" }
            )
          );
        case "widget-data":
          ...
          const userDashboards = customDashboardBuilder.getUserDashboards(
            user.uid
          );
          ...
          if (!dashboard) {
            return NextResponse.json(
              enforceProvenance(
                ...
                { path: "dashboard/custom", note: "not_found" }
              ),
              { status: 404 }
            );
          }
          ...
          const widgetData = await customDashboardBuilder.getWidgetData(
            widgetId,
            widget
          );
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "dashboard/custom", note: "widget_data" }
            )
          );
        default:
          return NextResponse.json(
