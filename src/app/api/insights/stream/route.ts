  ...
  try {
    ...
    return snap.docs.map((d) => {
      ...
      return {
        ...
        resultsSummary:
          typeof data?.["resultsSummary"] === "string"
            ? (data?.["resultsSummary"] as string)
            : undefined,
      };
    });
  } catch {
  ...
  try {
    ...
    const onClient: (client: SSEClient) => void = (client) => {
      const send = (obj: Record<string, unknown>, note?: string) => {
        const payload = enforceProvenanceOnChunk(obj, {
          path: "insights/stream",
          note,
        });
        client.send(payload);
      };
      ...
      void (async () => {
        ...
        try {
          ...
          send({
            type: "error",
            message: msg || "insight_generation_failed",
