    ...
    try {
      ...
      if (!authHeader) {
        const res = NextResponse.json(
          enforceProvenance(
            { error: "auth_required" },
            { path: "billing/invoices", note: "auth" }
          ),
          { status: 401 }
        );
      ...
      if (rawCursor) {
        if (rawCursor.includes("|")) {
          ...
          const createdAtDate = isNaN(createdAtMs)
            ? new Date(0)
      ...
      docs = docs.map((invRaw) => {
        ...
        const createdAt: Date =
          ...
          toDate(inv.issuedAt) ||
          new Date(String(inv.date || `${inv.period}-01T00:00:00Z`));
        return {
          ...inv,
          description:
            (inv.description as string) || `Invoice ${String(inv.period)}`,
      ...
      const nextCursor =
        hasMore && last
          ? `${String(last.period)}|${(last.createdAt as Date).getTime()}`
          : undefined;
      logger.debug("billing-invoices.page", {
        uid,
        count: docs.length,
        hasMore,
        nextCursor,
      ...
      const res = NextResponse.json(
        enforceProvenance(
          { invoices: docs, hasMore, nextCursor },
          { path: "billing/invoices", note: "page" }
        )
      );
      res.headers.set(
        "x-billing-diagnostics",
        `auth=ok; items=${docs.length}; hasMore=${hasMore}`
      );
      return res;
    } catch (e: unknown) {
      const msg =
      ...
      const res = NextResponse.json(
        enforceProvenance(
          ...
          { path: "billing/invoices", note: "exception" }
        ),
        { status: 500 }
      );
