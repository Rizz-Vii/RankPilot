...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const limit =
      Number.isFinite(limitRaw) && limitRaw > 0 ? Math.min(limitRaw, 200) : 50; // hard-cap
    const maxPreviewBytes =
      Number.isFinite(maxPreviewRaw) && maxPreviewRaw >= 0
        ? Math.min(maxPreviewRaw, 4096)
        : 1024; // guard
    ...
    const safe = items.map((it) => {
      ...
      const payloadPreview =
        includePayload && maxPreviewBytes > 0
          ? payloadStr.length > maxPreviewBytes
            ? `${payloadStr.slice(0, maxPreviewBytes)}…`
            : payloadStr
          : undefined;
      return {
        type: it.type,
        ...
        error: { message: msg },
        payloadBytes,
        ...(includePayload && payloadPreview !== undefined
          ? { payloadPreview }
          : {}),
      };
    });
