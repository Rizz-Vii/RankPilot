...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (!probe || probe !== (process.env.CRAWL_PROBE_TOKEN || process.env.NEXT_PUBLIC_CRAWL_PROBE_TOKEN)) {
        ...
        function buildTwiml(opts: { sayText: string; voice: string; language: string; rate: number; recordingUrl?: string | null; interactive?: boolean }) {
            ...
            if (recordingUrl) return `<?xml version="1.0" encoding="UTF-8"?>\n<Response>\n  <Play>${recordingUrl}</Play>\n</Response>`;
            if (interactive) {
        ...
        for (const doc of snap.docs) {
            const data = (doc.data() || {}) as {
                to?: string; from?: string | null; schedule?: unknown;
                config?: { voice?: string; language?: string; rate?: number; recordingUrl?: string | null; interactive?: boolean; script?: string | null } | null;
            };
            const to = (data?.to || '').toString();
            const from = (data?.from || defaultFrom || '').toString();
            if (!to) { await doc.ref.update({ status: 'skipped', reason: 'missing_to' }); continue; }
            ...
            await doc.ref.update({ status: 'processed', callSid: callSid || null, processedAt: new Date().toISOString() });
            processed++;
        }
