    ...
    try {
        ...
        if (!query || query.trim().length < 3) {
            return NextResponse.json(
                enforceProvenance({ results: [] } as SearchOutput, { path: 'search/features', note: 'empty-or-short' })
            );
        }
        const out = await searchFeatures({ query });
        return NextResponse.json(enforceProvenance(out as unknown as SearchOutput, { path: 'search/features' }));
    } catch (e) {
        const message = (e && typeof e === 'object' && 'message' in e && typeof (e as { message?: unknown }).message === 'string')
            ? (e as { message: string }).message
            : 'internal error';
        return NextResponse.json(
            enforceProvenance({ error: 'internal', message } as unknown as SearchOutput, { path: 'search/features', note: 'error' }),
            { status: 500 }
        );
    }
}
