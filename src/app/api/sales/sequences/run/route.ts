...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    for (const t of toCall) {
      ...
      try {
        const resp = await fetch(
          `${process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"}/api/telephony/call`,
          {
            method: "POST",
    ...
    await execRef.set(
      {
        results,
        stats: { attempted, succeeded, failed },
        completedAt: new Date(),
      },
      { merge: true }
    );
