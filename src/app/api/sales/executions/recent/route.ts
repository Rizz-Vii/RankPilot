...
function ok(data: unknown, init?: number) {
  return NextResponse.json<unknown>(data, { status: init ?? 200 });
}
  ...
  try {
    ...
    if (probe && probe === process.env.CRAWL_PROBE_TOKEN) {
      try {
        ...
        logger.degraded("probe_collection_group_failed", {
    ...
    if (!auth) {
      ...
      if (process.env.NODE_ENV !== "production") {
        logger.degraded("dev_unauth_recent_executions", {
          note: "returning empty list in dev without auth",
        });
