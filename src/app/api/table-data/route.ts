...
function normalizeRow(docData: RawRow): TableRow {
  ...
  const metric = String(
    docData.metric ?? docData.name ?? docData.label ?? "Metric"
  );

  // Value can be number or string; support valueNum as canonical numeric field
  let valueOut: string;
  if (typeof docData.valueNum === "number")
    valueOut = formatCurrency(docData.valueNum);
  else if (typeof docData.value === "number")
    valueOut = formatCurrency(docData.value);
  ...
  if (typeof docData.changeNum === "number")
    changeOut = formatPercent(docData.changeNum);
  else if (typeof docData.change === "number")
    changeOut = formatPercent(docData.change);
...
async function fetchFromFirestore(opts: {
  ...
  try {
    const { widgetId, sortBy, direction, page, pageSize, all, teamId, userId } =
      opts;
    ...
    try {
      ...
      const maybeCountFn: unknown = (
        base as unknown as Record<string, unknown>
      )["count"]; // Firestore aggregate query extension
      if (typeof maybeCountFn === "function") {
        ...
        const dataFn =
          typeof aggSnap?.data === "function" ? aggSnap.data() : {};
...
export async function GET(req: NextRequest): Promise<NextResponse> {
  try {
        ...
        const mappedField =
          field === "valuenum"
            ...
            : field === "changenum"
              ? "change"
              : field;
        sb = ["metric", "value", "change"].includes(mappedField)
          ? mappedField
          : "metric";
    ...
    let result = await fetchFromFirestore({
      widgetId,
      sortBy,
      direction,
      page,
    ...
    if (!result || result.rows.length === 0) {
      ...
      if (teamId || userId) {
        ...
        total = Math.min(
          2000,
          Math.max(50, parseInt(searchParams.get("total") || "500", 10) || 500)
        );
        ...
        const sorted = rowsAll.sort((a, b) => {
          ...
          if (sortBy === "value")
            return (parseNumeric(a.value) - parseNumeric(b.value)) * dir;
    ...
    if (format === "csv") {
      ...
      const body = outRows
        .map(
          (r) =>
            `${JSON.stringify(r.metric)},${JSON.stringify(r.value)},${JSON.stringify(r.change)}`
        )
        .join("\n");
      const csv = `${header}\n${body}`;
      ...
      res.headers.set(
        "x-provenance",
        teamId || userId ? "live_or_empty" : result ? "live" : "synthetic"
      );
      return res;
    }
    ...
    return NextResponse.json(
      enforceProvenance(
        { error: message, provenance: "synthetic" },
        { path: "table-data", note: "exception" }
      ),
      { status: 500 }
    );
  }
