...
export async function POST(req: NextRequest) {
    try {
        ...
        const ct = (form.get('contentType') as string) || file.type || 'application/octet-stream';
        ...
        await gcsFile.save(buffer, { contentType: ct, resumable: false, metadata: { contentType: ct } });

        // Generate a signed URL for direct playback; fallback to gs:// URL on error
        let url = `gs://${bucket.name}/${key}`;
        try {
            const [signed] = await gcsFile.getSignedUrl({ action: 'read', expires: Date.now() + 7 * 24 * 60 * 60 * 1000 });
            url = signed;
        } catch { /* non-fatal */ }
