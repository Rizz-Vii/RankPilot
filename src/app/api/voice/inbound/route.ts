...
export async function POST(req: NextRequest) {
    try {
        ...
        logger.info('voice.inbound.received', { probe: !!probe, event: body?.event || 'unknown' });
        ...
        if (action) {
            try {
                ...
                if (action === 'hold') {
                    ...
                    const ttlMs = typeof duration === 'number' ? duration * 1000 : 5 * 60 * 1000;
                    const hold = await tools.holdSlot({ slotId, ttlMs });
                    return NextResponse.json({ ok: !!hold?.ok, hold }, { status: hold?.ok ? 200 : 400 });
                }
                ...
                if (action === 'create') {
                    ...
                    if (!payload || (!payload.start && !payload.serviceId)) {
                        return NextResponse.json({ ok: false, error: 'invalid_payload', reason: 'missing start or serviceId' }, { status: 400 });
                    }

                    // Merge top-level customer fields if provided
                    if (body?.customer) payload.customer = { ...(payload.customer || {}), ...(body.customer || {}) };

                    const appt = await tools.createAppointment({ ...(payload || {}), holdId });
                    if (appt?.ok) {
                        // return appointment id and basic metadata
                        return NextResponse.json({ ok: true, apptId: appt.apptId, createdAt: new Date().toISOString() }, { status: 200 });
                    }
                    return NextResponse.json({ ok: false, error: appt?.error || 'failed' }, { status: 400 });
                }
            } catch (e) {
        ...
        logger.error('voice.inbound.error', { error: err instanceof Error ? err.message : String(err) });
