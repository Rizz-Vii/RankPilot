...
type Logger = {
  ...
  degraded: (m: string, o?: unknown) => void;
  audit: (m: string, o?: unknown) => void;
};
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const serviceId = String(
      (body as { serviceId?: unknown })?.serviceId || "voice-demo"
    );
    // Voice/tone options
    const sayVoice = String(
      (body as { voice?: unknown })?.voice ||
        process.env.TWILIO_SAY_VOICE ||
        "alice"
    ); // alice|man|woman|Polly.X
    const sayLanguage = String(
      (body as { language?: unknown })?.language ||
        process.env.TWILIO_SAY_LANGUAGE ||
        "en-US"
    );
    const sayRate = Number(
      (body as { rate?: unknown })?.rate || process.env.TWILIO_SAY_RATE || 1
    ); // 0.5-2.0
    // Optional: use a pre-recorded voice file URL instead of synthesized speech
    const recordingUrl = String(
      (body as { recordingUrl?: unknown })?.recordingUrl || ""
    ).trim();
    // Interactive dialog (beta)
    const interactive = Boolean(
      (body as { interactive?: unknown })?.interactive
    );
    // Optional: recurrence metadata (UI-level scheduling hint)
    const repeatRaw = String((body as { repeat?: unknown })?.repeat || "")
      .trim()
      .toLowerCase();
    const repeat =
      repeatRaw === "daily" || repeatRaw === "weekly" ? repeatRaw : "";
    // From number override (validated against allowed list if provided)
    const fromOverride = String(
    ...
    if (!schedule)
      return NextResponse.json(
        { ok: false, error: "missing_schedule" },
        { status: 400 }
      );
    ...
    const fromNumber = fromOverride
      ? allowedFrom.length
        ? allowedFrom.includes(fromOverride)
          ? fromOverride
          : null
        : fromOverride
      : defaultFrom;
    ...
    if (future && adminDb) {
      ...
      if (twilio && fromNumber) {
        try {
          ...
          const prosodyOpen =
            sayRate && sayRate !== 1
              ? `<prosody rate="${Math.round(sayRate * 100)}%">`
              : "";
          ...
          for (const to of recipients) {
            ...
            try {
              ...
              logger!.degraded("twilio.call.record_failed", {
                error: String(e),
              });
            }
          }
        ...
        logger!.degraded("twilio.not_configured", {
          hasClient: !!twilio,
          hasFrom: !!fromNumber,
        });
      }
    ...
    const res = await (
      tools as {
        createAppointment: (
          p: typeof payload
        ) => Promise<{ ok?: boolean; apptId?: string; error?: string }>;
      }
