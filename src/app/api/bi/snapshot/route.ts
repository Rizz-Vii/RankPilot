...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    let advisory:
      | {
          forecasts: {
            rateLimitNext: number | null;
            queueDepthNext: number | null;
            queueSuccessPctNext: number | null;
            fallbacksNext: number | null;
            route?: { p95Next: number | null; p99Next: number | null };
          };
          planner?: ReturnType<typeof computePlannerHints>;
        }
      | undefined;
    if (advisoryFlag) {
      const series = getTimeSeries(20, route || undefined);
      const base = series as BaseTimeSeries;
      const rateLimitSeries = Array.isArray(base.rateLimit)
        ? base.rateLimit
        : [];
      const queueDepthSeries = Array.isArray(base.queueDepth)
        ? base.queueDepth
        : [];
      const queueSuccessSeries = Array.isArray(base.queueSuccessPct)
        ? base.queueSuccessPct
        : [];
      const fallbacksSeries = Array.isArray(base.fallbacks)
        ? base.fallbacks
        : [];
      const rateLimitNext = forecastNext({ samples: rateLimitSeries }).forecast;
      const queueDepthNext = forecastNext({
        samples: queueDepthSeries,
      }).forecast;
      const queueSuccessPctNext = forecastNext({
        samples: queueSuccessSeries,
      }).forecast;
      const fallbacksNext = forecastNext({ samples: fallbacksSeries }).forecast;
      let routeNode:
        | { p95Next: number | null; p99Next: number | null }
        | undefined;
      if (route) {
        const withRoute = series as RouteTimeSeries | BaseTimeSeries;
        const p95Series =
          "p95" in withRoute &&
          Array.isArray((withRoute as RouteTimeSeries).p95)
            ? (withRoute as RouteTimeSeries).p95
            : [];
        const p99Series =
          "p99" in withRoute &&
          Array.isArray((withRoute as RouteTimeSeries).p99)
            ? (withRoute as RouteTimeSeries).p99
            : [];
    ...
    const body = enforceProvenance(
      {
        ...
        hints: {
          ...
          rateLimitRejections: Object.values(
            unified.rateLimitRejections
