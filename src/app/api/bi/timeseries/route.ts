import { ensureSamplerStarted, getTimeSeries, registerRouteForSampling } from '@/lib/metrics/time-series';
...
export async function GET(req: NextRequest) {
    ...
    const points = Math.max(1, Math.min(144, Number(url.searchParams.get('n') || '0') || 0));
    const snap = getTimeSeries(points || undefined, route);
    const hasRoute = !!route && 'p95' in snap && 'p99' in snap;
    const header = hasRoute
        ? ['ts', 'fallbacks', 'rateLimit', 'queueDepth', 'queueSuccessPct', 'p95', 'p99']
        : ['ts', 'fallbacks', 'rateLimit', 'queueDepth', 'queueSuccessPct'];
    const rows: string[] = [];
    rows.push(toCsvLine(header));
    const len = snap.ts.length;
    for (let i = 0; i < len; i++) {
        const base = [snap.ts[i], snap.fallbacks[i] ?? '', snap.rateLimit[i] ?? '', snap.queueDepth[i] ?? '', snap.queueSuccessPct[i] ?? ''];
        if (hasRoute) {
