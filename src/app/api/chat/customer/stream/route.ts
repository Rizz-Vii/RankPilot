...
import {
  buildQueryEmbedding,
  maybeClusterKeywords,
  retrieveSimilarMessages,
} from "@/lib/chat/retrievalAndClustering";
...
async function checkAndIncrementMessageQuota(uid: string, tier: string) {
  ...
  await adminDb.runTransaction(async (tx) => {
    ...
    const data = snap.exists
      ? (snap.data() as UsageCounter)
      : { messages: 0, tokens: 0, date: dateKey };
...
async function incrementTokenUsage(uid: string, tier: string, tokens: number) {
  ...
  await adminDb.runTransaction(async (tx) => {
    ...
    const data = snap.exists
      ? (snap.data() as UsageCounter)
      : { messages: 0, tokens: 0, tier, date: dateKey };
    if ((data.tokens || 0) + tokens > limits.tokens) {
      ...
      if (remaining > 0) {
        tx.set(
          docRef,
          {
            tokens: FieldValue.increment(remaining),
            updatedAt: FieldValue.serverTimestamp(),
          },
          { merge: true }
        );
...
export async function POST(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const { message, sessionId, url, teamId } = body as {
      message?: string;
      sessionId?: string;
      url?: string;
      teamId?: string;
    ...
    if (!message?.trim()) {
      ...
      return NextResponse.json(
        enforceProvenance(
          ...
          { path: "chat/customer/stream", note: "validation" }
        ),
        { status: 400 }
      );
    }
    ...
    try {
      ...
      if (sessDoc.exists) {
        const sData = sessDoc.data() as
          ...
          | undefined;
        if (sData?.sessionSummary)
          memoryBlock += `\nConversation Memory Summary:\n${sData.sessionSummary}`;
        if (
          Array.isArray(sData?.pendingActions) &&
          sData!.pendingActions!.length
        )
          memoryBlock += `\nPending Actions:\n- ${sData!.pendingActions!.slice(0, 8).join("\n- ")}`;
        if (Array.isArray(sData?.keywords) && sData!.keywords!.length)
    ...
    try {
      ...
      return NextResponse.json(
        enforceProvenance(
          { error: msg },
          { path: "chat/customer/stream", note: "quota" }
        ),
        { status: 429 }
      );
    }
    ...
    if (process.env.RANKPILOT_ENABLE_EMBEDDINGS === "1") {
      try {
        ...
        if (qEmb) {
          const similar = await retrieveSimilarMessages({
            uid,
          ...
          if (similar.length) {
            retrievalBlock =
              ...
              similar
                .map((s) => `Q: ${s.question}\nA: ${s.response.slice(0, 400)}`)
                .join("\n---\n");
          }
          try {
            const siteChunksResult = await retrieveSiteChunks({
              uid,
              queryEmbedding: qEmb,
              topK: 4,
            });
            ...
            if (siteChunks.length) {
              siteBlock =
                ...
                siteChunks
                  .map(
                    (c: SiteChunk) =>
                      `• (${c.score.toFixed(2)}) ${c.meta?.title ? c.meta.title + " - " : ""}${c.content.slice(0, 260)}`
                  )
                  .join("\n");
            }
          } catch {
            /* ignore site retrieval errors */
          }
        }
      } catch {
    ...
    if (totalTokens > MAX_CONTEXT_TOKENS) {
      ...
      for (const seg of segments) {
        ...
        if (used + segTokens <= MAX_CONTEXT_TOKENS * 0.96) {
          ...
          if (remaining > 40) {
            const words = seg.value
              .split(/\s+/)
              .slice(0, Math.floor(remaining / 1.3));
            kept += words.join(" ");
          }
          break;
        }
    ...
    if (provider !== "openai") {
      const reason = openAICircuitOpen()
        ? "circuit_open"
        : openaiKey
          ? "openai_failure"
          : "no_openai_key";
      ...
      void (async () => {
        ...
        try {
          ...
          if (openAIStream) {
            ...
            fullResponse = await fallbackOneShot(
              finalSystemPrompt,
              message,
              800
            );
          ...
          try {
            ...
            try {
              ...
              await adminDb
                ...
                .doc(`${uid}_${dateKey}`)
                .set(
                  { [`providerCounts.${provider}`]: FieldValue.increment(1) },
                  { merge: true }
                );
            } catch {}
            void maybeSummarizeSession({
              uid,
              sessionId: currentSessionId,
              latestUserMessage: message,
              latestAIResponse: fullResponse,
            })
              .then((r) => {
                if (r.updated) {
                  client.send({ info: "summary_updated" });
                }
              })
              .catch(() => {});
            maybeEmbedMessage({
    ...
    return NextResponse.json(
      enforceProvenance(
        {
          error:
            e instanceof Error ? e.message : String(e ?? "Stream init failed"),
        },
        { path: "chat/customer/stream", note: "exception" }
      ),
      { status: 500 }
    );
  }
