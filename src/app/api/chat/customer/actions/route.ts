  ...
  try {
    ...
    if (!authHeader?.startsWith("Bearer ")) {
      return NextResponse.json(
        enforceProvenance(
          { error: "Auth required" },
          { path: "chat/customer/actions", note: "auth" }
        ),
        { status: 401 }
      );
    }
    ...
    if (!sessionId) {
      return NextResponse.json(
        enforceProvenance(
          ...
          { path: "chat/customer/actions", note: "validation" }
        ),
        { status: 400 }
      );
    }
      ...
      if (forbidden.includes(k)) {
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "chat/customer/actions", note: "forbidden-field" }
          ),
          { status: 400 }
        );
      }
    ...
    const sData: SessionData = sessionSnap.exists
      ? (sessionSnap.data() as SessionData)
      : {};
    const pendingActions: string[] = Array.isArray(sData.pendingActions)
      ? (sData.pendingActions as string[])
      : [];
    ...
    return NextResponse.json(
      enforceProvenance(
        {
          success: true,
          actionStats: { totalCompleted, totalPending, completionRate },
        },
        { path: "chat/customer/actions", note: "ok" }
      )
    );
  } catch (e: unknown) {
    ...
    return NextResponse.json(
      enforceProvenance(
        ...
        { path: "chat/customer/actions", note: "exception" }
      ),
      { status: 500 }
    );
  }
