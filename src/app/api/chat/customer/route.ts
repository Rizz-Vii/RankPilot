...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn("api.chat.customer.GET.auth.verify_failed", {
        message: msg,
      });
      return NextResponse.json(
        { error: "Unauthorized" },
    ...
    const sessSnap = await sessRef.get().catch((e: unknown) => {
      ...
      console.error("api.chat.customer.GET.firestore.session_get_error", {
        message: msg,
      });
      return null;
    });
    const sData =
      (sessSnap?.exists ? (sessSnap.data() as Record<string, unknown>) : {}) ||
      {};
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      console.error("api.chat.customer.GET.firestore.messages_query_error", {
        message: msg,
      });
      docs = [];
    }
    ...
    for (const d of docs.slice(0, limit)) {
      ...
      const ts =
        (data.timestamp as unknown as { toDate?: () => Date })?.toDate?.() ||
        new Date();
      if (data.question) {
        messages.push({
          id: `${d.id}_user`,
    ...
    return NextResponse.json(
      {
        ...
        hasMore,
        sessionSummary:
          typeof sData["sessionSummary"] === "string"
            ? sData["sessionSummary"]
            : undefined,
        pendingActions: Array.isArray(sData["pendingActions"])
          ? sData["pendingActions"]
          : undefined,
        keywords: Array.isArray(sData["keywords"])
          ? sData["keywords"]
          : undefined,
        actionProgress:
          typeof sData["actionProgress"] === "object"
            ? sData["actionProgress"]
            : undefined,
        actionStats:
          typeof sData["actionStats"] === "object"
            ? sData["actionStats"]
            : undefined,
      },
      { headers: baseHeaders }
    ...
    return NextResponse.json(
      { error: msg },
      { status: 500, headers: { "Cache-Control": "no-store, no-transform" } }
    );
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e ?? "auth_error");
      console.warn("api.chat.customer.POST.auth.verify_failed", {
        message: msg,
      });
      return NextResponse.json(
        { error: "Unauthorized" },
      ...
      try {
        await sessRef.collection("messages").add({
          ...
          metadata: {
            ...
            mediaUrl: attachments[0]?.mediaUrl,
          },
        });
        await sessRef.set(
          { lastActivity: FieldValue.serverTimestamp(), chatType: "customer" },
          { merge: true }
        );
      } catch (e: unknown) {
        const msg = e instanceof Error ? e.message : String(e);
        console.error(
          "api.chat.customer.POST.firestore.attachment_persist_error",
          { message: msg }
        );
    ...
    try {
      ...
      const msg = e instanceof Error ? e.message : String(e);
      console.error("api.chat.customer.POST.firestore.persist_error", {
        message: msg,
      });
