...
export const POST = withAdmin(
  async function POST(request: NextRequest, admin) {
    const nreq = request;
    ...
    try {
      ...
      try {
        ...
        const udata = userDoc.exists
          ? (userDoc.data() as Record<string, unknown>)
          : undefined;
        const teamId =
          typeof udata?.teamId === "string" ? udata.teamId : undefined;
        if (teamId) {
          try {
      ...
      try {
        ...
        return NextResponse.json(
          enforceProvenance(
            {
              error: "Admin chat service unreachable",
              provenance: "synthetic",
            },
            { path: "chat/admin" }
          ),
      ...
      if (!res.ok) {
        ...
        const errMsg =
          ((
            (p as Record<string, unknown>)["error"] as
          ...
          ((
            (
              (p as Record<string, unknown>)["result"] as
                | Record<string, unknown>
                | undefined
            )?.["error"] as Record<string, unknown> | undefined
          )?.["message"] as string | undefined) ||
          (rawText || "").slice(0, 200) ||
          "Admin chat service unavailable";
        const code =
          res.status === 401
            ? 401
            : res.status === 403
        ...
        const hint =
          code === 401
            ...
            : code === 403
              ...
              : "";
        if (code >= 500) recordError("chat/admin", "5xx_server");
        else if (code >= 400) recordError("chat/admin", "4xx_user");
        recordRouteLatency("chat/admin", Date.now() - start);
        return NextResponse.json(
          enforceProvenance(
            {
      ...
      const data: ChatResponse = ((container as Record<string, unknown>)
        ?.result ??
        (container as Record<string, unknown>)?.data ??
        payload) as unknown as ChatResponse;
      if (!data) {
        recordError("chat/admin", "5xx_server");
        recordRouteLatency("chat/admin", Date.now() - start);
        return NextResponse.json(
          enforceProvenance(
            {
              error: "Invalid response from admin chat service",
              provenance: "synthetic",
            },
            { path: "chat/admin" }
          ),
      ...
      if (
        ...
        switch (firebaseError.code) {
          case "unauthenticated":
            ...
            return NextResponse.json(
              enforceProvenance(
                { error: "Authentication required", provenance: "synthetic" },
                { path: "chat/admin" }
              ),
              { status: 401 }
            );
          case "permission-denied":
            ...
            return NextResponse.json(
              enforceProvenance(
                { error: "Admin access required", provenance: "synthetic" },
                { path: "chat/admin" }
              ),
              { status: 403 }
            );
          case "invalid-argument":
            ...
            return NextResponse.json(
              enforceProvenance(
                { error: "Invalid request data", provenance: "synthetic" },
                { path: "chat/admin" }
              ),
              { status: 400 }
            );
      ...
      return NextResponse.json(
        enforceProvenance(
          { error: "Internal server error", provenance: "synthetic" },
          { path: "chat/admin" }
        ),
        { status: 500 }
      );
...
export const GET = withAdmin(
  async function GET(request: NextRequest, admin) {
    const nreq = request;
    const start = Date.now();
    try {
