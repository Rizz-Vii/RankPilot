...
export const POST = withAdmin(
  async (req: NextRequest, admin) => {
    ...
    try {
      ...
      if (!message?.trim()) {
        ...
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "chat/admin/stream", note: "validation" }
          ),
          { status: 400 }
        );
      }
              ...
              return new Response(
                JSON.stringify(
                  enforceProvenance(
                    ...
                    { path: "chat/admin/stream", note: "rate-limit" }
                  )
                ),
                {
                  status: 429,
      ...
      const now = Date.now();
      type AdminOpenAICB = { fail: number; until: number };
      const g = globalThis as typeof globalThis & {
        __adminOpenAICB?: AdminOpenAICB;
      };
      if (!g.__adminOpenAICB) {
        g.__adminOpenAICB = { fail: 0, until: 0 };
      }
      const cb = g.__adminOpenAICB as AdminOpenAICB;
      const circuitOpen = now < cb.until;
      if (provider === "openai" && circuitOpen) {
        provider = "synthetic";
        recordFallback("admin_stream:circuit_open");
      }
      ...
      const onClient: (client: SSEClient) => void = (client) => {
        void (async () => {
          try {
              ...
              try {
                ...
                const stream = (await openai.chat.completions.create({
                  ...
                  messages: [
                    {
                      role: "system",
                      content: "You are RankPilot Admin AI. Answer concisely.",
                    },
            ...
            if (provider !== "openai") {
              // fallback one-shot
              const text = await fallbackOneShot(
                "You are RankPilot Admin AI. Answer concisely.",
                message,
                600
              );
              fullResponse = text;
            ...
            client.send(
              enforceProvenanceOnChunk(
                { error: msg, provenance: "synthetic" },
                { path: "chat/admin/stream" }
              )
            );
            client.sendRaw("data: [DONE]\n\n");
      ...
      return NextResponse.json(
        enforceProvenance(
          { error: msg },
          { path: "chat/admin/stream", note: "exception" }
        ),
        { status: 500 }
      );
    }
