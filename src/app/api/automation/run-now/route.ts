...
function rangeVal(v: unknown): "30d" | "90d" | "ytd" {
  ...
  async function POST(req: Request): Promise<NextResponse> {
    try {
      ...
      if (!recipeId)
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "automation/run-now", note: "validation" }
          ),
          { status: 400 }
        );
      ...
      if (!snap.exists)
        return NextResponse.json(
          enforceProvenance(
            { success: false, error: "Not found", provenance: "synthetic" },
            { path: "automation/run-now", note: "not_found" }
          ),
          { status: 404 }
        );
        ...
        if (action === "runNeuroSEOAnalysis") {
          ...
          try {
            ...
            actionResults.push({
              action,
              status: "ok",
              message: "Digest enqueued",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Queue failed"),
            });
          }
        } else if (action === "generateContentRewrite") {
          try {
            const { RewriteGenEngine } = await import(
              "@/lib/neuroseo/rewrite-gen"
            );
            const engine = new RewriteGenEngine();
            const urls = strArray(cfg?.urls, ["https://example.com"]).slice(
              0,
              3
            );
            const sourceMap = await fetchRewriteSourceContents(urls, {
              max: 3,
              deterministicFallback: true,
            });
            const batchRef = await adminDb.collection("rewriteBatches").add({
              userId: data.userId,
              teamId: data.teamId || null,
              recipeId,
              createdAt: new Date(),
            ...
            for (const url of urls) {
              const originalContent =
                sourceMap[url] || `Placeholder content for ${url}`;
              const analysis = await engine.generateRewrites({
                originalContent,
                targetKeywords: strArray(cfg?.keywords, ["example"]).slice(
                  0,
                  5
                ),
                tone: "professional",
                ...
                constraints: [
                  { type: "preserve_facts", value: true, importance: "high" },
                ],
                seoRequirements: {
                  primaryKeyword:
                    strArray(cfg?.keywords, ["example"])[0] || "example",
              ...
              await adminDb.collection("rewriteVariants").add({
                batchId: batchRef.id,
                url,
                userId: data.userId,
                teamId: data.teamId || null,
            ...
            actionResults.push({
              action,
              status: "ok",
              message: "Rewrite batch generated",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Rewrite failed"),
            });
          }
          ...
          try {
            ...
            if (!deals.length)
              ...
              const closedWon = deals.filter(
                (d) => d.status === "ClosedWon"
              ).length;
              await adminDb.collection("salesMetricsSnapshots").add({
                userId: data.userId,
                teamId: data.teamId || null,
                range,
                pipeline,
            ...
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Metrics failed"),
            });
          }
          ...
          try {
            ...
            await adminDb.collection("salesForecastSnapshots").add({
              userId: data.userId,
              teamId: data.teamId || null,
              period,
              forecast: Math.round(pipeline * 0.8),
              actual: null,
              createdAt: new Date(),
            });
            actionResults.push({
              action,
              status: "ok",
              message: "Forecast snapshot added",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Forecast failed"),
            });
          }
          ...
          try {
            ...
            if (!deals.length)
              ...
              actionResults.push({
                action,
                status: "ok",
                message: "Pipeline digest enqueued",
              });
            }
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Pipeline digest failed"),
            });
          }
          ...
          try {
            ...
            if (!invoices.length)
              ...
              const periods = Array.from(
                new Set(invoices.map((i) => i.period))
              ).sort();
              ...
              const onTime = paid.filter((i) => {
                const paidAt = timestampToDate(i.paidAt);
                const due = timestampToDate(i.dueAt);
                return paidAt && due && paidAt.getTime() <= due.getTime();
              });
              const onTimePct = paid.length
                ? (onTime.length / paid.length) * 100
                : 0;
              const outstanding = current.filter(
                (i) => i.status !== "paid"
              ).length;
              await adminDb.collection("financeRevenueSnapshots").add({
                userId: data.userId,
                teamId: data.teamId || null,
                period: last,
                mrr,
              ...
              actionResults.push({
                action,
                status: "ok",
                message: "Revenue snapshot stored",
              });
            }
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Revenue snapshot failed"),
            });
          }
          ...
          try {
            ...
            invoices
              ...
              .forEach((i) => {
                ...
                if (!due) return;
                const days = Math.floor((nowTs - due) / 86400000);
            ...
            await adminDb.collection("emailQueue").add({
              ...
              recipeId,
              to,
              subject: "Invoice Aging Digest",
              body,
              status: "pending",
              createdAt: new Date(),
            });
            await adminDb.collection("financeInvoiceAgingSummaries").add({
              userId: data.userId,
              teamId: data.teamId || null,
              buckets,
              createdAt: new Date(),
            ...
            actionResults.push({
              action,
              status: "ok",
              message: "Invoice aging digest enqueued",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Aging digest failed"),
            });
          }
          ...
          try {
            ...
            // Revenue recognition
            await adminDb.collection("accountingJournalEntries").add({
              userId: data.userId,
              teamId: data.teamId || null,
            ...
            await adminDb.collection("accountingJournalEntries").add({
              ...
              period,
              lines: [
                { account: "COGS_HOSTING", side: "debit", amount: cogs },
                { account: "LIAB_AP", side: "credit", amount: cogs },
              ],
              source: "seed",
              createdAt: new Date(),
              updatedAt: new Date(),
            ...
            // Salaries OPEX
            const sal = Math.round(baseRevenue * 0.3);
            await adminDb.collection("accountingJournalEntries").add({
              userId: data.userId,
              teamId: data.teamId || null,
              date: now.toISOString().slice(0, 10),
              period,
              lines: [
                { account: "OPEX_SAL", side: "debit", amount: sal },
                { account: "ASSET_CASH", side: "credit", amount: sal },
              ],
              source: "seed",
              createdAt: new Date(),
              updatedAt: new Date(),
            ...
            actionResults.push({
              action,
              status: "ok",
              message: "Sample journals seeded",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Seed failed"),
            });
          }
        } else if (action === "financeAccountingGeneratePnL") {
          try {
            const period = new Date().toISOString().slice(0, 7);
            const jeSnap = await adminDb
              .collection("accountingJournalEntries")
              .where(
                data.teamId ? "teamId" : "userId",
                "==",
                data.teamId || data.userId
            ...
            if (jeSnap.empty) {
              ...
              let revenue = 0,
                ...
                opex = 0;
              jeSnap.docs
                .filter((d) => (d.data() as JournalEntryDoc).period === period)
                .forEach((d) => {
                  ((d.data() as JournalEntryDoc).lines || []).forEach(
                    (l: JournalLine) => {
                      const side = l.side;
                      const amt = l.amount || 0;
                      const acct = l.account || "";
                      const add = (normal: "debit" | "credit") =>
                        side === normal ? amt : -amt;
                      if (acct.startsWith("REV_")) revenue += add("credit");
                      else if (acct.startsWith("COGS_"))
                        cogs += Math.abs(add("debit"));
                      else if (acct.startsWith("OPEX_"))
                        opex += Math.abs(add("debit"));
                    }
                  );
                });
              const grossProfit = revenue - cogs;
              const netIncome = grossProfit - opex;
              await adminDb.collection("accountingReportSnapshots").add({
                userId: data.userId,
                teamId: data.teamId || null,
                type: "pnl",
                period,
                figures: { revenue, cogs, grossProfit, opex, netIncome },
                createdAt: new Date(),
              });
              actionResults.push({
                action,
                status: "ok",
                message: "P&L snapshot stored",
              });
            }
          } catch (e: unknown) {
            actionResults.push({
                ...
                .forEach((d) => {
                  ((d.data() as JournalEntryDoc).lines || []).forEach(
                    (l: JournalLine) => {
                      ...
                      const amt = l.amount || 0;
                      const acct = l.account || "";
                      const signed = (normal: "debit" | "credit") =>
                        side === normal ? amt : -amt;
                      if (acct.startsWith("ASSET_")) assets += signed("debit");
                      else if (acct.startsWith("LIAB_"))
                        liabilities += signed("credit");
                      else if (acct.startsWith("EQUITY_"))
                        equity += signed("credit");
                    }
                  );
                });
              await adminDb.collection("accountingReportSnapshots").add({
                ...
                type: "balance_sheet",
                period,
                figures: { assets, liabilities, equity },
                createdAt: new Date(),
              });
              actionResults.push({
                action,
                status: "ok",
                message: "Balance Sheet snapshot stored",
              });
            }
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Balance sheet failed"),
            });
          }
          ...
          try {
            const invSnap = await adminDb
              ...
              .get();
            const unpaid = invSnap.docs.filter(
              (d) => (d.data() as InvoiceDoc).status !== "paid"
            ).length;
            await adminDb.collection("accountingReportSnapshots").add({
              userId: data.userId,
              teamId: data.teamId || null,
            ...
            actionResults.push({
              action,
              status: "ok",
              message: "Reconciliation snapshot stored",
            });
          } catch (e: unknown) {
            actionResults.push({
              action,
              status: "error",
              message: errMsg(e, "Reconcile failed"),
            });
          }
        } else {
      ...
      try {
        await adminDb.collection("automationRuns").add({
          recipeId,
          userId: data.userId,
          teamId: data.teamId || null,
          startedAt: started,
          ...
          status: actionResults.some((a) => a.status === "error")
            ? "partial"
            : "ok",
          createdAt: new Date(),
        });
      ...
      return NextResponse.json(
        enforceProvenance(
          {
            success: false,
            error: errMsg(e, "Run now failed"),
            provenance: "synthetic",
          },
          { path: "automation/run-now", note: "exception" }
        ),
        { status: 500 }
      );
    }
