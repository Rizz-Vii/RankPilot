...
import type {
  ExportFormat,
  ServerChartArtifactData,
  ServerDashboardArtifactData,
} from "@/types/visualization-exports";
  ...
  async function POST(request: NextRequest) {
    try {
      ...
      try {
        ...
        console.warn(
          "[Visualizations API] Firebase admin initialization error:",
          error
        );
        return NextResponse.json(
          enforceProvenance(
      ...
      if (!userDoc.exists) {
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "visualizations", note: "not_found" }
          ),
          { status: 404 }
        );
      }
      ...
      const action: string | undefined =
        body && typeof body === "object"
          ? ((body as { action?: unknown }).action as string | undefined)
          : undefined;
      if (!action || typeof action !== "string") {
        return NextResponse.json(
          enforceProvenance(
            { error: "Invalid action" },
            { path: "visualizations", note: "invalid_action" }
          ),
          { status: 400 }
        );
      }
      ...
      return NextResponse.json(
        enforceProvenance(
          ...
          { path: "visualizations", note: "exception" }
        ),
        { status: 500 }
      );
    }
      ...
      switch (type) {
          ...
          if (!id) {
            return NextResponse.json(
              enforceProvenance(
                ...
                { path: "visualizations", note: "validation" }
              ),
              { status: 400 }
            );
          }
        ...
        default:
          return NextResponse.json(
            enforceProvenance(
              { error: "Invalid type" },
              { path: "visualizations", note: "invalid_type" }
            ),
            { status: 400 }
          );
      }
...
export const DELETE = withProvenance(
...
type ChartCreateInput = Partial<
  Omit<ChartDoc, "id" | "userId" | "metadata" | "settings">
> & {
  id?: string;
  title?: string;
  description?: string;
...
async function createChart(userId: string, chartData: unknown) {
  const cd: ChartCreateInput = isObj(chartData)
    ? (chartData as ChartCreateInput)
    : {};
  const chartDoc: ChartDoc = {
    id:
      cd.id || `chart_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
    userId,
    config: cd.config,
    ...
    settings: {
      ...
      refreshInterval:
        typeof cd.refreshInterval === "number" ? cd.refreshInterval : 60000,
    },
  };
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "create_chart" }
    )
  );
}

type ChartUpdateInput = Partial<
  ChartCreateInput & {
...
async function updateChart(userId: string, updateData: unknown) {
  const ud: ChartUpdateInput = isObj(updateData)
    ? (updateData as ChartUpdateInput)
    : {};
  ...
  const updatedChart: ChartDoc = {
    ...
    metadata: {
      ...
      ...(isObj((updates as { metadata?: unknown }).metadata)
        ? (updates as { metadata?: Record<string, unknown> }).metadata
        : {}),
      updated: new Date(),
    },
    settings: {
      ...
      ...(isObj((updates as { settings?: unknown }).settings)
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "update_chart" }
    )
  );
}

interface ChartExportInput {
  chartId?: string;
  format?: string;
  config?: unknown;
  artifact?: unknown;
...
async function exportChart(userId: string, exportData: unknown) {
  const ed: ChartExportInput = isObj(exportData)
    ? (exportData as ChartExportInput)
    : {};
  const { chartId, format, config, artifact } = ed;

  if (!chartId || !format) {
    return NextResponse.json(
      { error: "Chart ID and format required" },
      { status: 400 }
    );
  }
  ...
  const cConfig = isObj((chartData as { config?: unknown }).config)
    ? (chartData as { config?: Record<string, unknown> }).config
    : {};
  const cData = Array.isArray((chartData as { data?: unknown }).data)
    ? (chartData as { data?: unknown[] }).data
    : [];
  const cMetadata = isObj((chartData as { metadata?: unknown }).metadata)
    ? (chartData as { metadata?: Record<string, unknown> }).metadata
    : {};
  const cSettings = isObj((chartData as { settings?: unknown }).settings)
    ? (chartData as { settings?: Record<string, unknown> }).settings
    : {};
  const exportBase = {
    id: chartId,
    userId,
  ...
  const safeFormat =
    typeof format === "string" && allowedFormats.has(format) ? format : "json";
  ...
  const chartExportBase: ServerChartArtifactData = {
    ...
    config: isObj(exportBase.config)
      ? {
          type: (exportBase.config as { type?: unknown }).type as
            | string
            | undefined,
        }
      : undefined,
    ...
    metadata: isObj(exportBase.metadata)
      ? (Object.fromEntries(
          Object.entries(exportBase.metadata).filter(([_, v]) =>
  ...
  const chartExportConfig = isObj(config)
    ? (config as Partial<{ title?: string; width?: number; height?: number }>)
    : undefined;
  ...
  if (typeof artifact === "string" && artifact.length > 0) {
    ...
    exportUrl = await persistExportArtifact({
      userId,
      kind: "chart",
      id: chartId,
      format: safeFormat,
      artifact,
      metadata: { chartType: chartExportBase.config?.type },
    });
  } else {
    exportUrl = await generateChartExport(
      chartExportBase,
      serverFormat,
      chartExportConfig
    );
  ...
  return NextResponse.json(
    enforceProvenance(
      { success: true, exportUrl, message: "Chart exported successfully" },
      { path: "visualizations", note: "export_chart" }
    )
  );
}
...
async function getCharts(userId: string) {
  ...
  const charts = chartsSnapshot.docs.map(
    (doc) =>
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "charts" }
    )
  );
}
...
async function getChart(userId: string, chartId: string) {
  ...
  return NextResponse.json(
    enforceProvenance(
      {
        success: true,
        chart: { id: chartDoc.id, ...(chartData as Record<string, unknown>) },
      },
      { path: "visualizations", note: "chart" }
    )
  );
}
...
async function deleteChart(userId: string, chartId: string) {
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "delete_chart" }
    )
  );
}
...
type DashboardCreateInput = Partial<
  Omit<DashboardDoc, "id" | "userId" | "settings" | "metadata">
> & {
  id?: string;
  theme?: string;
  refreshInterval?: number;
...
async function createDashboard(userId: string, dashboardData: unknown) {
  const dd: DashboardCreateInput = isObj(dashboardData)
    ? (dashboardData as DashboardCreateInput)
    : {};
  const dashboardDoc: DashboardDoc = {
    id:
      dd.id ||
      `dashboard_${Date.now()}_${Math.random().toString(36).slice(2, 11)}`,
    userId,
    ...
    settings: {
      ...
      refreshInterval:
        typeof dd.refreshInterval === "number" ? dd.refreshInterval : 60000,
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "create_dashboard" }
    )
  );
}

type DashboardUpdateInput = Partial<
...
async function updateDashboard(userId: string, updateData: unknown) {
  const ud: DashboardUpdateInput = isObj(updateData)
    ? (updateData as DashboardUpdateInput)
    : {};
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "update_dashboard" }
    )
  );
}
...
async function exportDashboard(userId: string, exportData: unknown) {
  const ed: DashboardExportInput = isObj(exportData)
    ? (exportData as DashboardExportInput)
    : {};
  const { dashboardId, format, config, artifact } = ed;

  if (!dashboardId || !format) {
    return NextResponse.json(
      { error: "Dashboard ID and format required" },
      { status: 400 }
    );
  }
  ...
  const dWidgets = Array.isArray(
    (dashboardData as { widgets?: unknown }).widgets
  )
    ? (dashboardData as { widgets?: unknown[] }).widgets
    : [];
  const dSettings = isObj((dashboardData as { settings?: unknown }).settings)
    ? (dashboardData as { settings?: Record<string, unknown> }).settings
    : {};
  const dMetadata = isObj((dashboardData as { metadata?: unknown }).metadata)
    ? (dashboardData as { metadata?: Record<string, unknown> }).metadata
    : {};
  const dName =
    typeof (dashboardData as { name?: unknown }).name === "string"
      ? (dashboardData as { name?: string }).name
      : "Dashboard";
  ...
  const allowedDashFormats = new Set([
    "png",
    "jpeg",
    "json",
    "csv",
  ...
  const safeDashFormat =
    typeof format === "string" && allowedDashFormats.has(format)
      ? format
      : "json";
  ...
  const dashboardExportObj: ServerDashboardArtifactData = {
    ...
    widgets: Array.isArray(dashboardExportBase.widgets)
      ? dashboardExportBase.widgets.map((w) => ({
          id: (w as { id?: unknown }).id
            ? String((w as { id?: unknown }).id)
            : "w",
          type: (w as { type?: unknown }).type
  ...
  const dashFormat: ExportFormat = isDashExportFormat(dashServerFormat)
    ? dashServerFormat
    : "json";
  const dashExportConfig = isObj(config)
    ? (config as Partial<{ title?: string }>)
    : undefined;
  let exportUrl: string;
  if (typeof artifact === "string" && artifact.length > 0) {
    const isDataUrl = /^data:[^;]+;base64,[a-z0-9+/=]+$/i.test(artifact);
    ...
    exportUrl = await persistExportArtifact({
      ...
      kind: "dashboard",
      id: dashboardId,
      format: safeDashFormat,
      artifact,
      metadata: { widgetCount: dashboardExportObj.widgets?.length || 0 },
    });
  } else {
    exportUrl = await generateDashboardExport(
      dashboardExportObj,
      dashFormat,
      dashExportConfig
    );
  ...
  return NextResponse.json(
    enforceProvenance(
      { success: true, exportUrl, message: "Dashboard exported successfully" },
      { path: "visualizations", note: "export_dashboard" }
    )
  );
}
...
async function getDashboards(userId: string) {
  ...
  const dashboards = dashboardsSnapshot.docs.map(
    (doc) =>
      ({ id: doc.id, ...(doc.data() as Record<string, unknown>) }) as Record<
        string,
        unknown
  ...
  return NextResponse.json(
    enforceProvenance(
      { success: true, dashboards, count: dashboards.length },
      { path: "visualizations", note: "dashboards" }
    )
  );
}
...
async function getDashboard(userId: string, dashboardId: string) {
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "dashboard" }
    )
  );
}
...
async function deleteDashboard(userId: string, dashboardId: string) {
  ...
  return NextResponse.json(
    enforceProvenance(
      ...
      { path: "visualizations", note: "delete_dashboard" }
    )
  );
}
