...
async function getTeamForUser(uid: string) {
  ...
  const teamsSnap = await adminDb
    ...
    .get();
  if (!teamsSnap.empty)
    return { id: teamsSnap.docs[0].id, data: teamsSnap.docs[0].data() };
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    const invitesCol = adminDb
      .collection("teams")
      .doc(team.id)
      .collection("invites");
    const existingInv = await invitesCol
      .where("emailLower", "==", email.toLowerCase())
      .limit(1)
      .get();
    if (existingInv.size)
      return NextResponse.json({ error: "Already invited" }, { status: 409 });
    if ((teamData.memberIds || []).some((m) => m === email))
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  try {
    ...
    let found: {
      teamId: string;
      inviteDoc: FirebaseFirestore.DocumentSnapshot;
    } | null = null;
    ...
    if (tokenHash !== invData.tokenHash)
      return NextResponse.json({ error: "Invalid token" }, { status: 403 });
    if (invData.emailLower !== (decoded.email || "").toLowerCase())
    ...
    await adminDb.runTransaction(async (tx) => {
      ...
      if ((tData.memberIds || []).includes(uid))
      ...
      tx.update(teamRef.collection("invites").doc(inviteId), {
        status: "accepted",
        acceptedAt: new Date(),
      });
      ...
      tx.set(
        adminDb.collection("invites_index").doc(inviteId),
        { teamId: found!.teamId, status: "accepted", updatedAt: new Date() },
        { merge: true }
      );
    });
    ...
    if (team) {
      ...
      if (acting && ["owner", "admin"].includes(acting.role || "")) {
        const invSnap = await adminDb
          ...
          .get();
        invSnap.forEach((d) =>
          results.push({ id: d.id, teamId: team.id, ...d.data() })
        );
