...
export async function POST(req: NextRequest) {
    try {
        ...
        const members = Array.isArray(membersData) ? [...(membersData as unknown[])] : [];
        ...
        const ownerIndex = members.findIndex((m: unknown) => {
            ...
            return role === "owner" && (memberUserId === decoded.uid || memberEmail === decoded.email);
        });
        ...
        const targetRole = (members[targetIndex] as Record<string, unknown>).role as string | undefined;
        if (targetRole === "owner") return NextResponse.json({ error: "Target already owner" }, { status: 400 });

        // Perform transfer (retain traceability)
        const prevOwnerId = ((members[ownerIndex] as Record<string, unknown>).userId ?? (members[ownerIndex] as Record<string, unknown>).id) as string | undefined;
        const newOwnerId = ((members[targetIndex] as Record<string, unknown>).userId ?? (members[targetIndex] as Record<string, unknown>).id) as string | undefined;
