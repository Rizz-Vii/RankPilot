...
async function getTeam(
  uid: string
): Promise<{ id: string; data: TeamDoc } | null> {
...
export const DELETE = withProvenance(
  async function DELETE(
    ...
    try {
      ...
      if (!authHeader) {
        const missingAuthBody = enforceProvenance(
          { error: "Missing auth" },
          { path: "team/member", note: "auth" }
        );
      ...
      if (!team) {
        const notFoundBody = enforceProvenance(
          { error: "Team not found" },
          { path: "team/member", note: "not_found" }
        );
      ...
      if (!acting || !["owner", "admin"].includes(acting.role || "")) {
        const permBody = enforceProvenance(
          { error: "Insufficient permissions" },
          { path: "team/member", note: "perm" }
        );
      ...
      if (targetIndex === -1) {
        const memNotFoundBody = enforceProvenance(
          { error: "Member not found" },
          { path: "team/member", note: "member_not_found" }
        );
      ...
      if (target.role === "owner") {
        const ownerBody = enforceProvenance(
          { error: "Cannot remove owner" },
          { path: "team/member", note: "owner" }
        );
      ...
      if (target.role === "admin") {
        ...
        if (adminCount <= 1) {
          const lastAdminBody = enforceProvenance(
            { error: "Cannot remove last admin" },
            { path: "team/member", note: "last_admin" }
          );
      ...
      const errBody = enforceProvenance(
        { error: msg },
        { path: "team/member", note: "exception" }
      );
