    ...
    try {
      ...
      if (!authHeader) {
        const b = enforceProvenance(
          { error: "Missing auth" },
          { path: "team/member/resend", note: "auth" }
        );
      ...
      if (!team) {
        const b = enforceProvenance(
          ...
          { path: "team/member/resend", note: "not_found" }
        );
      ...
      if (!acting || !["owner", "admin"].includes(acting.role || "")) {
        const b = enforceProvenance(
          ...
          { path: "team/member/resend", note: "perm" }
        );
      ...
      if (!target) {
        const b = enforceProvenance(
          ...
          { path: "team/member/resend", note: "member_not_found" }
        );
        return NextResponse.json(b, { status: 404 });
      }
      if (target.status !== "pending") {
        const b = enforceProvenance(
          ...
          { path: "team/member/resend", note: "status" }
        );
      ...
      const invitedAtTs =
        invitedAtValue &&
        typeof (invitedAtValue as { toMillis?: unknown })?.toMillis ===
          "function"
          ? (invitedAtValue as { toMillis: () => number }).toMillis()
          : typeof invitedAtValue === "number"
            ? invitedAtValue
            : new Date(String(invitedAtValue)).getTime();
      if (now - invitedAtTs < 60_000) {
        const b = enforceProvenance(
          { error: "Please wait before resending" },
          { path: "team/member/resend", note: "cooldown" }
        );
      ...
      const ok = enforceProvenance(
        { success: true },
        { path: "team/member/resend", note: "ok" }
      );
      ...
      const b = enforceProvenance(
        { error: msg },
        { path: "team/member/resend", note: "exception" }
      );
