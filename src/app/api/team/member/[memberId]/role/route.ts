...
async function getTeam(
  uid: string
): Promise<{ id: string; data: TeamDoc | undefined } | null> {
  ...
  const uData = u.exists
    ? (u.data() as { teamId?: string | undefined })
    : undefined;
    ...
    try {
      ...
      if (!authHeader) {
        const body = enforceProvenance(
          { error: "Missing auth" },
          { path: "team/member/role", note: "auth" }
        );
      ...
      if (!role) {
        const b = enforceProvenance(
          { error: "Missing role" },
          { path: "team/member/role", note: "input" }
        );
      ...
      if (!team) {
        const b = enforceProvenance(
          ...
          { path: "team/member/role", note: "not_found" }
        );
      ...
      if (
        !acting ||
        !(privilegedRoles as readonly string[]).includes(acting.role || "")
      ) {
        const b = enforceProvenance(
          { error: "Insufficient permissions" },
          { path: "team/member/role", note: "perm" }
        );
      ...
      const targetIndex = members.findIndex(
        (m) =>
          m.userId === params.memberId ||
          m.id === params.memberId ||
          m.email === params.memberId
      );
      if (targetIndex === -1) {
        const b = enforceProvenance(
          { error: "Member not found" },
          { path: "team/member/role", note: "member_not_found" }
        );
        return NextResponse.json(b, { status: 404 });
      }
      ...
      if (target.role === "owner") {
        const b = enforceProvenance(
          { error: "Use transfer ownership flow" },
          { path: "team/member/role", note: "owner" }
        );
        return NextResponse.json(b, { status: 400 });
      }
      if (role === "owner") {
        const b = enforceProvenance(
          { error: "Cannot promote directly to owner" },
          { path: "team/member/role", note: "owner_promote" }
        );
        return NextResponse.json(b, { status: 400 });
      }
      ...
      const ok = enforceProvenance(
        { success: true },
        { path: "team/member/role", note: "updated" }
      );
      ...
      const b = enforceProvenance(
        { error: msg },
        { path: "team/member/role", note: "exception" }
      );
