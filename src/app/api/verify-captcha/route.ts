...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const token =
      ...
      "";
    if (!token) {
      const resBody = enforceProvenance(
        { error: "token_required" },
        { path: "verify-captcha:POST", note: "missing-token" }
      );
    ...
    if (!secret) {
      const resBody = enforceProvenance(
        ...
        { path: "verify-captcha:POST", note: "missing-secret" }
      );
    ...
    if (!resp.ok) {
      ...
      const resBody = enforceProvenance(
        ...
        { path: "verify-captcha:POST", note: `upstream-${resp.status}` }
      );
    ...
    const data = (await resp
      .json()
      .catch(() => ({}))) as RecaptchaVerifyResponse;
    if (!data?.success) {
      ...
      const resBody = enforceProvenance(
        ...
        { path: "verify-captcha:POST", note: "failed" }
      );
      return NextResponse.json(resBody, { status: 400 });
    }
    ...
    const resBody = enforceProvenance(
      ...
      { path: "verify-captcha:POST", note: "exception" }
    );
    return NextResponse.json(resBody, { status: 500 });
  }
}
