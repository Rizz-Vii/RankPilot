...
export const GET = withAdmin(
  async (request: Request, admin) => {
    try {
      // CORS support (GET/OPTIONS)
      const cors = handleCors(request as unknown as Request, {
        allowMethods: ["GET", "OPTIONS"],
      });
      if ((cors as unknown as { preflight?: Response }).preflight)
        return (cors as unknown as { preflight: Response })
          .preflight as unknown as NextResponse;
      ...
      const pageSize = Number.isFinite(sizeParam)
        ? Math.min(Math.max(sizeParam, 1), 50)
        : 25; // hard-cap to 50 for safety
      ...
      for (const userDoc of pageDocs) {
        ...
        const activitiesCol = adminDb
          .collection("users")
          .doc(userId)
          .collection("activities");
        const activitiesSnapshot = await activitiesCol
          .orderBy("timestamp", "desc")
          .limit(light ? 3 : 10)
          .get();
        const activities: ActivityRecord[] = activitiesSnapshot.docs.map(
          (activityDoc) => {
        ...
        const keywords: Array<Record<string, unknown>> =
          keywordsSnapshot.docs.map((keywordDoc) => {
            const keywordData = keywordDoc.data();
            return {
              id: keywordDoc.id,
              keyword:
                (keywordData as Record<string, unknown>)["keyword"] || "",
              data: keywordData,
            };
          });
        ...
        const competitors: Array<Record<string, unknown>> =
          competitorsSnapshot.docs.map((compDoc) => {
        ...
        const contentAnalyses: Array<Record<string, unknown>> =
          contentSnapshot.docs.map((contentDoc) => {
        ...
        const achievements: Array<Record<string, unknown>> =
          achievementsSnapshot.docs.map((achDoc) => {
        ...
        const subscriptionTier =
          ...
          userData.subscriptionTier.length > 0
            ? userData.subscriptionTier
            : "free";
        const role =
          typeof userData.role === "string" ? userData.role : undefined;
        const plan =
          typeof userData.plan === "string" ? userData.plan : undefined;
        const planType =
          typeof userData.planType === "string" ? userData.planType : undefined;
        ...
        const actualQuotas =
          userData &&
          typeof userData.quotas === "object" &&
          userData.quotas !== null
  ...
  },
  {
    path: "review-users",
    extraHeaders: (req) => ({
      ...(handleCors(req as unknown as Request, {
        allowMethods: ["GET", "OPTIONS"],
      }).headers || {}),
    }),
  }
...
export async function OPTIONS(request: Request): Promise<Response> {
  ...
  if ((cors as unknown as { preflight?: Response }).preflight)
    return (cors as unknown as { preflight: Response }).preflight;
