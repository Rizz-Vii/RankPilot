...
import {
  ...
  recordWorkflowRun,
} from "@/lib/neuroseo/metrics-registry";
import {
  enforceNeuroSeoRateLimit,
  NeuroSeoRateLimitError,
} from "@/lib/neuroseo/rate-limit";
import {
...
async function persistCompact(
  urls: string[],
  analysisType: string,
  userId: string | undefined,
  overallScore: number,
  provenance: "live" | "cache" | "synthetic"
): Promise<void> {
  if (!userId) return;
  try {
    const hashKey = Buffer.from(
      JSON.stringify({ u: [...urls].sort(), t: analysisType })
    )
      .toString("base64")
      .replace(/[^A-Za-z0-9]/g, "")
      .slice(0, 40);
    ...
    if (size > 5000) {
      logger.warn("persist.size_exceeded", { size });
      return;
    }
    await adminDb
    ...
    logger.info("persist.stream.success", {
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  if (urls.length > 50) {
    recordGuardStrip();
    return new Response(JSON.stringify({ error: "too_many_urls" }), {
      status: 400,
    });
  }
  ...
  let finished = false;
  let finalProvenance: string | undefined;
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      request.signal.addEventListener("abort", () => {
        ...
        finalize(finalProvenance || "aborted", false);
      });
      ...
      try {
        ...
        if (typeof teamId === "string" && teamId) {
          const { adminDb: _adminDb } = g.adminDb
            ? { adminDb: g.adminDb }
            : await import("@/lib/firebase-admin");
          await enforceTeamRateLimit(_adminDb, teamId, {
            routeKey: "neuroseo/stream",
          });
        } else {
          const admin =
            g.adminDb || (await import("@/lib/firebase-admin")).adminDb;
          await enforceNeuroSeoRateLimit(admin, userId, {});
        }
        const allowedTypes = ["comprehensive", "quick", "competitor"] as const;
        const atype = (allowedTypes as readonly string[]).includes(analysisType)
          ? (analysisType as (typeof allowedTypes)[number])
          : "comprehensive";
        const orchestration = neuroSEOOrchestrator.runAnalysisStream({
          urls,
          analysisType: atype,
          userId,
        });
        for await (const evt of orchestration) {
          type StreamEvt =
            | {
                type: "cached";
                data: { overallScore: number; provenance: "cache" };
              }
            ...
            | {
                type: "chunk.complete";
                data: { index: number; processed: number };
              }
          ...
          if (event.type === "cached") {
            recordCacheHit();
            finalProvenance = "cache";
          }
          if (event.type === "complete") {
            recordAnalysisRun();
            finalProvenance = event.data.provenance;
            await persistCompact(
              urls,
              analysisType,
              userId,
          ...
          write(
            controller,
            event.type,
            enforceProvenanceOnChunk(event.data as Record<string, unknown>, {
              path: "neuroseo/stream",
            })
          );
          if (Date.now() > deadline && !finished && evt.type !== "complete") {
            logger.warn("stream.timeout", { partial: true });
            ...
            await persistCompact(
              ...
              analysisType,
              userId,
              synthetic.overallScore,
              "synthetic"
            );
            finalProvenance = "synthetic";
            finalize("synthetic");
            return;
          }
        }
        ...
        if (
          error instanceof TeamRateLimitError ||
          error instanceof NeuroSeoRateLimitError
        ) {
          const retryAfterSeconds =
            error &&
            typeof error === "object" &&
            "retryAfterSeconds" in error &&
            typeof (error as { retryAfterSeconds?: unknown })
        ...
        write(
          ...
          "error",
          enforceProvenanceOnChunk(
            { message: errMsg, provenance: "synthetic" },
            { path: "neuroseo/stream" }
          )
        );
        const rng = createDeterministicRng([
        ...
        write(
          ...
          "fallback",
          enforceProvenanceOnChunk(
            { provenance: "synthetic", overallScore: synthetic.overallScore },
            { path: "neuroseo/stream" }
          )
        );
        await persistCompact(
