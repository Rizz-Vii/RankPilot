    ...
    if (!ENABLED) {
      return NextResponse.json(
        enforceProvenance(
          {
            success: false,
            error: "Live backend disabled",
            provenance: "synthetic",
          },
          { path: "neuroseo/live", note: "feature_gate" }
        ),
    ...
    try {
      ...
      if (!Array.isArray(urls) || urls.length === 0) {
        return NextResponse.json(
          enforceProvenance(
            ...
            { path: "neuroseo/live", note: "validation" }
          ),
      ...
      try {
        if (typeof teamId === "string" && teamId) {
          ...
          const candidate =
            g.adminDb && typeof g.adminDb === "object"
              ? g.adminDb
              : imported.adminDb;
          const resolvedAdmin =
            candidate &&
            ...
            "collection" in candidate
              ...
              : imported.adminDb;
          await enforceTeamRateLimit(resolvedAdmin, teamId, {
            routeKey: "neuroseo/live",
          });
        } else {
        ...
        if (
          e instanceof TeamRateLimitError ||
          e instanceof NeuroSeoRateLimitError
        ) {
          const retryAfterSeconds =
            e &&
            typeof e === "object" &&
            "retryAfterSeconds" in e &&
            typeof (e as { retryAfterSeconds?: unknown }).retryAfterSeconds ===
              "number"
          ...
          return NextResponse.json(
            enforceProvenance(
              ...
              { path: "neuroseo/live", note: "rate_limit" }
            ),
            {
              status: 429,
              headers: {
      ...
      const result = await executeNeuroLive(
        { urls: urlArray, analysisType: atype, userId: uid },
        { forceRefresh: force }
      );
      const resp = NextResponse.json(
        enforceProvenance({ ...result }, { path: "neuroseo/live" }),
      ...
      return NextResponse.json(
        enforceProvenance(
          { error: errMessage || "analysis failed", provenance: "synthetic" },
          { path: "neuroseo/live" }
        ),
        {
          status: errMessage?.includes("timeout") ? 503 : 500,
          headers: baseHeaders,
        }
      );
    }
