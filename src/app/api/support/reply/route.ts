...
function getTransport() {
  ...
  return {
    sendMail: async (opts: unknown) => {
      const details =
        (opts as nodemailer.SendMailOptions | undefined) ?? undefined;
      getLogger("support-reply").warn("email.send.fallback", {
        to: details?.to,
...
export const POST = withAdmin(
  async (req: NextRequest) => {
    ...
    if ("preflight" in cors)
      return cors.preflight as Response as unknown as NextResponse;
    try {
  ...
  },
  {
    path: "support/reply",
    extraHeaders: (req) => ({
      ...handleCors(req, { allowMethods: ["POST", "OPTIONS"] }).headers,
      ...noStoreHeaders(),
    }),
