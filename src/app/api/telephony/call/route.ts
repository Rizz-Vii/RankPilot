...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        const testMode: boolean = Boolean(body?.testMode || process.env.TWILIO_TEST_MODE === "1");
        ...
        if (testMode) {
            ...
            logger.info("telephony call simulated", { to, from, twimlUrl, testMode: true, callSid: fakeSid });
            return NextResponse.json({ callSid: fakeSid, test: true });
        }

        if (!client || !from) {
            logger.error("twilio_not_configured", { hasClient: Boolean(client), hasFrom: Boolean(from) });
            return NextResponse.json({ error: "telephony_unavailable" }, { status: 500 });
        }
        ...
        else if (process.env.TWILIO_DEFAULT_TWIML_URL) params.url = process.env.TWILIO_DEFAULT_TWIML_URL;

        const call = await client.calls.create(params);
        logger.info("telephony call initiated", { to, from, callSid: call.sid, status: call.status });
        return NextResponse.json({ callSid: call.sid, test: false });
    } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        getLogger("api.telephony.call").error("telephony_call_failed", { error: msg });
