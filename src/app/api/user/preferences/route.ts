...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const bodyToken =
      ...
      "authToken" in (payload as Record<string, unknown>) &&
      typeof (payload as Record<string, unknown>).authToken === "string"
    ...
    const prefs =
      payload &&
      typeof payload === "object" &&
      "preferences" in (payload as Record<string, unknown>)
    ...
    const message =
      err instanceof Error ? err.message : String(err ?? "internal");
