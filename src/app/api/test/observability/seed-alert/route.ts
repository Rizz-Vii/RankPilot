...
async function seed(
  dateKey: string,
  alertsIn: Array<unknown>,
  ma7Override?: Record<string, number | null>
) {
  const ref = adminDb.collection("kpiAlertsDaily").doc(dateKey);
  await adminDb.runTransaction(async (tx) => {
    const snap = await tx.get(ref);
    const base: { date: string; alerts?: unknown[]; createdAt?: Date } =
      snap.exists
        ? (snap.data() as {
            date: string;
            alerts?: unknown[];
    ...
    if (!snap.exists) tx.set(ref, { ...base, ...update });
...
export async function GET(req: NextRequest) {
  if (
    ...
    return NextResponse.json(
      enforceProvenance(
        ...
        { path: "test/observability/seed-alert", note: "disabled" }
      ),
      { status: 404 }
    );
  }
  ...
  const level =
    (url.searchParams.get("level") as "warn" | "critical") || "warn";
  ...
  if (maField && typeof ma7 === "number" && ma7Override[maField] == null)
    ma7Override[maField] = ma7;

  try {
    await seed(
      dateKey,
      [{ type, level, message, value, threshold: 0 }],
      ma7Override
    );
    return NextResponse.json(
      enforceProvenance(
        {
          ok: true,
          date: dateKey,
          seeded: { type, level, value },
          ma7Override,
        },
        { path: "test/observability/seed-alert" }
      )
    );
  } catch (e: unknown) {
    const msg =
    ...
    return NextResponse.json(
      enforceProvenance(
        { ok: false, error: msg },
        { path: "test/observability/seed-alert", note: "error" }
      ),
      { status: 500 }
    );
  }
...
export async function POST(req: NextRequest) {
  ...
  const alertsReq: unknown[] = Array.isArray(maybeBody?.alerts as unknown)
    ? (maybeBody!.alerts as unknown[])
    : [];
  const alerts = alertsReq.map((a: unknown) => {
    const ai = a as Record<string, unknown> | null;
    const typ =
      typeof ai?.type === "string"
        ? (ai!.type as string)
        : "provenanceCoverage";
    const lvl = (ai?.level === "critical" ? "critical" : "warn") as
      | "warn"
      | "critical";
    const msg =
      typeof ai?.message === "string"
        ? (ai!.message as string)
        : `${typ} seeded (batch)`;
    const val =
      typeof ai?.value === "number"
        ? (ai!.value as number)
        : typ === "provenanceCoverage"
          ? 95
          : 60;
  ...
  if (!alerts.length)
    alerts.push({
      type: "provenanceCoverage",
      level: "warn",
      message: "provenanceCoverage seeded (default)",
      value: 95,
      threshold: 0,
    });
  // Apply per-alert ma7 override if provided
  const ma7Overrides: Record<string, number> = {
    ...((maybeBody?.ma7 as Record<string, number>) || {}),
  };
