...
export async function GET(_req: NextRequest) {
    if (process.env.NODE_ENV === 'production') {
        return NextResponse.json(enforceProvenance({ error: 'forbidden' }, { path: 'test/auth/custom-token', note: 'disabled' }), { status: 403 });
    }
    ...
    try {
        ...
        try {
            ...
            return NextResponse.json(enforceProvenance({ token: 'stub-test-token', uid }, { path: 'test/auth/custom-token' }), { status: 200 });
        }
        try {
        ...
        return NextResponse.json(enforceProvenance({ token, uid }, { path: 'test/auth/custom-token' }));
    } catch (e) {
        return NextResponse.json(enforceProvenance({ error: (e as Error).message }, { path: 'test/auth/custom-token', note: 'error' }), { status: 500 });
    }
}
