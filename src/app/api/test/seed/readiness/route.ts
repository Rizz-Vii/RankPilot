...
async function seedFinance(testUser: string) {
    ...
    for (let idx = 0; idx < periods.length; idx++) {
        ...
        try {
            const r = await fetch(`${base}/api/test/finance/seed-invoice?testUser=${encodeURIComponent(testUser)}&amount=${amount}&status=paid&period=${p}`);
            ok = ok && r.ok;
        } catch {
            ok = false;
        }
    ...
    try {
        const draft = await fetch(`${base}/api/test/finance/seed-invoice?testUser=${encodeURIComponent(testUser)}&amount=75&status=draft&period=${current}`);
        ok = ok && draft.ok;
    } catch {
        ok = false;
    }
...
async function pollInvoicesCount(testUser: string, tries = 5, delayMs = 250) {
    ...
    for (let i = 0; i < tries; i++) {
        try {
            const r = await fetch(`${base}/api/finance/metrics?months=6&testUser=${encodeURIComponent(testUser)}`, { headers: { 'x-probe-token': process.env.CRAWL_PROBE_TOKEN || '' } });
            if (r.ok) {
                const json = (await r.json().catch(() => null)) as { invoicesCount?: number; invoices?: unknown[] } | null;
                if (json) {
...
export async function GET(req: Request) {
    ...
    try {
        if (wantFinance) {
            ...
            financePoll = await pollInvoicesCount(testUser, 6, 300).catch(() => ({ count: 0, ready: false, tries: 0 }));
        }
        if (wantBi) biSeeded = await seedBiLatency().catch(() => false);
    } catch {
        // swallow
    }
    return NextResponse.json({ ok: true, financeSeeded, biSeeded, durationMs: Date.now() - started, user: testUser, invoicesCount: financePoll?.count ?? 0, financeReady: financePoll?.ready ?? false, financePollTries: financePoll?.tries ?? 0 });
}
