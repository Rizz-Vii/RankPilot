...
export async function GET(req: NextRequest) {
    if (process.env.NODE_ENV === 'production') {
        const disabledBody = enforceProvenance({ error: 'Not available in production' }, { path: 'test/metrics/semantic-map', note: 'disabled' });
    ...
    const okBody = enforceProvenance({ ok: true, hitsIncremented: hits, fallbacksIncremented: fallbacks }, { path: 'test/metrics/semantic-map', note: 'ok' });
