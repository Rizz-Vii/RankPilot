...
export function OnTimeBreakdownModal({ open, onOpenChange }: Props) {
  ...
  const invoices: Invoice[] = Array.isArray(data?.invoices)
  ...
  const groups = paid.map((p) => ({
    ...
    tier: p.planTier || "-",
    onTime: (() => {
      const paidAt = p.paidAt?.toDate ? p.paidAt.toDate() : undefined;
      const due = p.dueAt?.toDate ? p.dueAt.toDate() : undefined;
      if (!paidAt || !due) return false;
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            {pct.toFixed(1)}% of {paid.length} paid invoices were on time.
          </DialogDescription>
        </DialogHeader>
              ...
              <span
                ...
                style={{
                  color: g.onTime
                    ...
                    : "var(--warning-foreground, var(--secondary))",
                }}
              >
