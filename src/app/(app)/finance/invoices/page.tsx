...
export default function InvoicesPage(): JSX.Element {
    ...
    void (async () => {
      try {
        ...
        if (r.length) {
          ...
          const first = r[0] as InvoiceSnapshot;
          setRevSnap({
            mrr: first.mrr,
            onTime: first.onTimePct,
            outstanding: first.outstanding,
            ts: first.createdAt?.toDate?.() || new Date(),
            period: first.period,
          });
        }
      } finally {
  ...
  const data = live.kpis.length
    ? live
    : {
        kpis: allowFinanceMocks() ? mock?.kpis || [] : [],
        rows: [],
        loading: false,
      };
      ...
      <div className="p-6 space-y-8">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Historical invoices, receipts, and billing document exports.
          </p>
          ...
          <LazyDataTable
            columns={[
              ...
              { key: "status", header: "Status" },
              {
                ...
                header: "Issued",
                render: (r) => {
                  const rec = r as unknown as InvoiceRowLite;
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-5">
            <ActionCard
              title="Download CSV"
              desc="Export invoice history"
              action="Export"
            />
            <ActionCard
              title="Generate Receipt"
              desc="Create receipt copy"
              action="Generate"
            />
            <ActionCard
              title="Update Billing"
              desc="Modify payment details"
              action="Update"
            />
            <ActionCard
