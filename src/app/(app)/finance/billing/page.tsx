...
export default function BillingOverviewPage() {
  ...
  type MockResp =
    | { kpis?: unknown[]; quotas?: unknown[]; [k: string]: unknown }
    | undefined;
  const live = useFinanceInvoiceMetrics(months) as unknown as LiveRaw;
  const { data: mock } = useMockDomainMetrics(
    "finance",
    allowFinanceMocks()
  ) as unknown as { data?: MockResp };
  interface FinanceKPI {
    ...
    delta?: number;
    trend?: number[];
    intent?: "success" | "neutral" | "warning" | "danger" | "accent";
  }
  interface FinanceQuota {
    key: string;
    label: string;
    used: number;
    limit: number;
  ...
  const adaptInvoice = (r: unknown): InvoiceLike | null => {
    ...
    if (
      typeof rec["period"] !== "string" ||
      typeof rec["planTier"] !== "string"
    )
      return null;
    return {
  ...
  const normalizeLive = (): FinanceDataShape => {
    ...
    const kpis: FinanceKPI[] = rawKpis.map((k: unknown) => {
      ...
      const label =
        typeof rec["label"] === "string"
          ...
          : typeof rec["key"] === "string"
            ...
            : "Metric";
      const value =
        typeof rec["value"] === "number" ? (rec["value"] as number) : 0;
      const delta =
        typeof rec["delta"] === "number" ? (rec["delta"] as number) : undefined;
      const trend = Array.isArray(rec["trend"])
        ? (rec["trend"] as unknown[]).filter(
      ...
      const intent = ((): FinanceKPI["intent"] | undefined => {
        ...
        return typeof v === "string" &&
          ["success", "neutral", "warning", "danger", "accent"].includes(v)
          ? (v as FinanceKPI["intent"])
          : undefined;
      })();
      return {
        key: String(key),
    ...
    const quotasSrc: unknown[] = Array.isArray(live.quotas)
      ? (live.quotas as unknown[])
      : [];
    const quotas: FinanceQuota[] = quotasSrc.map((q: unknown) => {
      ...
      const used =
        typeof rec["used"] === "number"
          ? (rec["used"] as number)
          : Number(rec["used"] as unknown) || 0;
      const limit =
        typeof rec["limit"] === "number"
          ? (rec["limit"] as number)
          : Number(rec["limit"] as unknown) || 0;
      return {
        key: String(key),
        label: String(label),
        used: Number(used),
    ...
    const rowsSrc: unknown[] = Array.isArray(live.rows)
      ? (live.rows as unknown[])
      : [];
    const rows: InvoiceLike[] = rowsSrc
      .map(adaptInvoice)
      .filter((x): x is InvoiceLike => x !== null);
    return { kpis, quotas, rows, loading: !!live.loading };
  };
  const data: FinanceDataShape =
    Array.isArray(live.kpis) && live.kpis.length
      ? normalizeLive()
      : {
          kpis: allowFinanceMocks() && mock ? (mock.kpis as FinanceKPI[]) : [],
          quotas:
            allowFinanceMocks() && mock && mock.quotas
              ? (mock.quotas as FinanceQuota[])
              : [],
          rows: [],
          loading: false,
        };
  ...
  useEffect(() => {
    if (kpiLen > 0) markLive();
    else markFallback();
  }, [kpiLen, markLive, markFallback]);
  return (
    <FeatureGate
      ...
      <div className="p-6 space-y-8">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Plan usage, upcoming charges & cross-engine quota consumption.
          </p>
        ...
        {allowFinanceMocks() &&
          !(Array.isArray(live.rows) && live.rows.length > 0) && (
            <Alert
            ...
            >
              <div className="flex items-start gap-3 text-sm">
                ...
                <p>
                  ...
                  (FINANCE_MOCK_MODE). This banner disappears once live metrics
                  load or mocks are disabled.
                </p>
              </div>
            </Alert>
          )}
          ...
          <LazyDataTable
            columns={[
              ...
              { key: "status", header: "Status" },
              {
                ...
                header: "Issued",
                render: (r) => {
                  const rec = r as unknown as InvoiceLike;
                  return rec.issuedAt?.toDate
                    ? rec.issuedAt.toDate().toISOString().slice(0, 10)
                    : "-";
                },
              },
              {
                ...
                render: (r) => {
                  const rec = r as unknown as InvoiceLike;
                  return rec.paidAt?.toDate
                    ? rec.paidAt.toDate().toISOString().slice(0, 10)
                    : "-";
                },
              },
            ]}
