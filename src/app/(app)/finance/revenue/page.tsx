...
interface FinanceRevenueSnapshot {
  mrr: number;
  onTime: number;
  outstanding: number;
  ts: Date;
...
export default function RevenueAnalyticsPage() {
  ...
  const teamId = (() => {
    ...
    if (
      u &&
      typeof u === "object" &&
      "teamId" in (u as Record<string, unknown>)
    ) {
      ...
      try {
        ...
        if (r.length) {
          ...
          const ts =
            ...
            first.createdAt &&
            "toDate" in (first.createdAt as Record<string, unknown>) &&
            typeof (first.createdAt as { toDate?: () => Date }).toDate ===
              "function"
              ? ((
                  first.createdAt as { toDate?: () => Date }
                ).toDate?.() as Date)
              : new Date();
          setRevSnap({
            ...
            outstanding:
              typeof first.outstanding === "number" ? first.outstanding : 0,
            ts,
  ...
  const adaptInvoice = (r: unknown): InvoiceRow | null => {
    ...
    if (typeof o.period !== "string" || typeof o.planTier !== "string")
      return null;
    return {
      period: o.period,
      planTier: o.planTier,
      ...
      issuedAt:
        o.issuedAt && typeof o.issuedAt === "object"
          ? (o.issuedAt as { toDate?: () => Date })
          : undefined,
      paidAt:
        o.paidAt && typeof o.paidAt === "object"
          ? (o.paidAt as { toDate?: () => Date } | null)
          : undefined,
    };
  };
  const data: InvoiceMetrics =
    Array.isArray(live.kpis) && live.kpis.length
      ? {
          kpis: live.kpis as KpiItem[],
          rows: Array.isArray(live.rows)
            ? live.rows
                .map(adaptInvoice)
      ...
      : {
          kpis: allowFinanceMocks()
            ? (mock?.kpis as KpiItem[] | undefined) || []
            : [],
          rows: [],
          loading: false,
        };
  ...
  useEffect(() => {
    trackDashboardView("finance");
  }, []);
  useEffect(() => {
    if (live.kpis.length) markLive();
    else markFallback();
  }, [live.kpis.length, markLive, markFallback]);
      ...
      <div className="p-6 space-y-8">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            MRR, churn & LTV trajectory with upcoming AI anomaly & retention
            models.
          </p>
        ...
        {allowFinanceMocks() && !live.rows.length && (
          <div
            className="rounded-md border border-warning/30 bg-warning/15 text-warning-foreground dark:bg-warning/20 dark:text-warning-foreground p-3 text-sm flex gap-3"
          ...
          >
            ...
            <span>
              ...
              (FINANCE_MOCK_MODE). This banner disappears once live metrics load
              or mocks are disabled.
            </span>
          </div>
        )}
        ...
        {!loadingSnap && revSnap && (
          <div
            className="rounded-lg border p-3 bg-background/60 flex items-center justify-between text-xs mb-2"
            aria-label="Latest revenue snapshot"
          >
            <div className="space-y-1">
              <p className="font-medium">Last Revenue Snapshot</p>
              <p className="text-muted-foreground">
                MRR {revSnap.mrr.toLocaleString()} · On-Time{" "}
                {revSnap.onTime.toFixed(1)}% · Outst. {revSnap.outstanding}
              </p>
            </div>
            <time
        ...
        {derived && (
          <section
            ...
            aria-label="Derived revenue metrics"
            data-testid="finance-derived-metrics"
          >
            <MetricCard
              key="mrr_formula"
              label="MRR (Derived)"
              value={derived.mrr.toLocaleString()}
              delta={0}
              deltaLabel=""
              trend={<TrendSparkline data={[derived.mrr]} />}
              intent="neutral"
              data-testid="finance-derived-mrr"
            />
            <MetricCard
              key="arr_formula"
              label="ARR"
              value={derived.arr.toLocaleString()}
              delta={0}
              deltaLabel=""
              trend={<TrendSparkline data={[derived.arr]} />}
              intent="neutral"
              data-testid="finance-derived-arr"
            />
            <MetricCard
              key="churn_formula"
              label="Churn %"
              value={derived.churnRatePct.toFixed(1) + "%"}
              delta={0}
              deltaLabel=""
              trend={<TrendSparkline data={[derived.churnRatePct]} />}
              intent={derived.churnRatePct < 5 ? "success" : "warning"}
              data-testid="finance-derived-churn"
            />
            <MetricCard
              key="ltv_formula"
              label="LTV"
              value={derived.ltv ? derived.ltv.toLocaleString() : "—"}
              delta={0}
              deltaLabel=""
              trend={<TrendSparkline data={[derived.ltv || 0]} />}
              intent={derived.ltv ? "accent" : "neutral"}
              data-testid="finance-derived-ltv"
            />
          </section>
        )}
            ...
            columns={[
              ...
              { key: "status", header: "Status" },
              {
                ...
                render: (r) => {
                  ...
                  const d =
                    rec.issuedAt &&
                    typeof rec.issuedAt === "object" &&
                    typeof rec.issuedAt.toDate === "function"
                      ? rec.issuedAt.toDate?.()
                      : undefined;
              ...
              },
              {
                ...
                render: (r) => {
                  ...
                  const d =
                    rec.paidAt &&
                    typeof rec.paidAt === "object" &&
                    typeof rec.paidAt?.toDate === "function"
                      ? rec.paidAt.toDate?.()
                      : undefined;
                  return d ? d.toISOString().slice(0, 10) : "-";
                },
        ...
        <section className="space-y-4" data-testid="finance-workbench">
          ...
          <div className="grid gap-4 md:grid-cols-5">
            <ActionCard
              data-testid="finance-action-cohort"
              title="Run Cohort"
              desc="Generate churn cohort table"
              action="Generate"
            />
            <ActionCard
              data-testid="finance-action-anomalies"
              title="Detect Anomalies"
              desc="Scan revenue series"
              action="Scan"
            />
            <ActionCard
              data-testid="finance-action-ltv"
              title="Update LTV Model"
              desc="Recalculate predictive LTV"
              action="Recalc"
            />
            <ActionCard
              data-testid="finance-action-snapshot"
              title="Revenue Snapshot"
              desc="Force revenue snapshot"
              action="Run"
              onClick={() => void trigger("financeRevenueSnapshot")}
              loading={!!running["financeRevenueSnapshot"]}
              loadingLabel="Running"
            />
            <ActionCard
              data-testid="finance-action-aging"
              title="Aging Digest"
              desc="Queue invoice aging digest"
              action="Run"
              onClick={() => void trigger("financeInvoiceAgingDigest")}
