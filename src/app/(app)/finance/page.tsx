...
import {
  fetchLatestFinanceInvoiceAging,
  fetchRecentFinanceRevenueSnapshots,
} from "@/lib/services/finance-automation-snapshots";
import type { AggregatedFinanceMetrics } from "@/lib/services/finance-metrics.service";
import {
  fetchFinanceMetrics,
  subscribeFinanceMetrics,
} from "@/lib/services/finance-metrics.service";
...
export default function FinanceDashboardRoot() {
  ...
  const userId: string | undefined =
    user &&
    typeof user === "object" &&
    "uid" in (user as unknown as Record<string, unknown>) &&
    typeof (user as unknown as Record<string, unknown>).uid === "string"
      ? ((user as unknown as Record<string, unknown>).uid as string)
      : undefined;
  const teamId =
    user &&
    typeof user === "object" &&
    "teamId" in (user as unknown as Record<string, unknown>) &&
    typeof (user as unknown as Record<string, unknown>).teamId === "string"
  ...
  const [revSnap, setRevSnap] = useState<{
    mrr: number;
    onTime: number;
    outstanding: number;
    period: string;
  ...
  const [agingSnap, setAgingSnap] = useState<{
  ...
  useEffect(() => {
    void trackDashboardView("finance");
  }, []);

  // Persist months selection
  useEffect(() => {
    if (typeof window === "undefined") return;
    const stored = window.localStorage.getItem("financeMonths");
    if (stored) {
      const num = parseInt(stored, 10);
      if ([3, 6, 9, 12].includes(num)) setMonths(num as 3 | 6 | 9 | 12);
    }
  }, []);
  useEffect(() => {
    if (typeof window !== "undefined")
      window.localStorage.setItem("financeMonths", String(months));
  }, [months]);

  interface RevenueSnapshotDoc {
    mrr: number;
    onTimePct: number;
    outstanding: number;
    period: string;
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const resp = await fetch(`/api/finance/metrics?${qs.toString()}`, {
        ...
        if (resp.ok) {
          ...
          if (process.env.NODE_ENV !== "production" && diag)
            console.debug("[finance] server diag:", diag);
          if (active) {
            setMetrics(json as AggregatedFinanceMetrics);
            setInitialLoading(false);
          }
        } else {
          const diag = resp.headers.get("x-finance-diagnostics");
          if (process.env.NODE_ENV !== "production" && diag)
            console.warn("[finance] server diag (non-200):", diag);
    ...
    void (async () => {
      try {
        const [rev, aging] = await Promise.all([
          fetchRecentFinanceRevenueSnapshots(userId, teamId, 1) as Promise<
            RevenueSnapshotDoc[]
          >,
          fetchLatestFinanceInvoiceAging(
            userId,
            teamId
          ) as Promise<AgingSnapshotDoc | null>,
        ]);
        if (rev.length) {
          const r = rev[0];
          const ts =
        ...
        if (aging) {
          const ts =
            aging.createdAt &&
            typeof aging.createdAt === "object" &&
            aging.createdAt.toDate
              ? aging.createdAt.toDate()
              : new Date();
  ...
  function exportSnapshot(format: "json" | "csv") {
    ...
    if (format === "json") {
      ...
      a.href = url;
      a.download = "finance-snapshot.json";
      a.click();
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "finance-snapshot.csv";
    a.click();
    URL.revokeObjectURL(url);
  }

  function runAutomation(
    action: "financeRevenueSnapshot" | "financeInvoiceAgingDigest"
  ) {
    void trigger(action, {
      optimistic:
        action === "financeRevenueSnapshot"
          ? () => {
    ...
    <FeatureGate feature="finance_dashboard" requiredTier="starter" showUpgrade>
      ...
      >
        <SuiteAccentProvider value="finance">
          <DashboardSurface
            as="section"
            aria-label="Finance dashboard surface"
          ...
          >
            <ToolPageHeader
              ...
              badges={[
                ...
                { label: "Financial", variant: "outline" },
              ]}
              showBreadcrumb
            >
              <div className="flex gap-2 flex-wrap">
                ...
                <Button
                  ...
                  className="gap-1"
                  aria-label="Export finance snapshot JSON"
                >
                ...
                <Button
                  ...
                  className="gap-1"
                  aria-label="Export finance snapshot CSV"
                >
                  <DownloadCloud className="h-4 w-4" />
                  CSV
                </Button>
            ...
            {!initialLoading && (!metrics || !metrics.kpis?.length) && (
              <DashboardEmptyState
                ...
                message="Connect your Stripe account above to see your real MRR, ARR, and active customers. We show nothing rather than sample numbers."
              />
            )}
            ...
            {allowFinanceMocks() &&
              (!metrics || !(metrics.kpis && metrics.kpis.length > 0)) && (
                <Alert
                  className="border-warning/30 bg-warning/15 text-warning-foreground dark:bg-warning/20 dark:text-warning-foreground"
                  aria-live="polite"
                  aria-label="Finance mock data banner"
                >
                  <div className="flex items-start gap-3 text-sm">
                    <AlertTriangle className="h-4 w-4 mt-0.5" />
                    <p>
                      Finance metrics are currently served from mock data
                      (FINANCE_MOCK_MODE). This banner disappears once live
                      metrics load or mocks are disabled.
                    </p>
                  </div>
                </Alert>
              )}
            ...
            <div className="sr-only" role="status" aria-live="polite">
              Finance summary: MRR {summary.mrr.toLocaleString()}, churn{" "}
              {summary.churn} percent, LTV {summary.ltv.toLocaleString()}.
            </div>
                  ...
                  <div className="rounded-lg border p-3 bg-background/60 flex items-center justify-between text-xs">
                    <div className="space-y-1">
                      ...
                      <p className="text-muted-foreground">
                        MRR {revSnap.mrr.toLocaleString()} · On-Time{" "}
                        {revSnap.onTime.toFixed(1)}% · Outst.{" "}
                        {revSnap.outstanding}
                      </p>
                    </div>
                    <time
                      className="text-[10px] text-muted-foreground"
                      dateTime={revSnap.ts.toISOString()}
                    >
                      {revSnap.ts.toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </time>
                ...
                {agingSnap && (
                  <div className="rounded-lg border p-3 bg-background/60 flex items-center justify-between text-xs">
                    <div className="space-y-1">
                      ...
                      <p className="text-muted-foreground">
                        0-30 {agingSnap.buckets["0-30"] || 0} · 31-60{" "}
                        {agingSnap.buckets["31-60"] || 0} · 61-90{" "}
                        {agingSnap.buckets["61-90"] || 0} · 90+{" "}
                        {agingSnap.buckets["90+"] || 0}
                      </p>
                    </div>
                    <time
                      className="text-[10px] text-muted-foreground"
                      dateTime={agingSnap.ts.toISOString()}
                    >
                      {agingSnap.ts.toLocaleTimeString([], {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </time>
            ...
            >
              ...
              {!initialLoading &&
                (() => {
                  ...
                  const baseKpis: AggregatedFinanceMetrics["kpis"] =
                    liveKpis && liveKpis.length > 0
                      ? liveKpis
                      : allowFinanceMocks() && mock
                        ? (mock.kpis as AggregatedFinanceMetrics["kpis"])
                        : [];
                      ...
                      <motion.div variants={dashboardItemVariants} key={k.key}>
                        <MetricCard
                          ...
                          badge={
                            k.target ? (
                              <Badge
                                variant={alertState ? "default" : "outline"}
                                className="text-[10px]"
                              >
                                {pctToTarget!.toFixed(0)}% target
                              </Badge>
                            ) : undefined
                          }
                          footer={
                            k.target ? (
                              <AdaptiveProgress
                                value={Math.min(100, pctToTarget!)}
            ...
            <section
              ...
              aria-label="Finance analytics modules"
            >
            ...
            <section
              className="space-y-4"
              aria-label="Finance workbench actions"
            >
              <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">
                Finance Workbench
              </h2>
              <div className="grid gap-4 md:grid-cols-5">
                <ActionCard
                  ...
                  desc="Draft and issue a customer invoice with tax logic."
                  action="Create"
                />
                <ActionCard
                  title="Update Runway"
                  desc="Recalculate runway assumptions based on burn inputs."
                  action="Recalc"
                />
                <ActionCard
                  title="On-Time Breakdown"
                  desc="View on-time payment performance by tier."
                  action="Open"
                  onClick={() => setShowOnTime(true)}
                />
                <ActionCard
                  title="Revenue Snapshot"
                  desc="Force revenue snapshot"
                  action="Run"
                  onClick={() => runAutomation("financeRevenueSnapshot")}
                  loading={!!running["financeRevenueSnapshot"]}
                  loadingLabel="Running"
                />
                <ActionCard
                  title="Aging Digest"
                  desc="Queue invoice aging digest"
                  action="Run"
                  onClick={() => runAutomation("financeInvoiceAgingDigest")}
                  loading={!!running["financeInvoiceAgingDigest"]}
