...
export default function RewriteGenPage() {
  ...
  const [analysisType, setAnalysisType] = useState<
    "comprehensive" | "seo-focused" | "readability" | "conversion"
  >("comprehensive");
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [analysisProgress, setAnalysisProgress] = useState(0);
  const [currentResult, setCurrentResult] = useState<RewriteGenResult | null>(
    null
  );
      ...
      titleSuggestions: [
        {
          ...
          reasoning:
            "Includes target keyword, year relevance, and compelling modifiers",
        ...
        },
        {
          ...
          reasoning:
            "Clear how-to format with specific promise of step-by-step guidance",
          targetKeywords: keywords.slice(0, 1),
        },
      ],
      metaDescriptionSuggestions: [
        {
          ...
          callToAction: true,
          reasoning:
            "Optimal length, includes target keywords, and compelling call-to-action",
        },
        {
      ...
      recommendations: [
        {
          ...
          description:
            "Balance primary keyword usage throughout content for better search visibility",
          examples: [
            "Use target keyword in H1 and H2 tags",
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const result = await simulateRewriteAnalysis(
        content,
        keywords,
        analysisType
      );
  ...
  const metricsData = currentResult
    ? [
        {
          name: "Readability",
          improvement: currentResult.metrics.readabilityImprovement,
        },
        { name: "SEO", improvement: currentResult.metrics.seoOptimization },
        {
          name: "Engagement",
          improvement: currentResult.metrics.engagementBoost,
        },
        {
          name: "Conversion",
          improvement: currentResult.metrics.conversionPotential,
        },
      ]
    : [];
              ...
              {isAnalyzing && (
                ...
                >
                  <div className="flex items-center justify-between text-sm">
                    <span>
                      Analyzing content and generating optimized rewrites...
                    </span>
              ...
              <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                ...
                <TabsContent value="suggestions" className="space-y-4">
                  ...
                  <div className="space-y-4">
                    {currentResult.contentSuggestions.map(
                      (suggestion, index) => (
                        <Card key={index}>
                          <CardContent className="p-6">
                            <div className="space-y-4">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                  <Label className="text-sm font-medium text-destructive-foreground">
                                    Original
                                  </Label>
                                ...
                                <div>
                                  <div className="flex items-center justify-between">
                                    <Label className="text-sm font-medium text-success-foreground">
                                      Suggested
                                    </Label>
                                    <Button
                                      size="sm"
                                  ...
                                  <div className="flex items-center gap-2 text-sm">
                                    ...
                                    >
                                      {suggestion.wordCountChange > 0
                                        ? "+"
                                        : ""}
                                      {suggestion.wordCountChange} words
                                    </span>
                                  </div>
                                )}
                              </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              ...
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <Progress
                                    value={title.readabilityScore}
                                    className="flex-1"
                                  />
                                  <span className="text-sm font-medium">
                                    {title.readabilityScore}/100
                                  </span>
                                </div>
                              </div>
                              <div>
                                ...
                                <div className="flex items-center gap-2 mt-1">
                                  <Progress
                                    value={title.engagementScore}
                                    className="flex-1"
                                  />
                                  <span className="text-sm font-medium">
                                    {title.engagementScore}/100
                                  </span>
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="space-y-2">
                              <div>
                                ...
                                <div className="flex flex-wrap gap-1 mt-1">
                                  {title.targetKeywords.map(
                                    (keyword, keyIndex) => (
                                      <Badge key={keyIndex} variant="outline">
                                        {keyword}
                                      </Badge>
                                    )
                                  )}
                ...
                <TabsContent value="meta" className="space-y-4">
                  <div className="space-y-4">
                    {currentResult.metaDescriptionSuggestions.map(
                      (meta, index) => (
                        <Card key={index}>
                          <CardContent className="p-6">
                            <div className="space-y-4">
                ...
                <TabsContent value="keywords" className="space-y-4">
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      {Object.entries(
                        currentResult.keywordOptimization.keywordDensity
                      ).map(([keyword, data]) => (
                        <div key={keyword} className="space-y-2">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            <Progress
                              value={data.original * 20}
                              className="h-2"
                            />
                            <Progress
                              value={data.suggested * 20}
                              className="h-2"
                            />
                          </div>
                        </div>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className="flex flex-wrap gap-2">
                        {currentResult.keywordOptimization.semanticKeywords.map(
                          (keyword, index) => (
                      ...
                      <Alert className="mt-4">
                        <Lightbulb className="h-4 w-4 text-warning-foreground" />
                        <AlertDescription>
                          Include these semantic keywords naturally throughout
                          your content to improve topical relevance and search
                          visibility.
                        </AlertDescription>
                      </Alert>
                    </CardContent>
