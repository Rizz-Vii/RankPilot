...
export default function TrustBlockPage() {
  ...
  const [currentResult, setCurrentResult] = useState<TrustBlockResult | null>(
    null
  );
  const [selectedTab, setSelectedTab] = useState("overview");

  const simulateTrustAnalysis = async (
    url: string
  ): Promise<TrustBlockResult> => {
    setProvenance(null);
    ...
    const pickStatus = (
      thresholds: number[],
      labels: Array<"present" | "missing" | "weak" | "strong">
    ) => {
      const r = rng();
      for (let i = 0; i < thresholds.length; i++)
        if (r < thresholds[i]) return labels[i];
      ...
      trustSignals: [
        {
          ...
          recommendations: [
            "Add comprehensive author biography",
      ...
      recommendations: [
        {
          ...
          description:
            "Display comprehensive author qualifications and expertise",
        ...
        },
        {
          ...
          description:
            "Include more references to reputable sources and studies",
        ...
        },
        {
          ...
          description:
            "Add clear contact information and organizational details",
  ...
  const radarData = currentResult
    ? [
        {
          subject: "Expertise",
          A: currentResult.eatScore.expertise,
          fullMark: 100,
        },
        {
          subject: "Authoritativeness",
          A: currentResult.eatScore.authoritativeness,
          fullMark: 100,
        },
        {
          subject: "Trustworthiness",
          A: currentResult.eatScore.trustworthiness,
          fullMark: 100,
        },
        {
          subject: "Content Quality",
          A: currentResult.eatScore.breakdown.contentQuality,
          fullMark: 100,
        },
        {
          subject: "Site Reputation",
          A: currentResult.eatScore.breakdown.siteReputation,
          fullMark: 100,
        },
        {
          subject: "Transparency",
          A: currentResult.eatScore.breakdown.transparency,
          fullMark: 100,
        },
      ]
    : [];

  const trustSignalData =
    currentResult?.trustSignals.map((signal) => ({
      name: signal.signal,
      score: signal.score,
      impact:
        signal.impact === "high" ? 100 : signal.impact === "medium" ? 70 : 40,
    })) || [];
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {Object.entries(currentResult.eatScore.breakdown).map(
                          ([key, value]) => (
                            <div key={key}>
                    ...
                    <CardContent className="space-y-6">
                      <div className="flex items-center gap-4 p-4 bg-muted rounded-lg">
                        <div className="w-16 h-16 bg-gradient-to-br from-primary to-accent rounded-full flex items-center justify-center text-primary-foreground font-bold text-xl">
                          {currentResult.authorCredibility.name
                            .split(" ")
                            .map((n) => n[0])
                            .join("")}
                        </div>
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {currentResult.authorCredibility.experience}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2 mt-2">
                            {currentResult.authorCredibility.credentials.map(
                              (credential, index) => (
                                <div
                                  key={index}
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-2 mt-2">
                            {currentResult.authorCredibility.expertiseAreas.map(
                              (area, index) => (
                ...
                <TabsContent value="content" className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <Card>
                      ...
                      <CardContent className="space-y-4">
                        {Object.entries(currentResult.contentQuality).map(
                          ([key, value]) => (
                            <div key={key}>
                              <div className="flex justify-between mb-2">
                                <span className="capitalize">
                    ...
                    <Card>
                      ...
                      <CardContent className="space-y-3">
                        {Object.entries(currentResult.siteCredibility).map(
                          ([key, value]) => (
                            <div
                              key={key}
                    ...
                    {currentResult.recommendations.map((rec, index) => (
                      <Card key={index}>
                        <CardContent className="p-6">
                          <div className="flex items-start gap-4">
                            <div className="flex-shrink-0">
                              {rec.category === "expertise" && (
                                <Brain className="h-6 w-6 text-primary" />
                              )}
                              {rec.category === "authoritativeness" && (
                                <Award className="h-6 w-6 text-accent-foreground" />
                              )}
                              {rec.category === "trustworthiness" && (
