...
interface BriefRow { brief?: { title?: string; primaryKeyword?: string; seoGuidelines?: { targetWordCount?: number }; targetWordCount?: number }; title?: string; primaryKeyword?: string; period?: string }
interface BriefLive { kpis: BriefMetric[]; rows: BriefRow[]; loading: boolean }
...
export default function ContentBriefsPage(): JSX.Element {
  ...
  <section className="space-y-3">
          ...
          <LazyDataTable
            columns={[
              {
                key: 'title', header: 'Title', render: (r: unknown) => {
                  const rec = r as BriefRow;
                  return rec?.brief?.title || rec?.title || '(Untitled)';
                }
              },
              {
                key: 'keyword', header: 'Primary Keyword', render: (r: unknown) => {
                  const rec = r as BriefRow;
                  return rec?.brief?.primaryKeyword || rec?.primaryKeyword || '-';
                }
              },
              {
                key: 'target', header: 'Word Target', render: (r: unknown) => {
                  const rec = r as BriefRow;
                  return rec?.brief?.seoGuidelines?.targetWordCount || rec?.brief?.targetWordCount || '-';
                }
              },
              {
