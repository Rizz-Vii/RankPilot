...
import type { SerpViewInput, SerpViewOutput } from "@/types";
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-3">
          {serpMetrics.kpis.map((k) => (
            <MetricCard
              ...
              key={k.key}
              label={k.label}
              value={Number(k.value).toFixed(1)}
              delta={k.delta}
              deltaLabel=""
        ...
        {(() => {
          ...
          const columns: {
            ...
            },
            {
              ...
              render: (r: Row) =>
                ...
                r.results.some(
                  (x) => typeof x.position === "number" && x.position <= 3
                )
                  ? "Yes"
                  : "No",
            },
          ...
          const rows: Row[] = serpMetrics.rows.map((raw) => {
            ...
            const results = Array.isArray(obj.results)
              ? (obj.results as unknown[]).map((x): Result => {
                  const o =
                    x && typeof x === "object"
                  ...
                  return {
                    position:
                      typeof o.position === "number" ? o.position : undefined,
            ...
            return {
              keyword:
                typeof obj.keyword === "string" ? obj.keyword : undefined,
              results,
