...
export default function SettingsPage() {
  ...
  const { isVoiceSupported, isVoiceEnabled, setIsVoiceEnabled, announcements } =
    useAccessibility();
  const [micStatus, setMicStatus] = useState<
    "idle" | "granted" | "blocked" | "error"
  >("idle");
  ...
  useEffect(() => {
    ...
    const prefsExt = prefs as Partial<ThemePreferences> &
      Record<string, unknown>;
    const initPrefs: Partial<ThemePreferences> & Record<string, unknown> = {};
    if (typeof prefsExt.highContrast === "boolean")
      initPrefs.highContrast = prefsExt.highContrast;
    if (typeof prefsExt.reducedMotion === "boolean")
      initPrefs.reducedMotion = prefsExt.reducedMotion;
    if (typeof prefsExt.fontSize === "string")
      initPrefs.fontSize = prefsExt.fontSize as ThemePreferences["fontSize"];
    if (typeof prefsExt.colorBlindnessSupport === "boolean")
      initPrefs.colorBlindnessSupport = prefsExt.colorBlindnessSupport;
    if (prefsExt.customColors) initPrefs.customColors = prefsExt.customColors;
    if (Object.keys(initPrefs).length)
      setPreferences(initPrefs as Partial<ThemePreferences>);
    ...
    if (typeof prefsExt.voiceCommands === "boolean")
      setIsVoiceEnabled(Boolean(prefsExt.voiceCommands));
  ...
  return (
    <div className={`max-w-6xl mx-auto space-y-8 ${isRTL ? "rtl" : "ltr"}`}>
      ...
      <ToolPageHeader
        ...
        badges={[
          { label: tr("common.secure", "Secure"), variant: "outline" },
          { label: "User", variant: "secondary" },
        ]}
        showBreadcrumb
      />
      <div role="status" aria-live="polite" className="sr-only">
        {loadingState
          ? tr("status.loading", "Loading settings…")
          : tr("status.ready", "Settings ready")}
      </div>
      <Tabs
        value={activeTab}
        onValueChange={(v) => {
        ...
        className="space-y-6"
        aria-label={tr("settings.tabs.label", "Account settings sections")}
      >
        <TabsList className="grid w-full grid-cols-8 lg:grid-cols-8">
          <TabsTrigger
            value="account"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.account", "Account")}
          >
            <User className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.account", "Account")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="appearance"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.theme", "Theme")}
          >
            <Palette className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.theme", "Theme")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="accessibility"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.accessibility", "Accessibility")}
          >
            <Accessibility className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.accessibilityShort", "A11y")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="language"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.language", "Language")}
          >
            <Globe className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.language", "Language")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="security"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.security", "Security")}
          >
            <Shield className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.security", "Security")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="notifications"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.notifications", "Notifications")}
          >
            <Bell className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.notifications", "Notifications")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="billing"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.billing", "Billing")}
          >
            <CreditCard className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.billing", "Billing")}
            </span>
          </TabsTrigger>
          <TabsTrigger
            value="privacy"
            className="flex items-center gap-2"
            aria-label={tr("settings.tabs.privacy", "Privacy")}
          >
            <Lock className="h-4 w-4" />
            <span className="hidden sm:inline">
              {tr("settings.tabs.privacy", "Privacy")}
            </span>
          </TabsTrigger>
        </TabsList>
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Switch
                      ...
                      onCheckedChange={(c) => {
                        setPreferences({ reducedMotion: c });
                      }}
                    />
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {micStatus !== "idle" && (
                        ...
                        >
                          {micStatus === "granted"
                            ? tr("settings.accessibility.micGranted", "Granted")
                            : tr(
                    ...
                    {announcements.length === 0 ? (
                      ...
                      announcements.map((a) => (
                        <div key={a.id} className="mb-2 last:mb-0">
                          <span
                            className={`inline-block w-2 h-2 rounded-full mr-2 ${a.priority === "assertive" ? "bg-destructive" : "bg-primary"}`}
                          />
                          {a.message}
                        </div>
                      ))
          ...
          <Card>
            ...
            <CardContent className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  <h4 className="font-medium">
                    {tr("settings.language.info", "Language Information")}
                  </h4>
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      <span>
                        {tr("settings.language.current", "Current Language:")}
                      </span>
                    ...
                    <div className="flex justify-between">
                      <span>
                        {tr("settings.language.direction", "Text Direction:")}
                      </span>
                      <span>{isRTL ? "Right-to-Left" : "Left-to-Right"}</span>
                    </div>
                    <div className="flex justify-between">
                      <span>
                        {tr("settings.language.sampleNumber", "Sample Number:")}
                      </span>
                    ...
                    <div className="flex justify-between">
                      <span>
                        {tr("settings.language.sampleDate", "Sample Date:")}
                      </span>
        ...
        <TabsContent value="privacy" className="space-y-6">
          ...
          {!loadingState &&
            Boolean(
              profileData?.lastExportAt || profileData?.deletionRequestedAt
            ) && (
              <Card>
                <CardHeader>
                  <CardTitle className="text-sm">
                    {tr("settings.privacy.auditTrail", "Privacy Audit Trail")}
                  </CardTitle>
                  <CardDescription>
                    {tr(
                      "settings.privacy.auditTrailDesc",
                      "Recent privacy-related account actions"
                    )}
                  </CardDescription>
                </CardHeader>
                <CardContent className="text-sm space-y-2">
                  {Boolean(profileData?.lastExportAt) && (
                    <div className="flex justify-between">
                      <span>
                        {tr("settings.privacy.lastExport", "Last Data Export")}:
                      </span>
                      <span>
                  ...
                  {Boolean(profileData?.deletionRequestedAt) && (
                    <div className="flex flex-col gap-1">
                      <div className="flex justify-between">
                        ...
                        <span>
                          {formatRelative(
                            toJsDate(profileData.deletionRequestedAt)
                          )}
                        </span>
                      </div>
                      <Button
                        ...
                        onClick={() => {
                          void (async () => {
                            const confirmMsg = tr(
                              ...
                              "Cancel scheduled deletion and keep your account?"
                            );
              ...
              <CardHeader>
                ...
                <CardDescription>
                  {tr(
                    ...
                    "Your account is pending deletion. Contact support to cancel."
                  )}
                </CardDescription>
              </CardHeader>
            </Card>
