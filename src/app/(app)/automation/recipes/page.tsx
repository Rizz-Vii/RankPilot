...
const ACTIONS: { id: string; label: string; desc: string }[] = [
  {
    id: "runNeuroSEOAnalysis",
    label: "Run NeuroSEO Analysis",
    desc: "Executes a fresh analysis with placeholder inputs",
  },
  {
    id: "sendDigestEmail",
    label: "Send Digest Email",
    desc: "Enqueue generic digest email",
  },
  {
    id: "generateContentRewrite",
    label: "Generate Content Rewrite",
    desc: "Create rewrite variants for up to 3 URLs",
  },
  {
    id: "salesRefreshMetrics",
    label: "Sales: Refresh Metrics Snapshot",
    desc: "Store pipeline + win-rate snapshot",
  },
  {
    id: "salesForecastSnapshot",
    label: "Sales: Forecast Snapshot",
    desc: "Persist lightweight forecast (pipeline * 0.8)",
  },
  {
    id: "salesPipelineDigest",
    label: "Sales: Pipeline Digest Email",
    desc: "Email current pipeline / win-rate",
  },
  {
    id: "financeRevenueSnapshot",
    label: "Finance: Revenue Snapshot",
    desc: "Store monthly revenue KPIs",
  },
  {
    id: "financeInvoiceAgingDigest",
    label: "Finance: Invoice Aging Digest",
    desc: "Email counts by aging bucket",
  },
];
...
export default function AutomationRecipesPage() {
  ...
  const [salesDigestTo, setSalesDigestTo] = useState<string>(
    "sales-digest@example.com"
  );
  const [financeAgingTo, setFinanceAgingTo] = useState<string>(
  ...
  const toggleActive = async (r: AutomationRecipe) => {
    try {
      const updated = await updateAutomationRecipe(r.id!, {
        active: !r.active,
      });
      ...
      <div className="space-y-6 p-4 md:p-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Schedule recurring AI-driven operations.
          </p>
        </div>
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="Weekly NeuroSEO Digest"
            ...
            <div className="space-y-3">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  disabled={hydrationDisabled || !!cronExpr.trim()}
                />
                <p className="text-xs text-muted-foreground">
                  1440 = daily, 10080 = weekly. Disabled if cron is set.
                </p>
              </div>
              <div className="space-y-2">
                <Label>Cron Expression (optional)</Label>
                <Input
                  placeholder="m h * * * or @daily"
                  value={cronExpr}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                ...
                />
                <p className="text-xs text-muted-foreground">
                  Supports m h * * * plus @daily / @hourly. Use instead of
                  interval.
                </p>
              </div>
              {validationError && (
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {ACTIONS.map((a) => {
                  ...
                  return (
                    <label
                      key={a.id}
                      className="flex items-start space-x-2 rounded border p-3 cursor-pointer hover:bg-muted/30"
                    >
                      <input
                        type="checkbox"
                        checked={checked}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={(e: ChangeEvent<HTMLTextAreaElement>) =>
                    setAnalysisUrls(e.target.value)
                  }
                  rows={3}
                  disabled={hydrationDisabled}
                />
            ...
            {selectedActions.includes("sendDigestEmail") && (
              <div className="space-y-2">
                ...
                <Input
                  value={digestSubject}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    setDigestSubject(e.target.value)
                  }
                  disabled={hydrationDisabled}
                />
              </div>
            ...
            {selectedActions.includes("generateContentRewrite") && (
              <div className="space-y-2 text-xs text-muted-foreground">
                <p>
                  Will generate rewrite variants for up to 3 URLs (placeholder
                  content) using provided keywords.
                </p>
              </div>
            )}
            {(selectedActions.includes("salesRefreshMetrics") ||
              ...
              <div className="space-y-2">
                ...
                <select
                  ...
                  onChange={(e: ChangeEvent<HTMLSelectElement>) =>
                    setRange(e.target.value as "30d" | "90d" | "ytd")
                  }
                >
            ...
            {selectedActions.includes("salesPipelineDigest") && (
              <div className="space-y-2">
                ...
                <Input
                  value={salesDigestTo}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    setSalesDigestTo(e.target.value)
                  }
                  disabled={hydrationDisabled}
                />
              </div>
            ...
            {selectedActions.includes("financeInvoiceAgingDigest") && (
              <div className="space-y-2">
                ...
                <Input
                  value={financeAgingTo}
                  onChange={(e: ChangeEvent<HTMLInputElement>) =>
                    setFinanceAgingTo(e.target.value)
                  }
                  disabled={hydrationDisabled}
                />
              </div>
            )}
            <Button onClick={() => void create()} disabled={hydrationDisabled}>
              Create
            </Button>
          </CardContent>
        </Card>
              ...
              return (
                <Card key={r.id} className="flex flex-col">
                  <CardHeader className="pb-2">
                    ...
                    <CardDescription className="text-xs">
                      {r.schedule.cron
                        ...
                        : r.schedule.intervalMinutes
                          ? `${r.schedule.intervalMinutes}m interval`
                          : r.schedule.atHourUTC !== undefined
                            ? `Daily @ ${r.schedule.atHourUTC}:00 UTC`
                            : "Custom"}
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="text-xs text-muted-foreground space-y-2 flex-1">
                    ...
                    <div>
                      Last Run: {r.lastRun ? r.lastRun.toLocaleString() : "—"}
                    </div>
                    <div>
                      Next Run: {r.nextRun ? r.nextRun.toLocaleString() : "—"}
                    </div>
                    ...
                    {logs.length > 0 && (
                      <div className="border-t pt-2 space-y-1">
                        ...
                        {logs.map((l) => (
                          <div
                            key={l.id}
                            className="flex flex-col border rounded p-1 bg-muted/30"
                          >
