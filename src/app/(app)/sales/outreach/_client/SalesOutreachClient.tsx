...
export function SalesOutreachClient() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const snapsRaw = await fetchRecentSalesMetricsSnapshots(
          userId,
          teamId,
          6
        );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        try {
          res = await fetch("/api/sales/executions/recent", {
          ...
          if (process.env.NODE_ENV !== "production") {
            try {
              res = await fetch("/api/sales/executions/recent", {
                headers: {
                  "x-probe-token":
                    process.env.NEXT_PUBLIC_CRAWL_PROBE_TOKEN || "dev",
                },
              });
            } catch {
              /* ignore */
        ...
        const items: Array<{
          ...
          startedAt?: unknown;
        }> = Array.isArray(json.executions) ? json.executions : [];
        const toDate = (v: unknown) =>
          v &&
          typeof v === "object" &&
          "toDate" in (v as Record<string, unknown>) &&
          typeof (v as { toDate?: () => unknown }).toDate === "function"
            ? ((v as { toDate: () => unknown }).toDate() as Date)
            : new Date();
        const mapped = items.map((x) => ({
          attempted: Number(x?.stats?.attempted || 0),
          succeeded: Number(x?.stats?.succeeded || 0),
          failed: Number(x?.stats?.failed || 0),
          ts: toDate(x?.startedAt),
        }));
        setExecSummary(mapped);
      } catch (e) {
        getLogger("sales.outreach.page").degraded("execSummary.load_failed", {
  ...
  function runRefresh() {
    void trigger("salesRefreshMetrics", {
      optimistic: () =>
    ...
    <div className="space-y-10">
      ...
      <section className="rounded-lg border p-4">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Configure voice, tone, speed, script, recipients, and caller ID.
        </p>
        <OutreachForm />
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 md:grid-cols-5">
          <ActionCard
            title="New Sequence"
            desc="AI-personalized multi-step"
            action={() => setSeqOpen(true)}
            label="Create"
          />
          <ActionCard
            title="Optimize Copy"
          ...
          <ActionCard
            ...
            action={runRefresh}
            label={running["salesRefreshMetrics"] ? "Refreshing…" : "Run"}
            disabled={!!running["salesRefreshMetrics"]}
          />
        </div>
      </section>
            ...
            {execSummary.map((x, i) => (
              <div key={i} className="rounded-md border p-3 text-xs">
                ...
                <div className="text-muted-foreground mt-1">
                  Attempted {x.attempted} · Succeeded {x.succeeded} · Failed{" "}
                  {x.failed}
                </div>
                <time
                  className="text-[10px] text-muted-foreground"
                  dateTime={x.ts.toISOString()}
                >
                  {x.ts.toLocaleString()}
                </time>
              </div>
