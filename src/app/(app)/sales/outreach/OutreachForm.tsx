...
export function OutreachForm() {
  ...
  const [pitch, setPitch] = useState(
    "Quick check-in about your RankPilot SEO campaign."
  );
  ...
  const [audioSource, setAudioSource] = useState<"tts" | "upload" | "record">(
    "tts"
  );
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const def: string =
          typeof j?.defaultFrom === "string" ? j.defaultFrom : "";
        if (!active) return;
  ...
  const scriptTemplates = useMemo(() => {
    const commonCTA =
      "If now is not ideal, feel free to reply with a better time.";
    return [
      {
        key: "intro",
        ...
        (snap) => {
          ...
          const entry: CallLive = {
            ...
            recording:
              ...
              typeof raw === "object" &&
              "recording" in raw &&
              typeof (raw as { recording?: unknown }).recording !== "undefined"
                ? ((
                    raw as {
                      recording?: {
                        url?: string | null;
  ...
  useEffect(() => {
    return () => {
      Object.values(callUnsubsRef.current).forEach((u) => {
        try {
          u();
        } catch {}
      });
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      if (invalidPhones.length) {
        setError(
          `Fix ${invalidPhones.length} invalid phone ${invalidPhones.length === 1 ? "number" : "numbers"} (E.164 like +15551234567). For example: ${invalidPhones[0]}`
        );
        setBusy(false);
        return;
      }
      ...
      if (!Number.isFinite(when) || when < now - 30_000) {
        ...
        setSchedule(fixed);
        toast.message("Schedule adjusted", {
          description: "Time was in the past. Set to 1 minute from now.",
        });
        setBusy(false);
        return;
      }
      ...
      if (!arr.length) {
        ...
        toast.message("Call scheduled", {
          description:
            "We queued your call for the selected time. Live updates will appear once it starts.",
        });
      }
    } catch (e: unknown) {
  ...
  async function processToWav(
    blob: Blob,
    opts: { normalize: boolean; sampleRate: number }
  ): Promise<Blob> {
  ...
  async function handleFileSelected(file: File) {
    try {
      const processed = optimizeRecording
        ? await processToWav(file, { normalize: true, sampleRate: 16000 })
        : file;
      ...
      toast.success("Audio uploaded", {
        description: `${derivedExt.toUpperCase()} | ${url.split("/").pop()}`,
      });
    } catch (e: unknown) {
      setError(toMessage(e) || "Failed to process/upload recording");
    }
  }
    ...
    try {
      ...
      mr.onstop = async () => {
        try {
          ...
          const processed = optimizeRecording
            ? await processToWav(raw, { normalize: true, sampleRate: 16000 })
            : raw;
          ...
          setError(toMessage(e) || "Recording upload failed");
        }
      };
      ...
      setError(toMessage(e) || "Microphone permission denied");
    }
  }
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <Textarea
            data-testid="outreach-recipients"
            className="h-28"
          ...
          <div className="flex items-center justify-between mt-1">
            <p className="text-xs text-muted-foreground">
              Use international E.164 format (e.g. +15551234567). One per line
              is okay.
            </p>
        ...
        <div>
          ...
          <Textarea
            data-testid="outreach-script"
            className="h-28"
            value={pitch}
            onChange={(e) => setPitch(e.target.value)}
          />
          <p className="text-xs text-muted-foreground mt-1">
            Spoken as a short greeting at call start. Keep it concise and clear.
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {[
              ...
              <Button
                ...
                onClick={() =>
                  setSchedule(
                    toLocalDatetimeValue(new Date(Date.now() + opt.addMs))
                  )
                }
              >
                {opt.label}
          ...
          <div className="mt-2">
            ...
            <Select
              onValueChange={(v) => {
                ...
                if (add)
                  setSchedule(toLocalDatetimeValue(new Date(Date.now() + add)));
              }}
            >
              <SelectTrigger>
          ...
          <div className="mt-3">
            ...
            <Select
              value={repeat}
              onValueChange={(v) =>
                setRepeat((v as "none" | "daily" | "weekly") || "none")
              }
            >
              <SelectTrigger>
                <SelectValue />
            ...
            {repeat !== "none" && (
              <p className="text-[11px] text-muted-foreground mt-1">
                Recurring scheduling is queued; first call will be placed at the
                selected time.
              </p>
            )}
          </div>
        ...
        <div>
          ...
          {fromOptions.length > 0 ? (
            <Select
              value={fromNum || AUTO_VALUE}
              onValueChange={(v) => setFromNum(v === AUTO_VALUE ? "" : v)}
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={`Auto (default ${defaultFrom || "Twilio"})`}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value={AUTO_VALUE}>
                  Auto (default {defaultFrom || "Twilio"})
                </SelectItem>
                {fromOptions.map((n) => (
          ...
          <p className="text-xs text-muted-foreground mt-1">
            Caller ID must be configured and allowed.
          </p>
        </div>
        ...
        <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            ...
            <Select
              value={audioSource}
              onValueChange={(v) =>
                setAudioSource(v as "tts" | "upload" | "record")
              }
            >
              <SelectTrigger>
                <SelectValue />
            ...
            <p className="text-[11px] text-muted-foreground mt-1">
              Tip: Upload or record to use a custom voice for the opening
              message.
            </p>
            <div className="flex flex-col gap-2 mt-2">
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Checkbox
                  id="useRecordingOnly"
                  checked={useRecordingOnly}
                  onCheckedChange={(v) => setUseRecordingOnly(Boolean(v))}
                />
                <Label htmlFor="useRecordingOnly" className="text-xs">
                  Use recording only (ignore script)
                </Label>
              </div>
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Checkbox
                  id="optimizeRecording"
                  checked={optimizeRecording}
                  onCheckedChange={(v) => setOptimizeRecording(Boolean(v))}
                />
                <Label htmlFor="optimizeRecording" className="text-xs">
                  Optimize recording (normalize volume)
                </Label>
              </div>
            </div>
          </div>
              ...
              {(uploading || recordingUrl) && (
                <p className="text-xs text-muted-foreground mt-1 break-all">
                  {uploading ? (
                    ...
                    <>
                      Uploaded: {recordingUrl}{" "}
                      {recordingType && (
                        <span className="ml-2 text-[10px] px-1 py-0.5 rounded bg-muted-foreground/10">
                          {recordingType}
                        </span>
                      )}
                    </>
          ...
          <div>
            <Label className="mb-1 block">
              Manual Recording URL (optional)
            </Label>
            <Input
              data-testid="outreach-recording-url"
              type="url"
            ...
            />
            <p className="text-xs text-muted-foreground mt-1">
              If provided, this audio plays instead of the script (takes
              precedence).
            </p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
        <Checkbox
          id="interactive"
          data-testid="outreach-interactive"
          checked={interactive}
          onCheckedChange={(v) => setInteractive(Boolean(v))}
        />
        <Label htmlFor="interactive" className="text-sm">
          Enable interactive prompt (collect keypad/speech after greeting)
        </Label>
      </div>
      {interactive && (
        <p className="text-[11px] text-muted-foreground -mt-2 mb-2">
          Calls will use a brief Gather prompt to capture a response before
          continuing.
        </p>
      )}
      ...
      <div className="flex gap-3">
        <Button
          data-testid="outreach-submit"
          disabled={busy}
          onClick={() => void submit()}
        >
          {busy ? "Sending..." : "Send Calls"}
        </Button>
      </div>
              ...
              result.results.map((r, i) => (
                <div key={i} className="rounded-md border p-3 text-xs">
                  ...
                  <div className="text-muted-foreground mt-1">
                    From {r.from || "—"} · Status {r.callStatus || "—"}
                  </div>
                  {r.callSid && (
                    <div className="mt-1">
        ...
        <div className="mt-4">
              ...
              <div key={c.id} className="border-b border-border/50 py-1">
                ...
                {c.lastEventAt && (
                  <div className="text-muted-foreground">
                    Updated {new Date(c.lastEventAt).toLocaleString()}
                  </div>
                )}
                {(() => {
                  type WithGather = CallLive & {
                    gather?: { digits?: string; speechResult?: string };
                  };
          ...
          {!listenAllowed && (
            <div className="text-[11px] text-muted-foreground mt-2">
              Listening disabled in dev (security rules). Status will not
              live-update.
            </div>
          )}
        </div>
