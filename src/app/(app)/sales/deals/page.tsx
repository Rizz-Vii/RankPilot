...
export default function SalesDealsPage() {
  ...
  useEffect(() => {
    ...
    async function loadSnapshots() {
      try {
        ...
        const m = (await fetchRecentSalesMetricsSnapshots(userId, teamIdLocal, 6)) as RawMetricsSnapshot[];
        if (!active) return;
        if (m.length) {
          const first = m[0];
  ...
  const metricsSnapshot = (!loadingSnap && snapMetrics) ? (
      ...
      <div className="p-6 space-y-10">
        <AddDealModal
          ...
          onCreated={(d: { amount?: number } | unknown) =>
            setSnapMetrics((s) => (s ? { ...s, deals: (s.deals || 0) + 1, pipeline: s.pipeline + ((d as { amount?: number })?.amount || 0) } : s))
          }
        />
        <ToolPageHeader
          title="Deals"
