...
export function NewSequenceModal({ open, onOpenChange, onCreated }: Props) {
  ...
  const [steps, setSteps] = useState('Hi {{firstName}},\nQuick note about...');
  const [saving, setSaving] = useState(false);

  async function save() {
    if(!user) return; if(!name.trim()) { toast({ title:'Name required', variant:'destructive'}); return; }
    setSaving(true);
    try {
      const seq = { name: name.trim(), steps, userId: user.uid, teamId: (user as any)?.teamId || null, createdAt: serverTimestamp(), updatedAt: serverTimestamp() };
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="space-y-4 py-2">
          <div className="space-y-1">
            ...
            <Input value={name} onChange={e=> setName(e.target.value)} placeholder="Outbound - Q4 Test" autoFocus />
          </div>
          <div className="space-y-1">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            <Button size="sm" onClick={save} disabled={saving}>{saving? 'Saving...':'Create Sequence'}</Button>
          </div>
        </div>
      </DialogContent>
    </Dialog>
