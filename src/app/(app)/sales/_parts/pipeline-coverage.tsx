    ...
    <div className="rounded-xl border p-4 bg-gradient-to-br from-background to-muted/30 h-[260px] flex flex-col">
      ...
      <div className="flex-1 flex flex-col items-center justify-center">
        <svg
          ...
          height={120}
          role="img"
          aria-label={`Pipeline coverage ${ratio.toFixed(1)} times target`}
        >
          <g transform="translate(90,100)">
            ...
            <circle
              ...
              transform="rotate(-90)"
            />
            <text textAnchor="middle" y={-10} fontSize={20} fontWeight={600}>
              {ratio.toFixed(1)}x
            </text>
            <text
              textAnchor="middle"
              y={12}
              fontSize={10}
              className="uppercase tracking-wide fill-[hsl(var(--muted-foreground))]"
            >
              Coverage
            </text>
          </g>
        ...
        <p className="text-[11px] text-muted-foreground mt-2">
          {coverage.pipeline.toLocaleString()} /{" "}
          {coverage.target.toLocaleString()} target
        </p>
      </div>
      <p className="mt-1 text-[11px] text-muted-foreground">
        Goal ≥ 3x; arc scales until 3x.
      </p>
    </div>
  );
}
