...
function CampaignDialog({
  ...
  const [body, setBody] = useState(
    "We are excited to share new growth levers..."
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="w-full max-w-xl rounded-xl border bg-card p-6 space-y-4 shadow-xl relative">
        ...
        <header className="space-y-1">
          <h3 className="font-semibold text-lg">Create Email Campaign</h3>
          <p className="text-xs text-muted-foreground">
            Define audience & schedule.
          </p>
        </header>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-xl border bg-card p-6 space-y-4 shadow-xl relative">
        <header className="space-y-1">
          <h3 className="font-semibold text-lg">Subject Variants</h3>
          <p className="text-xs text-muted-foreground">
            Generated AI subject suggestions.
          </p>
        </header>
...
export default function EmailCampaignsPage() {
  ...
  const [lastSubject, setLastSubject] = useState(
    "Unlock new SEO growth insights"
  );
  ...
  async function handleCreate(
    ...
    try {
      ...
      toast({
        ...
        description: `Impr: ${res.impressions} Leads:${res.leads}`,
      });
      ...
      <div className="space-y-8 p-6">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Sequence generation, deliverability & engagement optimization (AI
            optimizer upcoming).
          </p>
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <ActionCard
              ...
              desc="Start new multi-touch sequence."
              action={() => setCampOpen(true)}
              label={busy === "create" ? "Creating…" : "Start"}
              disabled={!!busy}
            />
            <ActionCard
              title="Optimize Send Time"
              desc="Run AI timing model."
              action={() => {
                void handleOptimize();
              }}
              label={busy === "opt" ? "Optimizing…" : "Optimize"}
              disabled={!!busy}
            />
            <ActionCard
              title="Generate Content"
              desc="Produce variant subject lines."
              action={openVariants}
              label="Generate"
              disabled={!!busy}
            />
          ...
          {sendTime && (
            <p className="text-[11px] text-muted-foreground">
              Suggested send window: {sendTime}
            </p>
          )}
        </section>
        <CampaignDialog
