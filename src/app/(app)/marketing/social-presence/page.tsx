...
interface PostDialogProps {
  open: boolean;
  onClose: () => void;
  onSchedule: (c: string, channel: string, when: Date) => Promise<void>;
  loading: boolean;
}
function PostDialog({ open, onClose, onSchedule, loading }: PostDialogProps) {
  const [content, setContent] = useState(
    "Announcing our new AI-driven optimization suite."
  );
      ...
      <div className="w-full max-w-lg rounded-xl border bg-card p-6 space-y-4 shadow-xl relative">
        ...
        <header className="space-y-1">
          ...
          <p className="text-xs text-muted-foreground">
            Create AI-instrumented social post across channels.
          </p>
        </header>
        <div className="space-y-2">
          <label className="text-xs font-medium flex flex-col gap-1">
            Channel
            <select
              className="bg-background border rounded-md h-9 px-2 text-sm"
              value={channel}
...
function OptimizeDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-xl border bg-card p-6 space-y-4 shadow-xl relative">
        <header className="space-y-1">
          <h3 className="font-semibold text-lg">Optimized Copy</h3>
          <p className="text-xs text-muted-foreground">
            AI-enhanced based on platform heuristics.
          </p>
        </header>
        <Textarea
          value={variant}
...
interface AccountsPanelProps {
  ...
  onConnect: (p: string, h: string) => Promise<void>;
  connecting: boolean;
}
    ...
    <div className="rounded-xl border p-4 space-y-3">
      ...
      <ul className="space-y-1 max-h-32 overflow-auto text-xs">
        {accounts.map((a) => (
          <li
            key={a.id}
            className="flex items-center justify-between rounded bg-background/60 px-2 py-1 border text-[11px]"
          >
            <span>
              {a.platform}:{a.handle}
            </span>
      ...
      <div className="flex gap-2 items-end">
        <select
          className="bg-background border rounded-md h-8 px-2 text-xs"
          value={platform}
          onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
        ...
        <Button
          ...
          onClick={() => {
            void onConnect(platform, handle);
          }}
        >
...
export default function SocialPresencePage() {
  ...
  const data:
    | CampaignMetrics
    | {
        kpis: NonNullable<typeof mock>["kpis"];
        quotas: NonNullable<typeof mock>["quotas"];
        rows: unknown[];
        loading: boolean;
      } = live.kpis.length
    ? live
  ...
  const [optOriginal, _setOptOriginal] = useState(
  ...
  useEffect(() => {
    void (async () => {
      const list = await listSocialAccounts(userId, teamId);
      setAccounts(list);
    })();
  }, [userId, teamId]);
  ...
  async function handleSchedule(content: string, channel: string, when: Date) {
    try {
      ...
      } as unknown as Parameters<typeof live.addOptimistic>[0]);
      await schedulePost({
        content,
        channel,
        scheduledAt: when,
      ...
      toast({
        ...
        description: `${channel} post queued.`,
      });
      setPostOpen(false);
    } catch (e) {
      const err =
  ...
  async function handleConnect(p: string, h: string) {
    try {
      setConnecting(true);
      const res = await connectSocialAccount(
        p as unknown as "linkedin" | "instagram" | "facebook" | "x",
        h,
        userId,
        teamId
      );
      toast({
        ...
        description: `${res.platform}:${res.handle}`,
      });
      const list = await listSocialAccounts(userId, teamId);
      setAccounts(list);
    } catch (e) {
  ...
  async function handleAnalyze() {
    try {
      setBusy("analyze");
      const t = await fetchPlatformTrends("linkedin");
      setTrends(t);
      toast({
        title: "Trends fetched",
        description: t.hashtags.slice(0, 3).join(" "),
      });
    } catch (e) {
      const err =
        e && typeof e === "object" && "message" in e
      ...
      <div className="space-y-8 p-6">
        <header className="space-y-2">
          ...
          <p className="text-muted-foreground max-w-3xl">
            Multi-channel scheduling, engagement velocity & AI content
            optimization (pipeline forthcoming).
          </p>
        ...
        <AccountsPanel
          accounts={accounts}
          onConnect={(p, h) => handleConnect(p, h)}
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <ActionCard
              title="Optimize Copy"
              desc="AI adjust for channel tone."
              action={() => {
                handleOptimize();
              }}
              label="Optimize"
            ...
            <ActionCard
              ...
              desc="Fetch latest engagement trend."
              action={() => {
                void handleAnalyze();
              }}
              label={busy === "analyze" ? "Analyzing…" : "Analyze"}
              disabled={!!busy}
            />
          </div>
          {trends && (
            <div
              className="text-xs rounded-md border p-3 bg-background/40 flex flex-wrap gap-2"
              aria-live="polite"
            >
              {trends.hashtags.map((h) => (
                <span
                  key={h}
