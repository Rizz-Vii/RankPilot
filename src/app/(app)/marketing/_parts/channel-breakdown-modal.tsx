  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg">
        <DialogHeader><DialogTitle>Channel Breakdown</DialogTitle></DialogHeader>
        <table className="w-full text-xs">
          ...
          <tbody>
            {rows.map(r=> (
              <tr key={r.channel} className="border-b border-border/40 last:border-none">
                <td className="py-1 pr-2">{r.channel}</td>
                <td className="py-1 pr-2 tabular-nums">{r.impressions.toLocaleString()}</td>
                <td className="py-1 pr-2 tabular-nums">{r.leads.toLocaleString()}</td>
                <td className="py-1 pr-2 tabular-nums">{r.roi.toFixed(1)}</td>
              </tr>
            ))}
            {!rows.length && (<tr><td colSpan={4} className="py-4 text-center text-muted-foreground">No data</td></tr>)}
          </tbody>
        </table>
      </DialogContent>
    </Dialog>
