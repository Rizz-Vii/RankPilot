  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader><DialogTitle>Campaign Details ({rows.length})</DialogTitle></DialogHeader>
        <ScrollArea className="h-[420px]">
          <table className="w-full text-xs">
            ...
            <tbody>
              {rows.map(r=> { const ctr = (r.impressions? (r.clicks||0)/r.impressions*100:0); const roi = (r.spend? ((r.revenue||0) - (r.spend||0))/(r.spend||0)*100:0); return (
                <tr key={r.id} className="border-b border-border/40 last:border-none">
                  ...
                  <td className="py-1 pr-2 tabular-nums">{(r.impressions||0).toLocaleString()}</td>
                  <td className="py-1 pr-2 tabular-nums">{ctr.toFixed(2)}</td>
                  <td className="py-1 pr-2 tabular-nums">{(r.leads||0).toLocaleString()}</td>
                  <td className="py-1 pr-2 tabular-nums">{roi.toFixed(1)}</td>
                </tr> ); })}
              {!rows.length && (<tr><td colSpan={6} className="py-4 text-center text-muted-foreground">No campaigns</td></tr>)}
            </tbody>
          </table>
        </ScrollArea>
      </DialogContent>
