...
type Snapshot = {
  ...
  hints?: {
    ...
    aiProvenanceCoveragePct: number;
  };
};
...
export default function BIDashboardPage() {
  ...
  useEffect(() => {
    ...
    setSelectedRoute((prev) =>
      prev && data.unified?.latency?.[prev] ? prev : top[0]
    );
  }, [data]);
      ...
      void (async () => {
        try {
          const res = await fetch(
            `/api/bi/snapshot${selectedRoute ? `?route=${encodeURIComponent(selectedRoute)}` : ""}`,
            { cache: "no-store" }
          );
          if (!res.ok) return;
    ...
    const tick = () => {
      void (async () => {
        try {
          ...
          const json: Snapshot = await res.json();
          if (canceled) return;
          const fallbacks = json.unified?.fallbackReasons
            ? Object.values(json.unified.fallbackReasons).reduce(
                (a, b) => a + (b || 0),
                0
              )
            : 0;
          const rl = json.unified?.rateLimitRejections
            ? Object.values(json.unified.rateLimitRejections).reduce(
                (a, b) => a + (b || 0),
                0
              )
            : 0;
      ...
      <div className="min-h-[50vh] grid place-items-center p-6">
        <div className="text-center">
          ...
          <p className="text-muted-foreground">
            This internal dashboard is disabled in production.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="p-6 space-y-6">
        ...
        <div className="flex items-center gap-3 text-sm">
          <div className="flex items-center gap-1">
            ...
            <select
              ...
              value={kind}
              onChange={(e) =>
                setKind(e.target.value as "all" | "latency" | "finance")
              }
            >
          ...
          <a
            className="underline"
            href={`/api/bi/export?format=csv&kind=${kind}${kind === "latency" && selectedRoute ? `&route=${encodeURIComponent(selectedRoute)}` : ""}`}
            target="_blank"
            rel="noreferrer"
          >
            Export CSV
          ...
          <a
            className="underline"
            href="/api/bi/export?format=csv&kind=ops"
            target="_blank"
            rel="noreferrer"
          >
            Export OPS CSV
        ...
        <div className="grid gap-6 md:grid-cols-3">
          <div className="rounded border p-4">
            ...
            <div className="text-2xl font-medium">
              {data.hints?.queueDepth ?? data.unified?.queue?.depth ?? 0}
            </div>
          </div>
          <div className="rounded border p-4">
            <div className="text-sm text-gray-500">Rate-limit rejections</div>
            <div className="text-2xl font-medium">
              {data.hints?.rateLimitRejections ?? 0}
            </div>
          </div>
          <div className="rounded border p-4">
            <div className="text-sm text-gray-500">AI provenance coverage</div>
            <div className="text-2xl font-medium">
              {data.hints?.aiProvenanceCoveragePct ??
                data.unified?.aiResponses?.coveragePct ??
                0}
              %
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            <div className="mt-6 rounded border p-4">
              ...
              <div className="text-xs text-gray-600 mb-2">
                Last ~4 minutes (10s interval) • ms
              </div>
              ...
              {trend.length > 0 && (
                <div className="mt-2 text-xs text-gray-600">
                  Forecast (EWMA): next ≈ {ewmaForecast(trend)} ms
                </div>
              )}
              {/* p99 companion */}
              <div className="mt-4">
              ...
              <div className="mt-2 text-xs">
                <a
                  className="underline"
                  href={`/api/bi/timeseries${selectedRoute ? `?route=${encodeURIComponent(selectedRoute)}` : ""}`}
                  target="_blank"
                  rel="noreferrer"
                >
      ...
      {!loading &&
        !error &&
        data?.finance &&
        (kind === "all" || kind === "finance") && (
          <div className="space-y-1">
            <h2 className="text-lg font-semibold">Finance snapshot</h2>
            <pre className="text-xs rounded border p-3 bg-gray-50 overflow-x-auto">
      ...
      {!loading &&
        ...
        (kind === "all" || kind === "latency") && (
          <div className="rounded border p-4">
            ...
            <div className="text-2xl font-medium">
              {Math.round((data.unified.queue.successRatio || 0) * 100)}%
            </div>
          </div>
        )}
