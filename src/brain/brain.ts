...
const toolRegistry = new Map<string, (input: UnknownObject) => Promise<UnknownObject> | UnknownObject>();
const planCache: Record<string, BrainPlan> = Object.create(null) as Record<string, BrainPlan>;
 ...
 */
export function registerTool(name: string, handler: (input: UnknownObject) => Promise<UnknownObject> | UnknownObject): void {
...
export async function invokeTool(t: ToolInvocation): Promise<UnknownObject> {
    ...
    const result = await Promise.resolve(handler(t.input ?? ({} as UnknownObject)));
    ...
    if (!validation.ok) {
        ...
        return { error: 'validation_failed', issues: validation.issues, raw: safeSerializable(result) } as UnknownObject;
    }
...
export function validate(result: UnknownObject): ValidationResult {
    ...
    if (!isObject) return { ok: false, issues: ['Result must be a non-null object'] };
...
export function serializePlan(p: BrainPlan): UnknownObject {
    return { steps: Array.isArray(p.steps) ? p.steps.map(safeSerializable) : [] } as UnknownObject;
}

/**
 * Cache and retrieve plans (optional helpers used by orchestrator UIs)
 */
export function cachePlan(key: string, plan: BrainPlan): void { planCache[key] = plan; }
export function getCachedPlan(key: string): BrainPlan | undefined { return planCache[key]; }
