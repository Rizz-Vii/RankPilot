...
let getQueueMetricsSnapshot: () => QueueMetricsSnapshot = () => ({
  enqueued: 0,
  started: 0,
  completed: 0,
  failed: 0,
...
void (async () => {
  try {
    ...
    const mod: QueueModule =
      imported && typeof imported === "object" ? (imported as QueueModule) : {};
...
export interface UnifiedMetricsSnapshot {
  ...
  >; // adds histogram buckets counts & percentiles
...
const latency: Record<
  string,
  RouteLatencyStats & { buckets: Record<string, number> }
> = {};
...
const inviteMaintenanceCounters = {
  markedExpired: 0,
  deletedAccepted: 0,
  deletedExpired: 0,
  orphanIndexes: 0,
};
const crawlerCounters = {
  success: 0,
  errors: 0,
  totalCrawlMs: 0,
  totalAnalysisMs: 0,
  ...
  quotaLimit: 0,
  quotaRemaining: 0,
  crawlSamples: [] as number[],
  analysisSamples: [] as number[],
};
...
export function recordRouteLatency(routeKey: string, ms: number) {
  if (!latency[routeKey])
    latency[routeKey] = { count: 0, totalMs: 0, maxMs: 0, buckets: {} };
...
export function recordError(
  routeKey: string,
  errorClass: "4xx_user" | "5xx_server"
) {
...
export function recordRateLimitRejection(scopeOrRoute: string) {
  rateLimitRejections[scopeOrRoute] =
    (rateLimitRejections[scopeOrRoute] || 0) + 1;
}

export function recordTeamRateLimitAllowed(scopeOrRoute: string) {
  teamRateLimitAllows[scopeOrRoute] =
    (teamRateLimitAllows[scopeOrRoute] || 0) + 1;
}

export function recordCompactDocSize(bytes: number) {
  if (bytes > 0) {
    compactDocSize.count += 1;
    compactDocSize.totalBytes += bytes;
  }
}

function computePercentileForRoute(
  stats: RouteLatencyStats & { buckets: Record<string, number> },
  percentile: number
): number | null {
...
export function getUnifiedMetricsSnapshot(): UnifiedMetricsSnapshot {
  ...
  const coveragePct =
    total === 0 ? 100 : +((withProvenance / total) * 100).toFixed(2);
  return {
    aiResponses: {
      total,
      withProvenance,
    ...
    latency: Object.fromEntries(
      Object.entries(latency).map(([k, v]) => [
        k,
        {
          ...v,
          buckets: { ...v.buckets },
          p90: computePercentileForRoute(v, 0.9),
          p95: computePercentileForRoute(v, 0.95),
          p99: computePercentileForRoute(v, 0.99),
        },
      ])
    ),
    ...
    compactDocs: {
      count: compactDocSize.count,
      totalBytes: compactDocSize.totalBytes,
      avgBytes: compactDocSize.count
        ? Math.round(compactDocSize.totalBytes / compactDocSize.count)
        : null,
    },
    inviteMaintenance: { ...inviteMaintenanceCounters },
    crawler: {
      ...crawlerCounters,
      crawlP95: computeSimpleP95(crawlerCounters.crawlSamples),
      analysisP95: computeSimpleP95(crawlerCounters.analysisSamples),
      crawlP99: computeSimpleP99(crawlerCounters.crawlSamples),
      analysisP99: computeSimpleP99(crawlerCounters.analysisSamples),
    },
...
export function recordInviteMaintenance(
  delta: Partial<typeof inviteMaintenanceCounters>
) {
  for (const k of Object.keys(
    delta
  ) as (keyof typeof inviteMaintenanceCounters)[]) {
...
export function recordCrawlerSuccess(crawlMs: number, analysisMs: number) {
  ...
  if (crawlerCounters.crawlSamples.length > 500)
    crawlerCounters.crawlSamples.splice(
      0,
      crawlerCounters.crawlSamples.length - 500
    );
  if (crawlerCounters.analysisSamples.length > 500)
    crawlerCounters.analysisSamples.splice(
      0,
      crawlerCounters.analysisSamples.length - 500
    );
}
...
export function __resetUnifiedMetricsTestOnly() {
  provenanceCounters.total = 0;
  provenanceCounters.withProvenance = 0;
  ...
  for (const k of Object.keys(rateLimitRejections))
    delete rateLimitRejections[k];
  for (const k of Object.keys(teamRateLimitAllows))
    delete teamRateLimitAllows[k];
  compactDocSize.count = 0;
  compactDocSize.totalBytes = 0;
  inviteMaintenanceCounters.markedExpired = 0;
  inviteMaintenanceCounters.deletedAccepted = 0;
  inviteMaintenanceCounters.deletedExpired = 0;
  inviteMaintenanceCounters.orphanIndexes = 0;
  crawlerCounters.success = 0;
  crawlerCounters.errors = 0;
  crawlerCounters.totalCrawlMs = 0;
  crawlerCounters.totalAnalysisMs = 0;
  crawlerCounters.aggregateHits = 0;
  crawlerCounters.legacyFallbacks = 0;
  crawlerCounters.quotaLimit = 0;
  crawlerCounters.quotaRemaining = 0;
  crawlerCounters.crawlSamples.length = 0;
  crawlerCounters.analysisSamples.length = 0;
  semanticMapCounters.aggregateHits = 0;
  semanticMapCounters.legacyFallbacks = 0;
  governanceCounters.provenanceInjected = 0;
  governanceCounters.forbiddenFieldStrips = 0;
  for (const k of Object.keys(provenanceReasonCounts))
    delete provenanceReasonCounts[k];
  phase3Counters.developerFeedbackRecordsTotal = 0;
  phase3Counters.predictiveForecastsGeneratedTotal = 0;
  phase3Counters.reportSummariesGeneratedTotal = 0;
  phase3Counters.eventRetriesTotal = 0;
  phase3Counters.eventRetrySuccessTotal = 0;
  phase3Counters.eventDeadLettersTotal = 0;
}
