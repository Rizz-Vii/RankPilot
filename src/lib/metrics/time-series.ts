...
export type BaseTimeSeries = { ts: number[]; fallbacks: number[]; rateLimit: number[]; queueDepth: number[]; queueSuccessPct: number[] };
type Series = BaseTimeSeries;
export type RouteTimeSeries = BaseTimeSeries & { route: string; p95: number[]; p99: number[] };
type RouteSeries = { p95: number[]; p99: number[] };

const MAX_POINTS = 144; // ~24 minutes at 10s
const series: Series = { ts: [], fallbacks: [], rateLimit: [], queueDepth: [], queueSuccessPct: [] };
const routeSeries = new Map<string, RouteSeries>();
const trackedRoutes = new Set<string>();
let started = false;

function push<T>(arr: T[], val: T) { arr.push(val); if (arr.length > MAX_POINTS) arr.splice(0, arr.length - MAX_POINTS); }

function sampleOnce() {
    const u = getUnifiedMetricsSnapshot();
    const ts = Math.floor(Date.now() / 1000);
    const fallbacks = Object.values(u.fallbackReasons || {}).reduce((a, b) => a + b, 0);
    const rl = Object.values(u.rateLimitRejections || {}).reduce((a, b) => a + b, 0);
    ...
    if (trackedRoutes.size && u.latency) {
        for (const route of trackedRoutes) {
            ...
            if (!rs) { rs = { p95: [], p99: [] }; routeSeries.set(route, rs); }
            push(rs.p95, p95 == null || Number.isNaN(p95) ? (rs.p95.at(-1) ?? 0) : p95);
            push(rs.p99, p99 == null || Number.isNaN(p99) ? (rs.p99.at(-1) ?? 0) : p99);
        }
    }
}
...
export function getTimeSeries(windowPoints?: number, route?: string): BaseTimeSeries | RouteTimeSeries {
    ...
    if (route) {
        ...
        if (rs) {
            ...
            const withRoute: RouteTimeSeries = { ...base, route, p95: rp95, p99: rp99 };
            return withRoute;
        }
    }
    return base;
