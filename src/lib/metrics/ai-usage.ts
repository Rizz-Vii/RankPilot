...
function getCostPerToken(model: string, dir: 'in' | 'out'): number {
    ...
    if (/claude/i.test(model)) return (dir === 'in' ? baseIn * 1.2 : baseOut * 1.2) / 1000;
    if (/gemini/i.test(model)) return (dir === 'in' ? baseIn * 0.9 : baseOut * 0.9) / 1000;
    return (dir === 'in' ? baseIn : baseOut) / 1000; // convert to per token
}

export function recordAIUsage(approxInputTokens: number, approxOutputTokens: number, model: string) {
    const now = Date.now();
    prune(now);
    const bucketKey = Math.floor(now / BUCKET_MS) * BUCKET_MS;
    let bucket = buckets.find(b => b.ts === bucketKey);
    if (!bucket) { bucket = { ts: bucketKey, in: 0, out: 0, cost: 0 }; buckets.push(bucket); }
    bucket.in += approxInputTokens;
    bucket.out += approxOutputTokens;
    bucket.cost += approxInputTokens * getCostPerToken(model, 'in') + approxOutputTokens * getCostPerToken(model, 'out');
}
...
export function getAIUsage24h() {
    prune(Date.now());
    const totals = buckets.reduce((acc, b) => { acc.in += b.in; acc.out += b.out; acc.cost += b.cost; return acc; }, { in: 0, out: 0, cost: 0 });
    return { tokensIn: totals.in, tokensOut: totals.out, costEstimate: +totals.cost.toFixed(4) };
}
