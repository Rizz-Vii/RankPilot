...
interface UnifiedSemanticMapStats {
  aggregateHits?: number;
  legacyFallbacks?: number;
}
...
export function getKpiSnapshot(): KpiSnapshot {
  const unified =
    getUnifiedMetricsSnapshot() as unknown as UnifiedSnapshotShape; // Scoped narrowing with explicit intermediate cast
  const neuro = getNeuroseoMetricsSnapshot();

  const cacheDenom = neuro.analysisRuns + neuro.analysisCacheHits;
  const cacheHitRatio = cacheDenom
    ? +((neuro.analysisCacheHits / cacheDenom) * 100).toFixed(2)
    : null;

  const fallbackTotal = Object.values(unified.fallbackReasons).reduce(
    (a, b) => a + b,
    0
  );
  const fallbackRate = unified.aiResponses.total
    ? +((fallbackTotal / unified.aiResponses.total) * 100).toFixed(2)
    : null;

  const p90Values = Object.values(unified.latency)
    .map((v) => v.p90)
    .filter((v) => v != null) as number[];
  const p95Values = Object.values(unified.latency)
    .map((v) => v.p95)
    .filter((v) => v != null) as number[];
  const p99Values = Object.values(unified.latency)
    .map((v) => v.p99)
    .filter((v) => v != null) as number[];
  ...
  const rejectionTotal = Object.values(unified.rateLimitRejections).reduce(
    (a, b) => a + b,
    0
  );
  const rateLimitRejectionRate = unified.aiResponses.total
    ? +((rejectionTotal / unified.aiResponses.total) * 100).toFixed(2)
    : null;
  ...
  if (unified.teamRateLimitAllows) {
    Object.entries(unified.teamRateLimitAllows).forEach(([k, v]) => {
      if (k.startsWith("team:")) teamAllows += v;
    });
  }
  Object.entries(unified.rateLimitRejections).forEach(([k, v]) => {
    if (k.startsWith("team:")) teamRejects += v;
  });
  const teamRateLimitUtilizationPct =
    teamAllows + teamRejects > 0
      ? +((teamAllows / (teamAllows + teamRejects)) * 100).toFixed(2)
      : null;

  const avgCompactDocBytes = unified.compactDocs?.avgBytes ?? null;

  const routesP95: Record<string, number | null> = {};
  Object.entries(unified.latency).forEach(([route, stats]) => {
    routesP95[route] = stats.p95;
  });
  ...
  if (
    crawlerAny &&
    (crawlerAny.aggregateHits != null || crawlerAny.legacyFallbacks != null)
  ) {
    const hits = crawlerAny.aggregateHits ?? 0;
    const fallbacks = crawlerAny.legacyFallbacks ?? 0;
    const denom = hits + fallbacks;
    if (denom > 0)
      crawlerAggregateAdoptionPct = +((hits / denom) * 100).toFixed(2);
  }
  ...
  if (smAny && (smAny.aggregateHits != null || smAny.legacyFallbacks != null)) {
    ...
    const denom = hits + fallbacks;
    if (denom > 0)
      semanticMapAggregateAdoptionPct = +((hits / denom) * 100).toFixed(2);
  }
