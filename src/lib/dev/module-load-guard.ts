...
interface CounterMap {
  [k: string]: { count: number; first: number };
}
declare global {
...
export function registerModuleLoad(
  ...
  try {
    ...
    if (rec.count === threshold) {
      console.warn(
        `[ModuleLoadGuard] Module '${opts.label || id}' loaded ${rec.count} times. Potential circular re-export or dynamic import loop.`
      );
    }
  } catch {
    /* swallow */
