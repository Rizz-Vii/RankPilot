...
function extractTable(
  input: ServerChartArtifactData | { table?: unknown[][]; data?: unknown[] }
): unknown[][] {
  const maybe: Partial<{ table: unknown[][]; data: unknown[] }> =
    input as Partial<{ table: unknown[][]; data: unknown[] }>;
  if (Array.isArray(maybe.table)) return maybe.table;
  if (Array.isArray(maybe.data))
    return maybe.data.map((d: unknown) => (Array.isArray(d) ? d : [d]));
...
export async function generateServerArtifact(
  ...
  chartData:
    | ServerChartArtifactData
    | { table?: unknown[][]; data?: unknown[]; svg?: string; image?: string },
  ...
  if (format === "png") {
    ...
    if (typeof svg === "string" && svg.trim().length) {
      ...
      if (typeof loaded !== "function")
        throw new Error("sharp module did not export a callable default");
      ...
      const pipeline = (w || h
        ...
        : img) as unknown as { png: () => { toBuffer(): Promise<Buffer> } };
      const buffer: Buffer = await pipeline.png().toBuffer();
      return { buffer, contentType: "image/png", ext: "png" };
    }
    throw new Error(
      "PNG server export requires data URL in chartData.image or svg markup in chartData.svg"
    );
  }
