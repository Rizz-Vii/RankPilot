...
export class D3VisualizationEngine {
  ...
  private initializeColorSchemes(): void {
    ...
    const clampByte = (n: number) =>
      Math.max(
        0,
        Math.min(255, Number.isFinite(Math.round(n)) ? Math.round(n) : 0)
      );
    ...
    const cssColorToHex = (v: string): string => {
      ...
      if (/^rgba?\(/i.test(v)) {
        ...
        if (parts.length >= 3 && parts.slice(0, 3).every(Number.isFinite))
          return hexFromRGB(parts[0], parts[1], parts[2]);
      }
      if (/^hsla?\(/i.test(v)) {
        const parts = v
          .replace(/hsla?\(|\)/g, "")
    ...
    const build = (
      defs: Array<{ token?: string; fb: [number, number, number] }>
    ) =>
      defs.map((d) =>
        d.token
          ? resolveTokenToHex(d.token, d.fb)
          : hexFromRGB(d.fb[0], d.fb[1], d.fb[2])
      );

    // Default scheme mapped to theme tokens where possible; others fallback to classic palette RGBs
    this.colorSchemes.set(
      "default",
      build([
  ...
  createLineChart(containerId: string, config: ChartConfig): void {
    ...
    series.forEach((seriesData, seriesName) => {
      const path = svg
        ...
        .attr(
          "stroke",
          colors[Array.from(series.keys()).indexOf(seriesName) % colors.length]
        )
        .attr("stroke-width", 2)
        .attr("d", line);
      ...
      if (config.options.interactive) {
        svg
          ...
          .attr(
            "fill",
            colors[
              Array.from(series.keys()).indexOf(seriesName) % colors.length
            ]
          )
  ...
  createPieChart(containerId: string, config: ChartConfig): void {
    ...
    const pie = d3
      .pie<ChartDataPoint>()
      .value((d) =>
        typeof d.value === "number" ? d.value : this.toNumber(d.y)
      )
      .sort(null);
    ...
    const strokeColor = this.resolveContainerColor(
      `#${containerId}`,
      [255, 255, 255]
    );
    ...
    if (config.options.animations) {
      paths
        ...
        .attrTween("d", function (d) {
          ...
          return (t: number) => {
            const mutable = d as d3.PieArcDatum<ChartDataPoint> & {
              endAngle?: number;
            };
    ...
    if (config.options.legend) {
      const labels = slices
        ...
        .attr(
          "transform",
          (d) =>
            `translate(${outerArc.centroid(d as d3.PieArcDatum<ChartDataPoint>).join(",")})`
        )
        .attr("dy", "0.35em")
    ...
    const points = svg
      ...
      .attr(
        ...
        (d: ChartDataPoint, i: number) =>
          colors[
            (typeof d.category === "number" ? d.category : i) % colors.length
          ]
      )
      .attr("opacity", 0.7);
  ...
  createHeatmap(containerId: string, config: ChartConfig): void {
    ...
    const xValues = Array.from(
      new Set(config.data.map((d) => String(d.x)))
    ).sort();
    const yValues = Array.from(
      new Set(config.data.map((d) => String(d.y)))
    ).sort();
  ...
  async exportChart(
    ...
    const root: SVGElement | null =
      group && group.ownerSVGElement
  ...
  updateChartData(chartId: string, newData: ChartDataPoint[]): void {
    ...
    switch (chart.type) {
      case "line":
        return this.createLineChart(containerId, chart.config);
      case "bar":
        return this.createBarChart(containerId, chart.config);
      case "pie":
        return this.createPieChart(containerId, chart.config);
      case "scatter":
        return this.createScatterPlot(containerId, chart.config);
      case "heatmap":
        return this.createHeatmap(containerId, chart.config);
    }
  }
  ...
  private createSVG(
    ...
    const svg = container
      ...
      .style(
        "background-color",
        config.styling.backgroundColor || "transparent"
      );
  ...
  private getChartDimensions(config: ChartConfig): {
    width: number;
    height: number;
  } {
    return {
  ...
  private addXAxis(
    ...
    if (axisConfig?.tickFormat) {
      ...
      axis.tickFormat(
        (d: number | Date | string) =>
          fmt(typeof d === "number" ? d : Number(d)) as unknown as string
      );
    }
    ...
    if (axisConfig?.label) {
      xAxis
        ...
        .attr(
          "x",
          (scale as unknown as { range(): [number, number] }).range()[1] / 2
        )
        .attr("y", 40)
        .style("text-anchor", "middle")
        .style("fill", "black")
  ...
  private addYAxis(
    ...
    const yAxis = svg.append("g").attr("class", "y-axis").call(axis);
    ...
    if (axisConfig?.label) {
      yAxis
        ...
        .attr(
          "x",
          -(scale as unknown as { range(): [number, number] }).range()[0] / 2
        )
        .style("text-anchor", "middle")
        .style("fill", "black")
        .text(axisConfig.label);
  ...
  private addGrid(
    ...
    svg
      ...
      .call(
        d3
          .axisBottom(xScale as d3.AxisScale<number | Date | string>)
          .tickSize(-height)
          .tickFormat(() => "")
      )
    ...
    svg
      ...
      .call(
        d3
          .axisLeft(yScale as d3.AxisScale<number | Date | string>)
          .tickSize(-width)
          .tickFormat(() => "")
      )
      .style("stroke-dasharray", "3,3")
  ...
  private addLegend(
    ...
    const legend = svg
      ...
      .attr(
        "transform",
        `translate(${config.width - config.margin.right + 20}, 20)`
      );
  ...
  private showTooltip(
    event: {
      pageX: number;
      pageY: number;
      target: EventTarget & { dispatchEvent(e: Event): boolean };
    },
    data: ChartDataPoint,
    config: ChartConfig
  ): void {
  ...
  private handleBarClick(
    event: { target: EventTarget & { dispatchEvent(e: Event): boolean } },
    data: ChartDataPoint,
    config: ChartConfig
  ): void {
  ...
  private exportAsSVG(svg: SVGElement, config: ChartExportConfig): string {
    const serializer:
      | XMLSerializer
      | { serializeToString: (n: Node) => string } =
      typeof XMLSerializer !== "undefined"
        ? new XMLSerializer()
        : {
            serializeToString: (n: Node) =>
              n instanceof Element ? (n as Element).outerHTML : String(n),
          };
    const parent = svg.parentElement;
    const originalRoot: SVGElement = (
      parent && parent instanceof SVGElement ? parent : svg
    ) as SVGElement;
    ...
    const w =
      originalRoot.getAttribute("width") ||
      (originalRoot instanceof SVGSVGElement &&
      originalRoot.width?.baseVal?.value != null
        ? String(originalRoot.width.baseVal.value)
        : "") ||
      String(svg.clientWidth) ||
      "800";
    const h =
      originalRoot.getAttribute("height") ||
      (originalRoot instanceof SVGSVGElement &&
      originalRoot.height?.baseVal?.value != null
        ? String(originalRoot.height.baseVal.value)
        : "") ||
    ...
    if (config.background) {
      ...
      const next = /background-color:/i.test(style)
        ? style
    ...
    if (
      (config.fontFacesCSS &&
        (Array.isArray(config.fontFacesCSS)
          ? config.fontFacesCSS.length
          : true)) ||
      (config.embedFonts && config.embedFonts.length)
    ) {
      try {
  ...
  private inlineComputedStyles(
    ...
    const origAll = [
      originalRoot,
      ...Array.from(originalRoot.querySelectorAll<SVGElement>("*")),
    ];
    const cloneAll = [
      cloneRoot,
      ...Array.from(cloneRoot.querySelectorAll<SVGElement>("*")),
    ];
  ...
  private embedFontFaces(root: SVGElement, config: ChartExportConfig): void {
    ...
    if (config.fontFacesCSS) {
      if (Array.isArray(config.fontFacesCSS))
        cssParts.push(...config.fontFacesCSS);
  ...
  private async exportAsPDF(
    ...
    const pdf = new jsPDF({
      orientation:
        (config.width || 842) >= (config.height || 595)
          ? "landscape"
          : "portrait",
      unit: "pt",
      format: "a4",
    ...
    if (config.watermark) {
      ...
      pdf.setFontSize(36);
      pdf.text("RankPilot", pageWidth / 2, pageHeight - 24, {
        align: "center",
      });
    }
...
declare module "./d3-visualization-engine" {
  interface D3VisualizationEngine {
    toDate(value: number | string | Date): Date;
    toNumber(value: number | string | Date | undefined): number;
    computeDateExtent(data: ChartDataPoint[]): [Date, Date];
    computeNumberExtent(
      data: ChartDataPoint[],
      key: "x" | "y"
    ): [number, number];
...
D3VisualizationEngine.prototype.toDate = function (
...
D3VisualizationEngine.prototype.toNumber = function (
...
D3VisualizationEngine.prototype.computeDateExtent = function (
  data: ChartDataPoint[]
): [Date, Date] {
  const dates = data
    .map((d) => this.toDate(d.x))
    .sort((a, b) => a.getTime() - b.getTime());
  return [dates[0] ?? new Date(), dates[dates.length - 1] ?? new Date()];
};

D3VisualizationEngine.prototype.computeNumberExtent = function (
  data: ChartDataPoint[],
  key: "x" | "y"
): [number, number] {
