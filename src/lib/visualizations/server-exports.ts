...
type ChartExportConfigInput =
  | Partial<{ title?: string; width?: number; height?: number }>
  | undefined;
function coerceChartConfig(cfg: ChartExportConfigInput): {
  title?: string;
  width?: number;
  height?: number;
} {
...
export async function generateChartExport(
  chartData: ServerChartArtifactData,
  format: ExportFormat,
  config: ChartExportConfigInput
): Promise<string> {
  ...
  const { buffer, contentType, ext } = await generateServerArtifact(
    format,
    {
...
function coerceDashboardConfig(cfg: DashboardExportConfigInput): {
  title?: string;
} {
  if (!cfg) return {};
...
export async function generateDashboardExport(
  dashboardData: ServerDashboardArtifactData,
  format: ExportFormat,
  config: DashboardExportConfigInput
): Promise<string> {
  const supported: ExportFormat[] = ["pdf", "excel", "json", "png", "svg"];
  if (!supported.includes(format))
    throw new Error(`Unsupported export format: ${format}`);
