...
export async function generateReport(
  ...
  if (Array.isArray(input.sections) && input.sections.length > 0) {
    ...
    const text = input.asCsv
      ? renderSectionsAsCsv(safeTitle, sections)
      : renderSectionsAsText(safeTitle, sections);
    try {
      const { recordReportSummariesGenerated } = await import(
        "../metrics/unified-metrics"
      );
      recordReportSummariesGenerated(1);
    } catch {
      /* optional */
    }
...
function mockSummarize({
  ...
  const over = new TextDecoder().decode(
    bytes.slice(0, Math.min(bytes.length, maxBytes + 16))
  );
  return over;
}
