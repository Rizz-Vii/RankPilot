...
class AppLoggerImpl {
    private component?: string; private traceId?: string; private startTime: number;
    constructor(opts: LoggerOptions = {}) { this.component = opts.component; this.traceId = opts.traceId; this.startTime = opts.startTime ?? (typeof performance !== 'undefined' ? performance.now() : Date.now()); }
    child(ctx: Partial<LoggerOptions>) { return new AppLoggerImpl({ component: ctx.component || this.component, traceId: ctx.traceId || this.traceId, startTime: this.startTime }); }
    withTrace(component?: string) { return this.child({ component, traceId: genId('trace') }); }
    private emit(level: LogLevel, message: string, context?: LogContext & { __audit?: boolean; __degraded?: boolean }) {
    ...
    warn(m: string, c?: LogContext) { this.emit('warn', m, c); }
    error(m: string, c?: LogContext) { this.emit('error', m, c); }
    /** Audit log (immutability / compliance events). Writes at info level with audit flag. */
    audit(m: string, c?: LogContext) { this.emit('info', m, { ...(c || {}), __audit: true }); }
    /** Degraded mode notice (feature fallback, partial failure). Warn level with degraded flag for observability. */
    degraded(m: string, c?: LogContext) { this.emit('warn', m, { ...(c || {}), __degraded: true }); }
}
export const appLogger = new AppLoggerImpl({ component: isServer ? 'server' : 'client' });
export function getLogger(component: string) { return appLogger.child({ component }); }
