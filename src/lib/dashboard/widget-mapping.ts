...
export function toServerMutation(
  _widget: ClientWidget,
  updates: Partial<ClientWidget>
): WidgetMutation {
  const m: WidgetMutation = {};
  if (updates.title) m.title = updates.title;
  if (updates.chartConfig)
    m.visualization = { chartType: updates.chartConfig.type as ChartType };
  return m;
}
...
export function mergeServerWidget(
  ...
  return {
    ...
    chartConfig: client.chartConfig
      ? {
          ...client.chartConfig,
          type:
            (server.visualization?.chartType as ChartType) ||
            client.chartConfig.type,
        }
      : client.chartConfig,
  };
}
...
export function serverToClientWidget(
  server: ServerDashboardWidget
): ClientWidget {
  const isChart =
    !!server.visualization?.chartType &&
    ["line", "bar", "pie", "scatter", "heatmap"].includes(
      server.visualization.chartType as string
    );
  return {
    id: server.id, // use server id as client id for hydrated widgets
    serverId: server.id,
    type:
      server.type === "iframe" || server.type === "map"
        ? ("text" as WidgetType)
        : (server.type as WidgetType),
    title: server.title,
