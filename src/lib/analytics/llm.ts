...
export function recordLlmUsage(provider: LlmProvider, ok: boolean, latencyMs: number): void {
    if (!features.llmVisibility) return;
    const bucket = latencyMs < 200 ? '<200' : latencyMs < 500 ? '200-499' : latencyMs < 1000 ? '500-999' : '>=1000';
