...
import { recordEventDeadLetter, recordEventRetry, recordEventRetrySuccess } from '../metrics/unified-metrics';
import { pushDeadLetter } from './dead-letter';
import { on } from './event-bus';
import type { EventType } from './event-types';

export type EventHandler = (evt: { type: EventType; payload?: { [k: string]: unknown } }) => Promise<void> | void;
...
interface RetryOptions {
    ...
    idempotencyKey?: (evt: { type: EventType; payload?: { [k: string]: unknown } }) => string | undefined;
    // Optional dead-letter sink: if provided, will be called on final failure; falls back to in-memory DLQ
    dlq?: (evt: { type: EventType; payload?: { [k: string]: unknown } }, error: unknown, attempts: number) => void;
}

export function registerWithRetry(eventType: EventType, handler: EventHandler, opts: RetryOptions = {}) {
    ...
    const unsubscribe = on(eventType, ({ type, payload }) => {
        ...
        if (typeof opts.idempotencyKey === 'function') {
            ...
            } catch { /* ignore idempotency errors */ }
        }
        const exec = async () => {
            attempt += 1;
            try {
                await handler(evt);
                if (attempt > 1) { try { recordEventRetrySuccess(1); } catch { /* optional */ } }
            } catch (err) {
                if (attempt < maxAttempts) {
                    try { recordEventRetry(1); } catch { /* optional */ }
                    const nextDelay = (() => {
                    ...
                    setTimeout(() => { void exec(); }, delay);
                } else {
                    // Exhausted: DLQ
                    try { recordEventDeadLetter(1); } catch { /* optional */ }
                    try {
