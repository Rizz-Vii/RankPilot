...
export interface FirestoreModularLike {
  /* marker – runtime checked via function presence */
}
...
async function sha256Hex(input: string): Promise<string> {
  ...
  if (
    ...
    g !== null &&
    "crypto" in g &&
    typeof (g as { crypto?: unknown }).crypto === "object" &&
    (g as { crypto: { subtle?: unknown } }).crypto.subtle &&
    typeof (g as { crypto: { subtle: { digest?: unknown } } }).crypto.subtle
      .digest === "function"
  ) {
...
function hasLegacyCollection(
  db: Firestore | undefined | null
): db is FirestoreLegacyLike {
  return (
    !!db && typeof (db as Record<string, unknown>).collection === "function"
  );
}
...
async function resolveAutoDb(): Promise<{
  db: Firestore;
  legacySanitized: boolean;
} | null> {
  ...
  try {
    ...
    if (clientDb) {
      ...
      if (hasLegacyCollection(clientDb as Firestore)) {
        return {
          db: wrapLegacyDbSanitize(clientDb as FirestoreLegacyLike),
  ...
  try {
    ...
    if (adminDb && hasLegacyCollection(adminDb as Firestore)) {
      ...
      return {
        db: wrapLegacyDbSanitize(adminDb as FirestoreLegacyLike),
        legacySanitized: true,
      };
    }
  } catch {
  ...
  while (attempt < 2) {
    ...
    try {
      ...
      if (hasLegacyCollection(db)) {
        ...
        await writeCreateOnlyViaLegacy(
          db as FirestoreLegacyLike,
          base,
          eventId,
          docData
        );
        ...
        await writeCreateOnlyViaModular(
          db as FirestoreModularLike,
          fullPath,
          docData
        );
      }
