...
async function loadSeedRandom(): Promise<(seed: string) => () => number> {
  ...
  const fn =
    ...
    (mod as unknown as { seedrandom?: unknown }).seedrandom ??
    mod;
  if (typeof fn !== "function")
    throw new Error("seedrandom module shape unsupported");
