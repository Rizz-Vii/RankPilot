...
function validateIds(
  ...
  if (requireTeamId && !teamId)
    throw new Error("Missing teamId in team-scoped write payload");
}

export async function guardedAdd<T = unknown>(
  colRef: CollectionReference,
  data: T,
  opts: GuardOptions = {}
) {
  const payload = (opts.stripUndefined ? sanitize(data) : data) as Record<
    string,
    unknown
  >;
...
export async function guardedSet<T = unknown>(
...
export async function guardedUpdate<T = unknown>(
  docRef: DocumentReference,
  data: Partial<T>,
  opts: GuardOptions = {}
) {
  const payload = (opts.stripUndefined ? sanitize(data) : data) as Record<
    ...
    unknown
  >;
  validateIds({ userId: payload["userId"] } as Record<string, unknown>, {
    requireTeamId: false,
  });
...
function getRegistry(): Map<string, RegistryEntry> {
  const g =
    getGlobal<Record<string, unknown>>() || ({} as Record<string, unknown>);
...
export function trackListener(key: string) {
  ...
  if (activeCounts[key] === 1) {
    console.log(
      `[FirestoreListener] + ${key} (unique:${Object.keys(activeCounts).length})`
    );
  }
}
...
export interface ManagedSnapshotOptions {
  debounceMs?: number;
}

export function managedOnSnapshot(
  q: unknown,
  onData: (snap: unknown) => void,
  onError?: (e: unknown) => void,
  opts: ManagedSnapshotOptions = {}
): Unsub {
  ...
  if (existing) {
    ...
    if (existing.lastSnap !== undefined) {
      queueMicrotask(() => {
        try {
          onData(existing.lastSnap as unknown);
        } catch {
          /* ignore */
        }
      });
  ...
  try {
    ...
    return () => {
      /* no-op: subscription never established */
    };
  }
...
export function getActiveListenerSummary() {
  return {
    totalUnique: Object.keys(activeCounts).length,
    keys: { ...activeCounts },
  };
}
...
function deriveReadableKey(q: unknown): string {
  try {
    ...
    const pathVal =
      anyQ && typeof anyQ === "object"
        ? (anyQ as { path?: unknown }).path
        : undefined;
    ...
    if (Array.isArray(filters)) {
      ...
      try {
        const parts = filters
          .map((f: unknown) => summarizeFilter(f))
    ...
    const orderBy =
      (qInternal as { orderBy?: unknown[] } | undefined)?.orderBy ||
      (anyQ as { _queryOptions?: { orderBy?: unknown[] } })._queryOptions
        ?.orderBy;
    if (Array.isArray(orderBy)) constraints.push(`orderBy:${orderBy.length}`);
    const limit =
      (qInternal as { limit?: number } | undefined)?.limit ||
    ...
    if (basePath) {
      const base = constraints.length
        ? `query:${basePath}?${constraints.join(",")}`
        : `query:${basePath}`;
...
function extractFieldName(anyF: Record<string, unknown>): string {
  ...
  const fieldPath = (anyF["fieldPath"] as { segments?: string[] } | undefined)
    ?.segments;
...
function extractValueSignature(anyF: Record<string, unknown>): string {
  ...
  if (typeof v === "object") {
    ...
    try {
      if (Array.isArray(v))
        return `a:${v.length}:${hashSmall(JSON.stringify(v).slice(0, 200))}`;
      // Firestore Timestamp
