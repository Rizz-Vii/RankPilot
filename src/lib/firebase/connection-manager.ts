...
const firebaseConfig = (() => {
    ...
    const missing = Object.entries(fromEnv).filter(([, v]) => !v).map(([k]) => k);
    if (process.env.NODE_ENV === 'production' && missing.length) {
        throw new Error(`[Firebase] Missing required env in production: ${missing.join(', ')}`);
    }
    return {
    ...
    private initializeDatabase() {
        try {
            ...
            if (typeof window !== 'undefined') {
                ...
                const lpMs = Number(process.env.NEXT_PUBLIC_FIRESTORE_LONG_POLLING_PING_MS || '0');
...
export const resetFirestoreConnection = (): Promise<void> => connectionManager.resetConnection();
