...
const FUNCTIONS_REGION = process.env.FIREBASE_REGION || process.env.NEXT_PUBLIC_FUNCTIONS_REGION || 'australia-southeast1';
...
export const app: FirebaseApp = isBrowser ? (_app ||= getClientApp()) : ({} as unknown as FirebaseApp);
export const db: Firestore = isBrowser ? (_db ||= getClientDb()) : ({} as unknown as Firestore);
export const auth: Auth = isBrowser ? (_auth ||= getAuth(app)) : ({} as unknown as Auth);
export const functions: Functions = isBrowser ? (_functions ||= getFunctions(app, FUNCTIONS_REGION)) : ({} as unknown as Functions);
export const storage: FirebaseStorage = isBrowser ? (_storage ||= getStorage(app)) : ({} as unknown as FirebaseStorage);

// Optionally connect Functions emulator (browser only)
if (isBrowser && process.env.NEXT_PUBLIC_USE_FUNCTIONS_EMULATOR === 'true' && functions) {
  ...
  if (measurementId) {
    void (async () => {
      try {
        ...
        const supported = await (mod.isSupported ? mod.isSupported() : Promise.resolve(false));
        if (supported && app) {
          try { analyticsInstance = mod.getAnalytics(app); } catch { analyticsInstance = null; }
        }
      } catch {
...
export const getFirebaseClient = () => ({ app, db, auth, functions, storage, analytics });
