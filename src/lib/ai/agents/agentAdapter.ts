...
export interface SimpleAgentDefinition {
  ...
  instructions:
    | string
    | ((context: Record<string, unknown>) => string | Promise<string>);
...
export interface AgentToolDefinition {
  ...
  execute: (
    ...
    ctx: { startTs: number; runId: string }
...
export interface InputGuardrailDefinition {
  ...
  execute: (args: {
    ...
    | Promise<{ tripwire: boolean; info?: unknown }>
    | { tripwire: boolean; info?: unknown };
}
...
function agentsEnabled(): boolean {
  if (
    ...
    process.env.RANKPILOT_AGENTS_ENABLED === "1"
  )
    return true;
...
export async function createSimpleAgent(
  ...
  const inputGuardrails = (def.inputGuardrails || []).map((g) => ({
    ...
    execute: async ({
      ...
      const tripwireTriggered =
        typeof tripwireVal === "boolean" ? tripwireVal : Boolean(tripwireVal);
  ...
  const baseConfig: unknown = {
    name: def.name,
    instructions: def.instructions as unknown as
      | string
      | ((context: Record<string, unknown>) => string | Promise<string>),
    model: def.model,
    tools,
  ...
  if (AgentCtor) {
    if (
      def.handoffs &&
      def.handoffs.length &&
      typeof (AgentCtor as Record<string, unknown>).create === "function"
    ) {
      return (
        AgentCtor as { create: (cfg: Record<string, unknown>) => Agent }
      ).create({
...
>(
  ...
  try {
    ...
    const result: unknown = await run(agent, opts.input, {
    ...
    return {
      ...
      meta: {
        ...
        model: (
          agent as unknown as Record<string, unknown> & { model?: string }
        ).model,
        inputTokens:
          (
            resAny as {
          ...
          (resAny as { promptTokens?: number }).promptTokens ??
          (resAny as { usage?: { prompt_tokens?: number } }).usage
            ?.prompt_tokens,
        outputTokens:
          (
            resAny as {
          ...
          (resAny as { completionTokens?: number }).completionTokens ??
          (resAny as { usage?: { completion_tokens?: number } }).usage
            ?.completion_tokens,
        totalTokens:
          (
            resAny as {
          ...
          (resAny as { usage?: { total_tokens?: number } }).usage?.total_tokens,
      },
    };
  } catch (e) {
    const msg = errToString(e);
...
export async function runAdHocSimpleAgent(
  input: string,
  instructions: string
): Promise<AgentRunResult> {
...
export async function runAgentStream(
  ...
  try {
    const started = Date.now();
    const stream: unknown = await run(agent, opts.input, {
      context: opts.context,
      maxTurns: opts.maxTurns,
      stream: true,
    });
    const sAny = stream as Record<string, unknown> & {
      toTextStream?: (opts: {
    ...
    // Text accumulation
    const chunks: string[] = [];
    const textStream = sAny.toTextStream
      ? sAny.toTextStream({ compatibleWithNodeStreams: false })
...
export async function createTriageAgent(
  ...
  if (
    AgentAny &&
    (typeof AgentAny === "function" || typeof AgentAny === "object")
  ) {
    if (typeof (AgentAny as Record<string, unknown>).create === "function") {
      try {
        return (
          AgentAny as { create: (cfg: Record<string, unknown>) => Agent }
        ).create({ name, instructions, handoffs: subAgents });
      } catch {
        return null;
      }
    }
    try {
      return new (AgentAny as new (cfg: Record<string, unknown>) => Agent)({
        name,
        instructions,
        handoffs: subAgents,
      });
