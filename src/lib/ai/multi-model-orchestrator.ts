...
const MCPServiceManager =
  ...
  class LocalMCPServiceManagerStub {
    ...
    async huggingfaceInference(_req: {
      model: string;
      inputs: unknown;
      parameters?: unknown;
 ...
 */
type MCPManagerInstance = {
  huggingfaceInference(req: {
    model: string;
    inputs: unknown;
    parameters?: unknown;
  }): Promise<{ data?: unknown }>;
...
export class MultiModelOrchestrator {
  ...
  async processRequest(
    ...
    try {
      ...
      const results = await this.executeParallelInference(
        request,
        selectedModels
      );
  ...
  private selectModels(request: MultiModelRequest): ModelConfig[] {
    ...
    candidates.sort((a, b) => {
      ...
      return (
        performanceScore[b.performance] +
  ...
  private async executeParallelInference(
    ...
    const promises = models.map(async (model) => {
      ...
      try {
        ...
        console.error(
          `[MultiModelOrchestrator] Model ${model.name} failed:`,
          error
        );
        return {
          model: model.name,
  ...
  private async processBatchRequest(
    ...
    for (const batch of batches) {
      ...
      const batchResults = await this.executeParallelInference(
        batchRequest,
        models
      );
  ...
  private aggregateSentimentResults(results: InferenceResult[]): unknown {
    ...
    const avgScore =
      ...
      sentiments.length;
    const dominantLabel = sentiments.sort(
    ...
    return {
      ...
      score: avgScore,
      consensus:
        sentiments.filter((s) => s?.label === dominantLabel).length /
        sentiments.length,
    };
  }
  ...
  private generateCacheKey(request: MultiModelRequest): string {
    ...
    const inputStr = Array.isArray(rest.input)
      ? rest.input.join("|")
      : rest.input;
  ...
  private estimateTokenUsage(input: unknown, output: unknown): number {
    const inputStrRaw = Array.isArray(input)
      ? input.join(" ")
      : String(input ?? "");
    const outputStr =
  ...
  private async processBatchQueue(): Promise<void> {
    ...
    for (const [, requests] of this.batchQueue.entries()) {
      if (requests.length >= 5) {
        // Process when batch reaches 5 requests
   ...
   */
  getPerformanceAnalytics(): Record<
    string,
    { avgTime: number; successRate: number; usage: number }
  > {
    ...
    this.performanceMetrics.forEach((metrics, modelName) => {
      ...
      analytics[modelName] = {
        ...
        successRate: count
          ? metrics.filter((time) => time > 0).length / count
          : 0,
        usage: count,
      };
    });
...
const g = globalThis as typeof globalThis & {
  __multiModelOrchestrator?: MultiModelOrchestrator;
};
if (!g.__multiModelOrchestrator) {
  g.__multiModelOrchestrator = new MultiModelOrchestrator();
}
export const multiModelOrchestrator: MultiModelOrchestrator =
  g.__multiModelOrchestrator;
