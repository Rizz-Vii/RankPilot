...
export class EnhancedNeuroSEOOrchestrator {
  ...
  private async performAnalysis(
    request: NeuroSEORequest
  ): Promise<OptimizedMergedResult> {
    try {
   ...
   */
  private mergeOptimizedResults(
    batchResults: NeuroSEOReport[],
    request: NeuroSEORequest
  ): OptimizedMergedResult {
    return {
  ...
  private mergeSummaries(
    ...
    const averageScore = batchResults.length
      ? Math.round(scoreAccumulator / batchResults.length)
      : 0;
  ...
  private mergeEngineResults(
    ...
    batchResults.forEach((r) => {
      ...
      if (r.rewriteRecommendations)
        engines.rewrite.push(...r.rewriteRecommendations);
      if (r.engagementAnalysis)
        engines.engagement.push(...r.engagementAnalysis);
    });
    return engines;
  }
  ...
  private logPerformanceMetrics(metrics: {
    ...
    const gobj = globalThis as unknown as {
...
export function useNeuroSEO() {
  return {
    ...
    getCacheStats:
      neuroSEOOrchestrator.getCacheStats.bind(neuroSEOOrchestrator),
    clearCache: neuroSEOOrchestrator.clearCache.bind(neuroSEOOrchestrator),
    preloadAnalysis:
      neuroSEOOrchestrator.preloadAnalysis.bind(neuroSEOOrchestrator),
  };
}
