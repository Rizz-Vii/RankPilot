...
export function sMaxage(seconds: number, staleWhileRevalidate = 60): Record<string, string> {
    const smax = Math.max(0, Math.floor(seconds));
    const swr = Math.max(0, Math.floor(staleWhileRevalidate));
    return { 'Cache-Control': `public, s-maxage=${smax}, stale-while-revalidate=${swr}` };
}
