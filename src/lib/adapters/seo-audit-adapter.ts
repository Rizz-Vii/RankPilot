...
+        const status = statusRaw === 'good' || statusRaw === 'warning' || statusRaw === 'error'
...
+
+    return {
+        url: typeof (r as Record<string, unknown>).url === 'string' ? (r as Record<string, unknown>).url : undefined,
+        overallScore,
+        items,
