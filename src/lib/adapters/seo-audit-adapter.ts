...
export function adaptSEOAuditResponse(
  ...
  const startedAt =
    typeof meta.startedAt === "number" ? meta.startedAt : Date.now();
  const end = Date.now();
  ...
  const itemsRaw: unknown[] = Array.isArray(rawItemsCandidate)
    ? rawItemsCandidate
    : [];
    ...
    return {
      ...
      score:
        typeof (o as Record<string, unknown>).score === "number"
          ? ((o as Record<string, unknown>).score as number)
          : 0,
      details: String(
        (o as Record<string, unknown>).details ??
  ...
  let overallScore =
    typeof providedOverall === "number" ? (providedOverall as number) : 0;
  ...
  let source: "live" | "cache" | "fallback" | undefined = (():
    | "live"
    | "cache"
    | "fallback"
    | undefined => {
  ...
  if (
    (r as Record<string, unknown>).quota &&
    typeof (r as Record<string, unknown>).quota === "object"
  ) {
    const { limit, used, remaining } = (r as Record<string, unknown>)
  ...
  return {
    ...
    items,
    summary: String(
      (r as Record<string, unknown>).summary ?? "No summary provided."
    ),
    totalProcessingTime:
