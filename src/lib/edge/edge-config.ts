...
function applyCachingStrategy(
  ...
  response.headers.set(
    "X-Cache-Strategy",
    Object.keys(PERFORMANCE_RULES).find(
 ...
 */
export function optimizeStaticAssets(
  request: NextRequest
): NextResponse | null {
  const pathname = request.nextUrl.pathname;

  // Check if this is a static asset
  const isStaticAsset = PERFORMANCE_RULES.STATIC_ASSETS.paths.some((path) =>
 ...
 */
export function optimizeAPIResponses(
  request: NextRequest
): NextResponse | null {
  const pathname = request.nextUrl.pathname;
