...
const allowedStreamTypes = new Set<string>([
  "seo-metrics",
  "keyword-ranking",
  "performance",
  "competitor",
...
export class RealTimeDataStreamer extends EventEmitter {
  ...
  async registerClient(
    ...
    try {
      ...
      console.log(
        `[RealTimeStreamer] Client ${clientId} registered for user ${userId} (${tier})`
      );

      return { success: true, client };
    } catch (error) {
      console.error(
        "[RealTimeStreamer] Client registration error:",
  ...
  async subscribeToStreams(
    ...
    const tierLimit =
      this.tierLimits[client.tier as keyof typeof this.tierLimits];
    const subscribed: string[] = [];

    for (const streamType of streamTypes) {
      if (client.subscriptions.size >= tierLimit.maxStreams) {
        console.warn(
          `[RealTimeStreamer] Stream limit reached for client ${clientId}`
        );
        break;
      }
  ...
  async streamData(dataPoint: StreamingDataPoint): Promise<void> {
    ...
    for (const client of relevantClients) {
      try {
        ...
        console.error(
          `[RealTimeStreamer] Failed to send data to client ${client.id}:`,
          safeErrorMessage(error)
        );
  ...
  async broadcastCollaboration(event: CollaborationEvent): Promise<void> {
    const dashboardClients = Array.from(this.clients.values()).filter(
      (client) =>
        client.dashboardId === event.dashboardId &&
    ...
    for (const client of dashboardClients) {
      try {
        ...
        console.error(
          `[RealTimeStreamer] Collaboration broadcast failed for ${client.id}:`,
          safeErrorMessage(error)
        );
      }
    }
    ...
    console.log(
      `[RealTimeStreamer] Collaboration event broadcasted to ${dashboardClients.length} clients`
    );
  }
  ...
  async disconnectClient(clientId: string): Promise<boolean> {
    ...
    if (client.connection) {
      try {
        if (
          client.connectionType === "websocket" &&
        ...
        console.error(
          `[RealTimeStreamer] Error closing connection for ${clientId}:`,
          safeErrorMessage(error)
        );
      }
    }
  ...
  private startHeartbeat(): void {
    this.heartbeatInterval = setInterval(() => {
      ...
      for (const clientId of staleConnections) {
        ...
        void this.disconnectClient(clientId).catch((err) =>
          console.error(
            "[RealTimeStreamer] disconnectClient error (stale cleanup):",
    ...
    const interval = setInterval(() => {
      void (async () => {
        ...
        try {
          ...
          console.error(
            "[RealTimeStreamer] Mock data generation failed:",
            safeErrorMessage(e)
          );
        ...
        console.error("[RealTimeStreamer] streamData error", err)
      );
  ...
  destroy(): void {
    ...
    for (const [streamType, interval] of this.dataStreams.entries()) {
      if (allowedStreamTypes.has(streamType)) {
        streamMetrics.typeCounts[streamType] =
          (streamMetrics.typeCounts[streamType] || 0) + 1;
      } else {
    ...
    for (const clientId of this.clients.keys()) {
      void this.disconnectClient(clientId).catch((err) =>
        console.error(
          "[RealTimeStreamer] disconnectClient error (destroy):",
          safeErrorMessage(err)
        )
      );
    }
