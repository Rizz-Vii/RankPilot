...
export async function retrieveSiteChunks({
  ...
  const colBase = adminDb
    .collection("siteContent")
    .doc(uid)
    .collection(collectionId) as unknown as QueryLike;
  let col: QueryLike;
  try {
    const adminLike = adminDb as unknown as {
      constructor?: { FieldPath?: FieldPathCtor };
      firestore?: { FieldPath?: FieldPathCtor };
    };
    const FieldPath =
      adminLike.constructor?.FieldPath || adminLike.firestore?.FieldPath;
    col =
      FieldPath && colBase.orderBy
        ? colBase.orderBy(FieldPath.documentId())
        : colBase;
  } catch {
    col = colBase;
  }
  ...
  while (scanned < maxScan && scored.length < Math.max(topK * 4, topK + 8)) {
    // oversample factor
    ...
    if (lastDocId) {
      const lastSnap = await (adminDb
        ...
        .doc(lastDocId)
        .get() as unknown as Promise<DocSnap>);
    ...
    snapshot.docs.forEach((doc) => {
      ...
      const data = doc.data();
      const embedding = Array.isArray(
        (data as Record<string, unknown>).embedding
      )
      ...
      if (embedding) {
        ...
        const content = (data as Record<string, unknown>).content as
          ...
          | undefined;
        const meta =
          ((data as Record<string, unknown>).meta as
