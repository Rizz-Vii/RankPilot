...
import type {
  CrawlIngestionConfig,
  IngestionResult,
  SiteContentChunk,
} from "./siteContentTypes";

export async function ingestSiteContentForOrg(
  uid: string,
  config: CrawlIngestionConfig
  ...
  while (queue.length && visited.size < maxPages) {
    ...
    visited.add(url);
    if (exclude.some((r) => r.test(url))) {
      skipped++;
      continue;
    }
    if (include.length && !include.some((r) => r.test(url))) {
      skipped++;
      continue;
    }
    try {
      interface CrawlIngestOptions {
      ...
      const chunks: SiteContentChunk[] = makeChunks({
        url: result.url,
        title: result.title,
        text: result.content,
      ...
      for (const chunk of chunks) {
        ...
        if (snap.exists) {
          ...
          const createdAt = ((): number => {
            if (
              prevMetaRaw &&
              typeof prevMetaRaw === "object" &&
              "createdAt" in (prevMetaRaw as { createdAt?: unknown })
            ) {
          ...
          const prevMeta =
            prevMetaRaw && typeof prevMetaRaw === "object"
              ? (prevMetaRaw as Record<string, unknown>)
              : {};
          await embedAndStoreChunk(
            uid,
            {
              ...
              meta: {
                ...chunk.meta,
                lastHash: prevMeta.hash as string | undefined,
                createdAt,
                updatedAt: Date.now(),
              },
            },
      ...
      const linksArr: unknown = (
        result as unknown as { technicalData?: { links?: unknown } }
      ).technicalData?.links;
      const links = Array.isArray(linksArr) ? linksArr : [];
      const isLink = (v: unknown): v is { href: string } =>
        !!v && typeof (v as { href?: unknown }).href === "string";
      links
        .filter(isLink)
        .filter(
          (l) => l.href.startsWith("http") && new URL(l.href).host === host
        )
        .slice(0, 50)
        .forEach((l) => {
          if (!visited.has(l.href)) queue.push(l.href);
        });
    } catch (e: unknown) {
      errors++;
