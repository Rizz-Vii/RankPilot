...
async function applyRateLimit(request: NextRequest): Promise<boolean> {
  ...
  try {
    if (pathname.startsWith("/api/auth/")) {
      ...
      await rateLimiters.uploads.check(
        SECURITY_CONFIG.RATE_LIMITS.UPLOADS.limit,
        ip
      );
    } else if (
      pathname.startsWith("/api/seo/") ||
      pathname.startsWith("/api/neuroseo/")
    ) {
      await rateLimiters.analysis.check(
        SECURITY_CONFIG.RATE_LIMITS.ANALYSIS.limit,
        ip
      );
 ...
 */
function detectThreats(request: NextRequest): {
  isThreat: boolean;
  threatType?: string;
} {
...
function applyCORS(request: NextRequest, response: NextResponse): NextResponse {
  ...
  const isAllowedOrigin =
    origin &&
    SECURITY_CONFIG.ALLOWED_ORIGINS.some((allowedOrigin) => {
...
export async function securityMiddleware(
  ...
  if (threatCheck.isThreat) {
    logSecurityEvent(request, "threat_detected", {
...
export class SecurityMonitor {
  ...
  getRecentEvents(
    limit = 100
  ): Array<{ id: string; timestamp: string; type: string; details: unknown }> {
