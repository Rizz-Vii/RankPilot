...
export const FORBIDDEN_DERIVED_FIELDS = ['roi', 'ctr', 'conversion', 'winRate', 'ltv'] as const;
export type ForbiddenDerivedField = typeof FORBIDDEN_DERIVED_FIELDS[number];

export interface StripResult<T> { doc: T; stripped: ForbiddenDerivedField[]; }

export function stripForbiddenDerivedFields<T extends Record<string, unknown>>(input: T): StripResult<T> {
...
export function cloneAndStripForbidden<T extends Record<string, unknown>>(input: T): StripResult<T> {
