...
export interface NeuroSEOReport {
  ...
  trustMeta?: {
    modelTag: string;
    generatedAt: string;
    dataIntegrity: "simulated" | "estimated" | "measured";
...
export class NeuroSEOSuite {
  ...
  async runAnalysis(request: NeuroSEOAnalysisRequest): Promise<NeuroSEOReport> {
    ...
    try {
      ...
      // Phase 7b: Upgrade keyInsights to REAL AI (gemini-2.5-flash) over the live-crawled page
      ...
      try {
        ...
        if (primary?.content) {
          ...
          if (aiInsights && aiInsights.length) {
            report.keyInsights = aiInsights as KeyInsight[];
            if (report.trustMeta) {
              ...
              report.trustMeta.notes = [
                "keyInsights generated by gemini-2.5-flash over the live-crawled page content",
                "scores remain heuristic pending measured-signal integration",
              ];
            }
            logger.info("phase.insights.ai", { count: aiInsights.length });
          }
        }
      } catch (e) {
        logger.info("phase.insights.ai_failed", {
...
export const NeuroSEOReportSchema = z.object({
  ...
  trustMeta: z
    .object({
      ...
      dataIntegrity: z.enum(["simulated", "estimated", "measured"]),
