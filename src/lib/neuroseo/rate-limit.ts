...
import {
  coerceWindowStart,
  windowStartToMs,
} from "@/lib/firestore/typed-snapshot";
...
interface RateDoc {
  count: number;
  windowStart: TimestampLike | Date;
}
 ...
 */
export async function enforceNeuroSeoRateLimit(
  ...
  const limit =
    opts?.limit ??
    (process.env.NEUROSEO_RATE_LIMIT
      ? parseInt(process.env.NEUROSEO_RATE_LIMIT, 10)
      : 50);
  const docRef = db.collection("neuroseoRateLimits").doc(scopeId);
  const now = Date.now();
  let retryAfterSeconds = 0;
  const res = await db.runTransaction(
    async (tx: AdminFirestore.Transaction) => {
      const snap = await tx.get(docRef);
      const raw = snap.exists ? (snap.data() as Partial<RateDoc>) : undefined;
      const data: RateDoc = {
        count: typeof raw?.count === "number" ? raw.count : 0,
        windowStart: raw?.windowStart
          ? coerceWindowStart(raw.windowStart, now)
          : new Date(now),
      };
      ...
      if (data.count + 1 > limit) {
        retryAfterSeconds = Math.max(
          1,
          Math.ceil((WINDOW_MS - (now - windowStartMs)) / 1000)
        );
        return {
          allowed: false,
          remaining: 0,
