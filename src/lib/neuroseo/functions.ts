...
export async function generateContentBrief(
    ...
    if (input && typeof input === "object") {
      ...
      return {
        ...
        targetAudience: String(
          (obj as { targetAudience?: unknown }).targetAudience ||
            "general audience"
        ),
        keywords: Array.isArray((obj as { keywords?: unknown }).keywords)
          ? ((obj as { keywords?: unknown[] }).keywords || [])
              .slice(0, 10)
              .map((k) => String(k))
          : [],
      };
  ...
  const sys =
    "You generate actionable SEO content briefs as JSON-like bullet lines.";
  const user =
    `Create a concise content brief for topic: "${i.topic}" targeting ${i.targetAudience}.\n` +
  ...
  const headings = lines
    .filter((l) => /^([Hh]2|##)\b/.test(l))
    .map((l) => l.replace(/^([Hh]2|##)\s*[:\-]?\s*/, ""))
    .slice(0, 8);
  const llmGeneratedOutline = headings.map((h) => ({
    heading: h,
    subheadings: [],
    keyPoints: [],
  }));
  const recs = lines
    .filter((l) => /^(-|\*)\s/.test(l) || /recommend/i.test(l))
    .slice(0, 8)
    .map((l) => l.replace(/^(-|\*)\s/, ""));
  ...
  return {
    ...
    competitorInsights: [],
    llmGeneratedOutline,
    seoRecommendations: recs.length
      ? recs
      : [
...
export async function generateInsights(
  ...
  const text = ((): string => {
    if (
      ...
      "content" in (_input as Record<string, unknown>)
    )
      return String((_input as { content?: unknown }).content || "").slice(
        0,
        6000
      );
      ...
      messages: [
        {
          role: "system",
          content:
            "Extract 5 concise, actionable SEO insights from the provided content. Return as bullet lines.",
        },
...
export async function getKeywordSuggestions(input: {
  seed: string;
  locale?: string;
  limit?: number;
...
export async function analyzeLinks(
  ...
  try {
    const res = await fetch(url, {
      signal: ac.signal,
      headers: { "User-Agent": "RankPilot-LinkAnalyzer/1.0" },
    });
  ...
  const backlinks = anchors.map((a) => {
    const isExternal =
      /^https?:\/\//i.test(a.href) && !a.href.startsWith(origin);
    const domain = ((): string => {
      try {
        return new URL(a.href, url).hostname.replace(/^www\./, "");
      } catch {
        return "unknown";
      }
    })();
    ...
    return {
      url: a.href.startsWith("http") ? a.href : new URL(a.href, url).toString(),
      domain,
      authority,
      anchorText: a.text,
  ...
  const avgAuthority = externals.length
    ? Math.round(
        externals.reduce((s, b) => s + b.authority, 0) / externals.length
      )
    : 0;
  return {
    backlinks: externals,
    domainMetrics: {
      totalBacklinks: externals.length,
      uniqueDomains,
...
export async function getSerpData(_input: unknown): Promise<SerpViewOutput> {
  const keyword = ((): string => {
    if (
      _input &&
      typeof _input === "object" &&
      "keyword" in (_input as Record<string, unknown>)
    ) {
      ...
      messages: [
        {
          role: "system",
          content:
            "Return top 10 likely SERP entries for the query as lines: position. title - url - snippet",
        },
    ...
    .map((l) => {
      ...
      return m
        ? {
            ...
            url: m[3],
            snippet: m[4].trim().slice(0, 220),
            features: [] as string[],
          }
        : null;
    })
    .filter((x): x is NonNullable<typeof x> => Boolean(x))
    .slice(0, 10) as SerpViewOutput["results"];
...
export async function auditUrl(input: {
  ...
  const req: SEOAuditRequest = {
    url: input.url,
    checkMobile: input.checkMobile ?? true,
  };
