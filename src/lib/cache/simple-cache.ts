...
export class InMemoryCache<V> implements ICache<V> {
  ...
  prune(maxAgeMs: number) {
    ...
    for (const [k, e] of this.store.entries())
      if (now - e.ts > maxAgeMs) this.store.delete(k);
  }
}
...
export class RedisCache<V> implements ICache<V> {
  ...
  private isPromiseLike<T>(val: unknown): val is Promise<T> {
    return (
      !!val &&
      typeof val === "object" &&
      ...
      this.client = {
        get: async (key: string) => {
          const res = await fetch(
            `${base}/get/${encodeURIComponent(this.prefix + key)}`,
        ...
        set: async (key: string, value: string) => {
          await fetch(
            `${base}/set/${encodeURIComponent(this.prefix + key)}/${encodeURIComponent(value)}`,
            { headers: { Authorization: `Bearer ${token}` } }
          );
        },
        del: async (key: string) => {
          await fetch(`${base}/del/${encodeURIComponent(this.prefix + key)}`, {
            headers: { Authorization: `Bearer ${token}` },
          });
        },
  ...
  private deserialize(s: string | null): CacheEntry<V> | undefined {
    ...
    try {
      return JSON.parse(s) as CacheEntry<V>;
    } catch {
      return undefined;
    }
  }
  ...
  get(key: string): CacheEntry<V> | undefined {
    ...
    if (this.isPromiseLike(out)) {
      ...
      console.warn(
        "RedisCache.get is async; returning undefined synchronously"
      );
      return undefined;
    }
...
export function createCache<V>(): ICache<V> {
  ...
  if (useRedis) {
    ...
    if (upstashUrl && upstashToken)
      return new RedisCache<V>({
        upstashUrl,
        upstashToken,
        prefix: "rankpilot:",
      });
    console.warn(
      "Redis cache enabled but Upstash env vars missing; falling back to in-memory"
    );
  }
