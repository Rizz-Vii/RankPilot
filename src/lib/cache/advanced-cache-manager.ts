...
export class AdvancedCacheManager {
  ...
  async set<T = unknown>(
    ...
    try {
      ...
      const compressedValue =
        !options?.skipCompression && config.compression
      ...
      if (options?.forceDistributed || size > 1024 * 1024) {
        // > 1MB goes to distributed
  ...
  async warmCache(
    ...
    const promises = warmingPlan.map(
      async ({ key, generator, userTier, tags }) => {
        try {
          const value = await generator();
          await this.set(key, value, userTier, { tags });
        } catch (error) {
          console.error(
            `[AdvancedCache] Cache warming failed for ${key}:`,
            error
          );
        }
      }
  ...
  private evictLeastUsed(userTier: string, requiredSpace: number): void {
    ...
    for (const [key, entry] of entries) {
      ...
      this.cacheStats.set(
        "evictions",
        (this.cacheStats.get("evictions") || 0) + 1
      );
    }
  }
  ...
  private updateAvgResponseTime(responseTime: number): void {
    ...
    const count =
      (this.cacheStats.get("hits") || 0) + (this.cacheStats.get("misses") || 0);
  ...
  private analyzeCacheLayer(cache: Map<string, CacheEntry>): {
    entries: number;
    size: number;
  } {
    let totalSize = 0;
  ...
  private getTopKeys(): Array<{ key: string; hits: number; size: number }> {
    const allEntries = Array.from(this.memoryCache.entries()).concat(
      Array.from(this.distributedCache.entries())
    );
  ...
  private getTierUsage(): Record<string, { entries: number; size: number }> {
    ...
    const allEntries = Array.from(this.memoryCache.values()).concat(
  ...
  private updateWarmingStrategy(): void {
    ...
    console.log(
      `[AdvancedCache] Top accessed keys: ${topKeys
        .slice(0, 5)
        .map((k) => k.key)
        .join(", ")}`
    );
  ...
  private async performPeriodicWarming(): Promise<void> {
    ...
    console.log(
      `[AdvancedCache] Performing periodic warming for ${commonPatterns.length} patterns`
    );
  }
