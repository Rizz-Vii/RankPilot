...
export class ThemeSystem {
  ...
  private listeners: Set<
    (theme: ThemeMode, preferences: ThemePreferences) => void
  > = new Set();
  ...
  private constructor() {
    ...
    if (typeof window !== "undefined") {
      ...
      try {
        const initialThemeClass = this.preferences.highContrast
          ? "high-contrast"
          : this.currentTheme;
        const cookiePayload = {
  ...
  private loadPreferences(): void {
    try {
      ...
      if (stored) {
        ...
        if (Object.prototype.hasOwnProperty.call(parsed, "highContrast"))
          this.userSetHighContrast = true;
        if (Object.prototype.hasOwnProperty.call(parsed, "reducedMotion"))
          this.userSetReducedMotion = true;
      }
    } catch (error) {
  ...
  private applyTheme(): void {
    ...
    // Typography sizes
    Object.entries(tokens.typography.fontSize).forEach(([k, v]) =>
    ...
    if (this.preferences.reducedMotion) {
      ...
      Object.entries(tokens.animation.duration).forEach(([k, v]) =>
        root.style.setProperty(`--animation-duration-${k}`, v)
      );
    }
    ...
    try {
      const resolvedTheme = this.preferences.highContrast
        ? "high-contrast"
        : this.currentTheme;
      const payload = {
        theme: resolvedTheme,
  ...
  private notifyListeners(): void {
    this.listeners.forEach((l) =>
      l(this.currentTheme, { ...this.preferences })
    );
  }
  ...
  private getThemeTokens(): ThemeTokens {
    if (this.preferences.highContrast || this.currentTheme === "high-contrast")
  ...
  setTheme(mode: ThemeMode): void {
    ...
    const effectiveUnchanged =
      prevMode === mode && this.currentTheme === nextTheme;
  ...
  private shallowEqualPrefs(
    a: Partial<ThemePreferences>,
    b: Partial<ThemePreferences>
  ): boolean {
      ...
      if (k === "customColors") {
        ...
        const bp =
          (bv as
            ...
            | undefined) || {};
        if (
          ap.primary !== bp.primary ||
          ap.secondary !== bp.secondary ||
          ap.accent !== bp.accent
        )
          return false;
        continue;
  ...
  setPreferences(preferences: Partial<ThemePreferences>): void {
    ...
    if ("mode" in preferences && typeof preferences.mode !== "undefined") {
      const { mode, ...rest } = preferences as Required<
        Pick<ThemePreferences, "mode">
      > &
    ...
    if (Object.prototype.hasOwnProperty.call(preferences, "highContrast"))
      this.userSetHighContrast = true;
    if (Object.prototype.hasOwnProperty.call(preferences, "reducedMotion"))
      this.userSetReducedMotion = true;
  ...
  subscribe(
    listener: (theme: ThemeMode, preferences: ThemePreferences) => void
  ): () => void {
  ...
  isHighContrast(): boolean {
    return (
      this.preferences.highContrast || this.currentTheme === "high-contrast"
    );
  }
...
export function useTheme() {
  ...
  const [preferences, setPrefsState] = React.useState<ThemePreferences>(() =>
    themeSystem.getPreferences()
  );

  React.useEffect(() => {
    setMounted(true);
    return themeSystem.subscribe((t, p) => {
      setThemeState(t);
      setPrefsState(p);
    });
  }, []);
  ...
  return {
    ...
    hasColorBlindnessSupport:
      themeSystem.hasColorBlindnessSupport.bind(themeSystem),
  };
}
