...
export async function fetchBillingData(
  ...
  try {
    ...
    if (subSnap.exists()) {
      ...
      const d: SubscriptionDoc =
        raw && typeof raw === "object" ? (raw as SubscriptionDoc) : {};
      const toMaybeDate = (
        value: SubscriptionDoc[keyof SubscriptionDoc]
      ): Date | null | undefined => {
    ...
    const invQ = query(
      collection(firestore, "financeInvoices"),
      where("userId", "==", userId),
      orderBy("period", "desc"),
      limit(invoiceLimit)
    );
    ...
    const invoices: BillingInvoice[] = invSnap.docs.map((d) => {
      ...
      const data: InvoiceDoc =
        rawInv && typeof rawInv === "object" ? (rawInv as InvoiceDoc) : {};
    ...
    const effectiveMonthly = latestPeriod
      ? invoices
          .filter((i) => i.period === latestPeriod && i.status === "paid")
          .reduce((s, i) => s + (i.amount || 0), 0)
      : 0;
    const nextInvoice = invoices.find((i) => i.status !== "paid") || null;
    logger.info("billing-ui.load.success", {
      userId,
