...
import(queueUtilsPath)
  .then((mod) => {
    ...
    const readCandidate =
      typeof m.readQueue === "function"
        ? m.readQueue
        : m.default && typeof m.default.readQueue === "function"
          ? m.default.readQueue
          : undefined;
    const appendCandidate =
      typeof m.appendTask === "function"
        ? m.appendTask
    ...
    if (appendCandidate) {
      appendTask = (t: DelegationQueueTask) =>
        (appendCandidate as (t: GenericQueueTask) => void)(
          t as unknown as GenericQueueTask
        );
    }
  })
  .catch(() => {
...
export function reviewerEmitTasks(
  options: { maxTasks?: number; includeWarnings?: boolean } = {}
): ReviewerPlanTask[] {
  const { maxTasks = 5, includeWarnings = false } = options;
  const ruleFilter = (process.env.TWO_AGENT_RULE_FILTER || "").trim();
  const ruleFilterRegex = ruleFilter
    ? new RegExp(
        ruleFilter
          .split(",")
          .map((r) => r.trim().replace(/[.*+?^${}()|[\]\\]/g, (r) => `\\${r}`))
          .join("|")
      )
    : null;
  const lintReportPath = path.resolve(
    process.env.TWO_AGENT_LINT_REPORT_PATH || "artifacts/eslint-report.json"
  );
  const tscDiagPath = path.resolve(
    process.env.TWO_AGENT_TSC_DIAGNOSTICS_PATH ||
      "artifacts/tsc-diagnostics.json"
  );
    ...
    try {
      ...
      if (Array.isArray(raw)) {
        ...
        const grouped = new Map<
          string,
          { issues: AnalysisIssue[]; fixable: boolean }
        >();
        ...
        for (const [key, bucket] of grouped.entries()) {
          ...
          const severity = bucket.issues.some(
          ...
          const task: ReviewerPlanTask = {
            ...
            summary:
              `Resolve ${rule}${bucket.fixable ? " (fixable)" : ""} in ${path.basename(file)} (${bucket.issues.length} occurrences)` +
  ...
  const tasks = combined.sort((a, b) => {
    const wa =
      (RULE_PRIORITY[a.rule] ?? (a.rule.startsWith("TS") ? 75 : 30)) +
      (a.fixable ? FIXABLE_BOOST : 0);
    const wb =
      (RULE_PRIORITY[b.rule] ?? (b.rule.startsWith("TS") ? 75 : 30)) +
      (b.fixable ? FIXABLE_BOOST : 0);
...
export function supervisorEnqueueReviewerTasks(
  tasks: ReviewerPlanTask[]
): number {
    ...
    try {
      ...
      if (f && existsSync(f)) {
        ...
        if (stat.mtimeMs > churnCutoff) {
          console.log(
            `[two-agent] skip (churn guard) ${t.taskId} recent mtime`
          );
          continue;
        }
      }
  ...
  if (added) {
    appendFileSync(
      path.resolve("metrics-snapshots.log"),
      `twoAgentEnqueue ${Date.now()} added=${added}\n`
    );
  }
  return added;
}
...
export async function runTwoAgentLintCycle(opts?: {
  maxTasks?: number;
  ...
  if (process.env.TWO_AGENT_SKIP_PREGEN !== "1") {
    const extractErr = (err: unknown): string =>
      ...
      typeof (err as { message?: unknown }).message === "string"
        ...
        : String(err);
    try {
      execSync("npm run lint:report:json --silent", { stdio: "pipe" });
    } catch (e) {
      console.warn("[two-agent] pregen eslint report failed", extractErr(e));
    }
    try {
      execSync("npm run diagnostics:tsc:json --silent", { stdio: "pipe" });
    } catch (e) {
      console.warn("[two-agent] pregen tsc diagnostics failed", extractErr(e));
    }
  }
  let requestedMax =
    opts?.maxTasks ?? Number(process.env.TWO_AGENT_MAX_TASKS || 5);
  ...
  if (process.env.TWO_AGENT_PLAN_RETURN_ALL === "1") {
    ...
    let filtered = full.filter((t) =>
      allowRequeue ? true : !(skipDone && doneSet.has(t.taskId))
    );
    if (!filtered.length) {
    ...
    try {
      if (!existsSync(path.dirname(offsetFile)))
        mkdirSync(path.dirname(offsetFile), { recursive: true });
    } catch {
      /* ignore */
    }
    try {
      offsetUsed = Number(readFileSync(offsetFile, "utf8")) || 0;
    } catch {
      offsetUsed = 0;
    }
    ...
    try {
      writeFileSync(offsetFile, String(offsetUsed));
    } catch {
      /* ignore */
    }
    plan = window;
  ...
  if (process.env.TWO_AGENT_PLANNER === "1" && plan.length) {
    try {
      const orderedRules = await planRulesWithAgents(
        plan.map((p) => ({ rule: p.rule, count: p.count }))
      );
  ...
  if (
    ...
    plan.some((t) => t.taskId.startsWith("AI-LINT-")) &&
    plan.some((t) => t.taskId.startsWith("AI-TSC"))
  ) {
  ...
  try {
    ...
    if (enableCodex) {
      const maxCodex = Math.max(
        0,
        Number(process.env.TWO_AGENT_CODEX_MAX_PER_BATCH || 2)
      );
      let tagged = 0;
      ...
      const extraCodexRulesRaw = (
        process.env.TWO_AGENT_CODEX_EXTRA_RULES || ""
      ).trim();
      ...
      if (extraCodexRules.size > effectiveMaxCodex) {
        effectiveMaxCodex = Math.min(
          hardCap,
          Math.max(extraCodexRules.size, effectiveMaxCodex)
        );
      }
      for (const t of plan) {
    ...
    /* ignore codex tagging errors */
  }
  // Hash plan to prevent churn (include occurrence counts and adaptive max)
  const hashPayload =
    plan.map((t) => `${t.taskId}:${t.count}`).join("|") +
    `#${adaptiveMax}#off${offsetUsed}`;
  const hash = String(hashCode(hashPayload));
  const forceReplan = process.env.TWO_AGENT_FORCE_REPLAN === "1" || false;
  const forceReplanAfter = Number(
    process.env.TWO_AGENT_FORCE_REPLAN_AFTER || 0
  );
  const iterationFile = ".codex/tmp/two-agent-plan-iteration.txt";
  let iterationCount = 0;
  try {
    if (existsSync(iterationFile))
      iterationCount = Number(readFileSync(iterationFile, "utf8")) || 0;
  } catch {
    /* ignore */
  }
  iterationCount++;
  try {
    ensureParentDir(iterationFile);
    writeFileSync(iterationFile, String(iterationCount));
  } catch {
    /* ignore */
  }
  try {
    const prev = existsSync(LAST_HASH_FILE)
      ? readFileSync(LAST_HASH_FILE, "utf8").trim()
      : "";
    const bypassHash =
      forceReplan ||
      (forceReplanAfter > 0 && iterationCount % forceReplanAfter === 0);
    if (!bypassHash && prev === hash) {
      if (drift !== undefined) {
        appendFileSync(
          path.resolve("metrics-snapshots.log"),
          `twoAgentPlannerDrift ${Date.now()} drift=${drift.toFixed(3)} unchangedHash=1\n`
        );
      }
      return {
        planned: 0,
    ...
    /* ignore hash persistence errors */
  }
  ...
  if (drift !== undefined) {
    appendFileSync(
      path.resolve("metrics-snapshots.log"),
      `twoAgentPlannerDrift ${timestamp} drift=${drift.toFixed(3)} adaptiveMax=${adaptiveMax} planned=${added}\n`
    );
  }
  // Trend line JSONL (lightweight): timestamp, planned, drift, considered, adaptiveMax
  try {
    appendFileSync(
      ".codex/tmp/two-agent-trends.jsonl",
      JSON.stringify({
        t: timestamp,
        planned: added,
        drift,
 ...
 */
async function planRulesWithAgents(
  rules: { rule: string; count: number }[]
): Promise<string[]> {
  const aggregated = new Map<string, number>();
  for (const r of rules)
    aggregated.set(r.rule, (aggregated.get(r.rule) || 0) + r.count);
  const list = Array.from(aggregated.entries()).map(([rule, count]) => ({
    rule,
    count,
  }));
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      try {
        ...
        const agentsMod = agentsUnknown as {
          AgentsClient?: new (cfg: { apiKey: string }) => AgentsClientLike;
        };
        const maybeClient = agentsMod?.AgentsClient
          ? new agentsMod.AgentsClient({
        ...
        if (maybeClient && maybeClient.responses?.create) {
          ...
          if (match) {
            ...
            if (Array.isArray(parsed))
              ordered = parsed.filter((r) => typeof r === "string");
          }
        }
      } catch {
        /* ignore agent SDK errors */
      }
      if (!ordered) {
        ...
        const completion = await client.chat.completions.create({
          ...
          messages: [
            {
              role: "system",
              content: "Return strictly JSON array of rule ids.",
            },
  ...
  const lastMsg =
    ...
    typeof (lastError as { message?: unknown }).message === "string"
      ...
      : String(lastError);
  console.warn(
    "[two-agent] planner retries exhausted, using fallback",
    lastMsg
  );
  return fallback;
}
