...
export async function fetchJournalEntries(
  ...
  const q = query(
    collection(db, "accountingJournalEntries"),
...
export async function seedSampleJournalEntries(
  ...
  for (let i = 0; i < months; i++) {
    const d = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth() - i, 5)
    );
    const period = d.toISOString().slice(0, 7);
    await addBalancedEntry(
      userId,
      teamId,
      d,
      period,
      ...
      [
        {
          account: "OPEX_SAL",
          side: "debit",
          amount: Math.round(baseRevenue * 0.3),
        },
        {
          account: "ASSET_CASH",
          side: "credit",
...
async function addBalancedEntry(
  userId: string,
