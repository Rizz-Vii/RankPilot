...
export class EnhancedAuthService {
  ...
  static async getUserAnalytics(): Promise<UserAnalytics> {
    try {
      ...
      const totalUsers = Object.values(tierCounts).reduce(
        (sum, count) => sum + count,
        0
      );
      ...
      return {
        ...
        subscriptionTrends: tiers.map((tier) => ({
          ...
          growth: Math.round(
            ((tierCounts[tier] || 0) / Math.max(1, totalUsers)) * 100
          ),
        })),
      };
    } catch (error) {
  ...
  static async updateLoginTracking(userId: string): Promise<void> {
    try {
      ...
      const prev =
        (snap.exists()
          ? (snap.data()?.loginCount as number | undefined)
          : undefined) ?? 0;
