...
function aggregateInvoices(
  invoices: FinanceInvoiceRuntime[],
  months: number
): AggregatedFinanceMetrics {
  if (!invoices.length) {
  ...
  const sum = (
    arr: FinanceInvoiceRuntime[],
    f: (x: FinanceInvoiceRuntime) => number
  ...
  const now = Date.now();
  function bucket(inv: FinanceInvoiceRuntime) {
    const due = inv.dueAt;
    if (!due) return ">60";
  ...
  unpaid.forEach((u) => {
    const b = bucket(u);
    if (!agingMap.has(b)) agingMap.set(b, { count: 0, amount: 0 });
    const rec = agingMap.get(b)!;
    rec.count++;
    rec.amount += u.amount || 0;
  });
  const aging = Array.from(agingMap.entries())
...
export async function fetchFinanceMetrics(
  ...
  months: number,
  teamId?: string
): Promise<AggregatedFinanceMetrics> {
  try {
    const conds = teamId
      ? [where("teamId", "==", teamId)]
      : [where("userId", "==", userId)];
    ...
    const invoices: FinanceInvoiceRuntime[] = snap.docs.map((d) =>
...
export function subscribeFinanceMetrics(
  userId: string,
  months: number,
  cb: (m: AggregatedFinanceMetrics) => void,
  teamId?: string
): Unsubscribe {
  const conds = teamId
    ? [where("teamId", "==", teamId)]
  ...
  const unsub = managedOnSnapshot(
    ...
    (snap: unknown) => {
      ...
      invoices = qs.docs.map((docSnap) =>
        mapFinanceInvoiceDoc(
          docSnap.id,
          docSnap.data() as FinanceInvoiceFirestore
        )
      );
    ...
    (err) => {
      if ((err as { code?: string })?.code === "permission-denied") {
        console.warn(
          "[FinanceMetrics] permission-denied accessing financeInvoices"
        );
        if (allowFinanceMocks()) {
          void getMockMetrics("finance")
            .then((mock) =>
