...
function aggregateCampaigns(
  ...
  const sum = (
    arr: MarketingCampaignDoc[],
    f: (x: MarketingCampaignDoc) => number
  ) => arr.reduce((a, x) => a + f(x), 0);
  const last = ordered.at(-1)!;
  const prev = ordered.at(-2);
  const lastArr = byPeriod[last];
  const prevArr = prev ? byPeriod[prev] : lastArr;
  const impressions = sum(lastArr, (c) => c.impressions || 0);
  const prevImpr = Math.max(
    1,
    sum(prevArr, (c) => c.impressions || 0)
  );
  const clicks = sum(lastArr, (c) => c.clicks || 0);
  const ctr = impressions ? (clicks / impressions) * 100 : 0;
  const prevClicks = sum(prevArr, (c) => c.clicks || 0);
  const prevCtr = prevImpr ? (prevClicks / prevImpr) * 100 : 0;
  const leads = sum(lastArr, (c) => c.leads || 0);
  const prevLeads = Math.max(
    1,
    sum(prevArr, (c) => c.leads || 0)
  );
  const spend = sum(lastArr, (c) => c.spend || 0);
  const revenue = sum(lastArr, (c) => c.revenue || 0);
  const roi = spend ? ((revenue - spend) / spend) * 100 : 0;
  const prevSpend = sum(prevArr, (c) => c.spend || 0);
  const prevRevenue = sum(prevArr, (c) => c.revenue || 0);
  const prevRoi = prevSpend ? ((prevRevenue - prevSpend) / prevSpend) * 100 : 0;
  const trendSeries = ordered.map((p) => {
    ...
    return {
      ...
      roi: sp ? ((rev - sp) / sp) * 100 : 0,
    };
  });
  const channelMap = new Map<
    string,
    { impressions: number; leads: number; spend: number; revenue: number }
  >();
  for (const c of filtered) {
    const key = c.channel || "unknown";
    if (!channelMap.has(key))
      channelMap.set(key, { impressions: 0, leads: 0, spend: 0, revenue: 0 });
  ...
  const channelPerformance = Array.from(channelMap.entries())
    .map(([channel, v]) => ({
      ...
      leads: v.leads,
      roi: v.spend ? ((v.revenue - v.spend) / v.spend) * 100 : 0,
    }))
  ...
  const kpis: AggregatedMarketingMetrics["kpis"] = [
    {
      ...
      label: "Impressions",
      value: impressions,
      delta: impressions ? ((impressions - prevImpr) / prevImpr) * 100 : 0,
      trend: trendSeries.map((t) => t.impressions),
      intent: "neutral",
      target: impressions * 1.1,
    },
    {
      key: "ctr",
      label: "CTR %",
      ...
      delta: ctr - prevCtr,
      trend: trendSeries.map((t) => Number(t.ctr.toFixed(2))),
      intent: ctr > 3 ? "success" : "warning",
      target: 3.5,
    },
    {
      key: "leads",
      label: "Leads",
      value: leads,
      delta: ((leads - prevLeads) / prevLeads) * 100,
      trend: trendSeries.map((t) => t.leads),
      intent: "neutral",
      target: leads * 1.15,
    },
    {
      ...
      delta: roi - prevRoi,
      trend: trendSeries.map((t) => Number(t.roi.toFixed(1))),
      intent: roi > 50 ? "success" : "warning",
      target: 60,
    },
  ];
  return {
    kpis,
    campaigns: filtered.slice(0, 300),
    channelPerformance,
    trendSeries,
  };
}
...
export async function fetchMarketingMetrics(
  ...
  months: number,
  teamId?: string
): Promise<AggregatedMarketingMetrics> {
  try {
    const conds = teamId
      ? [where("teamId", "==", teamId)]
      : [where("userId", "==", userId)];
    ...
    const campaigns: MarketingCampaignDoc[] = mapDocs(
      snap as QuerySnapshot<DocumentData>,
      (id, data) =>
        mapMarketingCampaignDoc(id, data as MarketingCampaignFirestore)
    );
...
export function subscribeMarketingMetrics(
  userId: string,
  months: number,
  cb: (m: AggregatedMarketingMetrics) => void,
  teamId?: string
): Unsubscribe {
  const conds = teamId
    ? [where("teamId", "==", teamId)]
  ...
  const unsub = managedOnSnapshot(
    q,
    (snap) => {
      campaigns = mapDocs(snap as QuerySnapshot<DocumentData>, (id, data) =>
        mapMarketingCampaignDoc(id, data as MarketingCampaignFirestore)
      );
      cb(aggregateCampaigns(campaigns, months));
    },
    (err) => {
      console.error("[MarketingMetrics] snapshot error", err);
    },
    { debounceMs: 120 }
  );
  return () => {
