...
export async function fetchRecentSalesMetricsSnapshots(
  userId: string,
  teamId?: string,
  max = 12
): Promise<SalesMetricsSnapshotDoc[]> {
  const q = query(
...
export async function fetchRecentSalesForecastSnapshots(
  userId: string,
  teamId?: string,
  max = 8
): Promise<SalesForecastSnapshotDoc[]> {
  const q = query(
...
function mapSalesMetricsSnapshotDoc(
  id: string,
  raw: unknown
...
function mapSalesForecastSnapshotDoc(
  id: string,
  raw: unknown
