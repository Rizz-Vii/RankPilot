...
export async function dualWriteNeuralCrawlerAggregate(
  full: FullResultLike & { _skipAgg?: boolean }
) {
  try {
    if (full._skipAgg) return; // optional escape hatch for tests
    if (process.env.NEXT_PUBLIC_DATA_MIN_NEURAL_CRAWLER_DUAL_WRITE !== "1")
      return;
