...
function synthetic(target: string, reason: string): FirecrawlCrawlResponse {
  return {
    pages: [
      {
        url: target,
        content: "SYNTHETIC_CRAWL_CONTENT",
        status: 200,
        title: "Synthetic Crawl",
        links: [],
        canonicalUrl: target,
        metaDescription: `Synthetic description for ${target}`,
      },
    ],
    fallback: true,
...
export async function runFirecrawl(
  ...
  if (!key) {
    recordFallback("firecrawl:no_api_key");
    return synthetic(targetUrl, "no_api_key");
  }
  ...
  try {
    ...
    if (!resp.ok) {
      recordFallback("firecrawl:http_" + resp.status);
      return synthetic(targetUrl, "http_" + resp.status);
    }
    const json = await resp.json();
    const pagesUnknown: unknown =
      json && typeof json === "object"
        ? (json as Record<string, unknown>).pages
        : undefined;
    ...
    const pages: FirecrawlPageResult[] = raw.slice(0, limit).map((p, idx) => {
      ...
      return {
        ...
        links,
        canonicalUrl:
          typeof obj.canonicalUrl === "string" ? obj.canonicalUrl : undefined,
        metaDescription: metaDescSource,
      };
    });
    const validation = firecrawlCrawlResponseSchema.safeParse({ pages });
    const elapsedMs = Date.now() - start;
    recordRouteLatency("firecrawl/crawl", elapsedMs);
    if (!validation.success) {
      recordFallback("firecrawl:validation");
      return { ...synthetic(targetUrl, "validation"), elapsedMs };
    }
