...
export class AgentSystemBootstrap {
  ...
  async executeAll(): Promise<boolean> {
    ...
    for (const [name, agent] of Array.from(this.agents)) {
      ...
      try {
        ...
        if (success) {
          console.log(
            `✅ ${name} completed successfully in ${executionTime}ms`
          );
        } else {
  ...
  async emergencyRollback(): Promise<boolean> {
    ...
    for (const [name, agent] of Array.from(this.agents)) {
      try {
        ...
        console.log(
          `${success ? "✅" : "❌"} Rollback ${name}: ${success ? "SUCCESS" : "FAILED"}`
        );
      } catch (error) {
    ...
    console.log(
      `📊 Emergency rollback complete: ${successCount}/${totalCount} agents rolled back successfully`
    );
  ...
  listAgents(): Array<{
    ...
    return Array.from(this.agents.values()).map((agent) => ({
      ...
      safetyLevel: agent.safetyConstraints.requiresHumanApproval
        ? "Manual Approval"
        : "Autonomous",
    }));
  }
