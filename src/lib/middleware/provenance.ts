...
import {
  ...
  recordProvenanceObservation,
  recordProvenanceReason,
...
export type ProvenanceTag =
  ...
  | "mixed"
  | "unknown"
  | "error"; // 'error' treated internally and normalized to 'synthetic'

const ALLOWED: ProvenanceTag[] = [
  "live",
  "cache",
  "synthetic",
  "hybrid",
...
export function enforceProvenance<T extends Record<string, unknown>>(
  obj: T,
  ctx: EnforcementContext = {}
): T & { __provenance: ProvenanceTag; provenance?: never } {
  ...
  if (typeof direct === "string" && ALLOWED.includes(direct as ProvenanceTag))
...
export function enforceProvenanceOnChunk<T extends Record<string, unknown>>(
  chunk: T,
  ctx: EnforcementContext = {}
): T {
  ...
  if (
    ...
    ALLOWED.includes(existing as ProvenanceTag)
  )
    tag = existing as ProvenanceTag;
...
export function provenanceTag(obj: unknown): string | undefined {
  ...
  return (
    ((obj as Record<string, unknown>).__provenance as string | undefined) ||
...
export function withProvenance<R, A extends unknown[]>(
    ...
    try {
      ...
      const message =
        ...
        typeof (e as { message?: unknown }).message === "string"
          ...
          : "internal error";
      return enforceProvenance(
        { error: "internal", message } as Record<string, unknown>,
        { ...ctx, note: "error-path" }
      ) as unknown as R;
    }
  };
