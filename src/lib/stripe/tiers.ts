...
export const STRIPE_PLANS = Object.freeze({
    ...
    enterprise: {
        name: 'Enterprise',
        priceId: {
            monthly: envOr('STRIPE_PRICE_ENTERPRISE_MONTHLY', 'price_enterprise_monthly'),
            yearly: envOr('STRIPE_PRICE_ENTERPRISE_YEARLY', 'price_enterprise_yearly'),
        },
...
const PriceIdSchema = z.object({ monthly: z.string().nullable(), yearly: z.string().nullable() });
const PlanSchema = z.object({
    name: z.string(),
    priceId: PriceIdSchema,
    price: z.object({ monthly: z.number(), yearly: z.number() }),
    features: z.array(z.string()),
    limits: z.object({ auditsPerMonth: z.number(), keywords: z.number(), reports: z.number(), competitors: z.number() }),
});
...
(() => {
    ...
    if (!parsed.success) {
        logger.warn('stripe.tiers.validation_failed', { issues: parsed.error.issues });
    }
...
export function getPriceId(tier: PlanType, interval: BillingInterval = 'monthly'): string | null {
