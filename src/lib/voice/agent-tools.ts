    ...
    if (db && typeof (db as any).collection === "function") {
      try {
        const q = await (db as any)
          .collection("voice_slots")
          .where("serviceId", "==", serviceId || null)
          .limit(50)
          .get();
        const slots: Array<Record<string, unknown>> = [];
        q.forEach((d: any) =>
          slots.push({ id: d.id, ...(d.data() as object) })
        );
        return { ok: true, slots };
      } catch (err) {
        logger.degraded("getAvailability.firestore_error", {
          error: String(err),
        });
        return { ok: true, slots: [] };
      }
...
export async function holdSlot({
  ...
  if (db && typeof (db as any).runTransaction === "function") {
    ...
    try {
      await (db as any).runTransaction(async (tx: any) => {
        ...
        if (snap.exists) {
          ...
          if (
            data?.status === "held" &&
            data?.heldUntil &&
            new Date(data.heldUntil) > new Date()
          ) {
      ...
      logger.error("holdSlot.transaction_error", {
        error: String(err),
        slotId,
      });
...
export async function createAppointment(payload: Record<string, unknown>) {
  logger.info("createAppointment", {
    payload: { serviceId: payload?.serviceId, start: payload?.start },
  });
    ...
    try {
      ...
      await (db as any).runTransaction(async (tx: any) => {
        if (holdRef) {
          ...
          if (hold.heldUntil && new Date(hold.heldUntil) < new Date())
      ...
      try {
        ...
        if (typeof (sendConfirmationImpl as any) === "function")
          await (sendConfirmationImpl as any)({ apptId, customer, payload });
      } catch (e) {
        logger.degraded("createAppointment.confirmation_failed", {
          error: String(e),
          apptId,
        });
      }
      ...
      logger.error("createAppointment.transaction_error", {
        error: String(err),
        payload,
      });
      return { ok: false, error: String(err) };
  ...
  try {
    if (db && typeof (db as any).collection === "function") {
      ...
      try {
        ...
        logger.degraded("sendConfirmation.voice_confirmations_failed", {
          error: String(e),
          apptId,
        });
      }
      ...
      try {
        ...
        logger.degraded("sendConfirmation.email_enqueue_failed", {
          error: String(e),
          apptId,
        });
      }
...
export function buildEmailQueuePayload({
  apptId,
  customer,
  payload,
}: {
