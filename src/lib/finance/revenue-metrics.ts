...
export function computeRevenueMetrics(
  ...
  const active = subs.filter(
    (s) =>
      s.status === "active" &&
      (!s.canceledAt || s.canceledAt.getTime() > asOf.getTime())
  );
  ...
  const startActive = subs.filter(
    (s) =>
      s.startedAt.getTime() <= periodStart.getTime() &&
      (!s.canceledAt || s.canceledAt.getTime() > periodStart.getTime())
  );
  const startSet = new Set(startActive.map((s) => s.userId));
  const churnedThisPeriod = subs
    .filter(
      (s) =>
        s.canceledAt &&
        s.canceledAt.getTime() >= periodStart.getTime() &&
        s.canceledAt.getTime() <= asOf.getTime()
    )
    .map((s) => s.userId);
  const churnUnique = new Set(
    churnedThisPeriod.filter((id) => startSet.has(id))
  );
  const churnRatePct = startSet.size
    ? +((churnUnique.size / startSet.size) * 100).toFixed(2)
    : 0;
  const arpu = activeCustomers.size
    ? +(mrr / activeCustomers.size).toFixed(2)
    : 0;
  const churnRateMonthly = churnRatePct / 100;
  const ltv =
    churnRateMonthly > 0 ? +(arpu / churnRateMonthly).toFixed(2) : null;
  return {
    mrr: +mrr.toFixed(2),
    arr: +arr.toFixed(2),
    churnRatePct,
