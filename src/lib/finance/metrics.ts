...
function detectMode(req?: Request): "mock" | "live" {
  ...
  const envMode =
    (process.env.FINANCE_METRICS_MODE as "mock" | "live" | undefined) || "mock";
  return envMode;
}

export async function getFinanceMetrics(
  req?: Request
): Promise<{ data: FinanceMetrics; headers: Record<string, string> }> {
  ...
  try {
    ...
    /* ignore auth errors; fall back to zeros below */
  }
  ...
  if (!uid && !scopeTeamId) {
    ...
    return {
      ...
      headers: {
        ...
        "x-finance-diagnostics": `auth=${authDiagnostics}; items=0; months=${months}; scope=unknown`,
      },
    };
  }
  ...
  try {
    ...
    if (/FAILED_PRECONDITION|9\b/.test(msg)) {
      ...
      return {
        ...
        headers: {
          "X-Finance-Metrics-Live": "1",
          "x-finance-diagnostics": `auth=${authDiagnostics}; error=fetch_invoices`,
        },
      };
    }
  }
  ...
  if (!invoices.length) {
    return {
      ...
      headers: {
        "X-Finance-Metrics-Live": "1",
        "x-finance-diagnostics": `auth=${authDiagnostics}; items=0; months=${months}; scope=${scopeTeamId ? "team" : "user"}`,
      },
    };
  }
  ...
  const sum = (arr: Inv[], f: (x: Inv) => number) =>
  ...
  const users = new Set(
    filtered
      .map(
        (i) =>
          ("userId" in i && typeof i.userId === "string" && i.userId) ||
          ("teamId" in i && typeof i.teamId === "string" && i.teamId) ||
          undefined
      )
      .filter(Boolean)
  ).size;
