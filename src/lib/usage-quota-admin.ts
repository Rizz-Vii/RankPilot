...
export class AdminUsageQuotaManager {
  ...
  private pickFields(usageType: UsageType): {
    usageField: string;
    limitField: keyof UsageQuota["limits"];
  } {
    switch (usageType) {
      case "audit":
        return { usageField: "auditsPerformed", limitField: "auditsPerMonth" };
      case "keyword":
        return { usageField: "keywordSearches", limitField: "keywords" };
      case "report":
        return { usageField: "reportsGenerated", limitField: "reports" };
      case "competitor":
        return { usageField: "competitorAnalyses", limitField: "competitors" };
      default:
        return { usageField: "auditsPerformed", limitField: "auditsPerMonth" };
    }
  }
  ...
  async checkUsageLimit(
    ...
    const currentUsage = quota.usage[
      usageField as keyof UsageQuota["usage"]
    ] as number;
  ...
  async incrementUsage(
    ...
    try {
      await adminDb.runTransaction(async (tx) => {
        ...
        let quota: UsageQuota | null = snap.exists
          ? (snap.data() as UsageQuota)
          : null;
        if (!quota) {
        ...
        const current =
          quota.usage?.[usageField as keyof UsageQuota["usage"]] ?? 0;
        const updated = { ...quota.usage, [usageField]: current + amount };
        tx.set(
          ref,
          { usage: updated, lastUpdated: new Date() },
          { merge: true }
        );
      });
      return true;
  ...
  async getUsageStats(userId: string) {
    ...
    return {
      ...
      daysUntilReset: Math.ceil(
        (q.currentPeriodEnd.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
      ),
    };
  }
  ...
  async enforceUsageLimit(
    ...
    if (!check.allowed)
      return { success: false, error: check.reason || "Usage limit exceeded" };
