...
export function computeNextRun(cfg: ScheduleConfig): number {
  ...
  if (cfg.cron && cfg.interval)
    throw new Error("Specify either cron or interval, not both");
  const now = Date.now();
  if (cfg.interval) {
  ...
  if (cfg.cron) {
    ...
    const matchMinute = (m: number) =>
      minTok === "*" ? true : Number(minTok) === m;
    const matchHour = (h: number) =>
      hourTok === "*" ? true : Number(hourTok) === h;
