...
export class ZapierWorkflowBuilder extends EventEmitter {
  ...
  constructor() {
    ...
    const disable =
      ...
      (process.env.ZAPIER_DISABLE_FIRESTORE === "1" ||
  ...
  private async syncTemplatesFromFirestore(force = false): Promise<void> {
    if (
      ...
      Date.now() - this.lastTemplateSync <
        ZapierWorkflowBuilder.TEMPLATE_SYNC_TTL
    )
      return;
    try {
      ...
      snapshot.forEach((d) => {
        ...
        if (raw.id && !this.templates.has(raw.id)) {
          this.templates.set(raw.id, {
            ...
            category:
              (raw.category as WorkflowTemplate["category"]) ||
              "seo-monitoring",
  ...
  async createWorkflowFromTemplate(
    ...
    if (!this.checkTierAccess(tier, template.requiredTier)) {
      throw new Error(
        `Insufficient tier access. Required: ${template.requiredTier}, Current: ${tier}`
      );
    }
    ...
    try {
      ...
      const triggerResults = await Promise.all(
        workflow.triggers.map((trigger) =>
          this.executeTrigger(trigger, workflow)
        )
      );
      ...
      const actionResults = await Promise.all(
        workflow.actions.map((action) =>
          this.executeAction(action, triggerResults, workflow)
        )
      );
      ...
      // Update success rate
      const successCount =
        workflow.metadata.runCount * (workflow.metadata.successRate / 100);
      workflow.metadata.runCount++;
      workflow.metadata.successRate =
        (successCount / workflow.metadata.runCount) * 100;
   ...
   */
  private async executeNeuroSEOTrigger(
    trigger: ZapierTrigger,
    workflow: ZapierWorkflow
  ): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        `NeuroSEO trigger error: ${error instanceof Error ? error.message : error}`
      );
    }
  }
   ...
   */
  private async executeKeywordTrigger(
    trigger: ZapierTrigger,
    workflow: ZapierWorkflow
  ): Promise<unknown> {
   ...
   */
  private async executeCompetitorTrigger(
    trigger: ZapierTrigger,
    workflow: ZapierWorkflow
  ): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        `Competitor trigger error: ${error instanceof Error ? error.message : error}`
      );
    }
  }
      ...
      try {
        switch (action.type) {
          ...
          case "update-dashboard":
            return await this.executeDashboardAction(
              action,
              triggerData,
              workflow
            );
          case "generate-report":
            return await this.executeReportAction(
              action,
              triggerData,
              workflow
            );
  ...
  private async executeEmailAction(
    ...
    return {
      ...
      subject:
        (mapped.subject as string | undefined) || "SEO Alert from RankPilot",
      timestamp: Date.now(),
    };
  }
  ...
  private async executeSlackAction(
    ...
    return {
      ...
      message:
        (mapped.message as string | undefined) ||
        "SEO update from RankPilot automation",
      timestamp: Date.now(),
    };
  }
  ...
  private async executeTaskAction(
    ...
    return {
      ...
      description:
        (mapped.description as string | undefined) ||
        "Automated task from RankPilot workflow",
      timestamp: Date.now(),
    };
  }
   ...
   */
  private async executeDashboardAction(
    action: ZapierAction,
    triggerData: unknown[],
    workflow: ZapierWorkflow
  ): Promise<unknown> {
    ...
    try {
      ...
      throw new Error(
        `Dashboard action error: ${error instanceof Error ? error.message : error}`
      );
    }
  }
   ...
   */
  private async executeSheetAction(
    action: ZapierAction,
    triggerData: unknown[],
    workflow: ZapierWorkflow
  ): Promise<unknown> {
   ...
   */
  private mapTriggerDataToAction(
   ...
   */
  private getValueFromPath<T = unknown>(
    data: unknown,
    path: string
  ): T | undefined {
  ...
  getWorkflowAnalytics(workflowId: string): {
    ...
    runCount: number;
    successRate: number;
    lastRun: ZapierWorkflow["lastRun"];
    created: number;
    updated: number;
  } | null {
    const workflow = this.workflows.get(workflowId);
    ...
    try {
      ...
      await setDoc(
        docRef,
        {
          ...
          __provenance:
            workflow.metadata.runCount === 0 ? "seed|user-init" : "user",
        },
        { merge: true }
      );
    } catch (err) {
  ...
  private async deletePersistedWorkflow(workflowId: string): Promise<void> {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(
        "[ZapierWorkflowBuilder] deletePersistedWorkflow failed:",
        msg
      );
    }
  }
  ...
  private async loadPersistedWorkflows(): Promise<void> {
    try {
      ...
      const msg = err instanceof Error ? err.message : String(err);
      console.warn(
        "[ZapierWorkflowBuilder] loadPersistedWorkflows failed:",
        msg
      );
    }
  }
