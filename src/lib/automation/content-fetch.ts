...
export async function fetchRewriteSourceContents(urls: string[], opts?: { max?: number; timeoutMs?: number; deterministicFallback?: boolean }) {
    ...
    for (const url of subset) {
        ...
        try {
            const crawl = await crawler.crawl(url, { respectRobots: true, cacheTtlMs: 5 * 60_000 });
            const text = (crawl.title ? crawl.title + '\n\n' : '') + (crawl.content || '');
            results[url] = text.trim().length > 80 ? truncate(text, 10_000) : deterministicPlaceholder(url, opts?.deterministicFallback);
        } catch {
            results[url] = deterministicPlaceholder(url, opts?.deterministicFallback);
        }
    }
