...
export async function listAutomationRecipes(userId: string, teamId?: string) {
  ...
  return snap.docs.map((d) =>
    deserializeRecipe({ id: d.id, ...(d.data() as Record<string, unknown>) })
  );
}
...
export async function listRecentAutomationRuns(
  ...
  return snap.docs.map((d) => {
    ...
    return {
      ...
      recipeId,
      startedAt:
        (data.startedAt as TsLike | undefined)?.toDate?.() || new Date(),
      finishedAt:
        (data.finishedAt as TsLike | undefined)?.toDate?.() || new Date(),
      actions: Array.isArray(data.actions)
        ? (data.actions as AutomationRunResult["actions"])
        : [],
      status:
        data.status === "ok" ||
        data.status === "partial" ||
        data.status === "error"
          ? (data.status as AutomationRunLog["status"])
          : "ok",
      createdAt:
        (data.createdAt as TsLike | undefined)?.toDate?.() || new Date(),
    } as AutomationRunLog;
  });
}
  ...
  return {
    ...
    schedule:
      typeof data.schedule === "object" && data.schedule
        ? (data.schedule as AutomationRecipe["schedule"])
        : {},
    actions: Array.isArray(data.actions)
      ? (data.actions.filter(
          (a) => typeof a === "string"
      ...
      : [],
    actionConfigs:
      typeof data.actionConfigs === "object" && data.actionConfigs
        ? (data.actionConfigs as Record<string, unknown>)
        : {},
    config:
...
export function defaultRecipeTemplate(
  userId: string,
  teamId?: string
): Omit<
  AutomationRecipe,
