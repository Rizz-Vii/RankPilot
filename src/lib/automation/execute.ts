...
const asCfg = <T extends keyof ActionConfigMap>(
  cfg: ActionConfigMap | undefined,
  key: T
): ActionConfigMap[T] | undefined => {
...
function normalizeInvoice(raw: unknown): NormalizedInvoice | null {
  const r =
    raw && typeof raw === "object"
      ? (raw as PossibleInvoiceShape)
      : ({} as PossibleInvoiceShape);
  let period: string;
  ...
  const paidAt: Date | undefined =
    r.paidAt && typeof r.paidAt.toDate === "function"
      ? r.paidAt.toDate()
      : undefined;
  const dueAt: Date | undefined =
    r.dueAt && typeof r.dueAt.toDate === "function"
      ? r.dueAt.toDate()
      : undefined;
  return {
    userId: typeof r.userId === "string" ? r.userId : "unknown",
    period,
    status: status as unknown as InvoiceRecord["status"],
    amount,
    paidAt,
    dueAt,
  };
...
export async function runDueAutomationRecipes(
  userId: string,
  teamId?: string
): Promise<AutomationRunResult[]> {
  const recipes = await listAutomationRecipes(userId, teamId);
  const now = new Date();
  const due = recipes.filter(
    (r) => r.active && r.nextRun && r.nextRun <= now
  ) as AutomationRecipeLike[];
  ...
  for (const recipe of due) {
    ...
    for (const action of recipe.actions) {
      try {
        const cfg = asCfg(
          recipe.actionConfigs,
          action as keyof ActionConfigMap
        );
        if (action === "runNeuroSEOAnalysis") {
          ...
          const urls: string[] =
            ...
            urlsCandidate.every((u) => typeof u === "string") &&
            urlsCandidate.length
              ? urlsCandidate
              : ["https://example.com"];
          const keywords: string[] =
            Array.isArray(keywordsCandidate) &&
            keywordsCandidate.every((k) => typeof k === "string") &&
            keywordsCandidate.length
              ? keywordsCandidate
              : ["example"];
          await new NeuroSEOSuite().runAnalysis({
            urls,
          ...
          const to =
            typeof (cfg as { to?: unknown })?.to === "string"
              ? (cfg as { to?: string }).to!
              : "digest@example.com";
          const subject =
            typeof (cfg as { subject?: unknown })?.subject === "string"
              ? (cfg as { subject?: string }).subject!
              : "NeuroSEO Digest";
          const urlsList = (cfg as { urls?: unknown })?.urls;
          const keywordsList = (cfg as { keywords?: unknown })?.keywords;
          const urls =
            Array.isArray(urlsList) &&
            urlsList.every((u) => typeof u === "string")
              ? urlsList
              : ["https://example.com"];
          const keywords =
            Array.isArray(keywordsList) &&
            keywordsList.every((k) => typeof k === "string")
              ? keywordsList
              : ["example"];
          ...
          try {
            ...
            const urls: string[] =
              ...
              urlsCandidate.every((u) => typeof u === "string") &&
              urlsCandidate.length
                ? urlsCandidate.slice(0, 3)
                : ["https://example.com"];
            const sourceMap = await fetchRewriteSourceContents(urls, {
              max: 3,
              deterministicFallback: true,
            });
            const batchId = await addDoc(collection(db, "rewriteBatches"), {
              userId,
              teamId: recipe.teamId || null,
              recipeId: recipe.id!,
              createdAt: Timestamp.fromDate(new Date()),
            ...
            for (const url of urls) {
              ...
              const analysis = await engine.generateRewrites({
                originalContent,
                targetKeywords: (() => {
                  const kw = (cfg as { keywords?: unknown })?.keywords;
                ...
                seoRequirements: {
                  primaryKeyword: (() => {
                    const kw = (cfg as { keywords?: unknown })?.keywords;
                    return Array.isArray(kw) && typeof kw[0] === "string"
                      ? kw[0]
                      : "example";
                  })(),
                  secondaryKeywords: (() => {
              ...
              await addDoc(collection(db, "rewriteVariants"), {
                batchId: batchId.id,
                url,
                userId,
                teamId: recipe.teamId || null,
            ...
            await updateDoc(doc(db, "rewriteBatches", batchId.id), {
              status: "complete",
          ...
          try {
            const range = ((): "30d" | "90d" | "ytd" => {
              const r = (cfg as { range?: unknown })?.range;
              return r === "90d" || r === "ytd" ? r : "30d";
            })();
            const dealsQ = query(
              collection(db, "salesDeals"),
              where(
                recipe.teamId ? "teamId" : "userId",
                "==",
                recipe.teamId || userId
              )
            );
            ...
            if (!deals.length)
              ...
              await addDoc(collection(db, "salesMetricsSnapshots"), {
                userId,
                teamId: recipe.teamId || null,
                range,
                pipeline,
          ...
          try {
            ...
            await addDoc(collection(db, "salesForecastSnapshots"), {
              userId,
              teamId: recipe.teamId || null,
              period,
          ...
          try {
            ...
            if (!invoices.length)
              ...
              await addDoc(collection(db, "financeRevenueSnapshots"), {
                userId,
                teamId: recipe.teamId || null,
                period: last,
                mrr,
          ...
          try {
            ...
            await addDoc(collection(db, "financeInvoiceAgingSummaries"), {
              userId,
              teamId: recipe.teamId || null,
              buckets,
              createdAt: Timestamp.fromDate(new Date()),
          ...
          try {
            const { seedSampleJournalEntries } = await import(
              "@/lib/services/accounting-ledger"
            );
            await seedSampleJournalEntries(userId, recipe.teamId || null, {
              months: 1,
            });
            actionsResults.push({
              type: action,
          ...
          try {
            ...
            const { fetchJournalEntries } = await import(
              "@/lib/services/accounting-ledger"
            );
            ...
            if (!entries.length) {
              actionsResults.push({
                type: action,
                status: "skipped",
                message: "No journal entries (seed first)",
              });
            } else {
              const figures = computePnL(period, entries);
              await addDoc(collection(db, "accountingReportSnapshots"), {
                userId,
                teamId: recipe.teamId || null,
                type: "pnl",
                period,
          ...
          try {
            ...
            const { computeBalanceSheet } = await import(
              "@/lib/accounting/aggregation"
            );
            const entries = await fetchJournalEntries(
              userId,
              recipe.teamId || null
          ...
          try {
            ...
            const invSnap = await getDocs(invQ);
            const unpaid = invSnap.docs
              .map((d) => normalizeInvoice(d.data()))
              .filter((i) => i && i.status !== "paid").length;
            await addDoc(collection(db, "accountingReportSnapshots"), {
              userId,
              teamId: recipe.teamId || null,
              type: "reconciliation",
          ...
          actionsResults.push({
            type: action as AutomationActionType,
            status: "error",
    ...
    const recipeForSchedule = {
      name: "auto",
      schedule:
        scheduleCandidate && typeof scheduleCandidate === "object"
          ? scheduleCandidate
          : { cron: "@daily" },
      createdAt:
        (recipe as unknown as { createdAt?: Date }).createdAt ?? new Date(),
      updatedAt: new Date(),
    ...
    const nextRun = computeNextRun(
      now,
      recipeForSchedule as unknown as {
        name: string;
        schedule: {
          intervalMinutes?: number;
          cron?: string;
