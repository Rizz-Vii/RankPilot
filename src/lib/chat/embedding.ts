 ...
 */
export async function maybeEmbedMessage(params: { uid: string; sessionId: string; messageDocId: string; question: string; }) {
    ...
    try {
        ...
        await adminDb.collection('chatLogs').doc(params.uid).collection('sessions').doc(params.sessionId)
            .collection('messages').doc(params.messageDocId).set({ embedding: { model: 'text-embedding-3-small', question: vector } }, { merge: true });
    } catch {
        // swallow errors
    }
}
