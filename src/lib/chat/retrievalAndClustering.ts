...
export async function retrieveSimilarMessages(params: {
  uid: string;
  sessionId: string;
  ...
  try {
    ...
    snap.forEach((doc) => {
      ...
      const vec = Array.isArray(d.embedding?.question)
    ...
    console.error("retrieveSimilarMessages error", err);
    return [];
  }
}
...
export async function maybeClusterKeywords(params: {
  uid: string;
  sessionId: string;
  apiKey: string;
  ...
  try {
    ...
    const rawData = sessSnap.data() as
      | Partial<{ messageCount: unknown }>
      | undefined;
    const data = rawData || {};
    const msgCount =
      typeof data.messageCount === "number" ? data.messageCount : 0;
    if (msgCount < clusteringInterval || msgCount % clusteringInterval !== 0)
      return false;
    ...
    msgsSnap.forEach((doc) => {
      const d = doc.data() as Partial<ChatMessageDoc>;
      if (
        Array.isArray(d.embedding?.question) &&
        typeof d.question === "string"
      ) {
    ...
    const k = Math.max(
      2,
      Math.min(6, Math.round(Math.sqrt(vectors.length / 2)))
    );
    ...
    for (let c = 0; c < k; c++) {
      ...
      try {
          ...
          messages: [
            {
              role: "system",
              content:
                "You label a cluster of user SEO intent queries with a concise 3-6 word topic.",
            },
            {
              role: "user",
              content: "Queries:\n" + sampleTexts.join("\n") + "\nLabel:",
        ...
        console.error("cluster labeling error", err);
      }
      clusters.push({ label, size: indices.length, examples: sampleTexts });
    }
    await sessionRef.set(
      { clusters, lastClusteredAt: new Date() },
      { merge: true }
    );
    return true;
  } catch (err) {
    console.error("maybeClusterKeywords error", err);
    return false;
  }
}
