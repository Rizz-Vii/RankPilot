...
export async function maybeSummarizeSession(params: {
  ...
  try {
    ...
    const sessionRaw: SessionData =
      (sessionSnap.data() as unknown as SessionData) || {};
    const messageCount: number =
      typeof sessionRaw.messageCount === "number"
        ? (sessionRaw.messageCount as number)
        : 0;
    const lastSummarizedCount: number =
      typeof sessionRaw.lastSummarizedCount === "number"
        ? (sessionRaw.lastSummarizedCount as number)
        : 0;
    if (
      messageCount < MESSAGE_INTERVAL ||
      messageCount - lastSummarizedCount < MESSAGE_INTERVAL
    ) {
    ...
    let existingSummary: string =
      typeof sessionRaw.sessionSummary === "string"
        ? (sessionRaw.sessionSummary as string)
        : "";
    // Completed actions (from persisted actionProgress) should be excluded from future pending actions.
    const actionProgress: Record<string, unknown> =
      sessionRaw &&
      typeof sessionRaw.actionProgress === "object" &&
      sessionRaw.actionProgress !== null
    ...
    const msgsSnap = await sessionRef
      .collection("messages")
      .orderBy("timestamp", "desc")
      .limit(40)
      .get();
    const msgs: ChatMessage[] = msgsSnap.docs.map((d) => ({
      id: d.id,
        ...
        messages: [
          {
            role: "system",
            content:
              "You condense SEO chat history into structured JSON memory.",
          },
    ...
    if (!jsonObj || typeof jsonObj.summary !== "string")
      return { updated: false };
    ...
    let pending: string[] = Array.isArray(jsonObj.pending_actions)
      ? (jsonObj.pending_actions as unknown[]).filter(
          (p): p is string => typeof p === "string"
        )
      : [];
    if (completedActions.length && pending.length) {
      const completedSet = new Set(
        completedActions.map((a) => a.toLowerCase().trim())
      );
      pending = pending.filter(
        (a) => !completedSet.has(String(a).toLowerCase().trim())
      );
    }
    await sessionRef.set(
      {
        ...
        keywords: Array.isArray(jsonObj.keywords)
          ? (jsonObj.keywords as unknown[]).filter(
